...
namespace StardewValley.Monsters
{
	public class Leaper : Monster
	{
		...
		protected override void initNetFields()
		{
			...
			leapProgress.Interpolated(true, true);
			leaping.Interpolated(true, true);
		...
		public override void update(GameTime time, GameLocation location)
		{
			...
			if (leaping.Value)
			{
				...
				{
					float total_length = (leapStartPosition.Value - leapEndPosition.Value).Length();
		...
		public override void behaviorAtGameTick(GameTime time)
		{
			...
			if (leaping.Value)
			{
				...
				if (leapProgress.Value == 1f)
				{
					...
					if (!IsValidLandingTile(base.Tile, true))
					{
						nextLeap = 0.1f;
					}
				}
		...
		public override void shedChunks(int number, float scale)
		{
			...
			Game1.createRadialDebris(base.currentLocation, Sprite.textureName, new Microsoft.Xna.Framework.Rectangle(0, 64, 16, 16), 8, standingPixel.X, standingPixel.Y, number, base.TilePoint.Y, Color.White, 4f);
		}
	}
}
