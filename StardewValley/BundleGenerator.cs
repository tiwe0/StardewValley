	...
	public class BundleGenerator
	{
		...
		public Dictionary<string, string> Generate(List<RandomBundleData> bundle_data, Random rng)
		{
			...
			foreach (RandomBundleData area_data in randomBundleData)
			{
				...
				if (bundle_set != null)
				{
					foreach (BundleData bundle4 in bundle_set.Bundles)
					{
						selected_bundles[bundle4.Index] = bundle4;
					}
				}
				List<BundleData> random_bundle_pool = new List<BundleData>();
				foreach (BundleData bundle3 in area_data.Bundles)
				{
					random_bundle_pool.Add(bundle3);
				}
				for (int i = 0; i < index_lookups.Count; i++)
				{
					...
					foreach (BundleData bundle2 in random_bundle_pool)
					{
						if (bundle2.Index == i)
						{
							index_bundles.Add(bundle2);
						}
					}
					if (index_bundles.Count > 0)
					{
						BundleData selected_bundle2 = random.ChooseFrom(index_bundles);
