	...
	public class Preconditions
	{
		...
		public static bool SawEvent(GameLocation location, string eventId, string[] args)
		{
			for (int i = 1; i < args.Length; i++)
			{
				string id;
				string error;
				if (!ArgUtility.TryGet(args, i, out id, out error, false))
				{
		...
		public static bool MissingPet(GameLocation location, string eventId, string[] args)
		{
			string petType;
			string error;
			if (!ArgUtility.TryGetOptional(args, 1, out petType, out error, null, false))
			{
				return Event.LogPreconditionError(location, eventId, args, error);
			}
		...
		public static bool HostMail(GameLocation location, string eventId, string[] args)
		{
			string mailId;
			string error;
			if (!ArgUtility.TryGet(args, 1, out mailId, out error, false))
			{
				return Event.LogPreconditionError(location, eventId, args, error);
			}
		...
		public static bool WorldState(GameLocation location, string eventId, string[] args)
		{
			string worldStateId;
			string error;
			if (!ArgUtility.TryGet(args, 1, out worldStateId, out error, false))
			{
				return Event.LogPreconditionError(location, eventId, args, error);
			}
		...
		public static bool EarnedMoney(GameLocation location, string eventId, string[] args)
		{
			int minMoney;
			string error;
			if (!ArgUtility.TryGetInt(args, 1, out minMoney, out error))
			{
				return Event.LogPreconditionError(location, eventId, args, error);
			}
		...
		public static bool FreeInventorySlots(GameLocation location, string eventId, string[] args)
		{
			int minFreeSpots;
			string error;
			if (!ArgUtility.TryGetInt(args, 1, out minFreeSpots, out error))
			{
				return Event.LogPreconditionError(location, eventId, args, error);
			}
		...
		public static bool DaysPlayed(GameLocation location, string eventId, string[] args)
		{
			int minDaysPlayed;
			string error;
			if (!ArgUtility.TryGetInt(args, 1, out minDaysPlayed, out error))
			{
				return Event.LogPreconditionError(location, eventId, args, error);
			}
		...
		public static bool Friendship(GameLocation location, string eventId, string[] args)
		{
			for (int i = 1; i < args.Length; i += 2)
			{
				...
				string error;
				int minPoints;
				if (!ArgUtility.TryGet(args, i, out npcName, out error, false) || !ArgUtility.TryGetInt(args, i + 1, out minPoints, out error))
				{
					return Event.LogPreconditionError(location, eventId, args, error);
				}
				Friendship friendship;
		...
		public static bool Random(GameLocation location, string eventId, string[] args)
		{
			float probability;
			string error;
			if (!ArgUtility.TryGetFloat(args, 1, out probability, out error))
			{
				return Event.LogPreconditionError(location, eventId, args, error);
			}
		...
		public static bool Shipped(GameLocation location, string eventId, string[] args)
		{
			for (int i = 1; i < args.Length; i += 2)
			{
				...
				string error;
				int minShipped;
				if (!ArgUtility.TryGet(args, i, out itemId, out error, false) || !ArgUtility.TryGetInt(args, i + 1, out minShipped, out error))
				{
					return Event.LogPreconditionError(location, eventId, args, error);
				}
				int countShipped;
		...
		public static bool SawSecretNote(GameLocation location, string eventId, string[] args)
		{
			int secretNoteId;
			string error;
			if (!ArgUtility.TryGetInt(args, 1, out secretNoteId, out error))
			{
				return Event.LogPreconditionError(location, eventId, args, error);
			}
		...
		public static bool ChoseDialogueAnswers(GameLocation location, string eventId, string[] args)
		{
			for (int i = 1; i < args.Length; i++)
			{
				string answerId;
				string error;
				if (!ArgUtility.TryGet(args, i, out answerId, out error, false))
				{
					return Event.LogPreconditionError(location, eventId, args, error);
				}
		...
		public static bool GoldenWalnuts(GameLocation location, string eventId, string[] args)
		{
			int minWalnuts;
			string error;
			if (!ArgUtility.TryGetInt(args, 1, out minWalnuts, out error))
			{
				return Event.LogPreconditionError(location, eventId, args, error);
			}
		...
		public static bool InUpgradedHouse(GameLocation location, string eventId, string[] args)
		{
			int minUpgradeLevel;
			string error;
			if (!ArgUtility.TryGetOptionalInt(args, 1, out minUpgradeLevel, out error, 2))
			{
				return Event.LogPreconditionError(location, eventId, args, error);
			}
		...
		public static bool Time(GameLocation location, string eventId, string[] args)
		{
			...
			string error;
			int maxTime;
			if (!ArgUtility.TryGetInt(args, 1, out minTime, out error) || !ArgUtility.TryGetInt(args, 2, out maxTime, out error))
			{
				return Event.LogPreconditionError(location, eventId, args, error);
			}
		...
		public static bool Weather(GameLocation location, string eventId, string[] args)
		{
			string weather;
			string error;
			if (!ArgUtility.TryGet(args, 1, out weather, out error, false))
			{
				return Event.LogPreconditionError(location, eventId, args, error);
			}
			if (!(weather == "rainy"))
		...
		public static bool DayOfWeek(GameLocation location, string eventId, string[] args)
		{
			...
			for (int i = 1; i < args.Length; i++)
			{
				string rawDayName;
				string error;
				if (!ArgUtility.TryGet(args, i, out rawDayName, out error, false))
				{
					return Event.LogPreconditionError(location, eventId, args, error);
				}
		...
		public static bool Season(GameLocation location, string eventId, string[] args)
		{
			for (int i = 1; i < args.Length; i++)
			{
				Season season;
				string error;
				if (!ArgUtility.TryGetEnum<Season>(args, 1, out season, out error))
				{
					return Event.LogPreconditionError(location, eventId, args, error);
				}
		...
		public static bool ReachedMineBottom(GameLocation location, string eventId, string[] args)
		{
			int minTimes;
			string error;
			if (!ArgUtility.TryGetOptionalInt(args, 1, out minTimes, out error, 1))
			{
				return Event.LogPreconditionError(location, eventId, args, error);
			}
		...
		public static bool Year(GameLocation location, string eventId, string[] args)
		{
			int desiredYear;
			string error;
			if (!ArgUtility.TryGetInt(args, 1, out desiredYear, out error))
			{
				return Event.LogPreconditionError(location, eventId, args, error);
			}
			if (desiredYear != 1)
		...
		public static bool Gender(GameLocation location, string eventId, string[] args)
		{
			string gender;
			string error;
			if (!ArgUtility.TryGet(args, 1, out gender, out error, false))
			{
				return Event.LogPreconditionError(location, eventId, args, error);
			}
		...
		public static bool HasItem(GameLocation location, string eventId, string[] args)
		{
			string itemId;
			string error;
			if (!ArgUtility.TryGet(args, 1, out itemId, out error, false))
			{
				return Event.LogPreconditionError(location, eventId, args, error);
			}
		...
		public static bool Tile(GameLocation location, string eventId, string[] args)
		{
			...
			for (int i = 1; i < args.Length - 1; i += 2)
			{
				Point tile;
				string error;
				if (!ArgUtility.TryGetPoint(args, i, out tile, out error))
				{
					return Event.LogPreconditionError(location, eventId, args, error);
				}
				if (tile == actualTile)
		...
		public static bool ActiveDialogueEvent(GameLocation location, string eventId, string[] args)
		{
			string id;
			string error;
			if (!ArgUtility.TryGet(args, 1, out id, out error, false))
			{
				return Event.LogPreconditionError(location, eventId, args, error);
			}
		...
		public static bool SendMail(GameLocation location, string eventId, string[] args)
		{
			...
			string error;
			bool inMailboxToday;
			if (!ArgUtility.TryGet(args, 1, out mailId, out error, false) || !ArgUtility.TryGetOptionalBool(args, 2, out inMailboxToday, out error))
			{
				return Event.LogPreconditionError(location, eventId, args, error);
			}
			if (inMailboxToday)
		...
		public static bool DayOfMonth(GameLocation location, string eventId, string[] args)
		{
			...
			for (int i = 1; i < args.Length; i++)
			{
				int day;
				string error;
				if (!ArgUtility.TryGetInt(args, i, out day, out error))
				{
					return Event.LogPreconditionError(location, eventId, args, error);
				}
		...
		public static bool UpcomingFestival(GameLocation location, string eventId, string[] args)
		{
			int numberOfDays;
			string error;
			if (!ArgUtility.TryGetInt(args, 1, out numberOfDays, out error))
			{
				return Event.LogPreconditionError(location, eventId, args, error);
			}
		...
		public static bool GameStateQuery(GameLocation location, string eventId, string[] args)
		{
			string query;
			string error;
			if (!ArgUtility.TryGetRemainder(args, 1, out query, out error))
			{
		...
		public static bool Skill(GameLocation location, string eventId, string[] args)
		{
			...
			string error;
			int minSkillLevel;
			if (!ArgUtility.TryGet(args, 1, out name, out error, false) || !ArgUtility.TryGetInt(args, 2, out minSkillLevel, out error))
			{
				return Event.LogPreconditionError(location, eventId, args, error);
			}
