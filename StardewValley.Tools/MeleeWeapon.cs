	...
	public class MeleeWeapon : Tool
	{
		...
		public override string getCategoryName()
		{
			if (!isScythe())
			{
				...
				return Game1.content.LoadString("Strings\\StringsFromCSFiles:Tool.cs.14303", getItemLevel(), Game1.content.LoadString(typeNameKey));
			}
		...
		public void triggerClubFunction(Farmer who)
		{
			...
			who.currentLocation.damageMonster(new Rectangle((int)who.Position.X - 192, who.GetBoundingBox().Y - 192, 384, 384), minDamage, maxDamage, false, 1.5f, 100, 0f, 1f, false, who);
			Game1.viewport.Y -= 21;
			...
			Game1.multiplayer.broadcastSprites(who.currentLocation, new TemporaryAnimatedSprite("TileSheets\\animations", new Rectangle(0, 128, 64, 64), 40f, 4, 0, v, false, who.FacingDirection == 1));
		...
		protected virtual void doAnimateSpecialMove()
		{
			...
			{
				AnimatedSprite.endOfAnimationBehavior endOfAnimFunc2 = triggerClubFunction;
				if (!lastUser.IsLocalPlayer)
				{
					endOfAnimFunc2 = null;
				}
				...
				{
				case 0:
					((FarmerSprite)lastUser.Sprite).animateOnce(176, 40f, 8, endOfAnimFunc2);
					Update(0, 0, lastUser);
					break;
				case 1:
					((FarmerSprite)lastUser.Sprite).animateOnce(168, 40f, 8, endOfAnimFunc2);
					Update(1, 0, lastUser);
					break;
				case 2:
					((FarmerSprite)lastUser.Sprite).animateOnce(160, 40f, 8, endOfAnimFunc2);
					Update(2, 0, lastUser);
					break;
				case 3:
					((FarmerSprite)lastUser.Sprite).animateOnce(184, 40f, 8, endOfAnimFunc2);
					Update(3, 0, lastUser);
					break;
				}
				beginSpecialMove(lastUser);
		...
		public virtual void DoDamage(GameLocation location, int x, int y, int facingDirection, int power, Farmer who)
		{
			...
			if (location.damageMonster(areaOfEffect, (int)((float)(int)minDamage * (1f + who.buffs.AttackMultiplier)), (int)((float)(int)maxDamage * (1f + who.buffs.AttackMultiplier)), false, knockback.Value * (1f + who.buffs.KnockbackMultiplier), (int)((float)(int)addedPrecision * (1f + who.buffs.WeaponPrecisionMultiplier)), effectiveCritChance * (1f + who.buffs.CriticalChanceMultiplier), critMultiplier.Value * (1f + who.buffs.CriticalPowerMultiplier), (int)type != 1 || !isOnSpecial, who) && (int)type == 2)
			{
		...
		public override void drawTooltip(SpriteBatch spriteBatch, ref int x, ref int y, SpriteFont font, float alpha, StringBuilder overrideText)
		{
			...
			Utility.drawWithShadow(spriteBatch, Game1.mouseCursors, new Vector2(x + 16 + 4, y + 16 + 4), new Rectangle(120, 428, 10, 10), Color.White, 0f, Vector2.Zero, 4f, false, 1f);
			...
			if ((int)speed != (((int)type == 2) ? (-8) : 0))
			{
				Utility.drawWithShadow(spriteBatch, Game1.mouseCursors, new Vector2(x + 16 + 4, y + 16 + 4), new Rectangle(130, 428, 10, 10), Color.White, 0f, Vector2.Zero, 4f, false, 1f);
				...
				Utility.drawTextWithShadow(spriteBatch, Game1.content.LoadString("Strings\\UI:ItemHover_Speed", (((((int)type == 2) ? ((int)speed - -8) : ((int)speed)) > 0) ? "+" : "") + (((int)type == 2) ? ((int)speed - -8) : ((int)speed)) / 2), font, new Vector2(x + 16 + 52, y + 16 + 12), negativeSpeed ? Color.DarkRed : (c7 * 0.9f * alpha));
				y += (int)Math.Max(font.MeasureString("TT").Y, 48f);
			}
			if ((int)addedDefense > 0)
			{
				...
				Utility.drawWithShadow(spriteBatch, Game1.mouseCursors, new Vector2(x + 16 + 4, y + 16 + 4), new Rectangle(110, 428, 10, 10), Color.White, 0f, Vector2.Zero, 4f, false, 1f);
				Utility.drawTextWithShadow(spriteBatch, Game1.content.LoadString("Strings\\UI:ItemHover_DefenseBonus", addedDefense), font, new Vector2(x + 16 + 52, y + 16 + 12), c6 * 0.9f * alpha);
				y += (int)Math.Max(font.MeasureString("TT").Y, 48f);
			}
			float effectiveCritChance = critChance.Value;
			if ((int)type == 1)
			...
			if ((double)effectiveCritChance / 0.02 >= 1.100000023841858)
			{
				...
				Utility.drawWithShadow(spriteBatch, Game1.mouseCursors, new Vector2(x + 16 + 4, y + 16 + 4), new Rectangle(40, 428, 10, 10), Color.White, 0f, Vector2.Zero, 4f, false, 1f);
			...
			if ((double)(critMultiplier.Value - 3f) / 0.02 >= 1.0)
			{
				...
				Utility.drawWithShadow(spriteBatch, Game1.mouseCursors, new Vector2(x + 16, y + 16 + 4), new Rectangle(160, 428, 10, 10), Color.White, 0f, Vector2.Zero, 4f, false, 1f);
				Utility.drawTextWithShadow(spriteBatch, Game1.content.LoadString("Strings\\UI:ItemHover_CritPowerBonus", (int)((double)(critMultiplier.Value - 3f) / 0.02)), font, new Vector2(x + 16 + 44, y + 16 + 12), c4 * 0.9f * alpha);
				y += (int)Math.Max(font.MeasureString("TT").Y, 48f);
			}
			if (knockback.Value != defaultKnockBackForThisType(type))
			{
				...
				Utility.drawWithShadow(spriteBatch, Game1.mouseCursors, new Vector2(x + 16 + 4, y + 16 + 4), new Rectangle(70, 428, 10, 10), Color.White, 0f, Vector2.Zero, 4f, false, 1f);
				Utility.drawTextWithShadow(spriteBatch, Game1.content.LoadString("Strings\\UI:ItemHover_Weight", (((float)(int)Math.Ceiling(Math.Abs(knockback.Value - defaultKnockBackForThisType(type)) * 10f) > defaultKnockBackForThisType(type)) ? "+" : "") + (int)Math.Ceiling(Math.Abs(knockback.Value - defaultKnockBackForThisType(type)) * 10f)), font, new Vector2(x + 16 + 52, y + 16 + 12), c3 * 0.9f * alpha);
				y += (int)Math.Max(font.MeasureString("TT").Y, 48f);
			}
			if (enchantments.Count > 0 && enchantments[enchantments.Count - 1] is DiamondEnchantment)
			{
			...
			foreach (BaseEnchantment enchantment in enchantments)
			{
				if (enchantment.ShouldBeDisplayed())
				{
					...
					{
						Utility.drawWithShadow(spriteBatch, Game1.mouseCursors_1_6, new Vector2(x + 16 + 4, y + 16 + 4), new Rectangle(502, 430, 10, 10), Color.White, 0f, Vector2.Zero, 4f, false, 1f);
						c = new Color(120, 50, 100);
					}
					else
					{
						Utility.drawWithShadow(spriteBatch, Game1.mouseCursors2, new Vector2(x + 16 + 4, y + 16 + 4), new Rectangle(127, 35, 10, 10), Color.White, 0f, Vector2.Zero, 4f, false, 1f);
					}
		...
		public override bool CanForge(Item item)
		{
			...
			if (other_weapon != null && other_weapon.type == type)
			{
				return true;
			}
