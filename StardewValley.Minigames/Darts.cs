	...
	public class Darts : IMinigame
	{
		...
		public bool tick(GameTime time)
		{
			...
			case GameState.Charging:
				if (chargeSound != null)
				{
					Game1.sounds.SetPitch(chargeSound, 2400f * (1f - chargeTime));
				}
		...
		public virtual int GetPointsForAim()
		{
			...
			lastHitString = base_points.ToString() ?? "";
			return base_points;
		}
		...
		public void draw(SpriteBatch b)
		{
			...
			{
				SpriteText.drawStringWithScrollCenteredAt(b, alternateTextString, (int)(score_position.X + score_shake.X), (int)(score_position.Y + score_shake.Y), "", 1f, SpriteText.color_Red);
			}
			else if (points >= 0)
			{
				...
				if (currentGameState == GameState.ShowScore || currentGameState == GameState.Scoring)
				{
					...
					string string_to_draw = ((currentGameState == GameState.ShowScore) ? (" " + lastHitString + " ") : (" " + lastHitAmount + " "));
			...
			if (gameOverString != "")
			{
				...
				if (points == 0)
				{
					...
					if (IsPerfectVictory())
					{
						SpriteText.drawStringWithScrollCenteredAt(b, Game1.content.LoadString("Strings\\UI:Darts_WinTextPerfect", throwsCount), (int)(score_position.X + score_shake.X), (int)(score_position.Y + score_shake.Y), "", 1f, SpriteText.color_Blue, 2);
					}
					else
					{
						SpriteText.drawStringWithScrollCenteredAt(b, Game1.content.LoadString("Strings\\UI:Darts_WinText", throwsCount), (int)(score_position.X + score_shake.X), (int)(score_position.Y + score_shake.Y), "", 1f, SpriteText.color_Blue, 2);
					}
				}
				else
				{
