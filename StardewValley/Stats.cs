...
namespace StardewValley
{
	public class Stats
	{
		...
		{
			int count;
			if (!specificMonstersKilled.TryGetValue(name, out count))
			{
				return 0;
			}
			return count;
		...
		{
			int timesShipped;
			if (Game1.player.basicShipped.TryGetValue(itemId, out timesShipped))
			{
		...
		public void checkForFriendshipAchievements()
		{
			...
			foreach (KeyValuePair<string, string> pair2 in DataLoader.CookingRecipes(Game1.content))
			{
				...
				if (!(ArgUtility.Get(getConditions2, 0) != "f"))
				{
					...
					if (npcName2 != null && Game1.player.friendshipData.TryGetValue(npcName2, out friendship2) && friendship2.Points >= minHearts2 * 250 && !Game1.player.cookingRecipes.ContainsKey(recipeKey2) && !Game1.player.hasOrWillReceiveMail(npcName2 + "Cooking"))
					{
			...
			foreach (KeyValuePair<string, string> pair in DataLoader.CraftingRecipes(Game1.content))
			{
				...
				if (!(ArgUtility.Get(getConditions, 0) != "f"))
				{
					...
					if (npcName != null && Game1.player.friendshipData.TryGetValue(npcName, out friendship) && friendship.Points >= minHearts * 250 && !Game1.player.craftingRecipes.ContainsKey(recipeKey) && !Game1.player.hasOrWillReceiveMail(npcName + "Crafting"))
					{
