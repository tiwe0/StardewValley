	...
	internal sealed class DeflateManager
	{
		...
		internal bool _tr_tally(int dist, int lc)
		{
			...
			if ((last_lit & 0x1FFF) == 0 && compressionLevel > CompressionLevel.Level2)
			{
				...
				{
					out_length = (int)(out_length + dyn_dtree[dcode * 2] * (5L + (long)Tree.ExtraDistanceBits[dcode]));
				}
				out_length >>= 3;
		...
		internal void send_compressed_block(short[] ltree, short[] dtree)
		{
			...
			if (last_lit != 0)
			{
				do
				{
					...
					int distance = ((pending[ix] << 8) & 0xFF00) | (pending[ix + 1] & 0xFF);
					int lc = pending[_lengthOffset + lx] & 0xFF;
					lx++;
					if (distance == 0)
					{
						send_code(lc, ltree);
		...
		private void _fillWindow()
		{
			do
			{
				...
				else if (strstart >= w_size + w_size - MIN_LOOKAHEAD)
				{
					...
					{
						int i = head[--p] & 0xFFFF;
						head[p] = (short)((i >= w_size) ? (i - w_size) : 0);
					}
					while (--j != 0);
					j = w_size;
					p = j;
					...
					{
						int i = prev[--p] & 0xFFFF;
						prev[p] = (short)((i >= w_size) ? (i - w_size) : 0);
					}
					while (--j != 0);
					more += w_size;
				}
				...
				j = _codec.read_buf(window, strstart + lookahead, more);
				lookahead += j;
		...
		internal BlockState DeflateSlow(FlushType flush)
		{
			...
			while (true)
			{
				...
				if (prev_length >= MIN_MATCH && match_length <= prev_length)
				{
					...
					bool bflush = _tr_tally(strstart - 1 - prev_match, prev_length - MIN_MATCH);
			...
			{
				bool bflush = _tr_tally(0, window[strstart - 1] & 0xFF);
				match_available = 0;
			}
			flush_block_only(flush == FlushType.Finish);
			if (_codec.AvailableBytesOut == 0)
		...
		internal int longest_match(int cur_match)
		{
			...
			byte scan_end2 = window[scan + best_len - 1];
			...
			{
				int match = cur_match;
				if (window[match + best_len] != scan_end || window[match + best_len - 1] != scan_end2 || window[match] != window[scan] || window[++match] != window[scan + 1])
				{
					continue;
				}
				scan += 2;
				...
				if (len > best_len)
				{
					...
					scan_end2 = window[scan + best_len - 1];
