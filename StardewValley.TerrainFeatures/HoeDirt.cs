...
namespace StardewValley.TerrainFeatures
{
	public class HoeDirt : TerrainFeature
	{
		...
		public override void initNetFields()
		{
			...
			nearWaterForPaddy.Interpolated(false, false);
			netCrop.Interpolated(false, false);
		...
		public override void doCollisionAction(Rectangle positionOfCollider, int speedOfCollision, Vector2 tileLocation, Character who)
		{
			...
			if (crop == null || (int)crop.currentPhase == 0)
			{
				return;
			}
		...
		public bool plant(string itemId, Farmer who, bool isFertilizer)
		{
			...
			if (!who.currentLocation.CheckItemPlantRules(itemId, isGardenPot, isIndoorPot || (location.GetData()?.CanPlantHere ?? location.IsFarm), out deniedMessage))
			{
				if (Game1.didPlayerJustClickAtAll(true))
				{
					if (deniedMessage == null && location.NameOrUniqueName != "Farm")
					{
						...
						if (farm.CheckItemPlantRules(itemId, isGardenPot, farm.GetData()?.CanPlantHere ?? true, out deniedMessage2))
						{
				...
				else
				{
					...
					else if (t.isScythe())
					{
						...
						if ((obj != null && obj.GetHarvestMethod() == HarvestMethod.Scythe) || (crop != null && t.ItemId == "66"))
						{
							...
							if (crop.harvest((int)tileLocation.X, (int)tileLocation.Y, this, null, true))
							{
								if (location is IslandLocation && Game1.random.NextDouble() < 0.05)
								{
									Game1.player.team.RequestLimitedNutDrops("IslandFarming", location, (int)tileLocation.X * 64, (int)tileLocation.Y * 64, 5);
		...
		public void updateNeighbors()
		{
			...
			foreach (Neighbor i in list)
			{
				...
				if ((int)state != 2)
				{
					i.feature.OnNeighborAdded(i.invDirection, state);
				}
				if (isWatered() && i.feature.isWatered())
				{
					if (i.feature.paddyWaterCheck() == paddyWaterCheck())
					{
