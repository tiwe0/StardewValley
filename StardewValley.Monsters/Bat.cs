				...
				if (base.Health <= 0)
				{
					...
					if ((bool)cursedDoll)
					{
						...
						if ((bool)magmaSprite)
						{
							...
							for (int i = 0; i < 20; i++)
							{
								base.currentLocation.temporarySprites.Add(new TemporaryAnimatedSprite("Characters\\Monsters\\Magma Sprite", new Rectangle(0, 64, 8, 8), pixelPosition + new Vector2(Game1.random.Next(64), Game1.random.Next(64)), false, 0f, Color.White)
								{
									scale = 4f,
									scaleChange = 0f,
						...
						{
							Game1.multiplayer.broadcastSprites(who.currentLocation, new TemporaryAnimatedSprite(Sprite.textureName, new Rectangle(0, 32, 16, 16), 2000f, 1, 9999, pixelPosition, false, false, 1f, 0.02f, Color.White, 4f, 0f, 0f, 0f)
							{
						...
						{
							Game1.multiplayer.broadcastSprites(who.currentLocation, new TemporaryAnimatedSprite("LooseSprites\\Cursors", new Rectangle(388, 1894, 24, 22), 40f, 6, 9999, pixelPosition, false, true, 1f, 0f, Color.Black * 0.67f, 4f, 0f, 0f, 0f)
							{
		...
		public override void drawAboveAllLayers(SpriteBatch b)
		{
			...
			if ((bool)cursedDoll)
			{
				...
				else if ((bool)magmaSprite)
				{
					...
					b.Draw(Sprite.Texture, getLocalPosition(Game1.viewport) + new Vector2(32 + Game1.random.Next(-6, 7), 32 + Game1.random.Next(-6, 7)) + offset3, Game1.getSourceRectForStandardTileSheet(Sprite.Texture, direction2 * 7 + (int)(Game1.currentGameTime.TotalGameTime.TotalMilliseconds % 490.0 / 70.0), 16, 16), Color.Red * 0.44f, 0f, new Vector2(8f, 16f), Math.Max(0.2f, scale.Value) * 4f, flip ? SpriteEffects.FlipHorizontally : SpriteEffects.None, 0.9955f);
					b.Draw(Sprite.Texture, getLocalPosition(Game1.viewport) + new Vector2(32 + Game1.random.Next(-6, 7), 32 + Game1.random.Next(-6, 7)) + offset3, Game1.getSourceRectForStandardTileSheet(Sprite.Texture, direction2 * 7 + (int)(Game1.currentGameTime.TotalGameTime.TotalMilliseconds % 490.0 / 70.0), 16, 16), Color.Yellow * 0.44f, 0f, new Vector2(8f, 16f), Math.Max(0.2f, scale.Value) * 4f, flip ? SpriteEffects.FlipHorizontally : SpriteEffects.None, 0.9975f);
					for (int j = previousPositions.Count - 1; j >= 0; j -= 2)
					{
						b.Draw(Sprite.Texture, new Vector2(previousPositions[j].X - (float)Game1.viewport.X, previousPositions[j].Y - (float)Game1.viewport.Y + (float)yJumpOffset) + drawOffset + new Vector2(32f, 32f) + offset3, Game1.getSourceRectForStandardTileSheet(Sprite.Texture, direction2 * 7 + (int)(Game1.currentGameTime.TotalGameTime.TotalMilliseconds % 490.0 / 70.0), 16, 16), Color.White * (0f + 0.125f * (float)j), 0f, new Vector2(8f, 16f), Math.Max(0.2f, scale.Value) * 4f, flip ? SpriteEffects.FlipHorizontally : SpriteEffects.None, 0.9985f);
					}
					b.Draw(Sprite.Texture, getLocalPosition(Game1.viewport) + new Vector2(32f, 32f) + offset3, Game1.getSourceRectForStandardTileSheet(Sprite.Texture, direction2 * 7 + (int)(Game1.currentGameTime.TotalGameTime.TotalMilliseconds % 490.0 / 70.0), 16, 16), Color.White, 0f, new Vector2(8f, 16f), Math.Max(0.2f, scale.Value) * 4f, flip ? SpriteEffects.FlipHorizontally : SpriteEffects.None, 1f);
				}
				else
				{
					int index = 103;
					...
					b.Draw(Game1.objectSpriteSheet, getLocalPosition(Game1.viewport) + new Vector2(32 + Game1.random.Next(-6, 7), 32 + Game1.random.Next(-6, 7)) + offset2, Game1.getSourceRectForStandardTileSheet(Game1.objectSpriteSheet, index, 16, 16), Color.Violet * 0.44f, 0f, new Vector2(8f, 16f), Math.Max(0.2f, scale.Value) * 4f, flip ? SpriteEffects.FlipHorizontally : SpriteEffects.None, (position.Y + 128f - 1f) / 10000f);
					b.Draw(Game1.objectSpriteSheet, getLocalPosition(Game1.viewport) + new Vector2(32 + Game1.random.Next(-6, 7), 32 + Game1.random.Next(-6, 7)) + offset2, Game1.getSourceRectForStandardTileSheet(Game1.objectSpriteSheet, index, 16, 16), Color.Lime * 0.44f, 0f, new Vector2(8f, 16f), Math.Max(0.2f, scale.Value) * 4f, flip ? SpriteEffects.FlipHorizontally : SpriteEffects.None, (position.Y + 128f) / 10000f);
					b.Draw(Game1.objectSpriteSheet, getLocalPosition(Game1.viewport) + new Vector2(32f, 32f) + offset2, Game1.getSourceRectForStandardTileSheet(Game1.objectSpriteSheet, index, 16, 16), (index == 789) ? Color.White : new Color(255, 50, 50), 0f, new Vector2(8f, 16f), Math.Max(0.2f, scale.Value) * 4f, flip ? SpriteEffects.FlipHorizontally : SpriteEffects.None, (position.Y + 128f + 1f) / 10000f);
				}
			}
			else
			{
				...
				if (cursedDoll.Value)
				{
					...
					if (shakeTimer < 0)
					{
						...
						if (magmaSprite.Value)
						{
							...
							base.currentLocation.temporarySprites.Add(new TemporaryAnimatedSprite("Characters\\Monsters\\Magma Sprite", new Rectangle(0, 64, 8, 8), base.Position + new Vector2(Game1.random.Next(32), -16 - Game1.random.Next(32)), false, 0f, Color.White)
							{
								scale = 4f,
								scaleChange = -0.05f,
