	...
	public class MineCart : IMinigame
	{
			...
			protected override void _GenerateTrack()
			{
				...
				for (int i = 0; i < straightAwayLength; i++)
				{
					...
					if (currentStaggerDistance <= 0)
					{
						...
						{
							_game.generatorPosition.Y += Game1.random.ChooseFrom(staggerPattern);
						}
			...
			protected override void _GenerateTrack()
			{
				...
				for (int i = 0; i < 4; i++)
				{
					...
					if (i != 1)
					{
						...
						{
							target_y += Game1.random.ChooseFrom(staggerPattern);
						}
					}
					...
					while (!has_landed)
					{
						if (_game.trackBuilderCharacter.position.Y < (float)(target_y * _game.tileSize) && Math.Abs(Math.Round(_game.trackBuilderCharacter.position.X / (float)_game.tileSize) - (double)_game.generatorPosition.X) > 0.0 && _game.trackBuilderCharacter.IsJumping() && Game1.random.NextDouble() < (double)releaseJumpChance)
						{
							_game.trackBuilderCharacter.ReleaseJump();
						}
						Vector2 old_position = _game.trackBuilderCharacter.position;
						_game.trackBuilderCharacter.Update(0.03f);
						if (old_position.Y < (float)(target_y * _game.tileSize) && _game.trackBuilderCharacter.position.Y >= (float)(target_y * _game.tileSize))
						{
							has_landed = true;
						}
						if (_game.trackBuilderCharacter.IsGrounded() || _game.trackBuilderCharacter.position.Y / (float)_game.tileSize > (float)_game.bottomTile)
						{
							...
							target_y = _game.KeepTileInBounds((int)(old_position.Y / (float)_game.tileSize));
							break;
						}
					}
					...
					if (i == 0 || i == 2)
					{
						...
						BalanceTrack track = new BalanceTrack(Track.TrackType.MushroomLeft, false);
						...
						_game.generatorPosition.X++;
						track = new BalanceTrack(Track.TrackType.MushroomMiddle, false);
						track.position.X = _game.generatorPosition.X * _game.tileSize;
						track.position.Y = _game.trackBuilderCharacter.position.Y + y_offset;
						track.startY = track.position.Y;
						AddTrack(track);
						current_balance_tracks.Add(track);
						_game.generatorPosition.X++;
						track = new BalanceTrack(Track.TrackType.MushroomRight, false);
						track.position.X = _game.generatorPosition.X * _game.tileSize;
						track.position.Y = _game.trackBuilderCharacter.position.Y + y_offset;
						track.startY = track.position.Y;
						AddTrack(track);
		...
		public class MushroomBunnyHopGenerator : BaseTrackGenerator
		{
			...
			protected override void _GenerateTrack()
			{
				...
				for (int i = 0; i < numberOfHops; i++)
				{
					...
					while (!has_landed)
					{
						if (_game.trackBuilderCharacter.position.Y < (float)(target_y * _game.tileSize) && Math.Abs(Math.Round(_game.trackBuilderCharacter.position.X / (float)_game.tileSize) - (double)_game.generatorPosition.X) > 1.0 && _game.trackBuilderCharacter.IsJumping() && Game1.random.NextDouble() < (double)releaseJumpChance)
						{
							_game.trackBuilderCharacter.ReleaseJump();
						}
						Vector2 old_position = _game.trackBuilderCharacter.position;
		...
		public class BaseTrackGenerator
		{
			...
			public void PopulateObstacles()
			{
				...
				{
					if (Game1.random.NextBool(_obstacleIndices[index].Value))
					{
						int track_index;
						switch (index)
						{
						case -12:
							track_index = _generatedTracks.Count - 1;
							break;
						case -11:
							track_index = 0;
							break;
						case -10:
							track_index = (_generatedTracks.Count - 1) / 2;
							break;
						case -13:
							track_index = Game1.random.Next(_generatedTracks.Count);
							break;
						default:
							track_index = index;
							break;
		...
		public class PlayerBubbleSpawner : Entity
		{
			...
			protected override void _Update(float time)
			{
				...
				{
					bubbleCount--;
					_game.AddEntity(new MineDebris(new Rectangle(0, 256, 16, 16), position + new Vector2(0f - _game.player.characterExtraHeight - 16f) / 2f, -10f, 10f, 0f, -1f, 1.5f, 0.5f, 4, 0.1f, 0.45f, true));
					if (bubbleCount == 0)
					{
		...
		public class PlayerMineCartCharacter : MineCartCharacter, ICollideable
		{
			...
			protected override void _Update(float time)
			{
				...
				if (_grounded && _game.respawnCounter <= 0)
				{
					...
					if (old_x_pos != (int)(position.X / (float)_game.tileSize) && Game1.random.NextBool())
					{
		...
		public MineCart(int whichTheme, int mode)
		{
			...
			Game1.playSound("minecartLoop", out minecartLoop);
			minecartLoop.Pause();
		...
		public void initLevelTransitions()
		{
			LEVEL_TRANSITIONS = new LevelTransition[15]
			{
				...
				new LevelTransition(1, 5, 6, 11, "rrurruuu", (Func<bool>)Game1.random.NextBool),
				new LevelTransition(1, 2, 6, 11, "rrurrrrddr"),
				new LevelTransition(8, 5, 8, 8, "ddrruuu", (Func<bool>)Game1.random.NextBool),
				...
				new LevelTransition(3, 9, 16, 8, "rruuluu", (Func<bool>)Game1.random.NextBool),
		...
		public Obstacle AddObstacle(Track track, ObstacleTypes obstacle_type)
		{
			...
			if (track == null || !_validObstacles.TryGetValue(obstacle_type, out obstacleTypes))
			{
				return null;
			}
		...
		public Track GetTrackForXPosition(float x)
		{
			...
			List<Track> tracks;
			if (!_tracks.TryGetValue(tile_position, out tracks))
			{
				return null;
			}
			return tracks[0];
		...
		protected void _GenerateMoreTrack()
		{
				...
				if (_trackGenerator == null)
				{
					...
					for (int tries = 0; tries < 2; tries++)
					{
						...
						if (_trackGenerator == null)
						{
							...
							_trackGenerator = new StraightAwayGenerator(this).SetLength(2, 2).SetStaggerChance(0f).SetCheckpoint(false);
							_forcedNextGenerator = null;
						}
					}
			...
			if (generatorPosition.X >= distanceToTravel)
			{
				...
				{
					CreatePickup(new Vector2((float)generatorPosition.X + 0.5f, generatorPosition.Y - 1) * tileSize, true);
				}
				generatorPosition.X++;
			}
		}
		...
		public Track AddTrack(Track track_object)
		{
			...
			List<Track> tracks;
			if (!_tracks.TryGetValue(x, out tracks))
			{
		...
		public void UpdateMapTick(float time)
		{
			...
			{
				MapJunimo junimo = entity as MapJunimo;
				if (junimo != null)
				{
					map_junimo = junimo;
					break;
		...
		public void UpdateCutsceneTick()
		{
			...
			if (currentTheme == 9)
			{
				if (cutsceneTick == 0)
				{
					...
					BalanceTrack track_piece = new BalanceTrack(Track.TrackType.MushroomLeft, false);
					track_piece.position.X = 15 * tileSize;
					track_piece.position.Y = 9 * tileSize;
					track_parts.Add(track_piece);
					AddTrack(track_piece);
					track_piece = new BalanceTrack(Track.TrackType.MushroomMiddle, false);
					track_piece.position.X = 16 * tileSize;
					track_piece.position.Y = 9 * tileSize;
					track_parts.Add(track_piece);
					AddTrack(track_piece);
					track_piece = new BalanceTrack(Track.TrackType.MushroomRight, false);
					track_piece.position.X = 17 * tileSize;
					track_piece.position.Y = 9 * tileSize;
					track_parts.Add(track_piece);
					AddTrack(track_piece);
					List<BalanceTrack> other_track_parts = new List<BalanceTrack>();
					track_piece = new BalanceTrack(Track.TrackType.MushroomLeft, false);
			...
			if (currentTheme == 7)
			{
				...
				if (cutsceneTick == 0)
				{
					...
					{
						AddEntity(new MineDebris(new Rectangle(256, 182, 48, 45), new Vector2((float)(20 * tileSize) + 12f, (float)(10 * tileSize) - 21.5f), 0f, 0f, 0f, 0f, 1000f, 1f, 1, 0f, 0.23f, true));
					}
					else
					{
						AddEntity(new MineDebris(new Rectangle(256, 112, 25, 32), new Vector2((float)(20 * tileSize) + 12f, (float)(10 * tileSize) - 16f), 0f, 0f, 0f, 0f, 1000f, 1f, 1, 0f, 0.23f, true));
					}
				}
				if (cutsceneTick == 200)
				{
		...
		public void UpdateFruitsSummary(float time)
		{
			...
			{
				perfectText = new SparklingText(Game1.dialogueFont, Game1.content.LoadString("Strings\\UI:BobberBar_Perfect"), Color.Lime, Color.White, true, 0.1, 2500, -1, 500, 0f);
			...
			if (_collectedFruit.Contains((CollectableFruits)currentFruitCheckIndex))
			{
				...
				Game1.playSound("newArtifact", currentFruitCheckIndex * 100);
				fruitEatCount++;
				if (fruitEatCount >= 3)
				{
					Game1.playSound("yoba");
			...
			{
				Game1.playSound("sell", currentFruitCheckIndex * 100);
			}
			stateTimer = 0.5f;
		...
		public bool tick(GameTime time)
		{
			...
			if (gameState == GameStates.Title)
			{
				...
				if (musicSW != null && Game1.currentSong?.Name == "junimoKart" && Game1.currentSong.IsPlaying && !musicSW.IsRunning)
				{
					musicSW.Start();
				}
				if (titleScreenJunimo != null && !titleJunimoStartedBobbing)
				{
					Stopwatch stopwatch = musicSW;
			...
			if (gameState == GameStates.Ingame)
			{
				...
				{
					int k = 0;
					while (k < checkpointPositions.Count && player.position.X >= (float)(checkpointPositions[k] * tileSize))
					{
						foreach (Entity entity2 in _entities)
						{
							CheckpointIndicator indicator2 = entity2 as CheckpointIndicator;
							if (indicator2 != null && (int)(indicator2.position.X / (float)tileSize) == checkpointPositions[k])
							{
								indicator2.Activate();
								break;
							}
						}
						checkpointPosition = ((float)checkpointPositions[k] + 0.5f) * (float)tileSize;
						ReapEntities();
				...
				if (_goalIndicator != null && distanceToTravel != -1 && player.position.X >= _goalIndicator.position.X && distanceToTravel != -1 && player.position.Y <= _goalIndicator.position.Y * (float)tileSize + 4f && !reachedFinish && fadeDelta < 0f)
				{
					...
					foreach (Entity entity in _entities)
					{
						GoalIndicator indicator = entity as GoalIndicator;
						if (indicator != null)
						{
							indicator.Activate();
						}
			...
			float parallax_scroll_speed = (screenLeftBound - old_screen_left_bound) / (float)tileSize;
		...
		public void submitHighScore()
		{
			...
			if (Game1.player.team.specialOrders != null)
			{
				...
				{
					specialOrder.onJKScoreAchieved?.Invoke(Game1.player, score);
				}
			}
			RefreshHighScore();
		}
		...
		public void receiveKeyPress(Keys k)
		{
			...
			else if ((CanPause() && !Game1.options.gamepadControls && (k.Equals(Keys.P) || k.Equals(Keys.Enter))) || (Game1.options.gamepadControls && Game1.input.GetGamePadState().IsButtonDown(Buttons.Start)))
			{
		...
		public void ShowMap()
		{
			...
			else if (currentTheme == 1)
			{
				...
				for (int n = 0; n < 4; n++)
				{
					AddEntity(new MineDebris(new Rectangle(421, 607, 5, 5), new Vector2(119f, 162f), Game1.random.Next(-30, 31), Game1.random.Next(-50, -39), 0.25f, 1f, 0.75f, 1f, 1, 1f, 0.45f, false, 0.8f));
				}
				for (int m = 0; m < 4; m++)
				{
					AddEntity(new MineDebris(new Rectangle(421, 607, 5, 5), new Vector2(49f, 166f), Game1.random.Next(-30, 31), Game1.random.Next(-50, -39), 0.25f, 1f, 0.75f, 1f, 1, 1f, 0.45f, false, 1.2f));
				}
			}
			else if (currentTheme == 3)
			{
				...
				for (int l = 0; l < 32; l++)
				{
					AddEntity(new MineDebris(new Rectangle(51, 53, 9, 9), new Vector2(250f, 136f) + new Vector2(Game1.random.Next(-20, 31), Game1.random.Next(-20, 21)), Game1.random.Next(-30, 31), Game1.random.Next(-70, -39), 0.25f, 1f, 0.75f, 1f, 1, 1f, 0.45f, false, 0.8f + 0.01f * (float)l));
				}
			}
			else if (currentTheme == 2)
			{
				AddEntity(new MineDebris(new Rectangle(416, 368, 24, 16), new Vector2(217f, 177f), 0f, 0f, 0f, 0f, 99f, 1f, 4, 0.1f, 0.54f, true, 0.8f));
			...
			{
				AddEntity(new MineDebris(new Rectangle(401, 591, 12, 11), new Vector2(328f, 197f), 0f, 0f, 0f, 0f, 99f, 1f, 4, 0.1f, 0.34f, false, 2.5f)).SetStartSound("fireball");
				AddEntity(new MineDebris(new Rectangle(401, 591, 12, 11), new Vector2(336f, 197f), 0f, 0f, 0f, 0f, 99f, 1f, 4, 0.1f, 0.35f, false, 2.625f));
				AddEntity(new MineDebris(new Rectangle(401, 591, 12, 11), new Vector2(344f, 197f), 0f, 0f, 0f, 0f, 99f, 1f, 4, 0.1f, 0.34f, false, 2.75f)).SetStartSound("fireball");
				AddEntity(new MineDebris(new Rectangle(401, 591, 12, 11), new Vector2(344f, 189f), 0f, 0f, 0f, 0f, 99f, 1f, 4, 0.1f, 0.35f, false, 2.825f));
				AddEntity(new MineDebris(new Rectangle(401, 591, 12, 11), new Vector2(344f, 181f), 0f, 0f, 0f, 0f, 99f, 1f, 4, 0.1f, 0.34f, false, 3f)).SetStartSound("fireball");
				AddEntity(new MineDebris(new Rectangle(401, 591, 12, 11), new Vector2(344f, 173f), 0f, 0f, 0f, 0f, 99f, 1f, 4, 0.1f, 0.35f, false, 3.125f));
				AddEntity(new MineDebris(new Rectangle(401, 591, 12, 11), new Vector2(344f, 165f), 0f, 0f, 0f, 0f, 99f, 1f, 4, 0.1f, 0.34f, false, 3.25f)).SetStartSound("fireball");
				AddEntity(new MineDebris(new Rectangle(401, 591, 12, 11), new Vector2(352f, 165f), 0f, 0f, 0f, 0f, 99f, 1f, 4, 0.1f, 0.35f, false, 3.325f));
				AddEntity(new MineDebris(new Rectangle(401, 591, 12, 11), new Vector2(360f, 165f), 0f, 0f, 0f, 0f, 99f, 1f, 4, 0.1f, 0.34f, false, 3.5f)).SetStartSound("fireball");
				AddEntity(new MineDebris(new Rectangle(401, 591, 12, 11), new Vector2(360f, 157f), 0f, 0f, 0f, 0f, 99f, 1f, 4, 0.1f, 0.35f, false, 3.625f));
				AddEntity(new MineDebris(new Rectangle(401, 591, 12, 11), new Vector2(360f, 149f), 0f, 0f, 0f, 0f, 99f, 1f, 4, 0.1f, 0.34f, false, 3.75f)).SetStartSound("fireball");
			}
			else if (currentTheme == 5)
			{
				...
				for (int k = 0; k < 8; k++)
				{
					AddEntity(new MineDebris(new Rectangle(427, 607, 6, 6), new Vector2(205 + Game1.random.Next(3, 14), 26 + Game1.random.Next(6, 14)), Game1.random.Next(-30, 31), Game1.random.Next(-60, -39), 0.25f, 1f, 0.75f, 1f, 1, 1f, 0.45f, false, 5f + (float)k * 0.005f));
				}
			}
			if (currentTheme == 9)
			{
				for (int j = 0; j < 8; j++)
				{
					AddEntity(new MineDebris(new Rectangle(368, 784, 16, 16), new Vector2(274 + Game1.random.Next(-19, 20), 46 + Game1.random.Next(6, 14)), Game1.random.Next(-4, 5), -16f, 0f, 0.05f, 2f, 1f, 3, 0.33f, 0.35f, true, 1f + (float)j * 0.1f)).SetStartSound("dirtyHit");
				}
			}
		...
		public void CreateBGDecor()
		{
			for (int i = 0; i < 16; i++)
			{
				lakeDecor.Add(new LakeDecor(this, currentTheme, true, i));
			}
		}
		...
		public void AddValidObstacle(ObstacleTypes obstacle_type, Type type)
		{
			...
			{
				List<Type> obstacleTypes;
				if (!_validObstacles.TryGetValue(obstacle_type, out obstacleTypes))
				{
		...
		public void setUpTheme(int whichTheme)
		{
			...
			case 6:
				...
				_generatorRolls.Add(new GeneratorRoll(0.25f, new RapidHopsGenerator(this).SetLength(3, 5).SetYStep(2).SetChaotic(true)
		...
		public T GetOverlap<T>(ICollideable source) where T : Entity
		{
			...
			foreach (Entity entity in _entities)
			{
				...
				ICollideable collideable_entity = entity as ICollideable;
		...
		public void draw(SpriteBatch b)
		{
			...
			if (gameState != GameStates.Map)
			{
				...
				else if (gameState != GameStates.Cutscene)
				{
					...
					{
						b.Draw(texture, TransformDraw(new Rectangle(n * tileSize - (int)lakeSpeedAccumulator % tileSize, tileSize * 9, tileSize, screenHeight - 96)), lakeBGSource, lakeTint, 0f, Vector2.Zero, SpriteEffects.None, 0.9f);
					}
					...
					{
						b.Draw(texture, TransformDraw(new Vector2(0f - backBGPosition + (float)(i3 * backBGSource.Width), backBGYOffset)), backBGSource, backBGTint, 0f, Vector2.Zero, GetPixelScale(), SpriteEffects.None, 0.7f);
					}
					for (int i4 = 0; i4 < screenWidth / midBGSource.Width + 2; i4++)
					{
						b.Draw(texture, TransformDraw(new Vector2(0f - midBGPosition + (float)(i4 * midBGSource.Width), 162 - midBGSource.Height + midBGYOffset)), midBGSource, midBGTint, 0f, Vector2.Zero, GetPixelScale(), SpriteEffects.None, 0.6f);
					}
				}
			}
			foreach (Entity entity in _entities)
			...
			if (gameState == GameStates.Title)
			{
				...
				if (gameMode == 2)
				{
					...
					Vector2 draw_position3 = new Vector2((float)(screenWidth / 2) - score_offset.X / 2f, 155f);
					for (int i5 = 0; i5 < 5 && i5 < _currentHighScores.Count; i5++)
					{
						...
						b.DrawString(Game1.dialogueFont, "#" + (i5 + 1), TransformDraw(draw_position3), color, 0f, Vector2.Zero, GetPixelScale() / 4f, SpriteEffects.None, 0.199f);
						b.DrawString(Game1.dialogueFont, score.Key, TransformDraw(draw_position3 + new Vector2(16f, 0f)), color, 0f, Vector2.Zero, GetPixelScale() / 4f, SpriteEffects.None, 0.199f);
						b.DrawString(Game1.dialogueFont, score.Value.ToString() ?? "", TransformDraw(draw_position3 + score_offset - new Vector2(score_text_width, 0f)), color, 0f, Vector2.Zero, GetPixelScale() / 4f, SpriteEffects.None, 0.199f);
						Vector2 shadow_offset = new Vector2(1f, 1f);
						b.DrawString(Game1.dialogueFont, "#" + (i5 + 1), TransformDraw(draw_position3 + shadow_offset), Color.Black, 0f, Vector2.Zero, GetPixelScale() / 4f, SpriteEffects.None, 0.1999f);
						b.DrawString(Game1.dialogueFont, score.Key, TransformDraw(draw_position3 + new Vector2(16f, 0f) + shadow_offset), Color.Black, 0f, Vector2.Zero, GetPixelScale() / 4f, SpriteEffects.None, 0.1999f);
						b.DrawString(Game1.dialogueFont, score.Value.ToString() ?? "", TransformDraw(draw_position3 + score_offset - new Vector2(score_text_width, 0f) + shadow_offset), Color.Black, 0f, Vector2.Zero, GetPixelScale() / 4f, SpriteEffects.None, 0.1999f);
			...
			else if (gameState != GameStates.FruitsSummary)
			{
				if (gameState == GameStates.Cutscene)
				{
					...
					b.DrawString(Game1.dialogueFont, cutsceneText, TransformDraw(new Vector2(screenWidth / 2 - (int)(Game1.dialogueFont.MeasureString(cutsceneText).X / 2f / 4f), 32f)), Color.White, 0f, Vector2.Zero, scale_adjustment, SpriteEffects.None, 0.199f);
				}
				else
				{
					for (int j2 = 0; j2 < waterfallWidth; j2 += 2)
					{
						...
						{
							b.Draw(texture, TransformDraw(new Vector2((float)(screenWidth + tileSize * j2) - waterFallPosition, m * tileSize + (int)(_totalTime * 48.0 + (double)(tileSize * 100)) % tileSize)), new Rectangle(48, 32, 16, 16), waterfallTint, 0f, Vector2.Zero, GetPixelScale(), SpriteEffects.None, 0.2f);
						}
					}
				}
			}
			...
			if (gamePaused)
			{
				...
				b.DrawString(Game1.dialogueFont, current_text, TransformDraw(draw_position - new Vector2(Game1.dialogueFont.MeasureString(current_text).X / 2f / 4f, 0f)), Color.White, 0f, Vector2.Zero, GetPixelScale() / 4f, SpriteEffects.None, 0.1f);
			}
