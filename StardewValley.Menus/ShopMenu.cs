...
namespace StardewValley.Menus
{
	public class ShopMenu : IClickableMenu
	{
		...
		private void Initialize(int currency, Func<ISalable, Farmer, int, bool> onPurchase, Func<ISalable, bool> onSell, bool playOpenSound)
		{
			...
			inventory = new InventoryMenu(xPositionOnScreen + width, yPositionOnScreen + IClickableMenu.spaceToClearTopBorder + IClickableMenu.borderWidth + 320 + 40, false, null, highlightItemToSell)
			{
		...
		public void UseFurnitureCatalogueTabs()
		{
			tabButtons = new List<ShopTabClickableTextureComponent>
			{
				...
				new ShopTabClickableTextureComponent(new Rectangle(0, 0, 64, 64), Game1.mouseCursors2, new Rectangle(80, 48, 16, 16), 4f)
				{
					...
					downNeighborID = -99998,
					rightNeighborID = 3546,
					Filter = delegate(ISalable item)
					{
						Furniture furniture5 = item as Furniture;
						return furniture5 != null && (furniture5.IsTable() || furniture5.furniture_type.Value == 4);
					}
				},
				new ShopTabClickableTextureComponent(new Rectangle(0, 0, 64, 64), Game1.mouseCursors2, new Rectangle(64, 48, 16, 16), 4f)
				{
					...
					Filter = delegate(ISalable item)
					{
						Furniture furniture4 = item as Furniture;
						return furniture4 != null && (furniture4.furniture_type.Value == 0 || furniture4.furniture_type.Value == 1 || furniture4.furniture_type.Value == 2 || furniture4.furniture_type.Value == 3);
					}
				},
				new ShopTabClickableTextureComponent(new Rectangle(0, 0, 64, 64), Game1.mouseCursors2, new Rectangle(64, 64, 16, 16), 4f)
				{
					...
					Filter = delegate(ISalable item)
					{
						Furniture furniture3 = item as Furniture;
						return furniture3 != null && (furniture3.furniture_type.Value == 6 || furniture3.furniture_type.Value == 13);
					}
				},
				new ShopTabClickableTextureComponent(new Rectangle(0, 0, 64, 64), Game1.mouseCursors2, new Rectangle(96, 64, 16, 16), 4f)
				{
					...
					Filter = delegate(ISalable item)
					{
						...
						return furniture2 != null && furniture2.furniture_type.Value == 12;
					}
				},
		...
		public void UseCatalogueTabs()
		{
			tabButtons = new List<ShopTabClickableTextureComponent>
			{
				...
				new ShopTabClickableTextureComponent(new Rectangle(0, 0, 64, 64), Game1.mouseCursors2, new Rectangle(48, 64, 16, 16), 4f)
				{
					...
					downNeighborID = -99998,
					rightNeighborID = 3546,
					Filter = (ISalable item) => (item as Wallpaper)?.isFloor.Value ?? false
				},
		...
		public void UseDresserTabs()
		{
			tabButtons = new List<ShopTabClickableTextureComponent>
			{
				...
				new ShopTabClickableTextureComponent(new Rectangle(0, 0, 64, 64), Game1.mouseCursors2, new Rectangle(16, 48, 16, 16), 4f)
				{
					...
					downNeighborID = -99998,
					rightNeighborID = 3546,
					Filter = delegate(ISalable salable)
					{
						Item item4 = salable as Item;
						return item4 != null && item4.Category == -95;
					}
				},
				new ShopTabClickableTextureComponent(new Rectangle(0, 0, 64, 64), Game1.mouseCursors2, new Rectangle(32, 48, 16, 16), 4f)
				{
					...
					Filter = delegate(ISalable salable)
					{
						Clothing clothing2 = salable as Clothing;
						return clothing2 != null && clothing2.clothesType.Value == Clothing.ClothesType.SHIRT;
					}
				},
				new ShopTabClickableTextureComponent(new Rectangle(0, 0, 64, 64), Game1.mouseCursors2, new Rectangle(48, 48, 16, 16), 4f)
				{
					...
					Filter = delegate(ISalable salable)
					{
						Clothing clothing = salable as Clothing;
						return clothing != null && clothing.clothesType.Value == Clothing.ClothesType.PANTS;
					}
				},
				new ShopTabClickableTextureComponent(new Rectangle(0, 0, 64, 64), Game1.mouseCursors2, new Rectangle(0, 64, 16, 16), 4f)
				{
					...
					Filter = delegate(ISalable salable)
					{
						Item item3 = salable as Item;
						return item3 != null && item3.Category == -97;
					}
				},
				new ShopTabClickableTextureComponent(new Rectangle(0, 0, 64, 64), Game1.mouseCursors2, new Rectangle(16, 64, 16, 16), 4f)
				{
					...
					Filter = delegate(ISalable salable)
					{
						Item item2 = salable as Item;
						return item2 != null && item2.Category == -96;
					}
				}
			};
			repositionTabs();
		...
		public Texture2D TryLoadPortrait(ShopOwnerData ownerData, NPC owner)
		{
			...
			if (ownerData.Portrait != null)
			{
				if (!string.IsNullOrWhiteSpace(ownerData.Portrait))
				{
					...
					NPC npc2 = Game1.getCharacterFromName(ownerData.Portrait);
		...
		public bool ParseDialogueSubstitution(string[] query, out string replacement, Random random, Farmer player)
		{
			if (query[0] == "SuggestedItem")
			{
				...
				if (Utility.TryGetRandom(itemPriceAndStock, out suggestedItem, out value, random))
				{
		...
		public override void receiveLeftClick(int x, int y, bool playSound = true)
		{
			...
			if (safetyTimer <= 0)
			{
				...
				{
					Item toSell = inventory.leftClick(x, y, null, false);
					if (toSell != null)
					{
						if (onSell != null)
						{
						...
						else
						{
							...
							{
								animations.Add(new TemporaryAnimatedSprite("TileSheets\\debris", new Rectangle(Game1.random.Next(2) * 16, 64, 16, 16), 9999f, 1, 999, snappedPosition + new Vector2(32f, 32f), false, false)
								{
									alphaFade = 0.025f,
							...
							if (sellObj != null && (int)sellObj.edibility != -300)
							{
								Item stackClone = sellObj.getOne();
								stackClone.Stack = sellObj.Stack;
								ISalable soldTomorrowItem;
								if (buyback_item != null && buyBackItemsToResellTomorrow.TryGetValue(buyback_item, out soldTomorrowItem))
								{
		...
		public virtual void BuyBuybackItem(ISalable bought_item, int price, int stack)
		{
			...
			if (buyBackItemsToResellTomorrow.TryGetValue(bought_item, out sold_tomorrow_item))
			{
		...
		public virtual ISalable AddBuybackItem(ISalable sold_item, int sell_unit_price, int stack)
		{
			...
			while (stack > 0)
			{
				...
				if (target == null)
				{
					...
					itemPriceAndStock.Add(target, new ItemStockInformation(sell_unit_price, amount_to_deposit2));
					target.Stack = 1;
		...
		private bool tryToPurchaseItem(ISalable item, ISalable held_item, int stockToBuy, int x, int y)
		{
			...
			if (held_item == null)
			{
				...
				if (getPlayerCurrencyAmount(Game1.player, currency) >= price && (extraTradeItem == null || HasTradeItem(extraTradeItem, extraTradeItemCount)))
				{
					...
					if (itemPriceAndStock[item].Stock != int.MaxValue && !item.IsInfiniteStock())
					{
						...
						ItemStockInformation stock2 = itemPriceAndStock[item];
						item.Stack = Math.Min(item.Stack, stock2.Stock);
			...
			else if (held_item.canStackWith(item))
			{
				...
				if (stockToBuy > 0)
				{
					int price2 = itemPriceAndStock[item].Price * stockToBuy;
					string extraTradeItem2 = null;
					int extraTradeItemCount2 = 5;
					if (itemPriceAndStock[item].TradeItem != null)
					{
						extraTradeItem2 = itemPriceAndStock[item].TradeItem;
						if (itemPriceAndStock[item].TradeItemCount.HasValue)
						{
							extraTradeItemCount2 = itemPriceAndStock[item].TradeItemCount.Value;
						}
					...
					if (getPlayerCurrencyAmount(Game1.player, currency) >= price2 && (extraTradeItem2 == null || HasTradeItem(extraTradeItem2, extraTradeItemCount2)))
					{
						heldItem.Stack += stacksToBuy2;
						if (CanBuyback() && buyBackItems.Contains(item))
						{
							BuyBuybackItem(item, price2, stacksToBuy2);
						}
						...
						{
							ConsumeTradeItem(extraTradeItem2, extraTradeItemCount2);
						}
						if (!_isStorageShop && item.actionWhenPurchased(ShopId))
						{
							heldItem = null;
		...
		public override void receiveRightClick(int x, int y, bool playSound = true)
		{
			...
			if (heldItem == null && !readOnly)
			{
				ISalable toSell = inventory.rightClick(x, y, null, false);
				if (toSell != null)
				{
					if (onSell != null)
					{
					...
					else
					{
						...
						else
						{
							...
							if (obj != null && (int)obj.edibility != -300 && Game1.random.NextDouble() < 0.03999999910593033)
							{
								ShopLocation shopLocation = Game1.currentLocation as ShopLocation;
								if (shopLocation != null)
								{
		...
		public override void performHoverAction(int x, int y)
		{
				...
				if (currentItemIndex + i < forSale.Count && forSaleButtons[i].containsPoint(x, y))
				{
					...
					if (canPurchaseCheck == null || canPurchaseCheck(currentItemIndex + i))
					{
						...
						if (!_isStorageShop)
						{
							...
							hoverPrice = ((itemPriceAndStock != null && itemPriceAndStock.TryGetValue(item, out stock)) ? stock.Price : item.salePrice());
						}
						hoveredItem = item;
			...
			foreach (ClickableComponent c in inventory.inventory)
			{
				...
				hoverPrice = (int)((float)j.sellToStorePrice(-1L) * sellPercentage) * j.Stack;
			}
		}
		...
		{
			ItemStockInformation stock;
			if (hoveredItem != null && itemPriceAndStock != null && itemPriceAndStock.TryGetValue(hoveredItem, out stock) && stock.TradeItem != null)
			{
		...
		{
			ItemStockInformation stock;
			if (hoveredItem != null && itemPriceAndStock != null && itemPriceAndStock.TryGetValue(hoveredItem, out stock) && stock.TradeItem != null && stock.TradeItemCount.HasValue)
			{
		...
		public override void draw(SpriteBatch b)
		{
			...
			for (int i = 0; i < forSaleButtons.Count; i++)
			{
				...
				IClickableMenu.drawTextureBox(b, theme.ItemRowBackgroundTexture, theme.ItemRowBackgroundSourceRect, forSaleButtons[i].bounds.X, forSaleButtons[i].bounds.Y, forSaleButtons[i].bounds.Width, forSaleButtons[i].bounds.Height, (forSaleButtons[i].containsPoint(Game1.getOldMouseX(), Game1.getOldMouseY()) && !scrolling) ? theme.ItemRowBackgroundHoverColor : Color.White, 4f, false);
				ISalable item = forSale[currentItemIndex + i];
				...
				if (item.ShouldDrawIcon())
				{
					...
					int drawnStack = 1;
					ItemStockInformation stock;
					if (itemPriceAndStock.TryGetValue(item, out stock))
					{
					...
					SpriteText.drawString(b, formattedDisplayName, forSaleButtons[i].bounds.X + 96 + 8, forSaleButtons[i].bounds.Y + 28, 999999, -1, 999999, failedCanPurchaseCheck ? 0.5f : 1f, 0.88f, false, -1, "", theme.ItemRowTextColor);
				}
				else
				{
					SpriteText.drawString(b, displayName, forSaleButtons[i].bounds.X + 32 + 8, forSaleButtons[i].bounds.Y + 28, 999999, -1, 999999, failedCanPurchaseCheck ? 0.5f : 1f, 0.88f, false, -1, "", theme.ItemRowTextColor);
				}
				...
				{
					SpriteText.drawString(b, itemPriceAndStock[forSale[currentItemIndex + i]].Price + " ", right - SpriteText.getWidthOfString(itemPriceAndStock[forSale[currentItemIndex + i]].Price + " ") - 60, forSaleButtons[i].bounds.Y + 28, 999999, -1, 999999, (getPlayerCurrencyAmount(Game1.player, currency) >= itemPriceAndStock[forSale[currentItemIndex + i]].Price && !failedCanPurchaseCheck) ? 1f : 0.5f, 0.88f, false, -1, "", theme.ItemRowTextColor);
					Utility.drawWithShadow(b, Game1.mouseCursors, new Vector2(forSaleButtons[i].bounds.Right - 52, forSaleButtons[i].bounds.Y + 40 - 4), new Rectangle(193 + currency * 9, 373, 9, 10), Color.White * ((!failedCanPurchaseCheck) ? 1f : 0.25f), 0f, Vector2.Zero, 4f, false, 1f, -1, -1, (!failedCanPurchaseCheck) ? 0.35f : 0f);
				...
				if (itemPriceAndStock[forSale[currentItemIndex + i]].TradeItem != null)
				{
					...
					Utility.drawWithShadow(b, texture, new Vector2((float)(right - 88) - textWidth, tradeIconDrawY), sourceRect, Color.White * (hasEnoughToTrade ? 1f : 0.25f), 0f, Vector2.Zero, -1f, false, -1f, -1, -1, hasEnoughToTrade ? 0.35f : 0f);
					SpriteText.drawString(b, "x" + required_item_count, right - (int)textWidth - 16, tradeTextDrawY, 999999, -1, 999999, hasEnoughToTrade ? 1f : 0.5f, 0.88f, false, -1, "", theme.ItemRowTextColor);
				}
			}
			if (IsOutOfStock())
			{
			...
			heldItem?.drawInMenu(b, new Vector2(Game1.getOldMouseX() + 8, Game1.getOldMouseY() + 8), 1f, 1f, 0.9f, StackDrawType.Draw, Color.White, true);
			base.draw(b);
			...
			if (portrait_draw_position > 0 && Game1.options.showMerchantPortraits)
			{
				if (portraitTexture != null)
				{
					Utility.drawWithShadow(b, theme.PortraitBackgroundTexture, new Vector2(portrait_draw_position, yPositionOnScreen), theme.PortraitBackgroundSourceRect, Color.White, 0f, Vector2.Zero, 4f, false, 0.91f);
					if (portraitTexture != null)
					{
		...
		public StackDrawType GetStackDrawType(ItemStockInformation stockInfo, ISalable item)
		{
			...
			{
				switch (ShopData.StackSizeVisibility)
				{
