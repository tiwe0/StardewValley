...
namespace StardewValley.Objects
{
	public class Mannequin : Object
	{
		...
		{
			Location.temporarySprites.Add(new TemporaryAnimatedSprite(GetMannequinData().Texture, new Rectangle((!(Game1.random.NextDouble() < 0.5)) ? 64 : 0, 64, 16, 32), TileLocation * 64f + new Vector2(0f, -1f) * 64f, false, 0.004f, Color.White)
			{
				scale = 4f,
				layerDepth = 1f,
		...
		public override void DayUpdate()
		{
			...
			if (Game1.IsMasterGame && GetMannequinData().Cursed && Location != null && (Location is FarmHouse || Location is IslandFarmHouse || Location is Shed))
			{
				if (Game1.random.NextDouble() < 0.05)
				{
					...
					Utility.spawnObjectAround(TileLocation, this, Location, false, delegate
					{
				...
				else
				{
					if (Game1.random.NextDouble() < 0.005)
					{
						...
						using (FarmerCollection.Enumerator enumerator = Location.farmers.GetEnumerator())
						{
							if (enumerator.MoveNext())
							{
								...
								if (Utility.spawnObjectAround(bedTile, this, Location, false, delegate
								{
					...
					{
						DecoratableLocation dec_location2 = Location as DecoratableLocation;
						string floorID = dec_location2.GetFloorID((int)TileLocation.X, (int)TileLocation.Y);
						string wallpaperID = null;
						for (int y2 = (int)TileLocation.Y; y2 > 0; y2--)
						{
							wallpaperID = dec_location2.GetWallpaperID((int)TileLocation.X, y2);
							if (wallpaperID != null)
							{
								break;
							}
						...
						if (floorID != null)
						{
							dec_location2.SetFloor("MoreFloors:6", floorID);
						}
						if (wallpaperID != null)
						{
							dec_location2.SetWallpaper("MoreWalls:21", wallpaperID);
						}
						shakeTimer = 10000;
					}
					else
					{
						...
						if (Game1.random.NextDouble() < 0.33)
						{
							for (int j = 0; j < 30; j++)
							{
								...
								{
									Vector2 spot2 = new Vector2(xPos, y);
									if (Location.isTileLocationOpen(spot2) && Location.isTilePlaceable(spot2) && !dec_location.isTileOnWall(xPos, y) && !Location.IsTileOccupiedBy(spot2))
									{
										facing.Value = 2;
										renderCache = null;
						...
						for (int i = 0; i < 30; i++)
						{
							...
							{
								Vector2 spot = new Vector2(x2, yPos);
								if (Location.isTileLocationOpen(spot) && Location.isTilePlaceable(spot) && !dec_location.isTileOnWall(x2, yPos) && !Location.IsTileOccupiedBy(spot))
								{
		...
		{
			renderCache = renderCache ?? _003CGetFarmerForRendering_003Eg__CreateInstance_007C36_0();
			return renderCache;
		}
	}
}
