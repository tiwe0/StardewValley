...
namespace StardewValley.Menus
{
	public class ShippingMenu : IClickableMenu
	{
		...
		public int getCategoryIndexForObject(Item item)
		{
			...
			{
				Object o = item as Object;
				if (o != null && (o.preserve.Value == Object.PreserveType.SmokedFish || o.preserve.Value == Object.PreserveType.AgedRoe || o.preserve.Value == Object.PreserveType.Roe))
				{
					return 2;
				}
			...
			else if (Game1.dayOfMonth != 28 && !outro)
			{
				if (!Game1.wasRainingYesterday)
				{
					...
					if (Game1.random.NextDouble() < 0.001)
					{
						...
						{
							animations.Add(new TemporaryAnimatedSprite("LooseSprites\\Cursors", new Rectangle(640, 826, 16, 8), 40f, 4, 0, new Vector2(Game1.random.Next(centerX * 2), Game1.random.Next(centerY)), false, flip)
							{
						...
						{
							animations.Add(new TemporaryAnimatedSprite("LooseSprites\\Cursors", new Rectangle(258, 1680, 16, 16), 40f, 4, 0, new Vector2(Game1.random.Next(centerX * 2), Game1.random.Next(centerY)), false, flip)
							{
								scale = 4f,
								motion = new Vector2(flip ? (-8) : 8, 8f),
								local = true
					...
					else if (Game1.random.NextDouble() < 5E-05)
					{
						...
						{
							TemporaryAnimatedSprite bird = new TemporaryAnimatedSprite("LooseSprites\\Cursors", sourceRect, Game1.random.Next(60, 101), 4, 100, startingPosition + new Vector2((i + 1) * Game1.random.Next(15, 18), (i + 1) * -20), false, false, 0.01f, 0f, Color.Black, 4f, 0f, 0f, 0f, true);
							bird.motion = new Vector2(-1f, 0f);
							animations.Add(bird);
							bird = new TemporaryAnimatedSprite("LooseSprites\\Cursors", sourceRect, Game1.random.Next(60, 101), 4, 100, startingPosition + new Vector2((i + 1) * Game1.random.Next(15, 18), (i + 1) * 20), false, false, 0.01f, 0f, Color.Black, 4f, 0f, 0f, 0f, true);
							bird.motion = new Vector2(-1f, 0f);
							animations.Add(bird);
						}
					}
				...
				{
					smokeTimer = 50;
					animations.Add(new TemporaryAnimatedSprite("LooseSprites\\Cursors", new Rectangle(684, 1075, 1, 1), 1000f, 1, 1000, new Vector2(188f, Game1.uiViewport.Height - 128 + 20), false, false)
					{
		...
		public override void draw(SpriteBatch b)
		{
			...
			if (Game1.wasRainingYesterday)
			{
				...
				for (int x2 = 0; x2 < width; x2 += 639)
				{
					b.Draw(Game1.mouseCursors, new Vector2(x2 * 4, Game1.uiViewport.Height - 192), new Rectangle(0, isWinter ? 1034 : 737, 639, 48), (isWinter ? (Color.White * 0.25f) : new Color(30, 62, 50)) * (0.5f - (float)introTimer / 3500f), 0f, Vector2.Zero, 4f, SpriteEffects.FlipHorizontally, 1f);
					b.Draw(Game1.mouseCursors, new Vector2(x2 * 4, Game1.uiViewport.Height - 128), new Rectangle(0, isWinter ? 1034 : 737, 639, 32), (isWinter ? (Color.White * 0.5f) : new Color(30, 62, 50)) * (1f - (float)introTimer / 3500f), 0f, Vector2.Zero, 4f, SpriteEffects.None, 1f);
				}
				...
				{
					b.Draw(Game1.mouseCursors, new Vector2((float)x3 + weatherX % 244f, -32f), new Rectangle(643, 1142, 61, 53), Color.SlateGray * 0.85f * (1f - (float)introTimer / 3500f), 0f, Vector2.Zero, 4f, SpriteEffects.None, 0.9f);
				}
				...
				{
					b.Draw(Game1.mouseCursors, new Vector2((float)x4 + weatherX * 1.5f % 244f, -128f), new Rectangle(643, 1142, 61, 53), Color.LightSlateGray * (1f - (float)introTimer / 3500f), 0f, Vector2.Zero, 4f, SpriteEffects.None, 0.9f);
				}
			}
			else
			{
				...
				for (int x5 = 0; x5 < width; x5 += 639)
				{
					b.Draw(Game1.mouseCursors, new Vector2(x5 * 4, 0f), new Rectangle(0, 1453, 639, 195), Color.White * (1f - (float)introTimer / 3500f), 0f, Vector2.Zero, 4f, SpriteEffects.None, 1f);
				}
				...
				foreach (ClickableTextureComponent c in categories)
				{
					if (introTimer < 2500 - j * 500)
					{
						...
						if (c.visible)
						{
							...
							categoryItems[j][0].drawInMenu(b, start + new Vector2(-88 + extraWidth, yOffset + 16), 1f, 1f, 0.9f, StackDrawType.Hide);
						}
						IClickableMenu.drawTextureBox(b, Game1.mouseCursors, new Rectangle(384, 373, 18, 18), (int)(start.X + (float)(-itemSlotWidth) - (float)categoryLabelsWidth - 12f), (int)(start.Y + (float)yOffset), categoryLabelsWidth + extraWidth, 104, Color.White, 4f, false);
						SpriteText.drawString(b, c.hoverText, (int)start.X - itemSlotWidth - categoryLabelsWidth + 8, (int)start.Y + 4);
						for (int k = 0; k < 6; k++)
						{
							b.Draw(Game1.mouseCursors, start + new Vector2(-itemSlotWidth + extraWidth - 192 - 24 + k * 6 * 4, 12f), new Rectangle(355, 476, 7, 11), Color.White, 0f, Vector2.Zero, 4f, SpriteEffects.None, 0.88f);
						}
						categoryDials[j].draw(b, start + new Vector2(-itemSlotWidth + extraWidth - 192 - 48 + 4, 20f), categoryTotals[j]);
