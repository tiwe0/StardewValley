...
namespace StardewValley.Menus
{
	public class InventoryPage : IClickableMenu
	{
		...
		{
			inventory = new InventoryMenu(xPositionOnScreen + IClickableMenu.spaceToClearSideBorder + IClickableMenu.borderWidth, yPositionOnScreen + IClickableMenu.spaceToClearTopBorder + IClickableMenu.borderWidth, true);
			...
			_horse = Game1.getCharacterFromName<Horse>(Game1.player.horseName, false);
			if (_horse == null && Game1.player.isRidingHorse() && Game1.player.mount.Name.Equals(Game1.player.horseName))
			{
		...
		public override void receiveLeftClick(int x, int y, bool playSound = true)
		{
			foreach (ClickableComponent c in equipmentIcons)
			{
				...
				case "Hat":
					...
					{
						Item oldItem5 = Utility.PerformSpecialItemGrabReplacement(Game1.player.Equip((Hat)newItem, Game1.player.hat));
				...
				case "Right Ring":
					if (newItem == null || newItem is Ring)
					{
						...
						Item oldItem4 = Utility.PerformSpecialItemGrabReplacement(Game1.player.Equip((Ring)newItem, ringField));
				...
				case "Shirt":
				{
					...
					Item oldItem2 = Utility.PerformSpecialItemGrabReplacement(Game1.player.Equip((Clothing)newItem, Game1.player.shirtItem));
				...
				case "Pants":
				{
					...
					Item oldItem = Utility.PerformSpecialItemGrabReplacement(Game1.player.Equip((Clothing)newItem, Game1.player.pantsItem));
					setHeldItem(oldItem);
			...
			if (checkHeldItem((Item i) => i?.QualifiedItemId == "(O)434"))
			{
				...
				Game1.player.eatObject(takeHeldItem() as Object, true);
			...
			else if (checkHeldItem() && Game1.oldKBState.IsKeyDown(Keys.LeftShift))
			{
				...
				else if (checkHeldItem(delegate(Item i)
				{
					Clothing clothing2 = i as Clothing;
				...
				{
					Clothing clothing = i as Clothing;
				...
				else if (checkHeldItem((Item i) => i is Trinket) && Game1.player.stats.Get("trinketSlots") != 0)
				{
					...
					for (int m = 0; m < Game1.player.trinketItems.Count; m++)
					{
						...
						{
							Game1.player.trinketItems[m] = takeHeldItem() as Trinket;
							success = true;
							break;
						}
					}
		...
		public override void draw(SpriteBatch b)
		{
			...
			Game1.player.FarmerRenderer.draw(b, new FarmerSprite.AnimationFrame(0, Game1.player.bathingClothes ? 108 : 0, false, false), Game1.player.bathingClothes ? 108 : 0, new Rectangle(0, Game1.player.bathingClothes ? 576 : 0, 16, 32), new Vector2(xPositionOnScreen + 192 - 8 - 32, yPositionOnScreen + IClickableMenu.borderWidth + IClickableMenu.spaceToClearTopBorder + 320 - 32 - 8), Vector2.Zero, 0.8f, 2, Color.White, 0f, 1f, Game1.player);
