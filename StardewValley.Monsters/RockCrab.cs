	...
	public class RockCrab : Monster
	{
		...
		public override bool hitWithTool(Tool t)
		{
			...
			if (t is Pickaxe && t.getLastFarmerToUse() != null && (int)shellHealth > 0)
			{
				...
				if ((int)shellHealth <= 0)
				{
					...
					Game1.createRadialDebris(base.currentLocation, 14, tile.X, tile.Y, Game1.random.Next(2, 7), false);
					Game1.createRadialDebris(base.currentLocation, 14, tile.X, tile.Y, Game1.random.Next(2, 7), false);
				}
				return true;
			}
		...
		public override void behaviorAtGameTick(GameTime time)
		{
			...
			else
			{
				...
				FacingDirection = getGeneralDirectionTowards(base.Player.getStandingPosition(), 0, true, false);
				moveUp = false;
				moveDown = false;
				moveRight = false;
				moveLeft = false;
