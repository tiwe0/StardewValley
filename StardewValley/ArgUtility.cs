	...
	public static class ArgUtility
	{
		...
		public static bool TryGetOptional(string[] array, int index, out string value, out string error, string defaultValue = null, bool allowBlank = true)
		{
			...
			if (!allowBlank && string.IsNullOrWhiteSpace(value))
			{
				...
				defaultInterpolatedStringHandler.AppendLiteral("optional index ");
				defaultInterpolatedStringHandler.AppendFormatted(index);
				defaultInterpolatedStringHandler.AppendLiteral(" can't have a blank value");
				error = defaultInterpolatedStringHandler.ToStringAndClear();
				return false;
			}
			error = null;
		...
		{
			bool value;
			if (!bool.TryParse(Get(array, index), out value))
			{
				return defaultValue;
			}
			return value;
		...
		{
			string raw;
			if (!TryGet(array, index, out raw, out error, false))
			{
				value = false;
				return false;
			}
			...
			{
				value = false;
				error = GetValueParseError(array, index, true, "a boolean (should be 'true' or 'false')");
				return false;
			}
			error = null;
			return true;
		...
		public static bool TryGetOptionalBool(string[] array, int index, out bool value, out string error, bool defaultValue = false)
		{
			...
			{
				error = GetValueParseError(array, index, false, "a boolean");
				value = defaultValue;
				return false;
			}
			error = null;
		...
		{
			int value;
			if (!Utility.TryParseDirection(Get(array, index), out value))
			{
				return defaultValue;
			}
			return value;
		...
		public static bool TryGetDirection(string[] array, int index, out int value, out string error)
		{
			...
			{
				value = 0;
				error = GetValueParseError(array, index, true, "a direction (should be 'up', 'down', 'left', or 'right')");
				return false;
			}
			error = null;
			return true;
		...
		public static bool TryGetOptionalDirection(string[] array, int index, out int value, out string error, int defaultValue = 0)
		{
			...
			{
				error = GetValueParseError(array, index, true, "a direction (should be one of 'up', 'down', 'left', or 'right')");
				value = defaultValue;
				return false;
			}
			error = null;
		...
		{
			TEnum value;
			if (!Utility.TryParseEnum<TEnum>(Get(array, index), out value))
			{
				return defaultValue;
			}
			return value;
		...
		public static bool TryGetEnum<TEnum>(string[] array, int index, out TEnum value, out string error) where TEnum : struct
		{
			...
			if (!Utility.TryParseEnum<TEnum>(raw, out value))
			{
				...
				error = GetValueParseError(array, index, true, defaultInterpolatedStringHandler.ToStringAndClear());
				return false;
			}
			error = null;
			return true;
		...
		public static bool TryGetOptionalEnum<TEnum>(string[] array, int index, out TEnum value, out string error, TEnum defaultValue = default(TEnum)) where TEnum : struct
		{
			...
			{
				Type type = typeof(TEnum);
				error = GetValueParseError(array, index, false, "an enum of type '" + (type.FullName ?? type.Name) + "'");
				value = defaultValue;
				return false;
			}
			error = null;
		...
		{
			float value;
			if (!float.TryParse(Get(array, index), out value))
			{
				return defaultValue;
			}
			return value;
		...
		public static bool TryGetFloat(string[] array, int index, out float value, out string error)
		{
			...
			{
				value = 0f;
				error = GetValueParseError(array, index, true, "a number");
				return false;
			}
			error = null;
			return true;
		...
		public static bool TryGetOptionalFloat(string[] array, int index, out float value, out string error, float defaultValue = 0f)
		{
			...
			{
				error = GetValueParseError(array, index, false, "a float");
				value = defaultValue;
				return false;
			}
			error = null;
		...
		{
			int value;
			if (!int.TryParse(Get(array, index), out value))
			{
				return defaultValue;
			}
			return value;
		...
		public static bool TryGetInt(string[] array, int index, out int value, out string error)
		{
			...
			{
				value = 0;
				error = GetValueParseError(array, index, true, "an integer");
				return false;
			}
			error = null;
			return true;
		...
		public static bool TryGetOptionalInt(string[] array, int index, out int value, out string error, int defaultValue = 0)
		{
			...
			{
				error = GetValueParseError(array, index, false, "an integer");
				value = defaultValue;
				return false;
			}
			error = null;
		...
		public static bool TryGetRectangle(string[] array, int index, out Rectangle value, out string error)
		{
			...
			int width;
			int height;
			if (!TryGetInt(array, index, out x, out error) || !TryGetInt(array, index + 1, out y, out error) || !TryGetInt(array, index + 2, out width, out error) || !TryGetInt(array, index + 3, out height, out error))
			{
		...
		public static bool TryGetVector2(string[] array, int index, out Vector2 value, out string error, bool integerOnly = false)
		{
			...
			if (integerOnly)
			{
				int x2;
				int y2;
				if (TryGetInt(array, index, out x2, out error) && TryGetInt(array, index + 1, out y2, out error))
				{
		...
		public static string GetRemainder(string[] array, int index, string defaultValue = null, char delimiter = ' ')
		{
			...
			return string.Join(delimiter, RuntimeHelpers.GetSubArray(array, Range.StartAt(index)));
		}
		...
		public static bool TryGetRemainder(string[] array, int index, out string value, out string error, char delimiter = ' ')
		{
			...
			{
				value = string.Join(delimiter, RuntimeHelpers.GetSubArray(array, Range.StartAt(index)));
			}
			error = null;
			return true;
		}
