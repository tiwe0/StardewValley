...
namespace StardewValley.Menus
{
	public class ChatBox : IClickableMenu
	{
		...
		protected virtual void runCommand(string command)
		{
			...
			case "ping":
			{
				...
				if (Game1.IsServer)
				{
					...
					{
						sb2.Clear();
						sb2.AppendFormat("Ping({0}) {1}ms ", farmer.Value.Name, (int)Game1.server.getPingToClient(farmer.Key));
						addMessage(sb2.ToString(), Color.White);
					}
					break;
				}
				sb2.AppendFormat("Ping: {0}ms", (int)Game1.client.GetPingToHost());
				addMessage(sb2.ToString(), Color.White);
				break;
			}
			case "mapscreenshot":
			...
			case "sleepannouncemode":
			{
				...
				defaultInterpolatedStringHandler.AppendLiteral("Strings\\UI:SleepAnnounceMode_");
				defaultInterpolatedStringHandler.AppendFormatted(Game1.player.team.sleepAnnounceMode.Value);
				array[0] = TokenStringBuilder.LocalizedText(defaultInterpolatedStringHandler.ToStringAndClear());
				multiplayer.globalChatInfoMessage("SleepAnnounceModeSet", array);
				break;
			}
			case "money":
				if (Program.enableCheats)
		...
		private void replyPrivateMessage(string command)
		{
			...
			Farmer lastPlayer;
			if (!Game1.otherFarmers.TryGetValue(lastReceivedPrivateMessagePlayerId, out lastPlayer) || !lastPlayer.isActive())
			{
		...
		{
			int index = 0;
			Farmer farmer = findMatchingFarmer(command, ref index, true);
			if (farmer != null)
			{
		...
		public override void draw(SpriteBatch b)
		{
			...
			if (drawBG)
			{
				IClickableMenu.drawTextureBox(b, Game1.mouseCursors, new Rectangle(301, 288, 15, 15), xPositionOnScreen, yPositionOnScreen - heightSoFar - 20 + ((!chatBox.Selected) ? chatBox.Height : 0), chatBox.Width, heightSoFar + 20, Color.White, 4f, false);
			}
			heightSoFar = 0;
			for (int i = messages.Count - 1; i >= 0; i--)
			{
