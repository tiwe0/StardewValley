...
namespace StardewValley.Menus
{
	public class DayTimeMoneyBox : IClickableMenu
	{
		...
		public void pingQuest(Quest quest)
		{
			...
			if (monsterQuest != null && monsterQuest.monster.Value != null && monsterQuest.monster.Value.Sprite != null && monsterQuest.monster.Value.Sprite.Texture != null && monsterQuest.monsterName != null)
			{
				...
				{
					questPingString = monsterQuest.numberKilled?.ToString() + "/" + monsterQuest.numberToKill;
				}
				return;
			}
			...
			{
				ParsedItemData data2 = ItemRegistry.GetData(resourceQuest.ItemId);
				questPingTexture = data2.GetTexture();
				questPingSourceRect = data2.GetSourceRect();
				if (resourceQuest.numberCollected != resourceQuest.number)
				{
					questPingString = resourceQuest.numberCollected?.ToString() + "/" + resourceQuest.number;
				}
				return;
			}
			...
			{
				ParsedItemData data = ItemRegistry.GetData(fishingQuest.ItemId);
				questPingTexture = data.GetTexture();
				questPingSourceRect = data.GetSourceRect();
				if (fishingQuest.numberFished != fishingQuest.numberToFish)
				{
					questPingString = fishingQuest.numberFished?.ToString() + "/" + fishingQuest.numberToFish;
				}
				return;
			}
			...
			if (socializeQuest != null)
			{
				...
				{
					questPingString = (int)socializeQuest.total - socializeQuest.whoToGreet.Count + "/" + socializeQuest.total;
				}
			}
			else
			{
		...
		public override void draw(SpriteBatch b)
		{
			...
			if (goldCoinTimer > 0)
			{
				SpriteText.drawSmallTextBubble(b, goldCoinString, position + new Vector2(5f, 73f) * 4f, -1, 0.99f, true);
			}
		}
