	...
	internal class DigitEntryMenu : NumberSelectionMenu
	{
		...
		private void onDigitPressed(string digit)
		{
			...
			else
			{
				...
				currentValue = Math.Min(val2: Convert.ToInt32((!(currentStr == "0")) ? (currentStr + digit) : digit), val1: maxValue);
				numberSelectedBox.Text = currentValue.ToString();
			}
		}
		...
		{
			base.draw(b);
			Game1.drawDialogueBox(calculatorX, calculatorY, calculatorWidth, calculatorHeight, false, true);
			foreach (ClickableComponent c in digits)
			{
				if (c.name == clear)
				{
