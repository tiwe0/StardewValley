...
namespace StardewValley.Menus
{
	public class MenuWithInventory : IClickableMenu
	{
			...
			: base(Game1.uiViewport.Width / 2 - (800 + IClickableMenu.borderWidth * 2) / 2, Game1.uiViewport.Height / 2 - (600 + IClickableMenu.borderWidth * 2) / 2 + menuOffsetHack, 800 + IClickableMenu.borderWidth * 2, 600 + IClickableMenu.borderWidth * 2)
		{
			...
			inventory = new InventoryMenu(xPositionOnScreen + IClickableMenu.spaceToClearSideBorder + IClickableMenu.borderWidth / 2 + inventoryXOffset, yPositionForInventory, false, null, highlighterMethod);
			HeldItemExitBehavior = heldItemExitBehavior;
			AllowExitWithHeldItem = allowExitWithHeldItem;
			if (okButton)
			{
		...
		{
			heldItem = inventory.rightClick(x, y, heldItem, true, true);
		}
		...
		public virtual void draw(SpriteBatch b, bool drawUpperPortion = true, bool drawDescriptionArea = true, int red = -1, int green = -1, int blue = -1)
		{
			...
			if (drawUpperPortion)
			{
				Game1.drawDialogueBox(xPositionOnScreen, yPositionOnScreen, width, height, false, true, null, false, false, red, green, blue);
				drawHorizontalPartition(b, yPositionOnScreen + IClickableMenu.borderWidth + IClickableMenu.spaceToClearTopBorder + 256, false, red, green, blue);
				if (drawDescriptionArea)
				{
					...
					if (!descriptionText.Equals(""))
					{
						...
						do
						{
							...
							parsed_text = Game1.parseText(descriptionText, Game1.smallFont, (int)(224f / scale));
						}
			...
			{
				Game1.drawDialogueBox(xPositionOnScreen - IClickableMenu.borderWidth / 2, yPositionOnScreen + IClickableMenu.borderWidth + IClickableMenu.spaceToClearTopBorder + 64, width, height - (IClickableMenu.borderWidth + IClickableMenu.spaceToClearTopBorder + 192), false, true);
			}
			okButton?.draw(b);
		...
		public override void gameWindowSizeChanged(Rectangle oldBounds, Rectangle newBounds)
		{
			...
			inventory = new InventoryMenu(xPositionOnScreen + IClickableMenu.spaceToClearSideBorder + IClickableMenu.borderWidth / 2, yPositionForInventory, false, null, inventory.highlightMethod);
