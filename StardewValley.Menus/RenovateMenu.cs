...
namespace StardewValley.Menus
{
	public class RenovateMenu : IClickableMenu
	{
		...
		public void SetupForRenovationPlacement()
		{
			...
			_oldPosition = Game1.player.TilePoint;
		...
		public virtual void AnimateRenovation()
		{
			...
			if (_buildAnimationCount > 0)
			{
				...
				if (_renovation.animationType == HouseRenovation.AnimationType.Destroy)
				{
					...
					for (int j = 0; j < 5; j++)
					{
						Microsoft.Xna.Framework.Rectangle rectangle2 = Game1.random.ChooseFrom(_renovation.renovationBounds[_animatingIndex]);
						...
						_renovation.location.temporarySprites.Add(new TemporaryAnimatedSprite(362, Game1.random.Next(30, 90), 6, 1, new Vector2(x2, y2), false, Game1.random.NextBool()));
						_renovation.location.temporarySprites.Add(new TemporaryAnimatedSprite(362, Game1.random.Next(30, 90), 6, 1, new Vector2(x2, y2), false, Game1.random.NextBool()));
						_renovation.location.temporarySprites.Add(new TemporaryAnimatedSprite("LooseSprites\\Cursors", new Microsoft.Xna.Framework.Rectangle(276, 1985, 12, 11), new Vector2(x2, y2), false, 0f, Color.White)
						{
							interval = 30f,
				...
				else
				{
					...
					for (int i = 0; i < 20; i++)
					{
						Microsoft.Xna.Framework.Rectangle rectangle = Game1.random.ChooseFrom(_renovation.renovationBounds[_animatingIndex]);
						...
						_renovation.location.temporarySprites.Add(new TemporaryAnimatedSprite(362, Game1.random.Next(30, 90) - 64, 6, 1, new Vector2(x, y), false, Game1.random.NextBool()));
						_renovation.location.temporarySprites.Add(new TemporaryAnimatedSprite(362, Game1.random.Next(30, 90) - 64, 6, 1, new Vector2(x, y), false, Game1.random.NextBool()));
					}
				}
			}
		...
		public virtual void CompleteRenovation(int selected_index)
		{
			...
			if (Game1.player.Money < _renovation.Price)
			{
				Game1.playSound("cancel");
				return;
			}
			bool isRefund = _renovation.Price < 0;
			if (!isRefund || Game1.player.mailReceived.Contains("FirstPurchase_" + _renovation.RoomId))
			{
				if (isRefund)
				{
					Game1.player._money -= _renovation.Price;
				}
				else
				{
					Game1.player.Money -= _renovation.Price;
					Game1.player.mailReceived.Add("FirstPurchase_" + _renovation.RoomId);
				}
			}
			freeze = true;
			if (_renovation.animationType == HouseRenovation.AnimationType.Destroy)
