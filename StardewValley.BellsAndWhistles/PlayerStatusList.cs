...
namespace StardewValley.BellsAndWhistles
{
	public class PlayerStatusList : INetObject<NetFields>
	{
		...
		public void InitNetFields()
		{
			...
			{
				NetString value2;
				if (_statusList.FieldDict.TryGetValue(key, out value2))
				{
		...
		public void AddSpriteDefinition(string key, string file, int x, int y, int width, int height)
		{
			...
			if (!_iconSprites.TryGetValue(file, out iconSprite) || iconSprite.IsDisposed)
			{
		...
		{
			string oldState;
			if (!_statusList.TryGetValue(Game1.player.UniqueMultiplayerID, out oldState) || oldState != newState)
			{
		...
		{
			string statusText;
			if (!TryGetStatusText(id, out statusText))
			{
				return fallback;
			}
			return statusText;
		...
		public int GetStatusInt(long id, int fallback = 0)
		{
			string statusText;
			int status;
			if (!TryGetStatusText(id, out statusText) || !int.TryParse(statusText, out status))
			{
				return fallback;
			}
			return status;
		...
		public void Draw(SpriteBatch b, Vector2 draw_position, float draw_scale = 4f, float draw_layer = 0.45f, HorizontalAlignment horizontal_origin = HorizontalAlignment.Left, VerticalAlignment vertical_origin = VerticalAlignment.Top)
		{
			...
			if (horizontal_origin == HorizontalAlignment.Right)
			{
				...
				else
				{
					...
					{
						string state2;
						if (_formattedStatusList.TryGetValue(farmer2.UniqueMultiplayerID, out state2))
						{
			...
			foreach (Farmer farmer in _sortedFarmers)
			{
				...
				string state;
				if (_formattedStatusList.TryGetValue(farmer.UniqueMultiplayerID, out state))
				{
					...
					if (displayMode == DisplayMode.Icons && _iconDefinitions.TryGetValue(state, out spriteDefinition))
					{
