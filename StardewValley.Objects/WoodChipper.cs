	...
	public class WoodChipper : Object
	{
		...
		public override void addWorkingAnimation()
		{
			...
			if (environment != null && environment.farmers.Any() && Game1.random.NextDouble() < 0.35)
			{
				for (int i = 0; i < 8; i++)
				{
					environment.temporarySprites.Add(new TemporaryAnimatedSprite(47, tileLocation.Value * 64f + new Vector2(0f, -76 + Game1.random.Next(-48, 0)), new Color(200, 110, 17), 8, false, 50f, 0, -1, 0.003f + Math.Max(0f, ((tileLocation.Y + 1f) * 64f - 24f) / 10000f) + tileLocation.X * 1E-05f)
					{
		...
		public override bool performObjectDropInAction(Item dropInItem, bool probe, Farmer who, bool returnFalseIfItemConsumed = false)
		{
			...
			if (PlaceInMachine(GetMachineData(), dropped_in_object, probe, who))
			{
				if (!probe)
				{
					...
					for (int i = 0; i < 12; i++)
					{
						location.temporarySprites.Add(new TemporaryAnimatedSprite(47, tileLocation.Value * 64f + new Vector2(0f, -76 + Game1.random.Next(-48, 0)), new Color(200, 110, 17), 8, false, 50f, 0, -1, 0.003f + Math.Max(0f, ((tileLocation.Y + 1f) * 64f - 24f) / 10000f) + tileLocation.X * 1E-05f)
						{
							delayBeforeAnimationStart = i * 100
						});
					}
		...
		public override void draw(SpriteBatch spriteBatch, int x, int y, float alpha = 1f)
		{
			...
			if (depositedItem.Value != null && shakeTimer > 0 && _isAnimatingChip)
			{
				...
				ParsedItemData itemDraw2 = ItemRegistry.GetDataOrErrorItem(depositedItem.Value.QualifiedItemId);
				Texture2D itemTexture2 = itemDraw2.GetTexture();
				spriteBatch.Draw(itemTexture2, draw_position, itemDraw2.GetSourceRect(), Color.White * alpha, 0f, new Vector2(8f, 8f), 4f * draw_scale, depositedItem.Value.flipped ? SpriteEffects.FlipHorizontally : SpriteEffects.None, draw_layer + 0.00175f);
			}
			...
			if (heldObject.Value != null)
			{
				...
				ColoredObject coloredObject = heldObject.Value as ColoredObject;
