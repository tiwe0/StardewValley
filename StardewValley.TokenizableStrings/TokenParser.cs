	...
	public class TokenParser
	{
		...
		public static class DefaultResolvers
		{
			...
			public static bool AchievementName(string[] query, out string replacement, Random random, Farmer player)
			{
				int achievementId;
				string error;
				if (!ArgUtility.TryGetInt(query, 1, out achievementId, out error))
				{
				...
				if (!Game1.achievements.TryGetValue(achievementId, out data))
				{
					...
					defaultInterpolatedStringHandler.AppendFormatted(achievementId);
					defaultInterpolatedStringHandler.AppendLiteral("'");
					return LogTokenError(query, defaultInterpolatedStringHandler.ToStringAndClear(), out replacement);
				}
			...
			public static bool ArticleFor(string[] query, out string replacement, Random random, Farmer player)
			{
				string word;
				string error;
				if (!ArgUtility.TryGet(query, 1, out word, out error))
				{
					return LogTokenError(query, error, out replacement);
				}
			...
			public static bool GenderedText(string[] query, out string replacement, Random random, Farmer player)
			{
				...
				string error;
				string femaleStr;
				if (!ArgUtility.TryGet(query, 1, out maleStr, out error) || !ArgUtility.TryGet(query, 2, out femaleStr, out error))
				{
					return LogTokenError(query, error, out replacement);
				}
			...
			public static bool ItemName(string[] query, out string replacement, Random random, Farmer player)
			{
				...
				string error;
				string fallbackText;
				if (!ArgUtility.TryGet(query, 1, out itemId, out error) || !ArgUtility.TryGetOptional(query, 2, out fallbackText, out error))
				{
					return LogTokenError(query, error, out replacement);
				}
			...
			public static bool LocalizedText(string[] query, out string replacement, Random random, Farmer player)
			{
				string key;
				string error;
				if (!ArgUtility.TryGet(query, 1, out key, out error))
				{
					return LogTokenError(query, error, out replacement);
				}
				object[] replacements;
			...
			public static bool MonsterName(string[] query, out string replacement, Random random, Farmer player)
			{
				...
				string error;
				string fallbackText;
				if (!ArgUtility.TryGet(query, 1, out monsterId, out error) || !ArgUtility.TryGetOptional(query, 2, out fallbackText, out error))
				{
					return LogTokenError(query, error, out replacement);
				}
				string data;
				replacement = (DataLoader.Monsters(Game1.content).TryGetValue(monsterId, out data) ? ArgUtility.Get(data.Split('/'), 14) : null);
			...
			public static bool MovieName(string[] query, out string replacement, Random random, Farmer player)
			{
				string movieId;
				string error;
				if (!ArgUtility.TryGet(query, 1, out movieId, out error))
				{
					return LogTokenError(query, error, out replacement);
				}
				MovieData data;
			...
			public static bool NumberWithSeparators(string[] query, out string replacement, Random random, Farmer player)
			{
				int number;
				string error;
				if (!ArgUtility.TryGetInt(query, 1, out number, out error))
				{
					return LogTokenError(query, error, out replacement);
				}
			...
			public static bool SpecialOrderName(string[] query, out string replacement, Random random, Farmer player)
			{
				string orderId;
				string error;
				if (!ArgUtility.TryGet(query, 1, out orderId, out error))
				{
					return LogTokenError(query, error, out replacement);
				}
			...
			public static bool SpouseFarmerText(string[] query, out string replacement, Random random, Farmer player)
			{
				...
				string error;
				string npcSpouse;
				if (!ArgUtility.TryGet(query, 1, out playerSpouse, out error) || !ArgUtility.TryGet(query, 2, out npcSpouse, out error))
				{
					return LogTokenError(query, error, out replacement);
				}
			...
			public static bool ToolName(string[] query, out string replacement, Random random, Farmer player)
			{
				...
				string error;
				int upgradeLevel;
				if (!ArgUtility.TryGet(query, 1, out itemId, out error) || !ArgUtility.TryGetOptionalInt(query, 2, out upgradeLevel, out error, -1))
				{
					return LogTokenError(query, error, out replacement);
				}
			...
			public static bool LocationName(string[] query, out string replacement, Random random, Farmer player)
			{
				string locationKey;
				string error;
				if (!ArgUtility.TryGet(query, 1, out locationKey, out error))
				{
					return LogTokenError(query, error, out replacement);
				}
			...
			public static bool FarmerStat(string[] query, out string replacement, Random random, Farmer player)
			{
				string statName;
				string error;
				if (!ArgUtility.TryGet(query, 1, out statName, out error))
				{
					return LogTokenError(query, error, out replacement);
				}
		...
		private static int ParseTagStartingAt(ref string text, int startIndex, Random random, TokenParserDelegate customParser, Farmer player)
		{
			for (int i = startIndex + 1; i < text.Length; i++)
			{
				...
				{
					string replacement;
					if (ParseTag(text.Substring(startIndex + 1, i - startIndex - 1), out replacement, random, customParser, player))
					{
		...
		private static bool ParseTag(string tag, out string replacement, Random random, TokenParserDelegate customParser, Farmer player)
		{
			...
			if (Parsers.TryGetValue(tagSplit[0], out parser) && parser(tagSplit, out replacement, random, player))
			{
				return true;
			}
			replacement = null;
