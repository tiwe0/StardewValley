...
namespace StardewValley.BellsAndWhistles
{
	public class ParrotPlatform
	{
		...
		public static List<ParrotPlatform> CreateParrotPlatformsForArea(GameLocation location)
		{
			...
			foreach (KeyValuePair<string, KeyValuePair<string, Point>> destination in GetDestinations(false))
			{
			...
			: this()
		{
			...
			parrots.Add(new Parrot(this, 15, 20, false, false));
			parrots.Add(new Parrot(this, 33, 20, true, false));
		}
		...
		public virtual void StartDeparture()
		{
			...
			_takeoffEvent = new Event("continue/follow/farmer " + Game1.player.TilePoint.X + " " + Game1.player.TilePoint.Y + " " + Game1.player.facingDirection?.ToString() + "/playerControl parrotRide", null, "-1")
			{
				showWorldCharacters = true,
				showGroundObjects = true
			};
			Game1.currentLocation.currentEvent = _takeoffEvent;
			_takeoffEvent.Update(Game1.player.currentLocation, Game1.currentGameTime);
		...
		public virtual void Update(GameTime time)
		{
			...
			{
				bool on_activation_tile = new Microsoft.Xna.Framework.Rectangle((int)position.X / 64, (int)position.Y / 64, 3, 1).Contains(Game1.player.TilePoint);
				if (_onActivationTile != on_activation_tile)
				{
					_onActivationTile = on_activation_tile;
					if (_onActivationTile && Game1.netWorldState.Value.ParrotPlatformsUnlocked)
					{
						Activate();
					}
				}
