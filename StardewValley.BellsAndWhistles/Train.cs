	...
	public class Train : INetObject<NetFields>
	{
		...
		public bool Update(GameTime time, GameLocation location)
		{
			...
			if (Game1.random.NextDouble() < 0.001 && location.Equals(Game1.currentLocation))
			{
				...
				whistleSteam = new TemporaryAnimatedSprite(27, new Vector2(position.X - 250f, 2624f), Color.White, 8, false, 100f, 0, 64, 1f, 64);
			}
			...
			if (smokeTimer <= 0f)
			{
				location.temporarySprites.Add(new TemporaryAnimatedSprite(25, new Vector2(position.X - 170f, 2496f), Color.White, 8, false, 100f, 0, 64, 1f, 128));
				smokeTimer = speed * 2000f;
			}
