	...
	public class AnimalQueryMenu : IClickableMenu
	{
		...
		public override void receiveLeftClick(int x, int y, bool playSound = true)
		{
			...
			if (movingAnimal)
			{
				...
				Vector2 clickTile = new Vector2((Game1.viewport.X + Game1.getOldMouseX(false)) / 64, (Game1.viewport.Y + Game1.getOldMouseY(false)) / 64);
			...
			if (confirmingSell)
			{
				if (yesButton.containsPoint(x, y))
				{
					...
					((AnimalHouse)animal.home.GetIndoors()).animalsThatLiveHere.Remove(animal.myID.Value);
		...
		public override void draw(SpriteBatch b)
		{
			if (!movingAnimal && !Game1.globalFade)
			{
				...
				Game1.drawDialogueBox(xPositionOnScreen, yPositionOnScreen + 128, width, height - 128, false, true);
				textBox.Draw(b);
				...
				{
					Utility.drawWithShadow(b, Game1.objectSpriteSheet_2, new Vector2((float)(xPositionOnScreen + width) - 105.6f, (float)yPositionOnScreen + 224f), new Microsoft.Xna.Framework.Rectangle(16, 240, 16, 16), Color.White, 0f, Vector2.Zero, 4f, false, 0.89f);
				}
				if (confirmingSell)
				{
					if (!Game1.options.showClearBackgrounds)
			...
			else if (!Game1.globalFade)
			{
				...
				Game1.drawDialogueBox(32, -64, (int)Game1.dialogueFont.MeasureString(s).X + IClickableMenu.borderWidth * 2 + 16, 128 + IClickableMenu.borderWidth * 2, false, true);
