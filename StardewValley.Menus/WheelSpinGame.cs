		...
		public override void update(GameTime time)
		{
			...
			if (timerBeforeStart <= 0)
			{
				...
				if (arrowRotationVelocity <= Math.PI / 80.0 && oldVelocity > Math.PI / 80.0)
				{
					...
					if (arrowRotation > Math.PI / 2.0 && arrowRotation <= 4.319689898685965 && Game1.random.NextDouble() < (double)((float)Game1.player.LuckLevel / 15f))
					{
						if (colorChoiceGreen)
						{
					...
					else if ((arrowRotation + Math.PI) % (Math.PI * 2.0) <= 4.319689898685965 && !colorChoiceGreen && Game1.random.NextDouble() < (double)((float)Game1.player.LuckLevel / 20f))
					{
						arrowRotationVelocity = Math.PI / 48.0;
						Game1.playSound("dwop");
					}
