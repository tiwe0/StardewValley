	...
	public class WitchEvent : BaseFarmEvent
	{
			...
			if (targetBuilding == null)
			{
				foreach (Building b in f.buildings)
				{
					if (b.buildingType.Value == "Slime Hutch")
					{
						...
						if (indoors.characters.Count > 0 && r.NextBool() && indoors.numberOfObjectsOfType("83", true) == 0)
						{
							targetBuilding = b;
						}
					}
		...
		public override void makeChangesToLocation()
		{
			...
			if (targetBuilding.buildingType.Value == "Slime Hutch")
			{
				...
				{
					GreenSlime slime = character as GreenSlime;
					if (slime != null)
					{
			...
			for (int tries = 0; tries < 200; tries++)
			{
				...
				if ((indoors.CanItemBePlacedHere(v) || (indoors.terrainFeatures.TryGetValue(v, out terrainFeature) && terrainFeature is Flooring)) && !indoors.objects.ContainsKey(v))
				{
