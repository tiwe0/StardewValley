...
namespace StardewValley.Tools
{
	public class FishingRod : Tool
	{
		...
		public FishingRod()
			: base("Fishing Rod", 0, 189, 8, false, 2)
		{
		}
		...
		public FishingRod(int upgradeLevel)
			: base("Fishing Rod", upgradeLevel, 189, 8, false, (upgradeLevel == 4) ? 3 : 2)
		{
			base.IndexOfMenuItemView = 8 + upgradeLevel;
		}

		public FishingRod(int upgradeLevel, int numAttachmentSlots)
			: base("Fishing Rod", upgradeLevel, 189, 8, false, numAttachmentSlots)
		{
			base.IndexOfMenuItemView = 8 + upgradeLevel;
		}
		...
		public override void DoFunction(GameLocation location, int x, int y, int power, Farmer who)
		{
			...
			if (!isFishing && !castedButBobberStillInAir && !pullingOutOfWater && !isNibbling && !hit && !showingTreasure)
			{
				...
				if (location.canFishHere() && location.isTileFishable(tileX, tileY))
				{
					...
					isFishing = true;
					location.temporarySprites.Add(new TemporaryAnimatedSprite(28, 100f, 2, 1, new Vector2(bobber.X - 32f, bobber.Y - 32f), false, false));
					if (who.IsLocalPlayer)
					{
						location.playSound("dropItemInWater", bobberTile);
						Game1.stats.TimesFished++;
					}
					timeUntilFishingBite = calculateTimeUntilFishingBite(bobberTile, true, who);
			...
			else
			{
				...
				if (isNibbling)
				{
					...
					if (isJunk || fromFishPond || non_fishable_fish)
					{
						...
						pullFishFromWater(o.QualifiedItemId, -1, 0, 0, false, false, fromFishPond, o.SetFlagOnPickup, false, 1);
					}
					else if (!hit && who.IsLocalPlayer)
					{
						hit = true;
						Game1.screenOverlayTempSprites.Add(new TemporaryAnimatedSprite("LooseSprites\\Cursors", new Rectangle(612, 1913, 74, 30), 1500f, 1, 0, Game1.GlobalToLocal(Game1.viewport, bobber.Value + new Vector2(-140f, -160f)), false, false, 1f, 0.005f, Color.White, 4f, 0.075f, 0f, 0f, true)
						{
				...
				if (fromFishPond)
				{
					...
					{
						pullFishFromWater(fishPondPull.QualifiedItemId, -1, 0, 0, false, false, true, null, false, 1);
						return;
					}
				}
				if (who.IsLocalPlayer)
				...
				if (who.FacingDirection == 1 || who.FacingDirection == 3)
				{
					...
					animations.Add(new TemporaryAnimatedSprite("TileSheets\\bobbers", sourceRect, t, 1, 0, bobber.Value + new Vector2(-32f, -48f), false, false, (float)playerPixel.Y / 10000f, 0f, Color.White, 4f, 0f, 0f, (float)Game1.random.Next(-20, 20) / 100f)
					{
				...
				else
				{
					...
					Rectangle sourceRect2 = Game1.getSourceRectForStandardTileSheet(Game1.bobbersTexture, getBobberStyle(who), 16, 32);
					sourceRect2.Height = 16;
					animations.Add(new TemporaryAnimatedSprite("TileSheets\\bobbers", sourceRect2, t2, 1, 0, bobber.Value + new Vector2(-32f, -48f), false, false, bobber.Y / 10000f, 0f, Color.White, 4f, 0f, 0f, (float)Game1.random.Next(-20, 20) / 100f)
					{
						motion = new Vector2(((float)who.StandingPixel.X - bobber.Value.X) / 800f, 0f - velocity2),
		...
		private float calculateTimeUntilFishingBite(Vector2 bobberTile, bool isFirstCast, Farmer who)
		{
			if (Game1.currentLocation.isTileBuildingFishable((int)bobberTile.X, (int)bobberTile.Y))
			{
				...
				if (pond != null && (int)pond.currentOccupants > 0)
				{
		...
		private void doPullFishFromWater(BinaryReader argReader)
		{
			...
			if (who.FacingDirection == 1 || who.FacingDirection == 3)
			{
				...
				animations.Add(new TemporaryAnimatedSprite(sprite_sheet_name, sprite_rect, t, 1, 0, bobber.Value, false, false, bobber.Y / 10000f, 0f, Color.White, 4f, 0f, 0f, 0f)
				{
			...
			else
			{
				...
				float distance2 = bobber.Y - (float)(playerStandingY - 64);
				...
				if (t != 0f)
				{
					xVelocity2 = (who.Position.X - bobber.X) / t;
				}
				animations.Add(new TemporaryAnimatedSprite(sprite_sheet_name, sprite_rect, t, 1, 0, bobber.Value, false, false, bobber.Y / 10000f, 0f, Color.White, 4f, 0f, 0f, 0f)
				{
				...
				if (numberOfFishCaught > 1)
				{
					...
					{
						distance2 = bobber.Y - (float)(playerStandingY - 64);
						...
						animations.Add(new TemporaryAnimatedSprite(sprite_sheet_name, sprite_rect, t, 1, 0, new Vector2(bobber.X, bobber.Y), false, false, bobber.Y / 10000f, 0f, Color.White, 4f, 0f, 0f, 0f)
						{
							motion = new Vector2(xVelocity2, 0f - velocity),
							acceleration = new Vector2(0f, gravity2),
							timeBasedMotion = true,
		...
		public override void draw(SpriteBatch b)
		{
			...
			{
				Vector2 bobberPos2 = bobber.Value;
				if (bobberTimeAccumulator > timePerBobberBob)
				{
					if ((!isNibbling && !isReeling) || Game1.random.NextDouble() < 0.05)
					{
						...
						who.currentLocation.temporarySprites.Add(new TemporaryAnimatedSprite("TileSheets\\animations", new Rectangle(0, 0, 64, 64), 150f, 8, 0, new Vector2(bobber.X - 32f, bobber.Y - 16f), false, Game1.random.NextBool(), 0.001f, 0.01f, Color.White, 0.75f, 0.003f, 0f, 0f));
					}
				...
				position.Height = 16;
				position.Y += 16;
				b.Draw(Game1.bobbersTexture, Game1.GlobalToLocal(Game1.viewport, bobberPos2), position, Color.White, 0f, new Vector2(8f, 8f), scale, (getLastFarmerToUse().FacingDirection == 1) ? SpriteEffects.FlipHorizontally : SpriteEffects.None, bobberLayerDepth);
			...
			for (int k = animations.Count - 1; k >= 0; k--)
			{
				animations[k].draw(b);
			}
			...
			if (!bobber.Value.Equals(Vector2.Zero) && (isFishing || pullingOutOfWater || castedButBobberStillInAir) && who.FarmerSprite.CurrentFrame != 57 && (who.FacingDirection != 0 || !pullingOutOfWater || whichFish == null))
			{
				...
				{
					if (who != null && who.IsLocalPlayer && Game1.didPlayerJustClickAtAll())
					{
						switch (who.FacingDirection)
						{
						case 0:
				...
				else
				{
					...
					{
					case 0:
						lastPosition = (pullingOutOfWater ? Game1.GlobalToLocal(Game1.viewport, who.Position + new Vector2(22f, who.armOffset.Y - 96f + 4f)) : Game1.GlobalToLocal(Game1.viewport, who.Position + new Vector2(28f, who.armOffset.Y - 64f - 12f)));
						break;
					case 2:
						lastPosition = (pullingOutOfWater ? Game1.GlobalToLocal(Game1.viewport, who.Position + new Vector2(8f, who.armOffset.Y - 96f + 4f)) : Game1.GlobalToLocal(Game1.viewport, who.Position + new Vector2(28f, who.armOffset.Y + 64f - 12f)));
						break;
					case 1:
						lastPosition = (pullingOutOfWater ? Game1.GlobalToLocal(Game1.viewport, who.Position + new Vector2(-48f, who.armOffset.Y - 96f - 8f)) : Game1.GlobalToLocal(Game1.viewport, who.Position + new Vector2(120f, who.armOffset.Y - 64f + 16f)));
						break;
					case 3:
						lastPosition = (pullingOutOfWater ? Game1.GlobalToLocal(Game1.viewport, who.Position + new Vector2(112f, who.armOffset.Y - 96f - 8f)) : Game1.GlobalToLocal(Game1.viewport, who.Position + new Vector2(-56f, who.armOffset.Y - 64f + 16f)));
						break;
					default:
						lastPosition = Vector2.Zero;
						break;
				...
				{
					Vector2 current = Utility.GetCurvePoint(j, v1, v2, v3, v4);
		...
		public override void tickUpdate(GameTime time, Farmer who)
		{
			...
			if (castingChosenCountdown > 0f)
			{
				...
				if (castingChosenCountdown <= 0f && who.CurrentTool != null)
				{
					...
					if (who.FacingDirection == 1 || who.FacingDirection == 3)
					{
						...
						if (who.IsLocalPlayer)
						{
							bobber.Set(new Vector2((float)playerPixel3.X + (float)((who.FacingDirection != 3) ? 1 : (-1)) * distance2, playerPixel3.Y));
						}
						Rectangle sourceRect2 = Game1.getSourceRectForStandardTileSheet(Game1.bobbersTexture, getBobberStyle(who), 16, 32);
						sourceRect2.Height = 16;
						animations.Add(new TemporaryAnimatedSprite("TileSheets\\bobbers", sourceRect2, t2, 1, 0, who.Position + new Vector2(0f, -96f), false, false, (float)playerPixel3.Y / 10000f, 0f, Color.White, 4f, 0f, 0f, (float)Game1.random.Next(-20, 20) / 100f)
						{
					...
					else
					{
						...
						if (who.IsLocalPlayer)
						{
							...
							bobber.Set(new Vector2(playerPixel2.X, (float)playerPixel2.Y - distance));
						}
						Rectangle sourceRect = Game1.getSourceRectForStandardTileSheet(Game1.bobbersTexture, getBobberStyle(who), 16, 32);
						sourceRect.Height = 16;
						animations.Add(new TemporaryAnimatedSprite("TileSheets\\bobbers", sourceRect, t, 1, 0, who.Position + new Vector2(0f, -96f), false, false, bobber.Y / 10000f, 0f, Color.White, 4f, 0f, 0f, (float)Game1.random.Next(-20, 20) / 100f)
						{
							alphaFade = 0.0001f,
			...
			else if (isFishing)
			{
				...
				if (timeUntilFishingBite != -1f)
				{
					...
					if (fishingBiteAccumulator > timeUntilFishingBite)
					{
						...
						timeUntilFishingNibbleDone = maxTimeToNibble;
						Point playerPixel = who.StandingPixel;
						Game1.screenOverlayTempSprites.Add(new TemporaryAnimatedSprite("LooseSprites\\Cursors", new Rectangle(395, 497, 3, 8), new Vector2(playerPixel.X - Game1.viewport.X, playerPixel.Y - 128 - 8 - Game1.viewport.Y), false, 0.02f, Color.White)
						{
							scale = 5f,
							scaleChange = -0.01f,
				...
				if (timeUntilFishingNibbleDone != -1f && !hit)
				{
					...
					if (fishingNibbleAccumulator > timeUntilFishingNibbleDone)
					{
						...
						timeUntilFishingNibbleDone = -1f;
						isNibbling = false;
						timeUntilFishingBite = calculateTimeUntilFishingBite(calculateBobberTile(), false, who);
					}
				}
			}
			...
			else if (fishCaught)
			{
				...
				{
					who.currentLocation.temporarySprites.Add(new TemporaryAnimatedSprite("LooseSprites\\Cursors", new Rectangle(653, 858, 1, 1), 9999f, 1, 1, who.Position + new Vector2(Game1.random.Next(-3, 2) * 4, -32f), false, false, (float)who.StandingPixel.Y / 10000f + 0.002f, 0.04f, Color.LightBlue, 5f, 0f, 0f, 0f)
					{
				...
				if (!treasureCaught && !gotTroutDerbyTag)
				{
					...
					bool fishIsObject = item2.HasTypeObject();
					...
					if (item2.QualifiedItemId == "(O)79" || item2.QualifiedItemId == "(O)842")
					{
						item2 = who.currentLocation.tryToCreateUnseenSecretNote(who);
						if (item2 == null)
						{
							return;
						}
					...
					if (!Game1.isFestival() && !caughtFromFishPond && fishIsObject && who.team.specialOrders != null)
					{
						...
						{
							specialOrder.onFishCaught?.Invoke(who, item2);
						}
					}
					if (!Game1.isFestival() && !who.addItemToInventoryBool(item2))
					{
						Game1.activeClickableMenu = new ItemGrabMenu(new List<Item> { item2 }, this).setEssential(true);
					}
					return;
				}
				fishCaught = false;
				...
				if (treasureCaught)
				{
					animations.Add(new TemporaryAnimatedSprite(goldenTreasure ? "LooseSprites\\Cursors_1_6" : "LooseSprites\\Cursors", goldenTreasure ? new Rectangle(256, 75, 32, 32) : new Rectangle(64, 1920, 32, 32), 500f, 1, 0, who.Position + new Vector2(-32f, -160f), false, false, (float)who.StandingPixel.Y / 10000f + 0.001f, 0f, Color.White, 4f, 0f, 0f, 0f)
					{
				...
				{
					animations.Add(new TemporaryAnimatedSprite("TileSheets\\Objects_2", new Rectangle(80, 16, 16, 16), 500f, 1, 0, who.Position + new Vector2(-8f, -128f), false, false, (float)who.StandingPixel.Y / 10000f + 0.001f, 0f, Color.White, 4f, 0f, 0f, 0f)
					{
						motion = new Vector2(0f, -0.128f),
						timeBasedMotion = true,
						endFunction = openChestEndFunction,
		...
		public void openChestEndFunction(int remainingFish)
		{
			...
			{
				who.playNearbySoundLocal("discoverMineral");
				animations.Add(new TemporaryAnimatedSprite("TileSheets\\Objects_2", new Rectangle(80, 16, 16, 16), 800f, 1, 0, who.Position + new Vector2(-8f, -196f), false, false, (float)who.StandingPixel.Y / 10000f + 0.001f, 0f, Color.White, 4f, 0f, 0f, 0f)
				{
			...
			else
			{
				...
				animations.Add(new TemporaryAnimatedSprite(goldenTreasure ? "LooseSprites\\Cursors_1_6" : "LooseSprites\\Cursors", goldenTreasure ? new Rectangle(256, 75, 32, 32) : new Rectangle(64, 1920, 32, 32), 200f, 4, 0, who.Position + new Vector2(-32f, -228f), false, false, (float)who.StandingPixel.Y / 10000f + 0.001f, 0f, Color.White, 4f, 0f, 0f, 0f)
				{
		...
		public void justGotDerbyTagEndFunction(int remainingFish)
		{
			...
			if (!who.addItemToInventoryBool(tag))
			{
				...
				ItemGrabMenu itemGrabMenu = new ItemGrabMenu(items, this).setEssential(true);
			...
			{
				ItemGrabMenu itemGrabMenu2 = new ItemGrabMenu(new List<Item> { fish }, this).setEssential(true);
		...
		public void openTreasureMenuEndFunction(int remainingFish)
		{
			...
			ItemGrabMenu itemGrabMenu = new ItemGrabMenu(treasures, this).setEssential(true);
			itemGrabMenu.source = 3;
			Game1.activeClickableMenu = itemGrabMenu;
			who.completelyStopAnimatingOrDoingAction();
		}
