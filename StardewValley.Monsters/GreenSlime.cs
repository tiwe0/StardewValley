	...
	public class GreenSlime : Monster
	{
			...
			: base("Green Slime", position)
		{
			...
			if (mineLevel < 40)
			{
				...
				color.Value = new Color(green2 / Game1.random.Next(2, 10), green2, (Game1.random.NextDouble() < 0.01) ? 255 : (255 - green2));
			...
			if (Game1.random.NextDouble() < 0.001)
			{
				...
				double extraChance = (double)(int)(Game1.stats.DaysPlayed / 28u) * 0.08;
		...
		public virtual void OnAttacked(Vector2 trajectory)
		{
			if (Game1.IsMasterGame && stackedSlimes.Value > 0)
			{
				...
				trajectory *= 16f;
				BasicProjectile projectile = new BasicProjectile(base.DamageToFarmer / 3 * 2, 13, 3, 0, (float)Math.PI / 16f, trajectory.X, trajectory.Y, base.Position, null, null, null, true, false, base.currentLocation, this);
		...
		public void moveTowardOtherSlime(GreenSlime other, bool moveAway, GameTime time)
		{
			...
			if (xToGo > 4 || yToGo > 4)
			{
				...
				{
					tryToMoveInDirection((dx > 0) ? 1 : 3, false, base.DamageToFarmer, false);
				}
				else
				{
					tryToMoveInDirection((dy > 0) ? 2 : 0, false, base.DamageToFarmer, false);
				}
			}
		...
		public override void behaviorAtGameTick(GameTime time)
		{
			...
			if ((bool)pursuingMate && mate != null)
			{
				...
				moveTowardOtherSlime(mate, false, time);
			...
			{
				moveTowardOtherSlime(mate, true, time);
				return;
			}
			...
			else if (Game1.random.NextDouble() < 0.1 && !base.focusedOnFarmers)
			{
				...
				{
					if ((bool)leftDrift && !base.currentLocation.isCollidingPosition(nextPosition(3), Game1.viewport, false, 1, false, this))
					{
						position.X -= base.speed;
					}
					else if (!leftDrift && !base.currentLocation.isCollidingPosition(nextPosition(1), Game1.viewport, false, 1, false, this))
					{
						position.X += base.speed;
					}
				}
				else if ((bool)leftDrift && !base.currentLocation.isCollidingPosition(nextPosition(0), Game1.viewport, false, 1, false, this))
				{
					position.Y -= base.speed;
				}
				else if (!leftDrift && !base.currentLocation.isCollidingPosition(nextPosition(2), Game1.viewport, false, 1, false, this))
				{
					position.Y += base.speed;
				}
			...
			else if (withinPlayerThreshold() && timeSinceLastJump > (base.focusedOnFarmers ? 1000 : 4000) && Game1.random.NextDouble() < 0.01 && stackedSlimes.Value <= 0)
			{
				...
				{
					addedSpeed = 2f;
					startGlowing(Color.Cyan, false, 0.15f);
				}
				else
				{
					addedSpeed = 0f;
