	...
	public class DialogueBox : IClickableMenu
	{
		...
		public override void draw(SpriteBatch b)
		{
			...
			if (isQuestion)
			{
				...
				if (characterIndexInDialogue >= getCurrentString().Length - 1)
				{
					...
					for (int i = 0; i < responses.Length; i++)
					{
						...
						{
							IClickableMenu.drawTextureBox(b, Game1.mouseCursors, new Rectangle(375, 357, 3, 3), x + 4, responseY - 8, width - 8, SpriteText.getHeightOfString(responses[i].responseText, width) + 16, Color.White, 4f, false);
						}
			...
			if (aboveDialogueImage != null)
			{
				...
				Utility.drawWithShadow(b, aboveDialogueImage.texture, new Vector2((float)(x + width / 2) - (float)(aboveDialogueImage.sourceRect.Width / 2) * aboveDialogueImage.scale, y - 64 - (int)((float)aboveDialogueImage.sourceRect.Height * aboveDialogueImage.scale)), aboveDialogueImage.sourceRect, Color.White, 0f, Vector2.Zero, aboveDialogueImage.scale, false, 1f);
			}
