	...
	public class GalaxySocket
	{
		...
		private LobbyType privacyToLobbyType(ServerPrivacy privacy)
		{
			...
			default:
			{
				...
				defaultInterpolatedStringHandler.AppendFormatted(privacy);
		...
		private void tryCreateLobby()
		{
			...
			try
			{
				GalaxyInstance.Matchmaking().CreateLobby(privacyToLobbyType(privacy), memberLimit, true, LobbyTopologyType.LOBBY_TOPOLOGY_TYPE_STAR);
			}
			catch (Exception e)
			{
		...
		private void onGalaxyMemberState(GalaxyID lobbyID, GalaxyID memberID, LobbyMemberStateChange memberStateChange)
		{
			...
			case LobbyMemberStateChange.LOBBY_MEMBER_STATE_CHANGED_ENTERED:
			{
				...
				defaultInterpolatedStringHandler.AppendFormatted(memberID);
		...
		private void onGalaxyLobbyLeft(GalaxyID lobbyID, ILobbyLeftListener.LobbyLeaveReason leaveReason)
		{
			...
			{
				Program.WriteLog(Program.LogType.Disconnect, "Forcibly left Galaxy lobby at " + DateTime.Now.ToLongTimeString() + " - " + leaveReason, true);
			}
