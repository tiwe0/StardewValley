...
namespace StardewValley
{
	public class Farm : GameLocation
	{
		...
		public virtual Vector2 GetStarterShippingBinLocation()
		{
			...
			{
				Vector2 position;
				if (!TryGetMapPropertyAs("ShippingBinLocation", out position, false))
				{
		...
		{
			Vector2 tile;
			if (!TryGetMapPropertyAs("PetBowlLocation", out tile, false))
			{
		...
		{
			Vector2 position;
			if (TryGetMapPropertyAs("GreenhouseLocation", out position, false))
			{
				return position;
			}
		...
		public void onNewGame()
		{
			...
			else if (Game1.whichFarm == 7 && Game1.whichModFarm.Id == "MeadowlandsFarm")
			{
				for (int x = 47; x < 63; x++)
				{
					objects.Add(new Vector2(x, 20f), new Fence(new Vector2(x, 20f), "322", false));
				}
				for (int y2 = 16; y2 < 20; y2++)
				{
					objects.Add(new Vector2(47f, y2), new Fence(new Vector2(47f, y2), "322", false));
				}
				for (int y = 7; y < 20; y++)
				{
		...
		public override void DayUpdate(int dayOfMonth)
		{
			...
			{
				Pet pet = characters[i] as Pet;
				if (pet != null && (getTileIndexAt(pet.TilePoint, "Buildings") != -1 || getTileIndexAt(pet.TilePoint.X + 1, pet.TilePoint.Y, "Buildings") != -1 || !CanSpawnCharacterHere(pet.Tile) || !CanSpawnCharacterHere(new Vector2(pet.TilePoint.X + 1, pet.TilePoint.Y))))
				{
					pet.WarpToPetBowl();
				}
			}
			...
			{
				if (CanItemBePlacedHere(new Vector2(5f, 32f), false, CollisionMask.All, CollisionMask.None) && CanItemBePlacedHere(new Vector2(6f, 32f), false, CollisionMask.All, CollisionMask.None) && CanItemBePlacedHere(new Vector2(6f, 33f), false, CollisionMask.All, CollisionMask.None) && CanItemBePlacedHere(new Vector2(5f, 33f), false, CollisionMask.All, CollisionMask.None))
				{
				...
				if (objects.Length > 0)
				{
					...
					{
						Object o2;
						if (Utility.TryGetRandom(objects, out key, out o2) && o2.IsWeeds() && o2.tileLocation.X < 36f && o2.tileLocation.Y < 34f)
						{
			...
			if (ShouldSpawnBeachFarmForage())
			{
				while (Game1.random.NextDouble() < 0.9)
				{
					...
					if (!CanItemBePlacedHere(v2) || getTileIndexAt((int)v2.X, (int)v2.Y, "AlwaysFront") != -1)
					{
						continue;
					}
					string whichItem2 = null;
					if (doesTileHavePropertyNoNull((int)v2.X, (int)v2.Y, "BeachSpawn", "Back") != "")
					{
						...
						if (Game1.stats.DaysPlayed > 1)
						{
							...
							{
								whichItem2 = Game1.random.Next(922, 925).ToString();
								objects.Add(v2, new Object(whichItem2, 1)
								{
									Fragility = 2,
									MinutesUntilReady = 3
								});
					...
					else if (Game1.season != Season.Winter && new Microsoft.Xna.Framework.Rectangle(20, 66, 33, 18).Contains((int)v2.X, (int)v2.Y) && doesTileHavePropertyNoNull((int)v2.X, (int)v2.Y, "Type", "Back") == "Grass")
					{
						whichItem2 = Utility.getRandomBasicSeasonalForageItem(Game1.season, (int)Game1.stats.DaysPlayed);
					}
					if (whichItem2 != null)
					{
						Object obj2 = ItemRegistry.Create<Object>("(O)" + whichItem2);
						obj2.CanBeSetDown = false;
						obj2.IsSpawnedObject = true;
						dropObject(obj2, v2 * 64f, Game1.viewport, true);
					}
				}
			}
			if (Game1.whichFarm == 2)
			{
				for (int x = 0; x < 20; x++)
				{
					...
					{
						if (getTileIndexAt(x, y, "Paths") == 21 && CanItemBePlacedHere(new Vector2(x, y), false, CollisionMask.All, CollisionMask.None) && CanItemBePlacedHere(new Vector2(x + 1, y), false, CollisionMask.All, CollisionMask.None) && CanItemBePlacedHere(new Vector2(x + 1, y + 1), false, CollisionMask.All, CollisionMask.None) && CanItemBePlacedHere(new Vector2(x, y + 1), false, CollisionMask.All, CollisionMask.None))
						{
			...
			if (ShouldSpawnForestFarmForage() && !Game1.IsWinter)
			{
				while (Game1.random.NextDouble() < 0.75)
				{
					...
					if (!CanItemBePlacedHere(v, false, CollisionMask.All, CollisionMask.None) || getTileIndexAt((int)v.X, (int)v.Y, "AlwaysFront") != -1 || ((Game1.whichFarm != 2 || !(v.X < 18f)) && !doesTileHavePropertyNoNull((int)v.X, (int)v.Y, "Type", "Back").Equals("Grass")))
					{
						continue;
					}
					string whichItem;
					...
					Object obj = ItemRegistry.Create<Object>(whichItem);
					obj.CanBeSetDown = false;
					obj.IsSpawnedObject = true;
					dropObject(obj, v * 64f, Game1.viewport, true);
				}
				if (objects.Length > 0)
				{
					for (int j = 0; j < 6; j++)
					{
						Object o;
						if (Utility.TryGetRandom(objects, out key, out o) && o.IsWeeds())
						{
			...
			if (terrainFeatures.Length > 0 && Game1.season == Season.Fall && Game1.dayOfMonth > 1 && Game1.random.NextDouble() < 0.05)
			{
				for (int tries = 0; tries < 10; tries++)
				{
					...
					if (Utility.TryGetRandom(terrainFeatures, out tile, out feature))
					{
						...
						if (tree != null && (int)tree.growthStage >= 5 && !tree.tapped && !tree.isTemporaryGreenRainTree.Value)
						{
		...
		public void doDailyMountainFarmUpdate()
		{
			...
			while (Game1.random.NextDouble() < chance)
			{
				...
				if (doesTileHavePropertyNoNull((int)v.X, (int)v.Y, "Type", "Back").Equals("Dirt") && CanItemBePlacedHere(v, false, CollisionMask.All, CollisionMask.None))
				{
					string stone_id = "668";
					int health = 2;
		...
		public void addCrows()
		{
			...
			{
				HoeDirt dirt2 = pair.Value as HoeDirt;
			...
			for (int i = 0; i < potentialCrows; i++)
			{
				...
				for (int attempts = 0; attempts < 10; attempts++)
				{
					...
					if (dirt == null || (int)dirt.crop?.currentPhase <= 1)
					{
						continue;
					}
		...
		public void grandpaStatueCallback(Item item, Farmer who)
		{
			...
			if (obj != null && obj.QualifiedItemId == "(BC)160")
			{
		...
		public void showShipment(Object o, bool playThrowSound = true)
		{
			...
			temporarySprites.Add(new TemporaryAnimatedSprite("LooseSprites\\Cursors", new Microsoft.Xna.Framework.Rectangle(524, 218, 34, 22), new Vector2(71f, 13f) * 64f + new Vector2(0f, 5f) * 4f, false, 0f, Color.White)
			{
				interval = 100f,
			...
			temporarySprites.Add(new TemporaryAnimatedSprite("LooseSprites\\Cursors", new Microsoft.Xna.Framework.Rectangle(524, 230, 34, 10), new Vector2(71f, 13f) * 64f + new Vector2(0f, 17f) * 4f, false, 0f, Color.White)
			{
				interval = 100f,
				totalNumberOfLoops = 1,
				animationLength = 3,
			...
			temporarySprites.Add(new TemporaryAnimatedSprite(itemData.TextureName, itemData.GetSourceRect(), new Vector2(71f, 13f) * 64f + new Vector2(8 + Game1.random.Next(6), 2f) * 4f, false, 0f, Color.White)
			{
				interval = 9999f,
				scale = 4f,
				alphaFade = 0.045f,
		...
		public override Item getFish(float millisecondsAfterNibble, string bait, int waterDepth, Farmer who, double baitPotency, Vector2 bobberTile, string location = null)
		{
			if (_fishLocationOverride == null)
			{
				...
				if (fields.Length != 0)
				{
					...
					string error;
					float chance;
					if (!ArgUtility.TryGet(fields, 0, out targetLocation, out error) || !ArgUtility.TryGetFloat(fields, 1, out chance, out error))
					{
		...
		protected override void resetLocalState()
		{
			...
			if (Game1.MasterPlayer.mailReceived.Contains("Farm_Eternal") && !Game1.player.mailReceived.Contains("Farm_Eternal_Parrots") && !IsRainingHere())
			{
				for (int k = 0; k < 20; k++)
				{
					temporarySprites.Add(new TemporaryAnimatedSprite("LooseSprites\\parrots", new Microsoft.Xna.Framework.Rectangle(49, 24 * Game1.random.Next(4), 24, 24), new Vector2(Game1.viewport.MaxCorner.X, Game1.viewport.Location.Y + Game1.random.Next(64, Game1.viewport.Height / 2)), false, 0f, Color.White)
					{
						scale = 4f,
		...
		public virtual Vector2 GetSpouseOutdoorAreaCorner()
		{
			...
			{
				Vector2 position;
				if (!TryGetMapPropertyAs("SpouseAreaLocation", out position, false))
				{
		...
		public void addGrandpaCandles()
		{
			...
			if ((int)grandpaScore > 0)
			{
				...
				temporarySprites.Add(new TemporaryAnimatedSprite("LooseSprites\\Cursors", candleSource, 99999f, 1, 9999, new Vector2((grandpa_shrine_location.X - 1) * 64 + 20, (grandpa_shrine_location.Y - 1) * 64 + 20), false, false, (float)((grandpa_shrine_location.Y - 1) * 64) / 10000f, 0f, Color.White, 4f, 0f, 0f, 0f));
				temporarySprites.Add(new TemporaryAnimatedSprite("LooseSprites\\Cursors", new Microsoft.Xna.Framework.Rectangle(536, 1945, 8, 8), new Vector2((grandpa_shrine_location.X - 1) * 64 + 12, (grandpa_shrine_location.Y - 1) * 64 - 4), false, 0f, Color.White)
				{
					interval = 50f,
				...
				{
					temporarySprites.Add(new TemporaryAnimatedSprite("LooseSprites\\Cursors", candleSource, 99999f, 1, 9999, new Vector2((grandpa_shrine_location.X - 1) * 64 + 40, (grandpa_shrine_location.Y - 2) * 64 + 24), false, false, (float)((grandpa_shrine_location.Y - 1) * 64) / 10000f, 0f, Color.White, 4f, 0f, 0f, 0f));
					temporarySprites.Add(new TemporaryAnimatedSprite("LooseSprites\\Cursors", new Microsoft.Xna.Framework.Rectangle(536, 1945, 8, 8), new Vector2((grandpa_shrine_location.X - 1) * 64 + 36, (grandpa_shrine_location.Y - 2) * 64), false, 0f, Color.White)
					{
						interval = 50f,
						totalNumberOfLoops = 99999,
						animationLength = 7,
				...
				{
					temporarySprites.Add(new TemporaryAnimatedSprite("LooseSprites\\Cursors", candleSource, 99999f, 1, 9999, new Vector2((grandpa_shrine_location.X + 1) * 64 + 20, (grandpa_shrine_location.Y - 2) * 64 + 24), false, false, (float)((grandpa_shrine_location.Y - 1) * 64) / 10000f, 0f, Color.White, 4f, 0f, 0f, 0f));
					temporarySprites.Add(new TemporaryAnimatedSprite("LooseSprites\\Cursors", new Microsoft.Xna.Framework.Rectangle(536, 1945, 8, 8), new Vector2((grandpa_shrine_location.X + 1) * 64 + 16, (grandpa_shrine_location.Y - 2) * 64), false, 0f, Color.White)
					{
						interval = 50f,
						totalNumberOfLoops = 99999,
						animationLength = 7,
				...
				{
					temporarySprites.Add(new TemporaryAnimatedSprite("LooseSprites\\Cursors", candleSource, 99999f, 1, 9999, new Vector2((grandpa_shrine_location.X + 1) * 64 + 40, (grandpa_shrine_location.Y - 1) * 64 + 20), false, false, (float)((grandpa_shrine_location.Y - 1) * 64) / 10000f, 0f, Color.White, 4f, 0f, 0f, 0f));
					temporarySprites.Add(new TemporaryAnimatedSprite("LooseSprites\\Cursors", new Microsoft.Xna.Framework.Rectangle(536, 1945, 8, 8), new Vector2((grandpa_shrine_location.X + 1) * 64 + 36, (grandpa_shrine_location.Y - 1) * 64 - 4), false, 0f, Color.White)
					{
						interval = 50f,
						totalNumberOfLoops = 99999,
						animationLength = 7,
			...
			{
				temporarySprites.Add(new TemporaryAnimatedSprite("LooseSprites\\Cursors2", new Microsoft.Xna.Framework.Rectangle(176, 157, 15, 16), 99999f, 1, 9999, new Vector2(grandpa_shrine_location.X * 64 + 4, (grandpa_shrine_location.Y - 2) * 64 - 24), false, false, (float)((grandpa_shrine_location.Y - 1) * 64) / 10000f, 0f, Color.White, 4f, 0f, 0f, 0f));
			}
		}
		...
		{
			if (isShippingBinLidOpen(true) && shippingBinLid.pingPongMotion == 1)
			{
		...
		public virtual Point GetMainMailboxPosition()
		{
			...
			{
				Point position;
				if (!TryGetMapPropertyAs("MailboxLocation", out position, false))
				{
		...
		public virtual Point GetGrandpaShrinePosition()
		{
			...
			{
				Point position;
				if (!TryGetMapPropertyAs("GrandpaShrineLocation", out position, false))
				{
		...
		public virtual Point GetMainFarmHouseEntry()
		{
			...
			{
				Point position;
				if (!TryGetMapPropertyAs("FarmHouseEntry", out position, false))
				{
		...
		private void doLightningStrike(LightningStrikeEvent lightning)
		{
			...
			if (lightning.createBolt && Game1.currentLocation.name.Equals("Farm"))
			{
				...
				{
					temporarySprites.Add(new TemporaryAnimatedSprite(362, 75f, 6, 1, lightning.boltPosition, false, false));
				}
		...
		public bool ShouldSpawnMountainOres()
		{
			if (!_mountainForageRectangle.HasValue)
			{
				...
				_mountainForageRectangle = (TryGetMapPropertyAs("SpawnMountainFarmOreRect", out area, false) ? area : Microsoft.Xna.Framework.Rectangle.Empty);
			}
