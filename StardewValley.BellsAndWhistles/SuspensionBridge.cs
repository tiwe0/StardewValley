	...
	public class SuspensionBridge
	{
		...
		public virtual void Update(GameTime time)
		{
			...
			if (Game1.player.bridge == this)
			{
				...
				if (playerBounds.Top >= bridgeBounds.Top && playerBounds.Bottom <= bridgeBounds.Bottom && (playerBounds.Intersects(bridgeBounds) || InEntranceArea(playerBounds)))
				{
					Game1.player.SetOnBridge(true);
				}
				else if (!InEntranceArea(playerBounds) && !playerBounds.Intersects(bridgeBounds))
				{
