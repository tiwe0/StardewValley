...
namespace StardewValley
{
	...
	public abstract class Item : IComparable, INetObject<NetFields>, ISalable, IHaveItemTypeId, IHaveModData
	{
		...
		protected string ValidateUnqualifiedItemId(string id)
		{
			if (ItemRegistry.IsQualifiedItemId(id))
			{
				...
				if (id.StartsWith(qualifier))
				{
					...
					defaultInterpolatedStringHandler.AppendFormatted(GetType().FullName);
		...
		{
			drawInMenu(spriteBatch, location, scaleSize, transparency, layerDepth, drawStackNumber, Color.White, true);
		}

		public void drawInMenu(SpriteBatch spriteBatch, Vector2 location, float scaleSize, float transparency, float layerDepth)
		{
			drawInMenu(spriteBatch, location, scaleSize, transparency, layerDepth, StackDrawType.Draw, Color.White, true);
		}

		public void drawInMenu(SpriteBatch spriteBatch, Vector2 location, float scaleSize)
		{
			drawInMenu(spriteBatch, location, scaleSize, 1f, 0.9f, StackDrawType.Draw, Color.White, true);
		}
		...
		public virtual bool canStackWith(ISalable other)
		{
			...
			if (otherItem == null || other.GetType() != GetType())
			{
				return false;
			}
		...
		{
			object rawValue;
			if (tempData == null || !tempData.TryGetValue(key, out rawValue))
			{
				value = default(T);
				return false;
			}
