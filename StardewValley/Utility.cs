	...
	public class Utility
	{
		...
		public static HorseWarpRestrictions GetHorseWarpRestrictionsForFarmer(Farmer who)
		{
			...
			foreach (Farmer farmer in Game1.getOnlineFarmers())
			{
				...
				{
					return restrictions | HorseWarpRestrictions.InUse;
				}
			}
			return restrictions;
		}
		...
		public static Microsoft.Xna.Framework.Rectangle ConstrainScissorRectToScreen(Microsoft.Xna.Framework.Rectangle scissor_rect)
		{
			...
			{
				int amount_to_trim4 = -scissor_rect.Top;
				scissor_rect.Height -= amount_to_trim4;
				scissor_rect.Y += amount_to_trim4;
			}
			if (scissor_rect.Bottom > Game1.viewport.Height)
			{
				int amount_to_trim3 = scissor_rect.Bottom - Game1.viewport.Height;
				scissor_rect.Height -= amount_to_trim3;
			}
			if (scissor_rect.Left < 0)
			{
				int amount_to_trim2 = -scissor_rect.Left;
				scissor_rect.Width -= amount_to_trim2;
				scissor_rect.X += amount_to_trim2;
			}
		...
		public static bool isFestivalDay(int day, Season season, string locationContext)
		{
			...
			defaultInterpolatedStringHandler.AppendFormatted(getSeasonKey(season));
			...
			if (locationContext != null)
			{
				...
				int startTime;
				int endTime;
				if (!Event.tryToLoadFestivalData(festivalId, out assetName, out data, out locationName, out startTime, out endTime))
				{
					return false;
				}
		...
		public static void ForEachLocation(Func<GameLocation, bool> action, bool includeInteriors = true, bool includeGenerated = false)
		{
			...
			foreach (MineShaft rawLevel2 in MineShaft.activeMines)
			{
		...
		public static void ForEachBuilding<TBuilding>(Func<TBuilding, bool> action, bool ignoreUnderConstruction = true) where TBuilding : Building
		{
			ForEachLocation(delegate(GameLocation location)
			{
				...
				{
					TBuilding val = building as TBuilding;
					if (val != null && (!ignoreUnderConstruction || !val.isUnderConstruction()) && !action(val))
					{
						return false;
					}
				}
		...
		public static bool TryParseEnum<TEnum>(string value, out TEnum parsed) where TEnum : struct
		{
			if (Enum.TryParse<TEnum>(value, true, out parsed))
			{
				...
				long result;
				if (typeof(TEnum).GetCustomAttribute<FlagsAttribute>() != null && !long.TryParse(parsed.ToString(), out result))
				{
					return true;
				}
			}
		...
		{
			int legacyId;
			if (int.TryParse(itemId, out legacyId))
			{
		...
		{
			int legacyId;
			if (int.TryParse(itemId, out legacyId) && legacyId >= lowerBound)
			{
		...
		public static int? fuzzyCompare(string query, string term)
		{
			...
			string formattedQuery = _003CfuzzyCompare_003Eg__FormatForFuzzySearch_007C36_0(query);
			string formattedTerm = _003CfuzzyCompare_003Eg__FormatForFuzzySearch_007C36_0(term);
		...
		{
			return CreateRandom(Game1.stats.DaysPlayed, Game1.uniqueIDForThisGame / 2uL, seedA, seedB, seedC);
		}
		...
		public static string getRandomSingleTileFurniture(Random r)
		{
			...
			{
			case 0:
				return "(F)" + r.Next(10) * 3;
			case 1:
				return "(F)" + r.Next(1376, 1391);
			default:
				return "(F)" + (r.Next(6) * 2 + 1391);
			}
		}
		...
		{
			Random giftRandom = CreateRandom(Game1.uniqueIDForThisGame / 2uL, Game1.year, Game1.dayOfMonth, Game1.seasonIndex, who.TilePoint.X);
		...
		{
			return (i as Object)?.canBeShipped() ?? false;
		}
		...
		{
			Season season;
			if (TryParseEnum<Season>(whichSeason, out season))
			{
				return (int)season;
			}
		...
		public static void addSprinklesToLocation(GameLocation l, int sourceXTile, int sourceYTile, int tilesWide, int tilesHigh, int totalSprinkleDuration, int millisecondsBetweenSprinkles, Color sprinkleColor, string sound = null, bool motionTowardCenter = false)
		{
			...
			for (int i = 0; i < numSprinkles; i++)
			{
				...
				l.temporarySprites.Add(new TemporaryAnimatedSprite(r.Next(10, 12), currentSprinklePosition, sprinkleColor, 8, false, 50f)
				{
					layerDepth = 1f,
		...
		public static void addRainbowStarExplosion(GameLocation l, Vector2 origin, int numStars)
		{
			...
			{
				sprites.Add(new TemporaryAnimatedSprite("TileSheets\\animations", new Microsoft.Xna.Framework.Rectangle(0, 640, 64, 64), origin + radPosition, false, 0.03f, GetPrismaticColor(Game1.random.Next(99999)))
				{
					motion = getVectorDirection(origin, origin + radPosition, true) * 0.06f * 150f,
					acceleration = -getVectorDirection(origin, origin + radPosition, true) * 0.06f * 6f,
		...
		{
			return GetPointsOnLine(x0, y0, x1, y1, false);
		}
		...
		public static Monster findClosestMonsterWithinRange(GameLocation location, Vector2 originPoint, int range, bool ignoreUntargetables = false)
		{
			...
			{
				if (i is Monster && (!ignoreUntargetables || !(i is Spiker)))
				{
					float distance = Vector2.Distance(originPoint, i.getStandingPosition());
					if (distance <= (float)range && distance < closest_distance && !(i as Monster).IsInvisible)
					{
						closest_monster = i as Monster;
		...
		public static void CollectSingleItemOrShowChestMenu(Chest chest, object context = null)
		{
			...
			Game1.activeClickableMenu = new ItemGrabMenu(items, false, true, InventoryMenu.highlightAllItems, chest.grabItemFromInventory, null, chest.grabItemFromChest, false, true, true, true, true, 1, null, -1, context);
		}
		...
		public static void fixAllAnimals()
		{
			...
			ForEachLocation(delegate(GameLocation f)
			{
				...
				{
					AnimalHouse animalHouse2 = current4.GetIndoors() as AnimalHouse;
				...
				foreach (FarmAnimal a2 in allFarmAnimals)
				{
					foreach (Building building2 in f.buildings)
					{
						building2.GetIndoors()?.animals.RemoveWhere((KeyValuePair<long, FarmAnimal> pair) => pair.Value.Equals(a2));
					}
					f.animals.RemoveWhere((KeyValuePair<long, FarmAnimal> pair) => pair.Value.Equals(a2));
				}
				foreach (Building b in f.buildings)
				{
					(b.GetIndoors() as AnimalHouse)?.animalsThatLiveHere.RemoveWhere((long id) => getAnimal(id)?.home != b);
				}
				foreach (FarmAnimal current5 in allFarmAnimals)
				{
					foreach (Building current6 in f.buildings)
					{
						if (current5.CanLiveIn(current6))
						{
							AnimalHouse animalHouse3 = current6.GetIndoors() as AnimalHouse;
							if (animalHouse3 != null && !animalHouse3.isFull())
							{
								animalHouse3.adoptAnimal(current5);
								break;
							}
						}
					}
		...
		public static Event getWeddingEvent(Farmer farmer)
		{
			...
			_003C_003Ec__DisplayClass112_.spouseActor = ((spouseFarmer != null) ? ("farmer" + getFarmerNumberFromFarmer(spouseFarmer)) : farmer.spouse);
			WeddingData data = DataLoader.Weddings(Game1.content);
			_003C_003Ec__DisplayClass112_.contextualAttendees = new List<WeddingAttendeeData>();
			if (data.Attendees != null)
			{
				...
				foreach (WeddingAttendeeData attendee in data.Attendees.Values)
				{
					...
					if (!exes.Contains(attendee.Id) && !(attendee.Id == farmer.spouse) && GameStateQuery.CheckConditions(attendee.Condition, null, farmer) && (attendee.IgnoreUnlockConditions || !NPC.TryGetData(attendee.Id, out characterData) || GameStateQuery.CheckConditions(characterData.UnlockConditions, null, farmer)))
					{
						_003C_003Ec__DisplayClass112_.contextualAttendees.Add(attendee);
					}
				}
			}
			string weddingEventString;
			if (!data.EventScript.TryGetValue(spouseFarmerId?.ToString() ?? farmer.spouse, out weddingEventString) && !data.EventScript.TryGetValue("default", out weddingEventString))
			{
				throw new InvalidOperationException("The Data/Weddings asset has no wedding script with the 'default' script key.");
			}
			weddingEventString = TokenParser.ParseText(weddingEventString, null, _003C_003Ec__DisplayClass112_._003CgetWeddingEvent_003Eg__ParseWeddingToken_007C0, farmer);
		...
		{
			CharacterData data;
			if (NPC.TryGetData(who, out data))
			{
		...
		public static int GetAllPlayerFriendshipLevel(NPC npc)
		{
			...
			{
				Friendship friendship;
				if (allFarmer.friendshipData.TryGetValue(npc.Name, out friendship) && friendship.Points > highest_friendship_points)
				{
		...
		{
			Item i = ItemQueryResolver.TryResolve("FLAVORED_ITEM " + baseID + " " + preservesID, new ItemQueryContext(Game1.currentLocation, Game1.player, Game1.random)).FirstOrDefault()?.Item as Item;
			if (i != null)
			{
				i.Quality = quality;
				i.Stack = stack;
		...
		public static string getRandomItemFromSeason(Season season, bool forQuest, Random random)
		{
			...
			if (forQuest)
			{
				foreach (string s in all_unlocked_cooking_recipes)
				{
					...
					if (!DataLoader.CookingRecipes(Game1.content).TryGetValue(s, out rawCraftingData))
					{
						continue;
					}
		...
		public static Color? StringToColor(string rawColor)
		{
			...
			if (rawColor.StartsWith('#'))
			{
				...
				byte green2;
				byte blue2;
				if ((rawColor.Length == 7 || rawColor.Length == 9) && byte.TryParse(rawColor.Substring(1, 2), NumberStyles.HexNumber, null, out red2) && byte.TryParse(rawColor.Substring(3, 2), NumberStyles.HexNumber, null, out green2) && byte.TryParse(rawColor.Substring(5, 2), NumberStyles.HexNumber, null, out blue2) && (rawColor.Length == 7 || byte.TryParse(rawColor.Substring(7, 2), NumberStyles.HexNumber, null, out alpha)))
				{
					return new Color(red2, green2, blue2, alpha);
				}
			}
			else if (rawColor.Contains(' '))
			{
				...
				int blue;
				int alpha2;
				if ((parts.Length == 3 || parts.Length == 4) && ArgUtility.TryGetInt(parts, 0, out red, out error) && ArgUtility.TryGetInt(parts, 1, out green, out error) && ArgUtility.TryGetInt(parts, 2, out blue, out error) && ArgUtility.TryGetOptionalInt(parts, 3, out alpha2, out error, 255))
				{
					return new Color(red, green, blue, alpha2);
				}
			}
			else
			{
		...
		public static void drawLightningBolt(Vector2 strikePosition, GameLocation l)
		{
			...
			{
				l.temporarySprites.Add(new TemporaryAnimatedSprite("LooseSprites\\Cursors", lightningSourceRect, 9999f, 1, 999, drawPosition, false, Game1.random.NextBool(), (strikePosition.Y + 32f) / 10000f + 0.001f, 0.025f, Color.White, 4f, 0f, 0f, 0f)
				{
					light = true,
					lightRadius = 2f,
		...
		public static void consolidateStacks(IList<Item> objects)
		{
			for (int j = 0; j < objects.Count; j++)
			{
				...
				{
					if (objects[k] != null && o.canStackWith(objects[k]))
					{
						o.Stack = objects[k].addToStack(o);
						if (o.Stack <= 0)
						{
							break;
						}
			...
			if (random.NextDouble() < 0.125 + Game1.player.team.AverageDailyLuck() + Game1.player.team.AverageLuckLevel() / 100.0)
			{
				...
				if (random.NextDouble() < 0.25 - Game1.player.team.AverageDailyLuck() - Game1.player.team.AverageLuckLevel() / 100.0)
				{
					try
					{
						...
						if (TryGetRandom(farm.terrainFeatures, out tile, out feature))
						{
							...
							if (fruitTree != null)
							{
								...
								lightningEvent2.createBolt = true;
								lightningEvent2.boltPosition = tile * 64f + new Vector2(32f, -128f);
							}
							else
							{
								...
								{
									lightningEvent2.createBolt = true;
									lightningEvent2.boltPosition = tile * 64f + new Vector2(32f, 0f);
								}
							}
						}
					}
		...
		public static Item getItemFromStandardTextDescription(string type, string itemId, int stock, Farmer who)
		{
			...
			{
				int index;
				item = (int.TryParse(itemId, out index) ? ItemRegistry.Create(((index >= 1000) ? "(S)" : "(P)") + itemId) : ItemRegistry.Create(itemId));
				break;
			}
			}
			item.Stack = stock;
		...
		public static List<string> GetJukeboxTracks(Farmer player, GameLocation location)
		{
			...
			{
				List<string> alternativeTrackIds = entry2.Value.AlternativeTrackIds;
				if (alternativeTrackIds == null || alternativeTrackIds.Count <= 0)
				{
					continue;
				}
				foreach (string id in entry2.Value.AlternativeTrackIds)
				{
					if (id != null)
					{
						cueNamesByAlternativeId[id] = entry2.Key;
					}
				}
			}
			...
			foreach (string heardId in player.songsHeard)
			{
				...
				if (IsValidTrackName(cueName) && seen.Add(cueName) && (!Game1.jukeboxTrackData.TryGetValue(cueName, out data) || !((!data.Available) ?? false)))
				{
					tracks.Add(cueName);
				}
			}
			...
			if (!string.IsNullOrWhiteSpace(cueName))
			{
				...
				if (!(text == "turn_off"))
				{
					...
					}
					{
						...
						{
							if (entry.AlternativeTrackIds?.Contains(cueName, StringComparer.OrdinalIgnoreCase) ?? false)
							{
		...
		public static bool isThereAnObjectHereWhichAcceptsThisItem(GameLocation location, Item item, int x, int y)
		{
			...
			foreach (Building building in location.buildings)
			{
				if (building.occupiesTile(tileLocation) && building.performActiveObjectDropInAction(Game1.player, true))
				{
					return true;
				}
			}
			Object obj;
			if (location.Objects.TryGetValue(tileLocation, out obj) && obj.heldObject.Value == null && obj.performObjectDropInAction((Object)item, true, Game1.player))
			{
				return true;
			}
			return false;
		...
		public static FarmAnimal getAnimal(long id)
		{
			...
			{
				FarmAnimal value;
				if (location.animals.TryGetValue(id, out value))
				{
					match = value;
					return false;
				}
		...
		public static bool TryOpenShopMenu(string shopId, string ownerName, bool playOpenSound = true)
		{
			...
			if (!TryParseEnum<ShopOwnerType>(ownerName, out ownerType))
			{
		...
		{
			Random r = CreateRandom(Game1.stats.DaysPlayed, Game1.uniqueIDForThisGame / 2uL, 470124797.0, Game1.player.UniqueMultiplayerID);
			if (Game1.weddingToday)
			{
				return null;
			}
			...
			else
			{
				...
				if (isMarriedOrRoommates && Game1.player.team.GetSpouse(Game1.player.UniqueMultiplayerID).HasValue && Game1.player.GetSpouseFriendship().NextBirthingDate == null && r.NextDouble() < 0.05)
				{
					...
					Farmer farmerSpouse;
					if (Game1.otherFarmers.TryGetValue(spouseID, out farmerSpouse))
					{
		...
		public static List<Object> getPurchaseAnimalStock(GameLocation location)
		{
			...
			foreach (KeyValuePair<string, FarmAnimalData> pair in Game1.farmAnimalData)
			{
				...
				{
					Object o = new Object("100", 1, false, data.PurchasePrice)
					{
						Name = pair.Key,
						Type = null
					};
		...
		public static bool checkForCharacterInteractionAtTile(Vector2 tileLocation, Farmer who)
		{
			...
			if (character != null && !character.IsMonster && !character.IsInvisible)
			{
				...
				else if (character.Name == "Pierre" && who.ActiveObject?.QualifiedItemId == "(O)897" && character.tryToReceiveActiveObject(who, true))
				{
		...
		public static bool canGrabSomethingFromHere(int x, int y, Farmer who)
		{
			...
			if (who.IsLocalPlayer)
			{
				...
				{
					if (!obj.readyForHarvest && !obj.isSpawnedObject)
					{
				...
				if (Game1.currentLocation.terrainFeatures.TryGetValue(tileLocation, out terrainFeature))
				{
		...
		public static bool TryGetPassiveFestivalDataForDay(int dayOfMonth, Season season, string locationContextId, out string id, out PassiveFestivalData data, bool ignoreConditionsCheck = false)
		{
			...
			foreach (string item in possibleIds)
			{
				string curId = (id = item);
		...
		public static bool IsPassiveFestivalOpen(string festivalId)
		{
			...
			if (IsPassiveFestivalDay(festivalId) && TryGetPassiveFestivalData(festivalId, out festival))
			{
		...
		public static bool isPlacementForbiddenHere(string location_name)
		{
			...
			{
				PassiveFestivalData festival;
				if (!TryGetPassiveFestivalData(activePassiveFestival, out festival) || festival.MapReplacements == null)
				{
					continue;
				}
		...
		public static bool playerCanPlaceItemHere(GameLocation location, Item item, int x, int y, Farmer f, bool show_error = false)
		{
			...
			if (!isWithinTileWithLeeway(x, y, item, f) && (!(item is Wallpaper) || !(location is DecoratableLocation)))
			{
			...
			if (furniture != null && !location.CanFreePlaceFurniture() && !furniture.IsCloseEnoughToFarmer(f, x / 64, y / 64))
			{
				return false;
			}
			Vector2 tileLocation = new Vector2(x / 64, y / 64);
		...
		{
			_003C_003Ec__DisplayClass276_0 CS_0024_003C_003E8__locals0 = new _003C_003Ec__DisplayClass276_0();
			CS_0024_003C_003E8__locals0.action = action;
			ForEachLocation(delegate(GameLocation l)
			{
				Chest fridge = l.GetFridge(false);
				fridge?.ForEachItem(CS_0024_003C_003E8__locals0._003CiterateChestsAndStorage_003Eg__Handle_007C0);
				foreach (Object current in l.objects.Values)
				{
					if (current != fridge)
					{
						if (current is Chest)
						{
							current.ForEachItem(CS_0024_003C_003E8__locals0._003CiterateChestsAndStorage_003Eg__Handle_007C0);
						}
						else
						{
							(current.heldObject.Value as Chest)?.ForEachItem(CS_0024_003C_003E8__locals0._003CiterateChestsAndStorage_003Eg__Handle_007C0);
						}
					}
				}
				foreach (Furniture item4 in l.furniture)
				{
					item4.ForEachItem(CS_0024_003C_003E8__locals0._003CiterateChestsAndStorage_003Eg__Handle_007C0);
				}
				foreach (Building building in l.buildings)
				{
					foreach (Chest buildingChest in building.buildingChests)
					{
						buildingChest.ForEachItem(CS_0024_003C_003E8__locals0._003CiterateChestsAndStorage_003Eg__Handle_007C0);
					}
				}
				return true;
			});
			foreach (Item item3 in Game1.player.team.returnedDonations)
			{
				if (item3 != null)
				{
					CS_0024_003C_003E8__locals0.action(item3);
				}
			}
			foreach (Inventory value in Game1.player.team.globalInventories.Values)
			{
				foreach (Item item2 in (IEnumerable<Item>)value)
				{
					if (item2 != null)
					{
						CS_0024_003C_003E8__locals0.action(item2);
					}
				}
			}
			foreach (SpecialOrder specialOrder in Game1.player.team.specialOrders)
			{
				foreach (Item item in specialOrder.donatedItems)
				{
					if (item != null)
					{
						CS_0024_003C_003E8__locals0.action(item);
					}
				}
			}
		}
		...
		public static NPC GetRandomWinterStarParticipant(Func<string, bool> ignoreNpc = null)
		{
			...
			}, CreateRandom(Game1.uniqueIDForThisGame / 2uL, Game1.year, Game1.player.UniqueMultiplayerID));
		}
		...
		public static Item addItemToThisInventoryList(Item i, IList<Item> list, int listMaxSpace = -1)
		{
			...
			foreach (Item it in list)
			{
				...
				{
					Object obj3 = i as Object;
					if (obj3 != null)
					{
						obj3.stack.Value = it.addToStack(i);
					}
					else
					{
		...
		public static Item addItemToInventory(Item item, int position, IList<Item> items, ItemGrabMenu.behaviorOnItemSelect onAddFunction = null)
		{
			...
			if (isCurrentPlayer)
			{
				...
				Game1.player.GetItemReceiveBehavior(item, out needsInventorySpace, out showNotification);
		...
		public static Item getTreasureFromGeode(Item geode)
		{
			...
			try
			{
				...
				Random r = CreateRandom(geodeId.Contains("MysteryBox") ? Game1.stats.Get("MysteryBoxesOpened") : Game1.stats.GeodesCracked, Game1.uniqueIDForThisGame / 2uL, (int)Game1.player.uniqueMultiplayerID.Value / 2);
				...
				if (geodeId.Contains("MysteryBox"))
				{
					...
					case 12:
						if (r.NextBool(0.4))
						{
							...
							{
							case 0:
								return ItemRegistry.Create<Ring>("(O)525");
							case 1:
								return ItemRegistry.Create<Ring>("(O)529");
							case 2:
								return ItemRegistry.Create<Ring>("(O)888");
							default:
								return ItemRegistry.Create<Ring>("(O)" + r.Next(531, 533));
							}
						}
				...
				if (Game1.objectData.TryGetValue(geode.ItemId, out data))
				{
					...
					if (geodeDrops != null && geodeDrops.Count > 0 && (!data.GeodeDropsDefaultItems || r.NextBool()))
					{
						foreach (ObjectGeodeDropData drop in data.GeodeDrops.OrderBy((ObjectGeodeDropData p) => p.Precedence))
						{
							...
							Item item = ItemQueryResolver.TryResolveRandomItem(drop, new ItemQueryContext(null, null, r), false, null, null, null, delegate(string query, string error)
							{
								IGameLogger log = Game1.log;
								DefaultInterpolatedStringHandler defaultInterpolatedStringHandler = new DefaultInterpolatedStringHandler(58, 5);
								defaultInterpolatedStringHandler.AppendLiteral("Geode item '");
								defaultInterpolatedStringHandler.AppendFormatted(geode.QualifiedItemId);
								defaultInterpolatedStringHandler.AppendLiteral("' failed parsing item query '");
								defaultInterpolatedStringHandler.AppendFormatted(query);
								defaultInterpolatedStringHandler.AppendLiteral("' for ");
								defaultInterpolatedStringHandler.AppendFormatted("GeodeDrops");
				...
				if (r.NextBool())
				{
					...
					default:
						...
						{
						case "(O)749":
							return ItemRegistry.Create("(O)" + (82 + r.Next(3) * 2));
						case "(O)535":
		...
		public static bool tryToPlaceItem(GameLocation location, Item item, int x, int y)
		{
			...
			playerCanPlaceItemHere(location, item, x, y, Game1.player, true);
			return false;
		}
		...
		public static void recursiveFindPositionForCharacter(NPC c, GameLocation l, Vector2 tileLocation, int maxIterations)
		{
			...
			for (; iterations < maxIterations; iterations++)
			{
				...
				if (!l.isCollidingPosition(c.GetBoundingBox(), Game1.viewport, false, 0, false, c, true))
				{
		...
		public static void spreadAnimalsAround(Building b, GameLocation environment, IEnumerable<FarmAnimal> animalsList)
		{
			...
			while (animals.Count > 0 && iterations < 40 && positionsToCheck.Count > 0)
			{
				...
				if (!environment.isCollidingPosition(animal.GetBoundingBox(), Game1.viewport, false, 0, false, animal, true))
				{
		...
		public static bool[] horizontalOrVerticalCollisionDirections(Microsoft.Xna.Framework.Rectangle boundingBox, Character c, bool projectile = false)
		{
			...
			if (c != null)
			{
				if (Game1.currentLocation.isCollidingPosition(rect, Game1.viewport, false, -1, projectile, c, false, projectile))
				{
					directions[1] = true;
				}
			}
			else if (Game1.currentLocation.isCollidingPosition(rect, Game1.viewport, false, -1, projectile, c, false, projectile))
			{
				directions[1] = true;
			}
		...
		{
			return generateNewFarm(skipFarmGeneration, true);
		}
