...
namespace Ionic.Zlib
{
	internal class ZlibBaseStream : Stream
	{
		...
		private void finish()
		{
			...
			if (_streamMode == StreamMode.Writer)
			{
				...
				if (_flavor == ZlibStreamFlavor.GZIP)
				{
					...
					int c2 = (int)(crc.TotalBytesRead & 0xFFFFFFFFu);
			...
			else
			{
				...
				int isize_actual = (int)(_z.TotalBytesOut & 0xFFFFFFFFu);
		...
		private int _ReadAndValidateGzipHeader()
		{
			...
			int i = _stream.Read(header, 0, header.Length);
			switch (i)
			{
			case 0:
				return 0;
			...
			case 10:
			{
				...
				if ((header[3] & 4) == 4)
				{
					i = _stream.Read(header, 0, 2);
					totalBytesRead += i;
					short extraLength = (short)(header[0] + header[1] * 256);
					byte[] extra = new byte[extraLength];
					i = _stream.Read(extra, 0, extra.Length);
					if (i != extraLength)
					{
		...
		public override int Read(byte[] buffer, int offset, int count)
		{
			...
			while (((!nomoreinput && rc != 1) || _z.AvailableBytesOut != count) && _z.AvailableBytesOut > 0 && !nomoreinput && rc == 0);
