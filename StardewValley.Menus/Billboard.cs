	...
	public class Billboard : IClickableMenu
	{
		...
		public virtual Dictionary<int, List<NPC>> GetBirthdays()
		{
			...
			Utility.ForEachVillager(delegate(NPC npc)
			{
				...
				List<NPC> value;
				if (!birthdays.TryGetValue(npc.Birthday_Day, out value))
				{
		...
		public virtual List<BillboardEvent> GetEventsForDay(int day, Dictionary<int, List<NPC>> birthdays)
		{
			...
			{
				string id = Game1.currentSeason + day;
				string festivalName3 = Game1.temporaryContent.Load<Dictionary<string, string>>("Data\\Festivals\\" + id)["name"];
				curEvents.Add(new BillboardEvent(BillboardEventType.Festival, new string[1] { id }, festivalName3));
			}
			string id2;
			PassiveFestivalData festivalData;
			if (Utility.TryGetPassiveFestivalDataForDay(day, Game1.season, null, out id2, out festivalData, true) && (festivalData?.ShowOnCalendar ?? false))
			{
				string festivalName4 = TokenParser.ParseText(festivalData.DisplayName);
				if (!GameStateQuery.CheckConditions(festivalData.Condition))
				{
					curEvents.Add(new BillboardEvent(BillboardEventType.PassiveFestival, new string[1] { id2 }, "???")
					{
						locked = true
					});
				}
				...
				{
					curEvents.Add(new BillboardEvent(BillboardEventType.PassiveFestival, new string[1] { id2 }, festivalName4));
				}
			}
			if (Game1.IsSummer && (day == 20 || day == 21))
			{
				string festivalName2 = Game1.content.LoadString("Strings\\1_6_Strings:TroutDerby");
				curEvents.Add(new BillboardEvent(BillboardEventType.FishingDerby, LegacyShims.EmptyArray<string>(), festivalName2));
			}
			...
			if (birthdays.TryGetValue(day, out birthdayVillagers))
			{
				...
				{
					char lastChar = i.displayName.Last();
					...
					curEvents.Add(new BillboardEvent(BillboardEventType.Birthday, new string[1] { i.Name }, displayText, character_texture, i.getMugShotSourceRect()));
				}
			}
		...
		public override void performHoverAction(int x, int y)
		{
			...
			foreach (ClickableTextureComponent c in calendarDays)
			{
				...
				{
					BillboardDay day;
					hoverText = (calendarDayData.TryGetValue(c.myID, out day) ? day.HoverText : string.Empty);
					break;
				}
			}
		}
		...
		public override void draw(SpriteBatch b)
		{
			...
			if (!dailyQuestBoard)
			{
				...
				for (int i = 0; i < calendarDays.Count; i++)
				{
					...
					BillboardDay day;
					if (calendarDayData.TryGetValue(component.myID, out day))
					{
						if (day.Texture != null)
						{
						...
						{
							Utility.drawWithShadow(b, Game1.mouseCursors, new Vector2(component.bounds.X + 12, (float)(component.bounds.Y + 60) - Game1.dialogueButtonScale / 2f), new Rectangle(346, 392, 8, 8), day.GetEventOfType(BillboardEventType.PassiveFestival).locked ? (Color.Black * 0.3f) : Color.White, 0f, Vector2.Zero, 4f, false, 1f);
						}
						if (day.Type.HasFlag(BillboardEventType.Festival))
						{
							Utility.drawWithShadow(b, billboardTexture, new Vector2(component.bounds.X + 40, (float)(component.bounds.Y + 56) - Game1.dialogueButtonScale / 2f), new Rectangle(1 + (int)(Game1.currentGameTime.TotalGameTime.TotalMilliseconds % 600.0 / 100.0) * 14, 398, 14, 12), Color.White, 0f, Vector2.Zero, 4f, false, 1f);
						}
						if (day.Type.HasFlag(BillboardEventType.FishingDerby))
						{
							Utility.drawWithShadow(b, Game1.mouseCursors_1_6, new Vector2(calendarDays[i].bounds.X + 8, (float)(calendarDays[i].bounds.Y + 60) - Game1.dialogueButtonScale / 2f), new Rectangle(103, 2, 10, 11), Color.White, 0f, Vector2.Zero, 4f, false, 1f);
						}
					...
					else if (Game1.dayOfMonth == i + 1)
					{
						...
						IClickableMenu.drawTextureBox(b, Game1.mouseCursors, new Rectangle(379, 357, 3, 3), component.bounds.X - offset, component.bounds.Y - offset, component.bounds.Width + offset * 2, component.bounds.Height + offset * 2, Color.Blue, 4f, false);
					}
				}
			}
			else
			{
				...
				else
				{
					...
					if (Game1.stats.Get("BillboardQuestsDone") % 3u == 2 && (acceptQuestButton.visible || !Game1.questOfTheDay.completed))
					{
				...
				bool drawAllStars = Game1.stats.Get("BillboardQuestsDone") % 3u == 0 && Game1.questOfTheDay != null && (bool)Game1.questOfTheDay.completed;
				for (int j = 0; j < (drawAllStars ? 3 : (Game1.stats.Get("BillboardQuestsDone") % 3u)); j++)
				{
