...
namespace StardewValley.Menus
{
	public class OptionsInputListener : OptionsElement
	{
		...
		public override void receiveLeftClick(int x, int y)
		{
			...
			if (whichOption == -1)
			{
				...
				}
				{
					...
					{
						OptionsInputListener listener = option as OptionsInputListener;
						if (listener != null)
						{
		...
		public override void receiveKeyPress(Keys key)
		{
			...
			else if (!Game1.options.isKeyInUse(key) || new InputButton(key).ToString().Equals(buttonNames[0]))
			{
		...
		public override void draw(SpriteBatch b, int slotX, int slotY, IClickableMenu context = null)
		{
			if (buttonNames.Count > 0 || whichOption == -1)
			{
				...
				{
					Utility.drawTextWithShadow(b, label + ": " + buttonNames.Last() + ((buttonNames.Count > 1) ? (", " + buttonNames[0]) : ""), Game1.dialogueFont, new Vector2(bounds.X + slotX, bounds.Y + slotY), Game1.textColor, 1f, 0.15f);
				}
			}
			Utility.drawWithShadow(b, Game1.mouseCursors, new Vector2(setbuttonBounds.X + slotX, setbuttonBounds.Y + slotY), setButtonSource, Color.White, 0f, Vector2.Zero, 4f, false, 0.15f);
			if (listening)
			{
