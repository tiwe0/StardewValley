	...
	public class QuestionEvent : BaseFarmEvent
	{
		...
		public override bool setUp()
		{
			...
			case 2:
			{
				...
				Utility.ForEachBuilding(delegate(Building b)
				{
					if ((b.owner.Value == Game1.player.UniqueMultiplayerID || !Game1.IsMultiplayer) && b.AllowsAnimalPregnancy())
					{
						...
						if (animalHouse != null && !animalHouse.isFull() && Game1.random.NextDouble() < (double)animalHouse.animalsThatLiveHere.Count * 0.0055)
						{
			...
			case 3:
			{
				...
				{
					Game1.currentLocation.createQuestionDialogue(Game1.content.LoadString("Strings\\Events:HavePlayerBabyQuestion", spouse2.displayName), answers2, answerPlayerPregnancyQuestion);
				}
				else
				{
					Game1.currentLocation.createQuestionDialogue(Game1.content.LoadString("Strings\\Events:HavePlayerBabyQuestion_Adoption", spouse2.displayName), answers2, answerPlayerPregnancyQuestion);
				}
				Game1.messagePause = true;
				return false;
			}
