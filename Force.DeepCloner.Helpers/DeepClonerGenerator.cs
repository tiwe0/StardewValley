...
namespace Force.DeepCloner.Helpers
{
	internal static class DeepClonerGenerator
	{
		...
		private static object CloneClassRoot(object obj)
		{
			...
			Func<object, DeepCloneState, object> cloner = (Func<object, DeepCloneState, object>)DeepClonerCache.GetOrAddClass(obj.GetType(), (Type t) => GenerateCloner(t, true));
			if (cloner == null)
			{
				return obj;
			}
		...
		internal static T[,] Clone2DimArrayInternal<T>(T[,] obj, DeepCloneState state)
		{
			...
			int l2 = obj.GetLength(0);
			int l3 = obj.GetLength(1);
			T[,] outArray = new T[l2, l3];
			state.AddKnownRef(obj, outArray);
			...
			if (typeof(T).IsValueType())
			{
				...
				for (int j = 0; j < l2; j++)
				{
					for (int l = 0; l < l3; l++)
					{
						outArray[j, l] = cloner(obj[j, l], state);
					}
				}
			}
			else
		...
		public static object CloneObjectTo(object objFrom, object objTo, bool isDeep)
		{
			...
			Func<object, object, DeepCloneState, object> cloner = (Func<object, object, DeepCloneState, object>)(isDeep ? DeepClonerCache.GetOrAddDeepClassTo(type, (Type t) => ClonerToExprGenerator.GenerateClonerInternal(t, true)) : DeepClonerCache.GetOrAddShallowClassTo(type, (Type t) => ClonerToExprGenerator.GenerateClonerInternal(t, false)));
			if (cloner == null)
			{
				return objTo;
			}
