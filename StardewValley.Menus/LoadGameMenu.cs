	...
	public class LoadGameMenu : IClickableMenu, IDisposable
	{
		...
		public class SaveFileSlot : MenuSlot
		{
				...
				: base(menu)
			{
				...
				versionComparison = Utility.CompareGameVersions(Game1.version, farmer.gameVersion, true);
			}
			...
			protected virtual void drawSlotSaveNumber(SpriteBatch b, int i)
			{
				...
				SpriteText.drawString(b, slotNumberLabel, button.bounds.X + 28 + 32 - SpriteText.getWidthOfString(slotNumberLabel) / 2, button.bounds.Y + 36);
			}
			...
			protected virtual void drawSlotFarmer(SpriteBatch b, int i)
			{
				...
				Farmer.FarmerRenderer.draw(b, new FarmerSprite.AnimationFrame(0, 0, false, false), 0, new Rectangle(0, 0, 16, 32), new Vector2((float)menu.slotButtons[i].bounds.X + offset.X, (float)menu.slotButtons[i].bounds.Y + offset.Y), Vector2.Zero, 0.8f, 2, Color.White, 0f, 1f, Farmer);
			...
			protected virtual void drawSlotMoney(SpriteBatch b, int i)
			{
				...
				Utility.drawWithShadow(b, Game1.mouseCursors, new Vector2(menu.slotButtons[i].bounds.X + menu.width - 192 - 100 - moneyWidth, menu.slotButtons[i].bounds.Y + 64 + 44), new Rectangle(193, 373, 9, 9), Color.White, 0f, Vector2.Zero, 4f, false, 1f);
			...
			if (!string.IsNullOrWhiteSpace(filter))
			{
				for (int i = 0; i < results.Count; i++)
				{
					...
					if (name != null && name.IndexOf(filter, StringComparison.OrdinalIgnoreCase) == -1)
					{
						...
						if (value != null && value.IndexOf(filter, StringComparison.OrdinalIgnoreCase) == -1)
						{
							results[i] = null;
						}
					}
		...
		public override void receiveLeftClick(int x, int y, bool playSound = true)
		{
			...
			if (!deleteConfirmationScreen)
			{
				for (int j = 0; j < slotButtons.Count; j++)
				{
					...
					SaveFileSlot menu_save_slot = MenuSlots[currentItemIndex + j] as SaveFileSlot;
		...
		public override void draw(SpriteBatch b)
		{
			...
			if (MenuSlots.Count > 4)
			{
				IClickableMenu.drawTextureBox(b, Game1.mouseCursors, new Rectangle(403, 383, 6, 6), scrollBarRunner.X, scrollBarRunner.Y, scrollBarRunner.Width, scrollBarRunner.Height, Color.White, 4f, false);
				scrollBar.draw(b);
			}
			if (deleteConfirmationScreen)
			{
				...
				if (slot != null)
				{
					...
					SpriteText.drawString(b, toDisplay, middlePosX - SpriteText.getWidthOfString(toDisplay) / 2, (int)Utility.getTopLeftPositionForCenteringOnScreen(192, 64).Y, 9999, -1, 9999, 1f, 1f, false, -1, "", SpriteText.color_White);
