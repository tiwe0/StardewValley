	...
	public class Pickaxe : Tool
	{
		...
		public override void DoFunction(GameLocation location, int x, int y, int power, Farmer who)
		{
			...
			if (o != null)
			{
				if (o.IsBreakableStone())
				{
					...
					if (o.MinutesUntilReady > 0)
					{
						...
						if (o.MinutesUntilReady > 0)
						{
							Game1.createRadialDebris(Game1.currentLocation, 14, tileX, tileY, Game1.random.Next(2, 5), false);
							return;
						}
					}
					TemporaryAnimatedSprite sprite = ((ItemRegistry.GetDataOrErrorItem(o.QualifiedItemId).TextureName == "Maps\\springobjects" && o.ParentSheetIndex < 200 && !Game1.objectData.ContainsKey((o.ParentSheetIndex + 1).ToString()) && o.QualifiedItemId != "(O)25") ? new TemporaryAnimatedSprite(o.ParentSheetIndex + 1, 300f, 1, 2, new Vector2(x - x % 64, y - y % 64), true, o.flipped)
					{
						alphaFade = 0.01f
					} : new TemporaryAnimatedSprite(47, new Vector2(tileX * 64, tileY * 64), Color.Gray, 10, false, 80f));
					Game1.multiplayer.broadcastSprites(location, sprite);
					Game1.createRadialDebris(location, 14, tileX, tileY, Game1.random.Next(2, 5), false);
					Game1.multiplayer.broadcastSprites(location, new TemporaryAnimatedSprite(46, new Vector2(tileX * 64, tileY * 64), Color.White, 10, false, 80f)
					{
			...
			else
			{
				...
				{
					Game1.multiplayer.broadcastSprites(location, new TemporaryAnimatedSprite(12, new Vector2(tileX * 64, tileY * 64), Color.White, 8, false, 80f)
					{
						alphaFade = 0.015f
					});
				}
