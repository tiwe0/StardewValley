	...
	public class DiscreteColorPicker : IClickableMenu
	{
		...
		public override void receiveLeftClick(int x, int y, bool playSound = true)
		{
			...
			if (area.Contains(x, y))
			{
				...
				Chest chest = itemToDrawColored as Chest;
				if (chest != null)
				{
		...
		public override void draw(SpriteBatch b)
		{
			...
			for (int i = 0; i < totalColors; i++)
			{
				...
				if (i == colorSelection)
				{
					IClickableMenu.drawTextureBox(b, Game1.mouseCursors, new Rectangle(375, 357, 3, 3), xPositionOnScreen + IClickableMenu.borderWidth / 2 - 4 + i * 9 * 4, yPositionOnScreen + IClickableMenu.borderWidth / 2 - 4, 36, 36, Color.Black, 4f, false);
				}
			}
			(itemToDrawColored as Chest)?.draw(b, xPositionOnScreen + width + IClickableMenu.borderWidth / 2, yPositionOnScreen + 16, 1f, true);
		}
	}
}
