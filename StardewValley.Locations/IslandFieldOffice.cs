...
namespace StardewValley.Locations
{
	public class IslandFieldOffice : IslandLocation
	{
		...
		private void ApplyFrogRestore()
		{
			...
			for (int i = 0; i < 3; i++)
			{
				temporarySprites.Add(new TemporaryAnimatedSprite("LooseSprites\\Cursors", new Microsoft.Xna.Framework.Rectangle(372, 1956, 10, 10), new Vector2(6.5f + (float)Game1.random.Next(-10, 11) / 100f, 3f) * 64f, false, 0.007f, Color.White)
				{
					alpha = 0.75f,
		...
		private void ApplyBatRestore()
		{
			...
			for (int i = 0; i < 3; i++)
			{
				temporarySprites.Add(new TemporaryAnimatedSprite("LooseSprites\\Cursors", new Microsoft.Xna.Framework.Rectangle(372, 1956, 10, 10), new Vector2(2.5f + (float)Game1.random.Next(-10, 11) / 100f, 3f) * 64f, false, 0.007f, Color.White)
				{
					alpha = 0.75f,
					motion = new Vector2(0f, -1f),
					acceleration = new Vector2(0.002f, 0f),
		...
		protected override void resetLocalState()
		{
			...
			{
				safariGuy = new NPC(new AnimatedSprite("Characters\\SafariGuy", 0, 16, 32), new Vector2(8f, 6f) * 64f, "IslandFieldOFfice", 2, "Professor Snail", false, Game1.content.Load<Texture2D>("Portraits\\SafariGuy"));
			...
			if (safariGuy != null)
			{
				...
				if (Game1.random.NextBool())
				{
					...
					safariGuy.faceTowardFarmerForPeriod(60000, 5, false, Game1.player);
				}
				else
				{
		...
		public override void UpdateWhenCurrentLocation(GameTime time)
		{
			...
			if (safariGuy != null)
			{
				...
				{
					speakerTimer = 600f;
					temporarySprites.Add(new TemporaryAnimatedSprite("LooseSprites\\Cursors2", new Microsoft.Xna.Framework.Rectangle(211, 161, 5, 5), new Vector2(74.75f, 20.75f) * 4f, false, 0f, Color.White)
					{
						scale = 5f,
						scaleChange = -0.05f,
		...
		public override bool answerDialogueAction(string questionAndAnswer, string[] questionParams)
		{
			...
			case "Safari_Collect":
			{
				Game1.activeClickableMenu = new ItemGrabMenu(new List<Item>(uncollectedRewards), false, true, null, null, "Rewards", OnCollectReward, false, true, false, false, false, 0, null, -1, this);
			...
			case "Survey_Yes":
				if (!plantsRestoredLeft)
				{
					...
					for (int j = 18; j < 25; j++)
					{
						responses2.Add(new Response((j == 22) ? "Correct" : "Wrong", j.ToString() ?? ""));
					}
				...
				else if (!plantsRestoredRight)
				{
					...
					for (int i = 11; i < 19; i++)
					{
						responses.Add(new Response((i == 18) ? "Correct" : "Wrong", i.ToString() ?? ""));
					}
			...
			if (!(text == "FieldOfficeDesk"))
			{
				if (text == "FieldOfficeSurvey" && safariGuy != null)
				{
					...
					if (!plantsRestoredLeft)
					{
						...
						createQuestionDialogue(Game1.content.LoadString("Strings\\Locations:IslandFieldOffice_Survey_Prompt_LeftPlant"), responses2, "Survey");
					...
					else if (!plantsRestoredRight)
					{
						...
						createQuestionDialogue(Game1.content.LoadString("Strings\\Locations:IslandFieldOffice_Survey_Prompt_RightPlant"), responses, "Survey");
