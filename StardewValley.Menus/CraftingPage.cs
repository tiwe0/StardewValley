...
namespace StardewValley.Menus
{
	public class CraftingPage : IClickableMenu
	{
			...
			: base(x, y, width, height)
		{
			...
			inventory = new InventoryMenu(xPositionOnScreen + IClickableMenu.spaceToClearSideBorder + IClickableMenu.borderWidth, yPositionOnScreen + IClickableMenu.spaceToClearTopBorder + IClickableMenu.borderWidth + 320 - 16, false);
		...
		protected virtual List<string> GetRecipesToDisplay()
		{
			...
			if (!cooking)
			{
				foreach (string key2 in CraftingRecipe.craftingRecipes.Keys)
				{
					if (Game1.player.craftingRecipes.ContainsKey(key2))
					{
						recipes.Add(key2);
					}
				}
			...
			recipes.Sort(delegate(string a, string b)
			{
				...
				int value = -1;
				string value2;
				if (a != null && CraftingRecipe.cookingRecipes.TryGetValue(a, out value2))
				{
					num = ArgUtility.GetInt(value2.Split('/'), 2, -1);
				}
				string value3;
				if (b != null && CraftingRecipe.cookingRecipes.TryGetValue(b, out value3))
				{
		...
		public override void draw(SpriteBatch b)
		{
			...
			ObjectData rawHoverData;
			if (cooking && lastCookingHover.HasTypeObject() && Game1.objectData.TryGetValue(lastCookingHover.ItemId, out rawHoverData))
			{
