...
namespace StardewValley.Menus
{
	public class Bundle : ClickableComponent
	{
			...
			: base(new Rectangle(position.X, position.Y, 64, 64), "")
		{
			...
			sprite = new TemporaryAnimatedSprite(textureName, new Rectangle(bundleColor * 256 % 512, 244 + bundleColor * 256 / 512 * 16, 16, 16), 70f, 3, 99999, new Vector2(bounds.X, bounds.Y), false, false, 0.8f, 0f, Color.White, 4f, 0f, 0f, 0f)
			{
				pingPong = true
			};
			sprite.paused = true;
			...
			if (complete)
			{
				completionAnimation(menu, false);
			}
		}
		...
		public bool IsValidItemForThisIngredientDescription(Item item, BundleIngredientDescription ingredient)
		{
			...
			{
				Object obj = ItemQueryResolver.TryResolve("FLAVORED_ITEM " + ingredient.id + " " + ingredient.preservesId, new ItemQueryContext(Game1.currentLocation, Game1.player, Game1.random)).FirstOrDefault()?.Item as Object;
				if (obj != null && item.QualifiedItemId == obj.QualifiedItemId && obj.preservedParentSheetIndex.Contains(ingredient.preservesId))
				{
					return true;
				}
				return false;
		...
		public Item tryToDepositThisItem(Item item, ClickableTextureComponent slot, string noteTextureName, JunimoNoteMenu parentMenu)
		{
			...
			for (int i = 0; i < ingredients.Count; i++)
			{
				...
				if (IsValidItemForThisIngredientDescription(item, ingredient) && slot.item == null)
				{
					...
					ingredient = (ingredients[i] = new BundleIngredientDescription(ingredient, true));
