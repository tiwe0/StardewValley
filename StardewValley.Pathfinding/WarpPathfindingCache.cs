...
namespace StardewValley.Pathfinding
{
	...
	public static class WarpPathfindingCache
	{
		...
		public static void PopulateCache()
		{
			...
			{
				if (!IgnoreLocationNames.Contains(j.NameOrUniqueName))
				{
					ExploreWarpPoints(j, new List<string>(), null);
				}
			}
		}
		...
		public static string[] GetLocationRoute(string startingLocation, string endingLocation, Gender gender)
		{
			...
			if (Routes.TryGetValue(startingLocation, out routes))
			{
				foreach (LocationWarpRoute route in routes)
				{
					if (route.LocationNames[route.LocationNames.Length - 1] == endingLocation && (!route.OnlyGender.HasValue || route.OnlyGender == gender || gender == Gender.Undefined))
					{
		...
		private static void ExploreWarpPoints(GameLocation location, List<string> route, Gender? genderRestriction)
		{
			...
			if (GenderRestrictions.TryGetValue(locationName, out newGenderRestriction))
			{
		...
		private static void ExploreWarpPoints(string locationName, List<string> route, Gender? genderRestriction, HashSet<string> seenTargets)
		{
			...
			if (OverrideTargetNames.TryGetValue(locationName, out newLocationName))
			{
				locationName = newLocationName;
			}
			int level;
			if (seenTargets.Add(locationName) && !IgnoreLocationNames.Contains(locationName) && !MineShaft.IsGeneratedLevel(locationName, out level) && !VolcanoDungeon.IsGeneratedLevel(locationName, out level))
			{
		...
		{
			List<LocationWarpRoute> routes;
			if (!Routes.TryGetValue(route[0], out routes))
			{
