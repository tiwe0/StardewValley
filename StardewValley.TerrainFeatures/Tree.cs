	...
	public class Tree : TerrainFeature
	{
		...
		protected static void _LoadWildTreeData()
		{
			...
			foreach (KeyValuePair<string, WildTreeData> pair in _WildTreeData)
			{
				...
				{
					List<string> itemIds;
					if (!_WildTreeSeedLookup.TryGetValue(seedData.QualifiedItemId, out itemIds))
					{
		...
		public static string ResolveTreeTypeFromSeed(string itemId)
		{
			...
			if (metadata?.TypeIdentifier == "(O)" && GetWildTreeSeedLookup().TryGetValue(metadata.LocalItemId, out possibles))
			{
		...
		{
			texture = new Lazy<Texture2D>(_003CresetTexture_003Eg__LoadTexture_007C65_0);
		}
		...
		{
			WildTreeData data;
			if (!TryGetData(treeType.Value, out data))
			{
				return null;
			}
			return data;
				...
				if ((double)Math.Abs(shakeRotation) > Math.PI / 2.0)
				{
					...
					if (data != null)
					{
						...
						if (data.DropWoodOnChop)
						{
							...
							Game1.createRadialDebris(location, 12, (int)tileLocation.X + (shakeLeft ? (-4) : 4), (int)tileLocation.Y, numToDrop, true);
							Game1.createRadialDebris(location, 12, (int)tileLocation.X + (shakeLeft ? (-4) : 4), (int)tileLocation.Y, (int)((Game1.getFarmer(lastPlayerToHit.Value).professions.Contains(12) ? 1.25 : 1.0) * (double)(12 + extraWoodCalculator(tileLocation))), false);
						}
		...
		protected Item TryGetDrop(WildTreeItemData drop, Random r, Farmer targetFarmer, string fieldName, Func<string, string> formatItemId = null, bool? isStump = null)
		{
			...
			if (chopItemData != null && !chopItemData.IsValidForGrowthStage(growthStage.Value, isStump ?? stump.Value))
			{
				return null;
			}
			return ItemQueryResolver.TryResolveRandomItem(drop, new ItemQueryContext(Location, targetFarmer, r), false, null, formatItemId, null, delegate(string query, string error)
			{
				...
				defaultInterpolatedStringHandler.AppendFormatted(treeType.Value);
				...
				if ((int)growthStage >= 5)
				{
					...
					if ((bool)hasSeed && (Game1.IsMultiplayer || Game1.player.ForagingLevel >= 1))
					{
						...
						if (data != null && data.SeedDropItems?.Count > 0)
						{
							...
							{
								Item seed2 = TryGetDrop(drop, Game1.random, Game1.player, "SeedDropItems");
								if (seed2 != null)
								{
			...
			if ((int)growthStage >= 5 && !Game1.IsWinter && !accelerateMoss)
			{
				for (int x = (int)tile.X - 2; (float)x <= tile.X + 2f; x++)
				{
					for (int y = (int)tile.Y - 2; (float)y <= tile.Y + 2f; y++)
					{
						...
						{
							Tree tree = Location.terrainFeatures[v] as Tree;
							if (tree != null && tree.growthStage.Value >= 5 && (tree.treeType.Value == "10" || tree.treeType.Value == "11") && !tree.isTemporaryGreenRainTree.Value && (bool)tree.hasMoss)
							{
								accelerateMoss = true;
								break;
							}
		...
		public override bool seasonUpdate(bool onLoad)
		{
			...
			if (tapped.Value && Location != null)
			{
				...
				if (tile_object != null && tile_object.IsTapper())
				{
					UpdateTapperProduct(tile_object, null, true);
				}
			}
			loadSprite();
			return false;
			...
			else if ((int)growthStage >= 3)
			{
				...
				else
				{
					...
					case 4:
						...
						break;
					default:
						damage2 = 10 + ((int)t.upgradeLevel - 4);
						break;
					}
				}
		...
		public void UpdateTapperProduct(Object tapper, Object previousOutput = null, bool onlyPerformRemovals = false)
		{
			...
			{
				float multiplier;
				if (contextTag.StartsWith("tapper_multiplier_") && float.TryParse(contextTag.Substring("tapper_multiplier_".Length), out multiplier))
				{
			...
			if (TryGetTapperOutput(data.TapItems, previousOutput?.ItemId, random, timeMultiplier, out output, out minutesUntilReady) && (!onlyPerformRemovals || output == null))
			{
		...
		protected void performBushDestroy(Vector2 tileLocation)
		{
			...
			Game1.createRadialDebris(location, 12, (int)tileLocation.X, (int)tileLocation.Y, Game1.random.Next(20, 30), false, -1, false, GetChopDebrisColor(data));
		...
		protected bool performTreeFall(Tool t, int explosion, Vector2 tileLocation)
		{
			...
			else
			{
				...
				if (data != null)
				{
					Game1.createRadialDebris(location, 12, (int)tileLocation.X, (int)tileLocation.Y, Game1.random.Next(30, 40), false, -1, false, GetChopDebrisColor(data));
					Random r;
					if (Game1.IsMultiplayer)
					{
					...
					else if (Game1.IsMultiplayer)
					{
						if (data.DropWoodOnChop)
						{
							Game1.createRadialDebris(location, 12, (int)tileLocation.X, (int)tileLocation.Y, (int)((Game1.getFarmer(lastPlayerToHit.Value).professions.Contains(12) ? 1.25 : 1.0) * 4.0), true);
						}
						List<WildTreeChopItemData> chopItems = data.ChopItems;
						if (chopItems != null && chopItems.Count > 0)
						{
							Farmer targetFarmer2 = Game1.getFarmer(lastPlayerToHit.Value);
							foreach (WildTreeChopItemData drop2 in data.ChopItems)
							{
								Item item2 = TryGetDrop(drop2, r, targetFarmer2, "ChopItems");
								if (item2 != null)
								{
					...
					else
					{
						if (data.DropWoodOnChop)
						{
							Game1.createRadialDebris(location, 12, (int)tileLocation.X, (int)tileLocation.Y, (int)((Game1.getFarmer(lastPlayerToHit.Value).professions.Contains(12) ? 1.25 : 1.0) * (double)(5 + extraWoodCalculator(tileLocation))), true);
						}
		...
		public override void draw(SpriteBatch spriteBatch)
		{
			...
			WildTreeData data;
			if (texture.Value == null || !TryGetData(treeType.Value, out data))
			{
			...
			{
				spriteBatch.Draw(texture.Value, Game1.GlobalToLocal(Game1.viewport, i.position), new Microsoft.Xna.Framework.Rectangle(16 + i.type % 2 * 8, 112 + i.type / 2 * 8, 8, 8), Color.White, i.rotation, Vector2.Zero, 4f, SpriteEffects.None, baseSortPosition / 10000f + 0.01f);
			}
		}
	}
}
