...
namespace StardewValley.BellsAndWhistles
{
	public class Seagull : Critter
	{
		...
		public Seagull(Vector2 position, int startingState)
			: base(0, position)
		{
			moveLeft = Game1.random.NextBool();
		...
		public override bool update(GameTime time, GameLocation environment)
		{
			...
			case 0:
			{
				int delta = (moveLeft ? (-2) : 2);
				if (!environment.isCollidingPosition(getBoundingBox(delta, 0), Game1.viewport, false, 0, false, null, false, false, true))
				{
					position.X += delta;
				}
			...
			case 2:
			{
				...
				if (yOffset > tmpY)
				{
					environment.temporarySprites.Add(new TemporaryAnimatedSprite("TileSheets\\animations", new Rectangle(0, 0, 64, 64), 150f, 8, 0, new Vector2(position.X - 32f, position.Y - 32f), false, Game1.random.NextBool(), 0.001f, 0.01f, Color.White, 1f, 0.003f, 0f, 0f));
				}
				break;
			}
			case 1:
			...
			case 3:
				if (Game1.random.NextDouble() < 0.003 && sprite.CurrentAnimation == null)
				{
					...
					case 0:
					{
						...
						{
							frames.Add(new FarmerSprite.AnimationFrame((short)(baseFrame + 4), 200));
							frames.Add(new FarmerSprite.AnimationFrame((short)(baseFrame + 5), 200));
						}
					...
					case 2:
					{
						List<FarmerSprite.AnimationFrame> frames = new List<FarmerSprite.AnimationFrame>
						{
							...
							new FarmerSprite.AnimationFrame((short)(baseFrame + 7), 100, false, false, hop),
							new FarmerSprite.AnimationFrame((short)(baseFrame + 8), 100)
						};
						int extra = Game1.random.Next(3);
						for (int j = 0; j < extra; j++)
						{
							frames.Add(new FarmerSprite.AnimationFrame((short)(baseFrame + 7), 100));
							frames.Add(new FarmerSprite.AnimationFrame((short)(baseFrame + 8), 100));
						}
						sprite.setCurrentAnimation(frames);
						break;
					}
