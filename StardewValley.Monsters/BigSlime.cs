...
namespace StardewValley.Monsters
{
	public class BigSlime : Monster
	{
		...
		{
			base.initNetFields();
			base.NetFields.AddField(c, "c").AddField(heldItem, "heldItem");
		}
		...
		{
			base.currentLocation.temporarySprites.Add(new TemporaryAnimatedSprite(44, base.Position, c.Value, 10, false, 70f));
			base.currentLocation.temporarySprites.Add(new TemporaryAnimatedSprite(44, base.Position + new Vector2(-32f, 0f), c.Value, 10, false, 70f)
			{
			...
			base.currentLocation.temporarySprites.Add(new TemporaryAnimatedSprite(44, base.Position + new Vector2(0f, -32f), c.Value, 10)
			{
		...
		protected override void updateAnimation(GameTime time)
		{
			...
			if (isMoving())
			{
				...
				heldObjectBobTimer += (float)time.ElapsedGameTime.TotalMilliseconds * 0.007853982f;
			}
			else
			{
				Sprite.interval = 200f;
				heldObjectBobTimer += (float)time.ElapsedGameTime.TotalMilliseconds * 0.003926991f;
			}
		...
		public override List<Item> getExtraDropItems()
		{
			...
			{
				return new List<Item> { heldItem.Value };
			}
		...
		public override void draw(SpriteBatch b)
		{
			if (!base.IsInvisible && Utility.isOnScreen(base.Position, 128))
			{
				...
				heldItem.Value?.drawInMenu(b, getLocalPosition(Game1.viewport) + new Vector2(28f, -16f + (float)Math.Sin(heldObjectBobTimer + 1f) * 4f), 1f, 1f, (float)(standingY - 1) / 10000f, StackDrawType.Hide, Color.White, false);
				b.Draw(Sprite.Texture, getLocalPosition(Game1.viewport) + new Vector2(56f, 16 + yJumpOffset), Sprite.SourceRect, c.Value, rotation, new Vector2(16f, 16f), Math.Max(0.2f, scale.Value) * 4f, flip ? SpriteEffects.FlipHorizontally : SpriteEffects.None, Math.Max(0f, drawOnTop ? 0.991f : ((float)standingY / 10000f)));
				if (isGlowing)
				{
