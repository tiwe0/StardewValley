	...
	public class TrashBear : NPC
	{
		...
		public void updateItemWanted()
		{
			...
			foreach (string v in CraftingRecipe.cookingRecipes.Values)
			{
				if (counter == position)
				{
					...
					craft_result = (itemWantedIndex = ArgUtility.SplitBySpaceAndGet(craft_result, 0));
					break;
				}
				counter++;
			}
		...
		public void doEatEvent(string item_index)
		{
			if (Game1.currentLocation is Forest)
			{
				...
				{
					new FarmerSprite.AnimationFrame(9, 1500, false, false, throwUpItem, true),
					new FarmerSprite.AnimationFrame(5, 1000, false, false),
					new FarmerSprite.AnimationFrame(6, 250, false, false, chew),
					new FarmerSprite.AnimationFrame(7, 250, false, false),
					new FarmerSprite.AnimationFrame(6, 250, false, false),
					new FarmerSprite.AnimationFrame(7, 250, false, false),
					new FarmerSprite.AnimationFrame(6, 250, false, false),
					new FarmerSprite.AnimationFrame(7, 500, false, false, doneAnimating, true)
				});
				...
				Game1.currentLocation.temporarySprites.Add(new TemporaryAnimatedSprite(textureName, sourceRect, 1500f, 1, 0, base.Position + new Vector2(96f, -92f), false, false, (float)(base.StandingPixel.Y + 1) / 10000f, 0f, Color.White, 4f, 0f, 0f, 0f));
			}
		}
		...
		private void throwUpItem(Farmer who)
		{
			...
			string textureName = dataOrErrorItem.GetTextureName();
			Rectangle sourceRect = dataOrErrorItem.GetSourceRect();
			Game1.currentLocation.temporarySprites.Add(new TemporaryAnimatedSprite(textureName, sourceRect, 1000f, 1, 0, base.Position + new Vector2(96f, -108f), false, false, (float)(base.StandingPixel.Y + 1) / 10000f, 0f, Color.White, 4f, -0.01f, 0f, 0f)
			{
		...
		private void chew(Farmer who)
		{
			...
			for (int i = 0; i < 8; i++)
			{
				...
				sourceRect.Width = 4;
				sourceRect.Height = 4;
				Game1.currentLocation.temporarySprites.Add(new TemporaryAnimatedSprite(textureName, sourceRect, 400f, 1, 0, base.Position + new Vector2(64f, -48f), false, false, (float)base.StandingPixel.Y / 10000f + 0.01f, 0f, Color.White, 4f, 0f, 0f, 0f)
				{
