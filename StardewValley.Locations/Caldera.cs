...
namespace StardewValley.Locations
{
	public class Caldera : IslandLocation
	{
		...
		protected override void resetLocalState()
		{
			...
			{
				temporarySprites.Add(new TemporaryAnimatedSprite("TileSheets\\critters", new Microsoft.Xna.Framework.Rectangle(15, 333, 13, 12), new Vector2(908.8f, 1792f), false, 0f, Color.White)
				{
					scale = 4f,
					interval = 99999f,
		...
		public override bool performToolAction(Tool t, int tileX, int tileY)
		{
			if (t is WateringCan && isTileOnMap(new Vector2(tileX, tileY)) && waterTiles[tileX, tileY])
			{
				for (int j = 0; j < 10; j++)
				{
					TemporaryAnimatedSprite s = new TemporaryAnimatedSprite("LooseSprites\\Cursors", new Microsoft.Xna.Framework.Rectangle(276, 1965, 8, 8), new Vector2((float)tileX + 0.5f, (float)tileY + 0.5f) * 64f + new Vector2(Game1.random.Next(-16, 16), Game1.random.Next(-16, 16)), false, 0.02f, Color.White)
					{
						scale = 3f,
						animationLength = 7,
				...
				for (int i = 0; i < 5; i++)
				{
					temporarySprites.Add(new TemporaryAnimatedSprite("LooseSprites\\Cursors", new Microsoft.Xna.Framework.Rectangle(372, 1956, 10, 10), new Vector2(tileX, (float)tileY - 0.5f) * 64f + new Vector2(Game1.random.Next(64), Game1.random.Next(64)), false, 0.007f, Color.White)
					{
						alpha = 0.75f,
