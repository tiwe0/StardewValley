	...
	public class MineShaft : GameLocation
	{
		...
		public void calicoStatueActivated(NetPoint field, Point oldVector, Point newVector)
		{
			...
			if (Game1.currentLocation != null && Game1.currentLocation.Equals(this))
			{
				...
				temporarySprites.Add(new TemporaryAnimatedSprite("LooseSprites\\Cursors_1_6", new Microsoft.Xna.Framework.Rectangle(20, 0, 19, 21), new Vector2(newVector.X * 64 - 4, ((float)newVector.Y - 2.5f) * 64f), false, 0f, Color.White)
				{
			...
			{
				if (!tryToAddCalicoStatueEffect(r, 0.15, 10) && !tryToAddCalicoStatueEffect(r, 0.01, 17, true) && !tryToAddCalicoStatueEffect(r, 0.05, 12, true) && !tryToAddCalicoStatueEffect(r, 0.1, 15, true) && !tryToAddCalicoStatueEffect(r, 0.2, 16, true) && !tryToAddCalicoStatueEffect(r, 0.1, 14, true) && !tryToAddCalicoStatueEffect(r, 0.5, 11, true))
				{
			...
			if (!tryToAddCalicoStatueEffect(r, 0.1, 4) && !tryToAddCalicoStatueEffect(r, 0.1, 9) && !tryToAddCalicoStatueEffect(r, 0.1, 5) && !tryToAddCalicoStatueEffect(r, 0.1, 6) && !tryToAddCalicoStatueEffect(r, 0.2, 7, true) && !tryToAddCalicoStatueEffect(r, 0.2, 8, true))
			{
				Game1.player.team.AddCalicoStatueEffect(13);
				signalCalicoStatueActivation(13);
			}
		...
		{
			MineInfo changes;
			if (permanentMineChanges.TryGetValue(mineLevel, out changes))
			{
		...
		{
			FishingRod r = who?.CurrentTool as FishingRod;
		...
		private void populateLevel()
		{
			...
			{
				Layer backLayer2 = map.RequireLayer("Back");
				...
				for (int k = 0; k < numBarrels; k++)
				{
					...
					{
						p = new Point(mineRandom.Next(backLayer2.LayerWidth), 0);
						motion = new Point(0, 1);
					}
					else if (mineRandom.NextBool())
					{
						p = new Point(0, mineRandom.Next(backLayer2.LayerHeight));
						motion = new Point(1, 0);
					}
					else
					{
						p = new Point(backLayer2.LayerWidth - 1, mineRandom.Next(backLayer2.LayerHeight));
						motion = new Point(-1, 0);
					}
					while (isTileOnMap(p.X, p.Y))
					{
						...
						{
							if (Utility.GetDayOfPassiveFestival("DesertFestival") > 0 && getMineArea() == 121 && !hasAddedDesertFestivalStatue && getTileIndexAt((int)objectPos5.X, (int)objectPos5.Y - 1, "Buildings") != -1)
							{
							...
							{
								objects.Add(objectPos5, BreakableContainer.GetBarrelForMines(objectPos5, this));
							}
						}
						break;
					}
			...
			if (mineLevel % 10 != 0 || (getMineArea() == 121 && !isForcedChestLevel(mineLevel) && !netIsTreasureRoom.Value))
			{
				...
				for (int j = 0; j < backLayer.LayerWidth; j++)
				{
					for (int l = 0; l < backLayer.LayerHeight; l++)
					{
						...
						if (isTileClearForMineObjects(j, l))
						{
							if (mineRandom.NextDouble() <= stoneChance)
							{
								...
								if (getMineArea() == 40 && mineRandom.NextDouble() < 0.15)
								{
									...
									base.Objects.Add(objectPos4, new Object(which2.ToString(), 1)
									{
										Fragility = 2,
										CanBeGrabbed = true
									});
								...
								if ((bool)rainbowLights && mineRandom.NextDouble() < 0.55)
								{
									if (mineRandom.NextDouble() < 0.25)
									{
										...
										base.Objects.Add(objectPos4, obj);
									}
									continue;
								}
								Object litter = createLitterObject(0.001, 5E-05, gemStoneChance, objectPos4);
								if (litter != null)
								{
									base.Objects.Add(objectPos4, litter);
							...
							else if (mineRandom.NextDouble() <= monsterChance && getDistanceFromStart(j, l) > 5f)
							{
									...
									foreach (int invasionId in calicoStatueInvasionIds)
									{
										...
										for (int invasion = 0; invasion < amount; invasion++)
										{
											if (mineRandom.NextBool(0.15))
											{
												...
												{
												case 3:
													monsterToAdd2 = new Bat(position, mineLevel);
													break;
												case 0:
													monsterToAdd2 = new Ghost(position, "Carbon Ghost");
													break;
												case 1:
													monsterToAdd2 = new Serpent(position);
													break;
												case 2:
													monsterToAdd2 = ((!(mineRandom.NextDouble() < 0.33)) ? ((Monster)new Skeleton(position, mineRandom.NextBool())) : ((Monster)new Bat(position, 77377)));
													monsterToAdd2.BuffForAdditionalDifficulty(1);
													break;
												}
												break;
											}
								...
								if (monsterToAdd2 == null)
								{
									monsterToAdd2 = BuffMonsterIfNecessary(getMonsterForThisLevel(mineLevel, j, l));
								}
								GreenSlime slime = monsterToAdd2 as GreenSlime;
								if (slime == null)
								{
									if (!(monsterToAdd2 is Leaper))
									{
										...
										{
											if (monsterToAdd2 is DustSpirit)
											{
							...
							else if (mineRandom.NextDouble() <= itemChance)
							{
								...
								base.Objects.Add(objectPos3, getRandomItemForThisLevel(mineLevel, objectPos3));
							}
						...
						else if (isContainerPlatform(j, l) && CanItemBePlacedHere(new Vector2(j, l)) && mineRandom.NextDouble() < 0.4 && (firstTime || canAdd(0, barrelsAdded)))
						{
						...
						{
							if (!(mineRandom.NextDouble() <= monsterChance) || !CanSpawnCharacterHere(new Vector2(j, l)) || !isTileOnClearAndSolidGround(j, l) || !(getDistanceFromStart(j, l) > 5f) || (AnyOnlineFarmerHasBuff("23") && getMineArea() != 121))
							{
								continue;
							}
							Monster monsterToAdd = BuffMonsterIfNecessary(getMonsterForThisLevel(mineLevel, j, l));
							if (monsterToAdd.GetBoundingBox().Width <= 64 || isTileClearForMineObjects(j + 1, l))
							{
				...
				if (stonesLeftOnThisLevel > 35)
				{
					...
					for (int i = 0; i < tries; i++)
					{
						Vector2 stone;
						Object obj2;
						if (!Utility.TryGetRandom(objects, out stone, out obj2) || !obj2.IsBreakableStone())
						{
							continue;
						}
		...
		{
			if (CanItemBePlacedHere(tile, false, CollisionMask.All, CollisionMask.None))
			{
			...
			else
			{
				...
				case 220:
					if (Game1.player.secretNotesSeen.Contains(10) && !Game1.player.mailReceived.Contains("qiCave"))
					{
						...
						currentEvent.exitLocation = new LocationRequest(base.Name, false, this);
						Game1.player.chestConsumedMineLevels[mineLevel] = true;
					}
					else
					{
		...
		public void loadLevel(int level)
		{
			...
			if (getMineArea(level) == 121)
			{
				...
				{
					double chance2 = 0.01;
					chance2 += Game1.player.team.AverageDailyLuck(this) / 10.0 + Game1.player.team.AverageLuckLevel(this) / 100.0;
		...
		public override bool checkAction(Location tileLocation, xTile.Dimensions.Rectangle viewport, Farmer who)
		{
			if (who.IsLocalPlayer)
			{
				...
				case 194:
					...
					Game1.createRadialDebris(this, 382, tileLocation.X, tileLocation.Y, 6, false, -1, true);
		...
		public override void drawAboveAlwaysFrontLayer(SpriteBatch b)
		{
			...
			SpriteText.drawString(b, txt, tsarea.Left + 16, tsarea.Top + 16, 999999, -1, height, 1f, 1f, false, 2, "", col);
			...
			{
				BuffsDisplay _bd = onScreenMenu as BuffsDisplay;
				if (_bd != null)
				{
		...
		private static void clearInactiveMines(bool keepUntickedLevels = true)
		{
			...
			{
				int player_mine_level;
				if (farmer.locationBeforeForcedEvent.Value == null || !IsGeneratedLevel(farmer.locationBeforeForcedEvent.Value, out player_mine_level))
				{
					continue;
				}
		...
		public static MineShaft GetMine(string name)
		{
			...
			int mineLevel;
			if (!IsGeneratedLevel(name, out mineLevel))
			{
