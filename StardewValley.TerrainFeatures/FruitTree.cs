	...
	public class FruitTree : TerrainFeature
	{
		...
		public override void loadSprite()
		{
			...
			if (texture == null || textureName != assetName)
			{
				...
				catch (Exception ex)
				{
					...
					defaultInterpolatedStringHandler.AppendFormatted(treeId.Value);
		...
		public override bool tickUpdate(GameTime time)
		{
			...
			if (!falling)
			{
				...
				{
					Game1.multiplayer.broadcastSprites(location, new TemporaryAnimatedSprite("LooseSprites\\Cursors", new Rectangle(372, 1956, 10, 10), new Vector2(tileLocation.X * 64f + (float)Game1.random.Next(-64, 96), tileLocation.Y * 64f - 192f + (float)Game1.random.Next(-64, 128)), false, 0.002f, Color.Gray)
					{
						alpha = 0.75f,
			...
			else
			{
				...
				if ((double)Math.Abs(shakeRotation) > Math.PI / 2.0)
				{
					...
					Game1.createRadialDebris(location, 12, (int)tileLocation.X + (shakeLeft ? (-4) : 4), (int)tileLocation.Y, (int)((Game1.getFarmer(lastPlayerToHit.Value).professions.Contains(12) ? 1.25 : 1.0) * 12.0), true);
					Game1.createRadialDebris(location, 12, (int)tileLocation.X + (shakeLeft ? (-4) : 4), (int)tileLocation.Y, (int)((Game1.getFarmer(lastPlayerToHit.Value).professions.Contains(12) ? 1.25 : 1.0) * 12.0), false);
		...
		public int GetQuality()
		{
			...
			switch ((int)daysUntilMature / -112)
			{
			case 0:
				return 0;
			case 1:
		...
		public static bool IsGrowthBlocked(Vector2 tileLocation, GameLocation environment)
		{
			...
			for (int num = 0; num < surroundingTileLocationsArray.Length; num++)
			{
				Vector2 v = surroundingTileLocationsArray[num];
				TerrainFeature terrainFeature;
				int num2;
				...
				goto IL_003d;
				IL_003d:
				bool isClearHoeDirt = (byte)num2 != 0;
		...
		{
			FruitTreeData data;
			if (!TryGetData(treeId.Value, out data))
			{
				return null;
			}
			return data;
		...
		private Item TryCreateFruit(FruitTreeFruitData drop)
		{
			...
			Item item = ItemQueryResolver.TryResolveRandomItem(drop, new ItemQueryContext(Location, null, null), false, null, null, null, delegate(string query, string error)
			{
				IGameLogger log = Game1.log;
				...
				defaultInterpolatedStringHandler.AppendLiteral("' for fruit '");
				...
				if (health.Value <= 0f)
				{
					...
					else
					{
						...
						else if (Game1.IsMultiplayer)
						{
							...
							Game1.createRadialDebris(location, 12, (int)tileLocation.X, (int)tileLocation.Y, Game1.getFarmer(lastPlayerToHit.Value).professions.Contains(12) ? 5 : 4, true);
						}
						else
						{
							Game1.createRadialDebris(location, 12, (int)tileLocation.X, (int)tileLocation.Y, (int)((Game1.getFarmer(lastPlayerToHit.Value).professions.Contains(12) ? 1.25 : 1.0) * 5.0), true);
		...
		public override void draw(SpriteBatch spriteBatch)
		{
			...
			if ((int)growthStage < 4)
			{
				...
				{
				case 0:
					sourceRect2 = new Rectangle(0, spriteRow * 5 * 16, 48, 80);
					break;
				case 1:
					sourceRect2 = new Rectangle(48, spriteRow * 5 * 16, 48, 80);
					break;
				case 2:
					sourceRect2 = new Rectangle(96, spriteRow * 5 * 16, 48, 80);
					break;
				default:
					sourceRect2 = new Rectangle(144, spriteRow * 5 * 16, 48, 80);
					break;
				}
				spriteBatch.Draw(this.texture, Game1.GlobalToLocal(Game1.viewport, new Vector2(tileLocation.X * 64f + 32f + positionOffset.X, tileLocation.Y * 64f - (float)sourceRect2.Height + 128f + positionOffset.Y)), sourceRect2, Color.White, shakeRotation, new Vector2(24f, 80f), 4f, flipped ? SpriteEffects.FlipHorizontally : SpriteEffects.None, (float)boundingBox.Bottom / 10000f - tileLocation.X / 1000000f);
			}
			else
			{
				if (!stump || (bool)falling)
