...
namespace StardewValley.SpecialOrders
{
	...
	public class SpecialOrder : INetObject<NetFields>, IQuest
	{
		...
		{
			bool acceptable;
			if (_highlightLookup != null && _highlightLookup.TryGetValue(item, out acceptable))
			{
				return acceptable;
			}
		...
		public static bool CheckTag(string tag)
		{
			...
			{
				string value5 = tag.Substring("completed_".Length);
				if (Game1.player.team.completedSpecialOrders.Contains(value5))
				{
					return true;
				}
			}
			...
			{
				string value2 = tag.Substring("event_".Length);
		...
		public bool IsIslandOrder()
		{
			...
			if (_isIslandOrder == -1 && DataLoader.SpecialOrders(Game1.content).TryGetValue(questKey.Value, out data))
			{
		...
		public virtual string Parse(string data)
		{
			...
			do
			{
				...
				if (_orderData.RandomizedElements != null)
				{
					string itemId;
					int index;
					if (preSelectedItems.TryGetValue(key, out itemId))
					{
		...
		public virtual void Update()
		{
			...
			if (!readyForRemoval.Value)
			{
				...
				case SpecialOrderStatus.InProgress:
					participants.TryAdd(Game1.player.UniqueMultiplayerID, true);
					break;
		...
		public static void RemoveSpecialRuleAtEndOfDay(string rule)
		{
			...
			case "QI_COOKING":
				Utility.ForEachItem(delegate(Item item)
				{
					...
					if (@object != null && @object.orderData.Value == "QI_COOKING")
					{
		...
		public void CheckCompletion()
		{
			...
			{
				unclaimedRewards.TryAdd(farmer_id, true);
			}
		...
		{
			seenParticipants.TryAdd(Game1.player.UniqueMultiplayerID, true);
		}
		...
		public int GetMoneyReward()
		{
			if (_moneyReward == -1)
			{
				...
				foreach (OrderReward reward in rewards)
				{
					MoneyReward moneyReward = reward as MoneyReward;
					if (moneyReward != null)
					{
