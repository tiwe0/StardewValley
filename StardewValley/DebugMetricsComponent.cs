...
namespace StardewValley
{
	public class DebugMetricsComponent : DrawableGameComponent
	{
		...
		protected override void LoadContent()
		{
			...
			int w = 2;
			int h = 2;
			_opaqueWhite = new Texture2D(base.GraphicsDevice, w, h, false, SurfaceFormat.Color);
		...
		private void DrawLine(Color color, StringBuilder sb, int x)
		{
			if (sb != null)
			{
				...
				int y = _drawY;
				int yoffset = _font.LineSpacing;
				yoffset -= yoffset / 10;
				_spriteBatch.Draw(_opaqueWhite, new Rectangle(x - 1, y, (int)size.X + 2, yoffset), null, Color.Black * 0.5f);
