...
namespace StardewValley.Menus
{
	public class JunimoNoteMenu : IClickableMenu
	{
			...
			: base(Game1.uiViewport.Width / 2 - 640, Game1.uiViewport.Height / 2 - 360, 1280, 720, true)
		{
			...
			tempSprites.Clear();
			inventory = new InventoryMenu(xPositionOnScreen + 128, yPositionOnScreen + 140, true, null, HighlightObjects, 36, 6, 8, 8, false)
			{
				capacity = 36
			};
		...
		public void setUpMenu(int whichArea, Dictionary<int, bool[]> bundlesComplete)
		{
			...
			foreach (string j in bundlesInfo.Keys)
			{
				if (j.Contains(areaName))
				{
					...
					bundles.Add(new Bundle(bundleIndex, bundlesInfo[j], bundlesComplete[bundleIndex], getBundleLocationFromNumber(bundlesAdded), "LooseSprites\\JunimoNote", this)
					{
				...
				else if (heldItem != null)
				{
					if (Game1.oldKBState.IsKeyDown(Keys.LeftShift))
					{
						for (int l = 0; l < ingredientSlots.Count; l++)
						{
							...
							{
								if (ingredientSlots[l].item == null)
								{
		...
		public void SwapPage(int direction)
		{
			...
			for (int i = 0; i < area_count; i++)
			{
				...
				if (cc.shouldNoteAppearInArea(area))
				{
					...
					JunimoNoteMenu new_menu = (JunimoNoteMenu)(Game1.activeClickableMenu = new JunimoNoteMenu(true, area, true)
					{
		...
		protected override void cleanupBeforeExit()
		{
			...
			if (gameMenuTabToReturnTo != -1)
			{
				Game1.activeClickableMenu = new GameMenu(gameMenuTabToReturnTo, -1, false);
			}
		...
		private void updateIngredientSlots()
		{
			...
			foreach (BundleIngredientDescription ingredient in currentPageBundle.ingredients)
			{
				if (ingredient.completed && slotNumber < ingredientSlots.Count)
				{
					...
					currentPageBundle.ingredientDepositAnimation(ingredientSlots[slotNumber], "LooseSprites\\JunimoNote", true);
					slotNumber++;
				}
			}
		}
		...
		private void openRewardsMenu()
		{
			...
			Game1.activeClickableMenu = new ItemGrabMenu(rewards, false, true, null, null, null, rewardGrabbed, false, true, true, true, false, 0, null, -1, this);
		...
		public override void draw(SpriteBatch b)
		{
			...
			else
			{
				...
				{
					tempSprite2.draw(b, true, 0, 0, completed_slot_alpha);
				}
				...
				for (int i = 0; i < ingredientList.Count; i++)
				{
					...
					{
						c.item.drawInMenu(b, new Vector2(c.bounds.X, c.bounds.Y), c.scale / 4f, 1f, 0.9f, StackDrawType.Draw, Color.White * (completed ? 0.25f : alpha_mult), false);
					}
				}
				inventory.draw(b);
			}
		...
		public override void gameWindowSizeChanged(Rectangle oldBounds, Rectangle newBounds)
		{
			...
			inventory = new InventoryMenu(xPositionOnScreen + 128, yPositionOnScreen + 140, true, null, HighlightObjects, Game1.player.maxItems, 6, 8, 8, false);
		...
		private void setUpBundleSpecificPage(Bundle b)
		{
			...
			for (int j = 0; j < ingredientListRectangles.Count; j++)
			{
				...
				if (itemData.HasTypeObject())
				{
					...
					ingredientList.Add(new ClickableTextureComponent("ingredient_list_slot", ingredientListRectangles[j], "", displayName, texture, sourceRect, 4f)
					{
						myID = j + 1000,
						item = item,
						upNeighborID = -99998,
