...
namespace StardewValley.Menus
{
	...
	public class MapPage : IClickableMenu
	{
			...
			: base(x, y, width, height)
		{
			...
			for (int i = 0; i < array.Length; i++)
			{
				...
				foreach (MapAreaTooltip tooltip2 in tooltips)
				{
					...
					if (points.TryGetValue(tooltip2.NamespacedId, out component2))
					{
						SetNeighborId(component2, "left", tooltip2.Data.LeftNeighbor);
						SetNeighborId(component2, "right", tooltip2.Data.RightNeighbor);
						SetNeighborId(component2, "up", tooltip2.Data.UpNeighbor);
						SetNeighborId(component2, "down", tooltip2.Data.DownNeighbor);
					}
				}
			}
		}
		...
		public void SetNeighborId(ClickableComponent component, string direction, string neighborKeys)
		{
			...
			int neighborId;
			bool foundIgnore;
			if (!TryGetNeighborId(neighborKeys, out neighborId, out foundIgnore))
			{
				if (!foundIgnore)
				{
		...
		public bool TryGetNeighborId(string keys, out int id, out bool foundIgnore, bool isAlias = false)
		{
			...
			if (!string.IsNullOrWhiteSpace(keys))
			{
				...
				for (int i = 0; i < array.Length; i++)
				{
					...
					if (points.TryGetValue(key, out neighbor))
					{
						id = neighbor.myID;
						return true;
					}
					string alias;
					if (!isAlias && mapRegion.Data.MapNeighborIdAliases.TryGetValue(key, out alias))
					{
						bool localFoundIgnore;
						if (TryGetNeighborId(alias, out id, out localFoundIgnore, true))
						{
		...
		public virtual void drawMap(SpriteBatch b, bool drawBorders = true)
		{
			if (drawBorders)
			{
				...
				Game1.drawDialogueBox(mapBounds.X - 32, boxY, (mapBounds.Width + 16) * 4, (mapBounds.Height + 32) * 4, false, true);
			}
			float sortLayer = 0.86f;
			MapAreaTexture baseTexture = mapRegion.GetBaseTexture();
			if (baseTexture != null)
			{
				Rectangle destRect2 = baseTexture.GetOffsetMapPixelArea(mapBounds.X, mapBounds.Y);
				b.Draw(baseTexture.Texture, destRect2, baseTexture.SourceRect, Color.White, 0f, Vector2.Zero, SpriteEffects.None, sortLayer);
				sortLayer += 0.001f;
			}
			MapArea[] array = mapAreas;
			for (int i = 0; i < array.Length; i++)
			...
			foreach (MapArea area in array)
			{
				...
				if (EnableDebugLines.HasFlag(WorldMapDebugLineType.Areas))
				{
					...
					if (pixelArea2.Width > 0 || pixelArea2.Height > 0)
					{
						...
						Utility.DrawSquare(b, pixelArea2, 4, Color.Black);
					}
				}
