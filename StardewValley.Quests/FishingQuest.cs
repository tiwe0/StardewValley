	...
	public class FishingQuest : Quest
	{
		...
		public void loadQuestInfo()
		{
			...
			if (random.NextBool())
			{
				...
				Item fish2 = ItemRegistry.Create(ItemId.Value);
				bool isOctopus = ItemId.Value == "(O)149";
				numberToFish.Value = (int)Math.Ceiling(90.0 / (double)Math.Max(1, GetGoldRewardPerItem(fish2))) + Game1.player.FishingLevel / 5;
				reward.Value = numberToFish.Value * GetGoldRewardPerItem(fish2);
				target.Value = "Demetrius";
				parts.Clear();
				parts.Add(new DescriptionElement("Strings\\StringsFromCSFiles:FishingQuest.cs.13228", fish2, numberToFish.Value));
				dialogueparts.Clear();
				dialogueparts.Add(new DescriptionElement("Strings\\StringsFromCSFiles:FishingQuest.cs.13231", fish2, random.Choose(new DescriptionElement("Strings\\StringsFromCSFiles:FishingQuest.cs.13233"), new DescriptionElement("Strings\\StringsFromCSFiles:FishingQuest.cs.13234"), new DescriptionElement("Strings\\StringsFromCSFiles:FishingQuest.cs.13235"), new DescriptionElement("Strings\\StringsFromCSFiles:FishingQuest.cs.13236", fish2))));
				objective.Value = (isOctopus ? new DescriptionElement("Strings\\StringsFromCSFiles:FishingQuest.cs.13243", 0, numberToFish.Value) : new DescriptionElement("Strings\\StringsFromCSFiles:FishingQuest.cs.13244", 0, numberToFish.Value, fish2));
			}
			else
			{
				switch (Game1.season)
		...
		{
			return (item as Object)?.Price ?? ((int)((float)item.salePrice() * 1.5f));
		}
	}
}
