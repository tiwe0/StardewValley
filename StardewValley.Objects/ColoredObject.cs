...
namespace StardewValley.Objects
{
	public class ColoredObject : Object
	{
		...
		protected override void GetOneCopyFrom(Item source)
		{
			...
			ColoredObject fromObj = source as ColoredObject;
			if (fromObj != null)
			{
		...
		public override void draw(SpriteBatch spriteBatch, int x, int y, float alpha = 1f)
		{
			if ((bool)bigCraftable)
			{
				...
				{
					Rectangle coloredSourceRect2 = itemData2.GetSourceRect(indexOffset + 1, base.ParentSheetIndex);
					spriteBatch.Draw(texture2, destination, itemData2.GetSourceRect(indexOffset, base.ParentSheetIndex), Color.White, 0f, Vector2.Zero, SpriteEffects.None, Math.Max(0f, (float)((y + 1) * 64 - 1) / 10000f));
					spriteBatch.Draw(texture2, destination, coloredSourceRect2, color.Value, 0f, Vector2.Zero, SpriteEffects.None, Math.Max(0f, (float)((y + 1) * 64 - 1) / 10000f));
				}
				else
				{
					spriteBatch.Draw(texture2, destination, itemData2.GetSourceRect(0, base.ParentSheetIndex), color.Value, 0f, Vector2.Zero, SpriteEffects.None, Math.Max(0f, (float)((y + 1) * 64 - 1) / 10000f));
				}
