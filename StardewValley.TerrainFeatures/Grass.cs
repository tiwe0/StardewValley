	...
	public class Grass : TerrainFeature
	{
		...
		public override void doCollisionAction(Rectangle positionOfCollider, int speedOfCollision, Vector2 tileLocation, Character who)
		{
			...
			if (who is Farmer)
			{
				...
				if (weapon != null && weapon.isOnSpecial && (int)weapon.type == 0 && Math.Abs(shakeRotation) < 0.001f && performToolAction(Game1.player.CurrentTool, -1, tileLocation))
				{
				...
				else if (tool != null && tool.isScythe())
				{
					...
					if (obj.NextDouble() < chance)
					{
						...
						if (GameLocation.StoreHayInAnySilo(num, Location) == 0)
						{
							if (addAnimation)
							{
								TemporaryAnimatedSprite tmpSprite = new TemporaryAnimatedSprite("Maps\\springobjects", Game1.getSourceRectForStandardTileSheet(Game1.objectSpriteSheet, 178, 16, 16), 750f, 1, 0, player.Position - new Vector2(0f, 128f), false, false, player.Position.Y / 10000f, 0.005f, Color.White, 4f, -0.005f, 0f, 0f);
