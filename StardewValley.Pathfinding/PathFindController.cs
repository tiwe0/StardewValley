...
namespace StardewValley.Pathfinding
{
	...
	public class PathFindController
	{
			...
			try
			{
				...
				while (!openList.IsEmpty())
				{
					...
					for (int i = 0; i < 4; i++)
					{
						...
						if (!ignore_obstructions && location.isCollidingPosition(new Rectangle(neighbor.x * 64 + 1, neighbor.y * 64 + 1, 62, 62), Game1.viewport, character is Farmer, 0, false, character, true))
						{
							closedList.Add(nid);
							continue;
						}
		...
		public void handleWarps(Rectangle position)
		{
			...
			{
				w = new Warp(w.X, w.Y, "Trailer_Big", 13, 24, false);
			}
			NPC spouse = character as NPC;
			if (spouse != null && spouse.isMarried() && spouse.followSchedule)
			{
				...
				if (!(gameLocation is FarmHouse))
				{
					if (gameLocation is BusStop && w.X <= 9)
					{
						...
						w = new Warp(w.X, w.Y, home.name, homeEntry.X, homeEntry.Y, false);
					}
				}
				else
				{
					w = new Warp(w.X, w.Y, "BusStop", 10, 23, false);
				}
			...
			foreach (string activePassiveFestival in Game1.netWorldState.Value.ActivePassiveFestivals)
			{
				...
				string newName;
				if (Utility.TryGetPassiveFestivalData(activePassiveFestival, out data) && data.MapReplacements != null && data.MapReplacements.TryGetValue(targetLocationName, out newName))
				{
			...
			if (npc != null && (w.TargetName == "FarmHouse" || w.TargetName == "Cabin") && npc.isMarried() && npc.getSpouse() != null)
			{
				...
				w = new Warp(w.X, w.Y, location.name, entryPoint.X, entryPoint.Y, false);
