...
namespace StardewValley.Locations
{
	public class DesertFestival : Desert
	{
		...
		{
			Game1.changeMusicTrack(GetFestivalMusic(), true);
		}
		...
		public virtual void CollectRacePrizes()
		{
			...
			Game1.activeClickableMenu = new ItemGrabMenu(rewards, false, true, null, null, "Rewards", null, false, true, false, false, false, 0, null, -1, this);
		}
		...
		public override bool performAction(string action, Farmer who, Location tile_location)
		{
			...
			case "DesertRacerMan":
				...
				if (currentRaceState.Value >= RaceState.Go && currentRaceState.Value < RaceState.AnnounceWinner4)
				{
					...
					{
						int guessed_racer2 = raceGuesses[Game1.player.UniqueMultiplayerID];
						Game1.drawObjectDialogue(Game1.content.LoadString("Strings\\1_6_Strings:Race_Guess_Already_Made", Game1.content.LoadString("Strings\\1_6_Strings:Racer_" + guessed_racer2)));
						return true;
					}
		...
		public override bool answerDialogueAction(string question_and_answer, string[] question_params)
		{
			...
			if (question_and_answer.StartsWith("Cook"))
			{
				...
				else if (question_and_answer.StartsWith("Cook_ChoseSauce"))
				{
					...
					Game1.afterDialogues = (Game1.afterFadeFunction)Delegate.Combine(Game1.afterDialogues, (Game1.afterFadeFunction)delegate
					{
						temporarySprites.Add(new TemporaryAnimatedSprite("Maps\\desert_festival_tilesheet", new Microsoft.Xna.Framework.Rectangle(320, 280, 29, 24), new Vector2(480f, 1372f), false, 0f, Color.White)
						{
							id = 1001,
							animationLength = 2,
							interval = 200f,
						...
						temporarySprites.Add(new TemporaryAnimatedSprite("Maps\\desert_festival_tilesheet", new Microsoft.Xna.Framework.Rectangle(378, 280, 29, 24), new Vector2(480f, 1372f), false, 0f, Color.White)
						{
							id = 1002,
							animationLength = 4,
							interval = 100f,
						...
						for (int k = 0; k < 12; k++)
						{
							temporarySprites.Add(new TemporaryAnimatedSprite(30, new Vector2(460.8f + (float)Game1.random.Next(-10, 10), 1388 + Game1.random.Next(-10, 10)), Color.White, 4, false, 100f, 2)
							{
							...
							temporarySprites.Add(new TemporaryAnimatedSprite(30, new Vector2(544f + (float)Game1.random.Next(-10, 10), 1388 + Game1.random.Next(-10, 10)), Color.White, 4, false, 100f, 2)
							{
								delayBeforeAnimationStart = 2700 + k * 80,
								motion = new Vector2(1f + (float)Game1.random.Next(-5, 5) / 10f, -1f + (float)Game1.random.Next(-5, 5) / 10f),
								drawAboveAlwaysFront = true
				...
				else if (question_and_answer.StartsWith("Cook_PickedIngredient"))
				{
					...
					for (int j = 0; j < 5; j++)
					{
						if (j != _cookIngredient || _cookIngredient == 4)
						{
							sauces.Add(new Response(j.ToString() ?? "", Game1.content.LoadString("Strings\\1_6_Strings:Cook_Sauce" + j)));
						}
					}
		...
		{
			temporarySprites.Add(new TemporaryAnimatedSprite("LooseSprites\\Cursors", new Microsoft.Xna.Framework.Rectangle(372, 1956, 10, 10), v, false, 0.002f, Color.Gray)
			{
				alpha = 0.75f,
		...
		public override void draw(SpriteBatch spriteBatch)
		{
			...
			if (!hasAcceptedFishingQuestToday)
			{
				...
				spriteBatch.Draw(Game1.mouseCursors, Game1.GlobalToLocal(Game1.viewport, new Vector2(984f, 842f + yOffset2)), new Microsoft.Xna.Framework.Rectangle(395, 497, 3, 8), Color.White, 0f, new Vector2(1f, 4f), 4f + Math.Max(0f, 0.25f - yOffset2 / 16f), SpriteEffects.None, 1f);
			}
		...
		public Vector3 GetTrackPosition(int track_index, float horizontal_position)
		{
			...
			bool flag = inner_edge == outer_edge;
		...
		protected override void resetLocalState()
		{
			...
			for (int i = 0; i < 8; i++)
			{
				critters.Add(new Butterfly(this, getRandomTile(), false, true));
			}
			eggMoneyDial = new MoneyDial(4, false);
		...
		public static void SetupFestivalDay()
		{
			...
			if (characters.Count > 0)
			{
				...
				SetupMerchantSchedule(character4, 0);
			}
			if (characters.Count > 0)
			{
				NPC character3 = r.ChooseFrom(characters);
				characters.Remove(character3);
				SetupMerchantSchedule(character3, 1);
			}
			...
			if (festival_location != null)
			{
				...
				for (int j = 0; j < festival_location.totalRacers; j++)
				{
					racers.Add(j);
				}
