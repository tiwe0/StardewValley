	...
	public class LavaLurk : Monster
	{
		...
		public virtual void Initialize()
		{
			...
			resubmergeAnimation.AddRange(new FarmerSprite.AnimationFrame[3]
			{
				new FarmerSprite.AnimationFrame(3, 250),
				new FarmerSprite.AnimationFrame(2, 250),
				new FarmerSprite.AnimationFrame(1, 250, false, false, OnDiveAnimationEnd)
			});
			emergeAnimation.AddRange(new FarmerSprite.AnimationFrame[4]
			{
				...
				new FarmerSprite.AnimationFrame(5, 150, false, false, OnEmergeAnimationEnd, true)
			});
			diveAnimation.AddRange(new FarmerSprite.AnimationFrame[4]
			{
				...
				new FarmerSprite.AnimationFrame(4, 150),
				new FarmerSprite.AnimationFrame(3, 150),
				new FarmerSprite.AnimationFrame(2, 150, false, false, OnDiveAnimationEnd, true)
			});
		...
		{
			PlayAnimation(submergedAnimation, true);
		}
		...
		protected override void updateAnimation(GameTime time)
		{
			...
			case State.Submerged:
				...
				break;
			case State.Lurking:
				if (PlayAnimation(lurkAnimation, false) && base.currentLocation == Game1.currentLocation && Utility.isOnScreen(base.Position, 64))
				{
			...
			case State.Emerged:
				if (locallyPlayingAnimation != emergeAnimation && locallyPlayingAnimation != idleAnimation)
				{
					...
					PlayAnimation(emergeAnimation, false);
				}
				break;
			case State.Firing:
				PlayAnimation(fireAnimation, true);
				break;
			case State.Diving:
				if (locallyPlayingAnimation != diveAnimation && locallyPlayingAnimation != submergedAnimation && locallyPlayingAnimation != resubmergeAnimation)
				{
					...
					{
						PlayAnimation(resubmergeAnimation, false);
					}
					else
					{
						PlayAnimation(diveAnimation, false);
					}
				}
				break;
			}
		...
		public override void behaviorAtGameTick(GameTime time)
		{
			...
			case State.Firing:
				...
				if (fireTimer <= 0f)
				{
					...
					if (targettedFarmer != null)
					{
						...
						BasicProjectile projectile = new BasicProjectile(25, 10, 0, 3, (float)Math.PI / 16f, shot_velocity.X, shot_velocity.Y, shot_origin, null, null, null, false, false, base.currentLocation, this);
			...
			if (targettedFarmer != null && approachFarmer)
			{
				...
				Point playerTile = targettedFarmer.TilePoint;
