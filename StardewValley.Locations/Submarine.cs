...
namespace StardewValley.Locations
{
	public class Submarine : GameLocation
	{
		...
		{
			base.initNetFields();
			base.NetFields.AddField(submerged, "submerged").AddField(ascending, "ascending");
		}
		...
		{
			if (getTileIndexAt(tileLocation, "Buildings") == 217)
			{
				if (doneUntilReset)
				{
				...
				else if (submergeTimer <= 0f && curtainOpenPercent >= 1f)
				{
