...
namespace StardewValley.Locations
{
	public class IslandHut : IslandLocation
	{
		...
		public virtual int ShowNutHint()
		{
			...
			if (MissingTheseNuts(ref missing, "IslandLeftPlantRestored", "IslandRightPlantRestored", "IslandBatRestored", "IslandFrogRestored"))
			{
				tent_nut_missing = true;
			}
			if (MissingTheseNuts(ref missing, "IslandCenterSkeletonRestored"))
			{
				missing += 5;
				tent_nut_missing = true;
			}
			if (MissingTheseNuts(ref missing, "IslandSnakeRestored"))
			{
				missing += 2;
				tent_nut_missing = true;
			}
			...
			missing += north_nuts;
			if (MissingTheseNuts(ref missing, "TreeNut"))
			{
			...
			if (MissingTheseNuts(ref missing, "StardropPool") && resort_unlocked)
			{
				valid_hints.Add(new KeyValuePair<string, int>("Strings\\Locations:NutHint_StardropPool", 0));
			}
			if (MissingTheseNuts(ref missing, "Bush_Caldera_28_36", "Bush_Caldera_9_34"))
			{
				valid_hints.Add(new KeyValuePair<string, int>("Strings\\Locations:NutHint_Caldera", 0));
			}
			MissingTheseNuts(ref missing, "Bush_CaptainRoom_2_4");
			if (MissingTheseNuts(ref missing, "BananaShrine"))
			{
				missing += 2;
			}
			MissingTheseNuts(ref missing, "Bush_IslandEast_17_37");
			MissingLimitedNutDrops(ref missing, "Darts", 3);
			...
			if (MissingTheseNuts(ref missing, "IslandShrinePuzzle"))
			{
				missing += 4;
				valid_hints.Add(new KeyValuePair<string, int>("Strings\\Locations:NutHint_IslandShrine", 0));
			}
			MissingTheseNuts(ref missing, "Bush_IslandShrine_23_34");
		...
		public virtual void SpitTreeNut()
		{
			...
			sprite = new TemporaryAnimatedSprite("LooseSprites\\Cursors2", new Microsoft.Xna.Framework.Rectangle(32, 192, 16, 32), 1250f, 1, 1, new Vector2(10f, 7f) * 64f, false, false, 0.0001f, 0f, Color.White, 4f, 0f, 0f, 0f);
			...
			for (int i = 0; i < 5; i++)
			{
				sprite = new TemporaryAnimatedSprite("LooseSprites\\Cursors", new Microsoft.Xna.Framework.Rectangle(352, 1200, 16, 16), 50f, 11, 3, new Vector2(10f, 5f) * 64f, false, false, 0.1f, 0.01f, Color.White, 4f, 0f, 0f, 0f);
		...
		public override void drawAboveAlwaysFrontLayer(SpriteBatch b)
		{
			if (hintDialogues.Count > 0)
			{
				...
				SpriteText.drawStringWithScrollCenteredAt(b, hintDialogues[0], (int)position.X, (int)position.Y, "", Math.Min(1f, hintShowTime * 2f), null, 1, 1f);
			}
		...
		protected override void resetLocalState()
		{
			...
			{
				temporarySprites.Add(new TemporaryAnimatedSprite("LooseSprites\\shadow", new Microsoft.Xna.Framework.Rectangle(0, 0, 12, 7), new Vector2(5.15f, 2.25f) * 64f, false, 0f, Color.White)
				{
					id = 777,
					scale = 4f,
				...
				temporarySprites.Add(new TemporaryAnimatedSprite("Characters\\ParrotBoy", new Microsoft.Xna.Framework.Rectangle(32, 128, 16, 32), new Vector2(5f, 0.5f) * 64f, false, 0f, Color.White)
				{
					id = 777,
					scale = 4f,
					totalNumberOfLoops = 99999,
