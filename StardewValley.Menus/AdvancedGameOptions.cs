...
namespace StardewValley.Menus
{
	public class AdvancedGameOptions : IClickableMenu
	{
		...
		public virtual void PopulateOptions()
		{
			...
			AddDropdown(Game1.content.LoadString("Strings\\UI:Character_StartingCabins"), Game1.content.LoadString("Strings\\UI:AGO_StartingCabins_Tooltip"), false, () => Game1.startingCabins, delegate(int val)
			{
			...
			}, new KeyValuePair<string, bool>(Game1.content.LoadString("Strings\\UI:Character_Close"), false), new KeyValuePair<string, bool>(Game1.content.LoadString("Strings\\UI:Character_Separate"), true));
			AddHeader(Game1.content.LoadString("Strings\\UI:AGO_OtherOptions_Label"));
			AddTextEntry(Game1.content.LoadString("Strings\\UI:AGO_RandomSeed"), Game1.content.LoadString("Strings\\UI:AGO_RandomSeed_Tooltip"), true, () => (!Game1.startingGameSeed.HasValue) ? "" : Game1.startingGameSeed.Value.ToString(), delegate(string val)
			{
				val.Trim();
		...
		public override void performHoverAction(int x, int y)
		{
			...
			if (_lastHoveredIndex >= 0 && _hoverDuration >= 500)
			{
				...
				string tooltip;
				if (tooltips.TryGetValue(option, out tooltip))
				{
		...
		public override void draw(SpriteBatch b)
		{
			...
			if (options.Count > 7)
			{
				upArrow.draw(b);
				downArrow.draw(b);
				IClickableMenu.drawTextureBox(b, Game1.mouseCursors, new Rectangle(403, 383, 6, 6), scrollBarBounds.X, scrollBarBounds.Y, scrollBarBounds.Width, scrollBarBounds.Height, Color.White, 4f, false);
				scrollBar.draw(b);
			}
			if (!hoverText.Equals(""))
			{
