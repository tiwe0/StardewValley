		...
		public bool ApplySaveFix(SaveFixes saveFix)
		{
			...
			case SaveFixes.BedsToFurniture:
				Utility.ForEachLocation(delegate(GameLocation location)
				{
					...
					if (farmHouse != null)
					{
						bool hasOwner = farmHouse.HasOwner;
			...
			case SaveFixes.ChildBedsToFurniture:
				Utility.ForEachLocation(delegate(GameLocation location)
				{
					FarmHouse farmHouse2 = location as FarmHouse;
					if (farmHouse2 != null)
					{
			...
			case SaveFixes.FixFlooringFlags:
				Utility.ForEachLocation(delegate(GameLocation location)
				{
					...
					{
						(value as Flooring)?.ApplyFlooringFlags();
					}
					return true;
				});
				return true;
			...
			case SaveFixes.MakeDarkSwordVampiric:
				Utility.ForEachItem(delegate(Item item)
				{
					...
					if (meleeWeapon != null && meleeWeapon.QualifiedItemId == "(W)2")
					{
			...
			case SaveFixes.OstrichIncubatorFragility:
				Utility.ForEachItem(delegate(Item item)
				{
					...
					if (@object != null && @object.Fragility == 2 && @object.Name == "Ostrich Incubator")
					{
						@object.Fragility = 0;
					}
					return true;
