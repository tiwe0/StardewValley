...
namespace StardewValley.TerrainFeatures
{
	public class GiantCrop : ResourceClump
	{
		...
		public override bool performToolAction(Tool t, int damage, Vector2 tileLocation)
		{
			...
			Game1.createRadialDebris(Game1.currentLocation, 12, (int)tileLocation.X + (int)width / 2, (int)tileLocation.Y + (int)height / 2, r.Next(4, 9), false);
			if (shakeTimer <= 0f)
			{
				shakeTimer = 100f;
			...
			if (t.hasEnchantmentOfType<ShavingEnchantment>() && r.NextBool((float)power / 5f) && data?.HarvestItems != null)
			{
				...
				{
					Item item2 = TryGetDrop(drop2, r, player, true, healthDeducted);
					if (item2 != null)
					{
						...
						Debris d = new Debris(item2, new Vector2((tileLocation.X + (float)((int)width / 2)) * 64f, (tileLocation.Y + (float)((int)height / 2)) * 64f), Game1.player.getStandingPosition());
			...
			if (health.Value <= 0f)
			{
				...
				if (data?.HarvestItems != null)
				{
					...
					{
						Item item = TryGetDrop(drop, r, player, false, healthDeducted);
						if (item == null)
						{
							continue;
						}
				...
				Game1.createRadialDebris(Game1.currentLocation, 12, (int)tileLocation.X + (int)width / 2, (int)tileLocation.Y + (int)width / 2, r.Next(4, 9), false);
				...
				for (int x = 0; x < (int)width; x++)
				{
					for (int y = 0; y < (int)height; y++)
					{
						...
						{
							Game1.multiplayer.broadcastSprites(location, new TemporaryAnimatedSprite(5, (tileLocation + new Vector2((float)x + 0.5f, (float)y + 0.5f)) * 64f, Color.White, 8, false, 70f));
						}
						Game1.multiplayer.broadcastSprites(location, new TemporaryAnimatedSprite(5, (tileLocation + new Vector2(x, y)) * 64f, Color.White, 8, false, animationInterval));
					}
				}
				return true;
			}
		...
		public static IReadOnlyList<KeyValuePair<string, GiantCropData>> GetGiantCropsFor(string cropId)
		{
			...
			if (cropId != null)
			{
				...
				if (CacheByCropId.TryGetValue(cropId, out giantCrops))
				{
					return giantCrops;
				}
			}
		...
		public static bool RebuildCropIdCacheIfNeeded(bool forceRebuild = false)
		{
			...
			foreach (KeyValuePair<string, GiantCropData> pair in DataLoader.GiantCrops(Game1.content))
			{
				...
				if (fromItemId != null)
				{
					...
					if (!CacheByCropId.TryGetValue(fromItemId, out list))
					{
		...
		private Item TryGetDrop(GiantCropHarvestItemData drop, Random r, Farmer targetFarmer, bool isShaving, float healthDeducted)
		{
			...
			Item item = ItemQueryResolver.TryResolveRandomItem(drop, new ItemQueryContext(Location, targetFarmer, r), false, null, null, null, delegate(string query, string error)
			{
				...
				defaultInterpolatedStringHandler.AppendLiteral("' for harvest item '");
