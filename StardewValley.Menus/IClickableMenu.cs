	...
	public abstract class IClickableMenu
	{
		...
		public virtual void populateClickableComponentList()
		{
			...
			foreach (FieldInfo f in fields)
			{
				...
				{
					List<ClickableComponent> l6 = (List<ClickableComponent>)f.GetValue(this);
					if (l6 == null)
					{
						continue;
					}
					for (int m = l6.Count - 1; m >= 0; m--)
					{
						if (l6[m] != null)
						{
							allClickableComponents.Add(l6[m]);
						}
					}
				}
				else if (f.FieldType == typeof(List<ClickableTextureComponent>))
				{
					List<ClickableTextureComponent> l5 = (List<ClickableTextureComponent>)f.GetValue(this);
					if (l5 == null)
					{
						continue;
					}
					for (int l = l5.Count - 1; l >= 0; l--)
					{
						if (l5[l] != null)
						{
							allClickableComponents.Add(l5[l]);
						}
					}
				}
				else if (f.FieldType == typeof(List<List<ClickableTextureComponent>>))
				{
					List<List<ClickableTextureComponent>> l4 = (List<List<ClickableTextureComponent>>)f.GetValue(this);
					if (l4 == null)
					{
						continue;
					}
					for (int k = l4.Count - 1; k >= 0; k--)
					{
						if (l4[k] != null)
						{
							allClickableComponents.AddRange(l4[k]);
						}
					}
				}
				else if (f.FieldType == typeof(List<ClickableAnimatedComponent>))
				{
					List<ClickableAnimatedComponent> l3 = (List<ClickableAnimatedComponent>)f.GetValue(this);
					for (int j = l3.Count - 1; j >= 0; j--)
					{
						if (l3[j] != null)
						{
							allClickableComponents.Add(l3[j]);
						}
					}
				}
				else if (f.FieldType == typeof(List<Bundle>))
				{
					List<Bundle> l2 = (List<Bundle>)f.GetValue(this);
					for (int i = l2.Count - 1; i >= 0; i--)
					{
						if (l2[i] != null)
						{
							allClickableComponents.Add(l2[i]);
						}
					}
				}
				...
				else if (f.FieldType == typeof(Dictionary<int, List<List<ClickableTextureComponent>>>))
				{
					foreach (List<List<ClickableTextureComponent>> value in ((Dictionary<int, List<List<ClickableTextureComponent>>>)f.GetValue(this)).Values)
					{
						...
						{
							allClickableComponents.AddRange(l7);
						}
					}
				}
				else
				{
					...
					foreach (ClickableTextureComponent n in ((Dictionary<int, ClickableTextureComponent>)f.GetValue(this)).Values)
					{
						allClickableComponents.Add(n);
					}
				}
			}
			GameMenu game_menu = Game1.activeClickableMenu as GameMenu;
			if (game_menu != null && this == game_menu.GetCurrentPage())
			{
		...
		public virtual void snapCursorToCurrentSnappedComponent()
		{
			if (currentlySnappedComponent != null)
			{
				Game1.setMousePosition(currentlySnappedComponent.bounds.Right - currentlySnappedComponent.bounds.Width / 4, currentlySnappedComponent.bounds.Bottom - currentlySnappedComponent.bounds.Height / 4, true);
			}
		}
		...
		public static void drawToolTip(SpriteBatch b, string hoverText, string hoverTitle, Item hoveredItem, bool heldItem = false, int healAmountToDisplay = -1, int currencySymbol = 0, string extraItemToShowIndex = null, int extraItemToShowAmount = -1, CraftingRecipe craftingIngredients = null, int moneyAmountToShowAtBottom = -1)
		{
			...
			bool edibleItem = hoveredObj != null && (int)hoveredObj.edibility != -300;
		...
		public static void drawHoverText(SpriteBatch b, StringBuilder text, SpriteFont font, int xOffset = 0, int yOffset = 0, int moneyAmountToDisplayAtBottom = -1, string boldTitleText = null, int healAmountToDisplay = -1, string[] buffIconsToDisplay = null, Item hoveredItem = null, int currencySymbol = 0, string extraItemToShowIndex = null, int extraItemToShowAmount = -1, int overrideX = -1, int overrideY = -1, float alpha = 1f, CraftingRecipe craftingIngredients = null, IList<Item> additional_craft_materials = null, Texture2D boxTexture = null, Rectangle? boxSourceRect = null, Color? textColor = null, Color? textShadowColor = null, float boxScale = 1f, int boxWidthOverride = -1, int boxHeightOverride = -1)
		{
			...
			if (hoveredItem != null)
			{
				...
				if (obj != null && (int)obj.edibility != -300)
				{
			...
			if (boldTitleText != null)
			{
				...
				drawTextureBox(b, boxTexture, boxSourceRect.Value, x, y, width + ((craftingIngredients != null) ? 21 : 0), (int)Game1.dialogueFont.MeasureString(boldTitleText).Y + 32 + (int)((hoveredItem != null && categoryName.Length > 0) ? font.MeasureString("asd").Y : 0f) - 4, Color.White * alpha, 1f, false);
			...
			if (hoveredItem != null && categoryName.Length > 0)
			{
				...
				if (weapon != null && weapon.GetEnchantmentLevel<GalaxySoulEnchantment>() > 0)
				{
			...
			if (healAmountToDisplay != -1)
			{
				...
				if (stamina_recovery >= 0)
				{
					...
					if (stamina_recovery > 0)
					{
						Utility.drawWithShadow(b, Game1.mouseCursors, new Vector2(x + 16 + 4, y + 16), new Rectangle((stamina_recovery < 0) ? 140 : 0, 428, 10, 10), Color.White, 0f, Vector2.Zero, 3f, false, 0.95f);
					...
					{
						Utility.drawWithShadow(b, Game1.mouseCursors, new Vector2(x + 16 + 4, y + 16), new Rectangle(0, 438, 10, 10), Color.White, 0f, Vector2.Zero, 3f, false, 0.95f);
				...
				{
					Utility.drawWithShadow(b, Game1.mouseCursors, new Vector2(x + 16 + 4, y + 16), new Rectangle(140, 428, 10, 10), Color.White, 0f, Vector2.Zero, 3f, false, 0.95f);
			...
			if (buffIconsToDisplay != null)
			{
				...
				for (int i = 0; i < buffIconsToDisplay.Length; i++)
				{
					...
					if (i == 12)
					{
						Utility.drawWithShadow(b, Game1.mouseCursors, new Vector2(x + 16 + 4, y + 16), new Rectangle(410, 501, 9, 9), Color.White, 0f, Vector2.Zero, 3f, false, 0.95f);
						Utility.drawTextWithShadow(b, buffIconsToDisplay[i], font, new Vector2(x + 16 + 34 + 4, y + 16), Game1.textColor);
					}
					else
					{
						Utility.drawWithShadow(b, Game1.mouseCursors, new Vector2(x + 16 + 4, y + 16), new Rectangle(10 + i * 10, 428, 10, 10), Color.White, 0f, Vector2.Zero, 3f, false, 0.95f);
