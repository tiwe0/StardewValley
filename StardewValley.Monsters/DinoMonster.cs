	...
	public class DinoMonster : Monster
	{
		...
		public class BreathProjectile : INetObject<NetFields>
		{
			...
			public void Update(GameTime time, GameLocation location, DinoMonster parent)
			{
				...
				if (!location.isTileOnMap(position.Value / 64f) || location.isCollidingPosition(boundingBox, Game1.viewport, false, 0, true, parent, false, true))
				{
					ExplosionAnimation(location);
					active.Value = false;
				}
		...
		public override void behaviorAtGameTick(GameTime time)
		{
			...
			else
			{
				...
				if (wanderState)
				{
					...
					tryToMoveInDirection(facingDirection.Value, false, base.DamageToFarmer, isGlider);
				}
			}
			...
			else
			{
				...
				if (nextFireTime > 0)
				{
					...
					if (nextFireTime <= 0)
					{
						...
						Vector2 vector3 = (projectile.position.Value = (projectile.startPosition.Value = shot_origin));
