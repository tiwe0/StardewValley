...
namespace StardewValley
{
	...
	public class Character : INetObject<NetFields>, IHaveModData
	{
		...
		public void extendSourceRect(int horizontal, int vertical, bool ignoreSourceRectUpdates = true)
		{
			...
			Microsoft.Xna.Framework.Rectangle originalSourceRect2 = originalSourceRect;
		...
		{
			doEmote(whichEmote, true, nextEventCommand);
		}

		public void doEmote(int whichEmote, int emoteYOffset)
		{
			doEmote(whichEmote, true, false);
		...
		public void playNearbySoundLocal(string audioName, int? pitch = null, SoundContext context = SoundContext.Default)
		{
			...
			ICue cue;
			Game1.sounds.PlayLocal(audioName, currentLocation, Tile, pitch, context, out cue);
		}
		...
		{
			faceGeneralDirection(target, yBias, opposite, true);
		}
		...
		public virtual void draw(SpriteBatch b, int ySourceRectOffset, float alpha = 1f)
		{
			...
			Sprite.draw(b, Game1.GlobalToLocal(Game1.viewport, Position) + new Vector2(GetSpriteWidthForPositioning() * 4 / 2, box.Height / 2), (float)box.Center.Y / 10000f, 0, ySourceRectOffset, Color.White, false, 4f, 0f, true);
			if (IsEmoting)
			{
				Vector2 emotePosition = getLocalPosition(Game1.viewport);
				emotePosition.Y -= 96f;
		...
		public virtual void updateFaceTowardsFarmer(GameTime time, GameLocation location)
		{
			...
			{
				faceGeneralDirection(whoToFace.Value.getStandingPosition(), 0, false, true);
		...
		public virtual void DrawShadow(SpriteBatch b)
		{
			...
			CharacterData data;
			if (IsVillager && NPC.TryGetData(Name, out data) && data.Shadow != null)
			{
