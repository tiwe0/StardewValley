...
namespace StardewValley.Buildings
{
	public class JunimoHut : Building
	{
		...
		public override void updateWhenFarmNotCurrentLocation(GameTime time)
		{
			...
			JunimoHarvester i = new JunimoHarvester(location, new Vector2((int)tileX + 1, (int)tileY + 1) * 64f + new Vector2(0f, 32f), this, junimoNumber, gemColor);
		...
		public bool areThereMatureCropsWithinRadius()
		{
			...
			for (int x = (int)tileX + 1 - cropHarvestRadius; x < (int)tileX + 2 + cropHarvestRadius; x++)
			{
				for (int y = (int)tileY - cropHarvestRadius + 1; y < (int)tileY + 2 + cropHarvestRadius; y++)
				{
					...
					if (location.terrainFeatures.TryGetValue(new Vector2(x, y), out terrainFeature))
					{
		...
		public override bool doAction(Vector2 tileLocation, Farmer who)
		{
			...
			if (occupiesTile(tileLocation))
			{
				...
				{
					Game1.activeClickableMenu = new ItemGrabMenu(output.Items, false, true, InventoryMenu.highlightAllItems, output.grabItemFromInventory, null, output.grabItemFromChest, false, true, true, true, true, 1, null, 1, this);
				});
				return true;
			}
