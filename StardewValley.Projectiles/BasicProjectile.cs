...
namespace StardewValley.Projectiles
{
	public class BasicProjectile : Projectile
	{
		...
		public BasicProjectile(int damageToFarmer, int spriteIndex, int bouncesTillDestruct, int tailLength, float rotationVelocity, float xVelocity, float yVelocity, Vector2 startingPosition)
			: this(damageToFarmer, spriteIndex, bouncesTillDestruct, tailLength, rotationVelocity, xVelocity, yVelocity, startingPosition, "flameSpellHit", "flameSpell", null, true)
		{
		}
		...
		public override void behaviorOnCollisionWithPlayer(GameLocation location, Farmer player)
		{
			...
			player.takeDamage(damageToFarmer, false, null);
		...
		public override void behaviorOnCollisionWithMonster(NPC n, GameLocation location)
		{
			...
			if (n is Monster)
			{
				location.damageMonster(n.GetBoundingBox(), damageToFarmer, (int)damageToFarmer + 1, false, player, true);
		...
		protected virtual void explosionAnimation(GameLocation location)
		{
			if ((int)projectileID == 14)
			{
				for (int i = 0; i < 12; i++)
				{
					...
					location.temporarySprites.Add(new TemporaryAnimatedSprite("LooseSprites\\Cursors_1_6", new Rectangle(144, 249, 7, 7), 80f, 6, 1, position.Value + new Vector2(8f, 8f) * 4f, false, false, 1f, 0f, Utility.Get2PhaseColor(Color.White, Color.Cyan, 0, 1f, Game1.random.Next(1000)), 4f, 0f, 0f, 0f)
					{
			...
			else
			{
				...
				if (itemId.Value != null)
				{
					...
					Game1.createRadialDebris(location, whichDebris, (int)(position.X + 32f) / 64, (int)(position.Y + 32f) / 64, 6, false);
				}
				else
				{
			...
			if ((bool)explode)
			{
				Game1.multiplayer.broadcastSprites(location, new TemporaryAnimatedSprite(362, Game1.random.Next(30, 90), 6, 1, position.Value, false, Game1.random.NextBool()));
			}
