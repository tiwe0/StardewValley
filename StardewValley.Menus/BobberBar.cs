	...
	public class BobberBar : IClickableMenu
	{
		...
		public override void update(GameTime time)
		{
			...
			else
			{
				...
				if (bobberInBar)
				{
					...
					{
						for (int j = 0; j < Utility.getStringCountInList(bobbers, "(O)691"); j++)
						{
							if (bobberPosition + 16f < bobberBarPos + (float)(bobberBarHeight / 2))
							{
								bobberBarSpeed -= ((j > 0) ? 0.05f : 0.2f);
							}
							else
							{
								bobberBarSpeed += ((j > 0) ? 0.05f : 0.2f);
				...
				else if (distanceFromCatching >= 1f)
				{
					...
					if (perfect)
					{
						sparkleText = new SparklingText(Game1.dialogueFont, Game1.content.LoadString("Strings\\UI:BobberBar_Perfect"), Color.Yellow, Color.White, false, 0.1, 1500);
						if (Game1.isFestival())
						{
		...
		public override void draw(SpriteBatch b)
		{
			...
			if (scale == 1f)
			{
				...
				{
					int xPosition2 = (((float)xPositionOnScreen > (float)Game1.viewport.Width * 0.75f) ? (xPositionOnScreen - 80) : (xPositionOnScreen + 216));
					bool flip = xPosition2 < xPositionOnScreen;
					b.Draw(Game1.mouseCursors_1_6, new Vector2(xPosition2 - 12, yPositionOnScreen + 40) + everythingShake, new Rectangle(227, 6, 29, 24), Color.White, 0f, new Vector2(10f, 10f), 4f, flip ? SpriteEffects.FlipHorizontally : SpriteEffects.None, 0.88f);
					fishObject.drawInMenu(b, new Vector2(xPosition2, yPositionOnScreen) + new Vector2(flip ? (-8) : (-4), 4f) * 4f + everythingShake, 1f);
				}
				if (challengeBaitFishes > -1)
				{
					...
					for (int y = 0; y < 3; y++)
					{
						...
						{
							Utility.drawWithShadow(b, Game1.mouseCursors_1_6, new Vector2(xPosition - 12, (float)yPos + (float)(y * 20) * 2f) + everythingShake, new Rectangle(236, 205, 19, 19), Color.White, 0f, new Vector2(0f, 0f), 2f, false, 0.88f);
						}
						else
						{
