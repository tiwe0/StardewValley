	...
	public class BuildingSkinMenu : IClickableMenu
	{
			...
			: base(Game1.uiViewport.Width / 2 - WindowWidth / 2, Game1.uiViewport.Height / 2 - WindowHeight / 2, WindowWidth, WindowHeight)
		{
			...
			if (buildingData.Skins != null)
			{
				...
				{
					if (!(skin2.Id != Building.skinId.Value) || ((!ignoreSeparateConstructionEntries || !skin2.ShowAsSeparateConstructionEntry) && GameStateQuery.CheckConditions(skin2.Condition, Building.GetParentLocation())))
					{
						Skins.Add(new SkinEntry(index++, skin2));
					}
				}
			}
			RepositionElements();
