		...
		private void enterIPPressed()
		{
			...
			setMenu(new TitleTextInputMenu(title, delegate(string address)
			{
				try
				{
					...
					startupPreferences2.savePreferences(false);
				}
				catch (Exception)
				{
				}
		...
		private void drawTabs(SpriteBatch b)
		{
			if (isSetUp)
			{
				...
				if (joinTab.visible)
				{
					...
					IClickableMenu.drawTextureBox(b, Game1.menuTexture, new Rectangle(0, 256, 60, 60), joinTab.bounds.X, joinTab.bounds.Y, joinTab.bounds.Width, joinTab.bounds.Height + ((!smallScreenFormat) ? 64 : 0), colorSelect2 ? selectColor : (colorHover2 ? hoverColor : Color.White), 1f, false);
					Utility.drawTextWithColoredShadow(b, joinTab.label, Game1.dialogueFont, new Vector2(joinTab.bounds.Center.X, joinTab.bounds.Y + 40) - Game1.dialogueFont.MeasureString(joinTab.label) / 2f, Game1.textColor, colorHover2 ? hoverShadow : (colorSelect2 ? selectShadow : Game1.textShadowDarkerColor), 1.01f);
				}
				if (hostTab.visible)
				{
					...
					IClickableMenu.drawTextureBox(b, Game1.menuTexture, new Rectangle(0, 256, 60, 60), hostTab.bounds.X, hostTab.bounds.Y, hostTab.bounds.Width, hostTab.bounds.Height + ((!smallScreenFormat) ? 64 : 0), colorSelect ? selectColor : (colorHover ? hoverColor : Color.White), 1f, false);
