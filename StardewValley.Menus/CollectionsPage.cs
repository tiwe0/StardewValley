	...
	public class CollectionsPage : IClickableMenu
	{
			...
			: base(x, y, width, height)
		{
			...
			foreach (ParsedItemData data in dataEntries)
			{
				...
				if (yPos5 > yPositionOnScreen + height - 128)
				{
				...
				list.Add(new ClickableTextureComponent(id + " " + farmerHas2 + " " + farmerHasButNotMade, new Rectangle(xPos5, yPos5, 64, 64), null, "", itemData.GetTexture(), itemData.GetSourceRect(), 4f, farmerHas2)
				{
					...
					downNeighborID = ((yPos5 + 68 > yPositionOnScreen + height - 128) ? (-7777) : (list.Count + collectionWidth)),
			...
			foreach (KeyValuePair<int, string> kvp in Game1.achievements)
			{
				...
				if (farmerHas || (split2[2].Equals("true") && (split2[3].Equals("-1") || farmerHasAchievements(split2[3]))))
				{
					...
					collections[5][0].Add(new ClickableTextureComponent(kvp.Key + " " + farmerHas, new Rectangle(xPos3, yPos3, 64, 64), null, "", Game1.mouseCursors, Game1.getSourceRectForStandardTileSheet(Game1.mouseCursors, 25), 1f));
					widthUsed[5]++;
				}
				else
				{
					...
					collections[5][0].Add(new ClickableTextureComponent("??? false", new Rectangle(xPos4, yPos4, 64, 64), null, "???", Game1.mouseCursors, Game1.getSourceRectForStandardTileSheet(Game1.mouseCursors, 25), 1f));
					widthUsed[5]++;
				}
			}
			if (Game1.player.secretNotesSeen.Count > 0)
			{
				...
				foreach (int i in secretNotesData.Keys)
				{
					...
					if (i >= GameLocation.JOURNAL_INDEX)
					{
						collections[6][0].Add(new ClickableTextureComponent(i + " " + Game1.player.secretNotesSeen.Contains(i), new Rectangle(xPos2, yPos2, 64, 64), null, "", Game1.objectSpriteSheet, Game1.getSourceRectForStandardTileSheet(Game1.objectSpriteSheet, 842, 16, 16), 4f, Game1.player.secretNotesSeen.Contains(i)));
					}
					else
					{
						collections[6][0].Add(new ClickableTextureComponent(i + " " + Game1.player.secretNotesSeen.Contains(i), new Rectangle(xPos2, yPos2, 64, 64), null, "", Game1.objectSpriteSheet, Game1.getSourceRectForStandardTileSheet(Game1.objectSpriteSheet, 79, 16, 16), 4f, Game1.player.secretNotesSeen.Contains(i)));
					}
					widthUsed[6]++;
				}
			}
			...
			{
				string rawText;
				if (mail.TryGetValue(s, out rawText))
				{
					...
					letters.Add(new ClickableTextureComponent(s + " true " + ((split.Length > 1) ? split[1] : "???"), new Rectangle(xPos, yPos, 64, 64), null, "", Game1.mouseCursors, new Rectangle(190, 423, 14, 11), 4f, true)
					{
						myID = letters.Count,
			...
			case 7:
			{
				Dictionary<string, string> mail = DataLoader.Mail(Game1.content);
				{
					foreach (ClickableTextureComponent c2 in collections[currentTab][currentPage])
					{
						...
						{
							string id = ArgUtility.SplitBySpaceAndGet(c2.name, 0);
							letterviewerSubMenu = new LetterViewerMenu(mail[id], id, true);
						}
					}
					break;
				}
		...
		public string createDescription(string id)
		{
			...
			case 6:
			{
				...
				description = ((index2 >= GameLocation.JOURNAL_INDEX) ? (description + Game1.content.LoadString("Strings\\Locations:Journal_Name") + " #" + (index2 - GameLocation.JOURNAL_INDEX)) : (description + Game1.content.LoadString("Strings\\Locations:Secret_Note_Name") + " #" + index2));
				if (secretNotesData[index2][0] == '!')
				{
					secretNoteImage = Convert.ToInt32(ArgUtility.SplitBySpaceAndGet(secretNotesData[index2], 1));
					break;
				}
				string letter_text = Game1.parseText(Utility.ParseGiftReveals(secretNotesData[index2]).TrimStart(' ', '^').Replace("^", Environment.NewLine)
					.Replace("@", Game1.player.name), Game1.smallFont, 512);
				string[] split2 = letter_text.Split(Environment.NewLine);
				int max_lines = 15;
			...
			default:
			{
				...
				else if (data.ObjectType == "Cooking")
				{
					...
					if (hoverItem == null || hoverItem.ItemId != id)
					{
						...
						hoverCraftingRecipe = new CraftingRecipe(last_minute_1_5_hack_name, true);
					}
				}
				else if (!(data.ObjectType == "Fish"))
				{
					int timesFound2;
					int timesFound;
					description = ((!(data.ObjectType == "Minerals") && data.Category != -2) ? (description + Game1.content.LoadString("Strings\\UI:Collections_Description_NumberShipped", Game1.player.basicShipped.TryGetValue(id, out timesFound2) ? timesFound2 : 0)) : (description + Game1.content.LoadString("Strings\\UI:Collections_Description_MineralsFound", Game1.player.mineralsFound.TryGetValue(id, out timesFound) ? timesFound : 0)));
				}
		...
		public override void draw(SpriteBatch b)
		{
			...
			if (hoverItem != null)
			{
				...
				IClickableMenu.drawToolTip(b, desc, name, hoverItem, false, -1, 0, null, -1, hoverCraftingRecipe);
			}
