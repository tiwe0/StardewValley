...
namespace StardewValley.Locations
{
	public class MermaidHouse : GameLocation
	{
		...
		protected override void resetLocalState()
		{
			...
			sparkles = new TemporaryAnimatedSpriteList();
			alwaysFrontTempSprites = new TemporaryAnimatedSpriteList();
		...
		public override void UpdateWhenCurrentLocation(GameTime time)
		{
			...
			{
				if (stopWatch != null && !stopWatch.IsRunning && main_player_music?.Name == "mermaidSong" && main_player_music.IsPaused)
				{
					stopWatch.Start();
				}
			...
			if (Game1.shouldTimePass())
			{
				...
				if (((main_player_music?.Name == "mermaidSong" && main_player_music.IsPlaying) || (Game1.options.musicVolumeLevel <= 0f && Game1.options.ambientVolumeLevel <= 0f)) && !stopWatch.IsRunning)
				{
					stopWatch.Start();
				}
			...
			pearlRecipient.addItemByMenuIfNecessaryElseHoldUp(ItemRegistry.Create("(O)797"));
