	...
	public class BreakableContainer : Object
	{
		...
		public override bool performToolAction(Tool t)
		{
			...
			if (t != null && t.isHeavyHitter())
			{
				...
				if (weapon != null && (int)weapon.type == 2)
				{
					health.Value--;
				}
				if ((int)health <= 0)
				{
					...
					{
						Game1.multiplayer.broadcastSprites(location, new TemporaryAnimatedSprite("LooseSprites\\Cursors", Game1.random.NextBool() ? breakDebrisSource.Value : breakDebrisSource2.Value, 999f, 1, 0, tileLocation.Value * 64f + new Vector2(32f, 32f), false, Game1.random.NextBool(), (tileLocation.Y * 64f + 32f) / 10000f, 0.01f, chipColor, 4f, 0f, (float)Game1.random.Next(-5, 6) * (float)Math.PI / 8f, (float)Game1.random.Next(-5, 6) * (float)Math.PI / 64f)
						{
				...
				else if (hitSound != null)
				{
					...
					Game1.createRadialDebris(location, debris.Value, (int)tileLocation.X, (int)tileLocation.Y, Game1.random.Next(4, 7), false, -1, false, debrisColor);
				}
			}
			return false;
		}
		...
		public void releaseContents(Farmer who)
		{
			...
			case "174":
			{
				...
				if (dungeon != null && (int)dungeon.level == 5 && x == 34)
				{
