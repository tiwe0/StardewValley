...
namespace StardewValley.Monsters
{
	public class BlueSquid : Monster
	{
		...
		protected override void initNetFields()
		{
			...
			lastRotation.Interpolated(false, false);
			projectileIntroTimer.Interpolated(false, false);
			projectileOutroTimer.Interpolated(false, false);
		}
		...
		protected override void sharedDeathAnimation()
		{
			...
			if (Sprite.Texture.Height > Sprite.getHeight() * 4)
			{
				...
				Game1.createRadialDebris(base.currentLocation, Sprite.textureName, new Rectangle(0, 48, 16, 16), 8, standingPixel.X, standingPixel.Y, 6, base.TilePoint.Y, Color.White, 4f * scale.Value);
			}
		}
		...
		public override void behaviorAtGameTick(GameTime time)
		{
			...
			{
				Game1.multiplayer.broadcastSprites(base.currentLocation, new TemporaryAnimatedSprite("LooseSprites\\Cursors2", new Rectangle(Game1.random.Choose(135, 140), 234, 5, 5), base.Position + new Vector2(32f, 32 + Game1.random.Next(-8, 8)), false, 0.01f, Color.White)
				{
					interval = 9999f,
					holdLastFrame = true,
					alphaFade = 0.01f,
			...
			if (projectileIntroTimer.Value > 0f)
			{
				...
				{
					Game1.multiplayer.broadcastSprites(base.currentLocation, new TemporaryAnimatedSprite("LooseSprites\\Cursors2", new Rectangle(Game1.random.Choose(135, 140), 234, 5, 5), base.Position + new Vector2(21 + Game1.random.Next(-21, 21), squidYOffset / 2 + 32 + Game1.random.Next(-32, 32)), false, 0.01f, Color.White)
					{
						interval = 9999f,
						holdLastFrame = true,
						alphaFade = 0.01f,
			...
			if (withinPlayerThreshold(6) && nextFire == 0f && projectileIntroTimer.Value <= 0f && Math.Abs(xVelocity) < 1f && Math.Abs(yVelocity) < 1f && Game1.random.NextDouble() < 0.003 && canMoveTimer <= 0f && base.currentLocation.getTileIndexAt(base.TilePoint.X, base.TilePoint.Y, "Back") != -1 && base.currentLocation.getTileIndexAt(base.TilePoint.X, base.TilePoint.Y, "Buildings") == -1 && base.currentLocation.getTileIndexAt(base.TilePoint.X, base.TilePoint.Y, "Front") == -1)
			{
