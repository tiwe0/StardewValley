...
namespace StardewValley
{
	public class TemporaryAnimatedSprite
	{
		...
		{
			TemporaryAnimatedSprite s = GetTemporaryAnimatedSprite("TileSheets\\animations", new Rectangle(0, rowInAnimationTexture * 64, sourceRectWidth, sourceRectHeight), animationInterval, animationLength, numberOfLoops, position, false, flipped, layerDepth, 0f, color, 1f, 0f, 0f, 0f);
		...
		public TemporaryAnimatedSprite(int rowInAnimationTexture, Vector2 position, Color color, int animationLength = 8, bool flipped = false, float animationInterval = 100f, int numberOfLoops = 0, int sourceRectWidth = -1, float layerDepth = -1f, int sourceRectHeight = -1, int delay = 0)
			: this("TileSheets\\animations", new Rectangle(0, rowInAnimationTexture * 64, sourceRectWidth, sourceRectHeight), animationInterval, animationLength, numberOfLoops, position, false, flipped, layerDepth, 0f, color, 1f, 0f, 0f, 0f)
		{
			if (sourceRectWidth == -1)
			{
				sourceRectWidth = 64;
		...
		{
			TemporaryAnimatedSprite temporaryAnimatedSprite = GetTemporaryAnimatedSprite(0, 999999f, 1, 0, position, false, flipped);
			temporaryAnimatedSprite.textureName = textureName;
			temporaryAnimatedSprite.loadTexture();
			temporaryAnimatedSprite.sourceRect = sourceRect;
			temporaryAnimatedSprite.sourceRectStartingPos = new Vector2(sourceRect.X, sourceRect.Y);
		...
		public void Write(BinaryWriter writer, GameLocation location)
		{
			...
			checkDirty(dirtyBits, ref i, flicker, false);
			checkDirty(dirtyBits, ref i, timeBasedMotion, false);
			checkDirty(dirtyBits, ref i, overrideLocationDestroy, false);
			checkDirty(dirtyBits, ref i, pingPong, false);
			checkDirty(dirtyBits, ref i, holdLastFrame, false);
			checkDirty(dirtyBits, ref i, pulse, false);
			checkDirty(dirtyBits, ref i, extraInfoForEndBehavior, 0);
			checkDirty(dirtyBits, ref i, lightID, 0);
			checkDirty(dirtyBits, ref i, bigCraftable, false);
			checkDirty(dirtyBits, ref i, swordswipe, false);
			checkDirty(dirtyBits, ref i, flash, false);
			checkDirty(dirtyBits, ref i, flipped, false);
			checkDirty(dirtyBits, ref i, verticalFlipped, false);
			checkDirty(dirtyBits, ref i, local, false);
			checkDirty(dirtyBits, ref i, light, false);
			checkDirty(dirtyBits, ref i, lightFade, 0);
			checkDirty(dirtyBits, ref i, hasLit, false);
			checkDirty(dirtyBits, ref i, xPeriodic, false);
			checkDirty(dirtyBits, ref i, yPeriodic, false);
			checkDirty(dirtyBits, ref i, destroyable, true);
			checkDirty(dirtyBits, ref i, paused, false);
			...
			checkDirty(dirtyBits, ref i, stopAcceleratingWhenVelocityIsZero, false);
			...
			checkDirty(dirtyBits, ref i, positionFollowsAttachedCharacter, false);
			checkDirty(dirtyBits, ref i, dontClearOnAreaEntry, false);
			checkDirty(dirtyBits, ref i, drawAboveAlwaysFront, false);
		...
		public virtual void draw(SpriteBatch spriteBatch, bool localPosition = false, int xOffset = 0, int yOffset = 0, float extraAlpha = 1f)
		{
			...
			if (text != null)
			{
				if (extraInfoForEndBehavior == -777)
				{
					...
					SpriteText.drawString(spriteBatch, text, (int)v.X, (int)v.Y, 999999, -1, 999999, alpha, layerDepth, false, -1, "", color.Equals(Color.White) ? SpriteText.color_White : SpriteText.color_Black);
				}
				else
				{
					...
					if (currentNumberOfLoops >= totalNumberOfLoops)
					{
						if (bombRadius > 0)
						{
							...
							if (Game1.IsMasterGame)
							{
								...
								parent.explode(new Vector2((int)(position.X / 64f), (int)(position.Y / 64f)), bombRadius, owner, true, bombDamage);
							}
						}
						if (fireworkType >= 0)
						{
							...
							if (Game1.currentLocation == parent)
							{
								Game1.screenGlowOnce(col * 0.8f, false);
							}
							if (Game1.IsMasterGame)
							{
								float outMult = 0.3f;
								...
								for (int j = 0; j < array.Length; j++)
								{
									...
									fireworkSprites.Add(new TemporaryAnimatedSprite("LooseSprites\\Cursors_1_6", new Rectangle(0, 0, 1, 1), 1800f * mult, 1, 0, position, false, false, -1f, 0f, Color.Transparent, 1f, 0f, 0f, 0f)
									{
										motion = point,
								...
								for (int j = 0; j < array.Length; j++)
								{
									...
									fireworkSprites.Add(new TemporaryAnimatedSprite("LooseSprites\\Cursors_1_6", new Rectangle(304, 364 + fireworkType * 11, 11, 11), 75f * mult + (float)Game1.random.Next(-20, 21), 12, 1, position, false, false, -1f, 0f, Color.White, 4f, 0f, (float)(Game1.random.NextDouble() * Math.PI) * 0.5f, 0f)
									{
										motion = point2,
									...
									fireworkSprites.Add(new TemporaryAnimatedSprite("TileSheets\\animations", new Rectangle(0, 64 * (10 + which), 64, 64), 100f * mult, (which == 0) ? 9 : 6, 2, position, false, false, -1f, 0f, Utility.getBlendedColor(col, Color.White), 1f, 0f, (float)(Game1.random.NextDouble() * Math.PI) * 0.5f, 0f)
									{
										motion = point2 * 0.75f,
										acceleration = point2 * outMult,
										accelerationChange = -point2 / inDiv,
								...
								if (id == 30)
								{
									for (int i = 0; i < 8; i++)
									{
										...
										fireworkSprites.Add(new TemporaryAnimatedSprite("LooseSprites\\Cursors_1_6", new Rectangle(304, 397, 11, 11), 75f * mult, 12, 5, position, false, false, -1f, 0f, Utility.getBlendedColor(Color.White, Utility.getRandomRainbowColor()), 4f, 0f, 0f, 0f)
										{
											motion = mot * 1.1f,
											alpha = 1f,
											alphaFade = 0.01f,
