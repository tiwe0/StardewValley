	...
	public class Chest : Object
	{
		...
		private ChestHitTimer HitTimerInstance
		{
			get
			{
				...
				if (!Game1.player.team.chestHit.SavedTimers.TryGetValue(Location.NameOrUniqueName, out localTimers))
				{
					return hitTimerInstance;
				}
		...
		public override bool performToolAction(Tool t)
		{
			...
			if (t is Pickaxe && currentLidFrame == getLastLidFrame() && (int)frameCounter == -1 && isEmpty())
			{
				...
				for (int i = 0; i < 8; i++)
				{
					Game1.multiplayer.broadcastSprites(Location, new TemporaryAnimatedSprite("LooseSprites\\Cursors", (Game1.random.NextDouble() < 0.5) ? new Rectangle(598, 1275, 13, 4) : new Rectangle(598, 1275, 13, 4), 999f, 1, 0, tileLocation.Value * 64f + new Vector2(32f, 64f), false, Game1.random.NextDouble() < 0.5, (tileLocation.Y * 64f + 64f) / 10000f, 0.01f, new Color(204, 132, 87), 4f, 0f, (float)Game1.random.Next(-5, 6) * (float)Math.PI / 8f, (float)Game1.random.Next(-5, 6) * (float)Math.PI / 64f)
					{
						motion = new Vector2((float)Game1.random.Next(-25, 26) / 10f, Game1.random.Next(-11, -8)),
						acceleration = new Vector2(0f, 0.3f)
					});
				}
				Game1.createRadialDebris(Location, 12, (int)tileLocation.X, (int)tileLocation.Y, Game1.random.Next(4, 7), false, -1, false, new Color(204, 132, 87));
				return true;
			}
			return false;
		}
		...
		public bool TryMoveToSafePosition(int? preferDirection = null)
		{
			...
			_003C_003Ec__DisplayClass53_.location = Location;
			Vector2? prioritizeDirection;
			switch (preferDirection)
			{
			case 1:
				prioritizeDirection = new Vector2(1f, 0f);
				break;
			case 3:
				prioritizeDirection = new Vector2(-1f, 0f);
				break;
			case 0:
				prioritizeDirection = new Vector2(0f, -1f);
				break;
			default:
				prioritizeDirection = new Vector2(0f, 1f);
				break;
			}
			return _003CTryMoveToSafePosition_003Eg__TryMoveRecursively_007C53_0(tileLocation.Value, 0, prioritizeDirection, ref _003C_003Ec__DisplayClass53_);
		}
		...
		public override bool performObjectDropInAction(Item dropInItem, bool probe, Farmer who, bool returnFalseIfItemConsumed = false)
		{
			if (dropInItem != null && dropInItem.QualifiedItemId != base.QualifiedItemId && dropInItem.HasContextTag("swappable_chest") && HasContextTag("swappable_chest") && Location != null)
			{
				if (!probe)
				{
					...
					GameLocation location = Location;
					Chest otherChest = new Chest(true, TileLocation, dropInItem.ItemId);
		...
		public void dumpContents()
		{
			...
			{
				if (giftbox.Value && giftboxIsStarterGift.Value)
				{
				...
				foreach (Item item in items)
				{
					...
					item.SetTempData("FromStarterGiftBox", true);
				...
				ItemGrabMenu grabMenu2 = activeClickableMenu as ItemGrabMenu;
				if (grabMenu2 != null)
				{
					ItemGrabMenu itemGrabMenu = grabMenu2;
			...
			if ((bool)giftbox)
			{
				TemporaryAnimatedSprite sprite = new TemporaryAnimatedSprite("LooseSprites\\Giftbox", new Rectangle(0, (int)giftboxIndex * 32, 16, 32), 80f, 11, 1, tileLocation.Value * 64f - new Vector2(0f, 52f), false, false, tileLocation.Y / 10000f, 0f, Color.White, 4f, 0f, 0f, 0f)
				{
					destroyable = false,
					holdLastFrame = true
				};
		...
		public virtual void CheckAutoLoad(Farmer who)
		{
			...
			if (location != null && location.objects.TryGetValue(new Vector2(tile.X, tile.Y + 1f), out beneath_object))
			{
		...
		public virtual void ShowMenu()
		{
			...
			{
			case SpecialChestTypes.MiniShippingBin:
				Game1.activeClickableMenu = new ItemGrabMenu(GetItemsForPlayer(), false, true, Utility.highlightShippableObjects, grabItemFromInventory, null, grabItemFromChest, false, true, true, true, false, 1, fridge ? null : this, -1, this);
				break;
			case SpecialChestTypes.JunimoChest:
				Game1.activeClickableMenu = new ItemGrabMenu(GetItemsForPlayer(), false, true, InventoryMenu.highlightAllItems, grabItemFromInventory, null, grabItemFromChest, false, true, true, true, true, 1, fridge ? null : this, -1, this);
				break;
			case SpecialChestTypes.AutoLoader:
			{
				ItemGrabMenu itemGrabMenu = new ItemGrabMenu(GetItemsForPlayer(), false, true, InventoryMenu.highlightAllItems, grabItemFromInventory, null, grabItemFromChest, false, true, true, true, true, 1, fridge ? null : this, -1, this);
			...
			case SpecialChestTypes.Enricher:
				Game1.activeClickableMenu = new ItemGrabMenu(GetItemsForPlayer(), false, true, Object.HighlightFertilizers, grabItemFromInventory, null, grabItemFromChest, false, true, true, true, true, 1, fridge ? null : this, -1, this);
				break;
			default:
				Game1.activeClickableMenu = new ItemGrabMenu(GetItemsForPlayer(), false, true, InventoryMenu.highlightAllItems, grabItemFromInventory, null, grabItemFromChest, false, true, true, true, true, 1, fridge ? null : this, -1, this);
				break;
		...
		public IInventory GetItemsForPlayer(long id)
		{
			...
			case SpecialChestTypes.MiniShippingBin:
				...
				{
					Inventory items;
					if (!separateWalletItems.TryGetValue(id, out items))
					{
		...
		public override void draw(SpriteBatch spriteBatch, int x, int y, float alpha = 1f)
		{
			...
			if ((bool)playerChest && (base.QualifiedItemId == "(BC)130" || base.QualifiedItemId == "(BC)232" || base.QualifiedItemId.Equals("(BC)BigChest") || base.QualifiedItemId.Equals("(BC)BigStoneChest")))
			{
				...
				Texture2D texture3 = dataOrErrorItem.GetTexture();
				...
				Rectangle drawRect = dataOrErrorItem.GetSourceRect(0, spriteIndex);
				Rectangle lidRect = dataOrErrorItem.GetSourceRect(0, lidIndex);
				Rectangle coloredLidRect = dataOrErrorItem.GetSourceRect(0, coloredLidIndex);
				spriteBatch.Draw(texture3, Game1.GlobalToLocal(Game1.viewport, new Vector2(draw_x * 64f, (draw_y - 1f) * 64f + (float)((shakeTimer > 0) ? Game1.random.Next(-1, 2) : 0))), drawRect, playerChoiceColor.Value * alpha, 0f, Vector2.Zero, 4f, SpriteEffects.None, base_sort_order);
				spriteBatch.Draw(texture3, Game1.GlobalToLocal(Game1.viewport, new Vector2(draw_x * 64f, draw_y * 64f + 20f)), new Rectangle(0, spriteIndex / 8 * 32 + 53, 16, 11), Color.White * alpha, 0f, Vector2.Zero, 4f, SpriteEffects.None, base_sort_order + 2E-05f);
				spriteBatch.Draw(texture3, Game1.GlobalToLocal(Game1.viewport, new Vector2(draw_x * 64f, (draw_y - 1f) * 64f + (float)((shakeTimer > 0) ? Game1.random.Next(-1, 2) : 0))), lidRect, Color.White * alpha, 0f, Vector2.Zero, 4f, SpriteEffects.None, base_sort_order + 2E-05f);
				spriteBatch.Draw(texture3, Game1.GlobalToLocal(Game1.viewport, new Vector2(draw_x * 64f, (draw_y - 1f) * 64f + (float)((shakeTimer > 0) ? Game1.random.Next(-1, 2) : 0))), coloredLidRect, playerChoiceColor.Value * alpha * alpha, 0f, Vector2.Zero, 4f, SpriteEffects.None, base_sort_order + 1E-05f);
				return;
			}
			if ((bool)playerChest)
			{
				...
				spriteBatch.Draw(texture2, Game1.GlobalToLocal(Game1.viewport, new Vector2(draw_x * 64f + (float)((shakeTimer > 0) ? Game1.random.Next(-1, 2) : 0), (draw_y - 1f) * 64f)), itemData2.GetSourceRect(), tint.Value * alpha, 0f, Vector2.Zero, 4f, SpriteEffects.None, base_sort_order);
				spriteBatch.Draw(texture2, Game1.GlobalToLocal(Game1.viewport, new Vector2(draw_x * 64f + (float)((shakeTimer > 0) ? Game1.random.Next(-1, 2) : 0), (draw_y - 1f) * 64f)), itemData2.GetSourceRect(0, currentLidFrame), tint.Value * alpha * alpha, 0f, Vector2.Zero, 4f, SpriteEffects.None, base_sort_order + 1E-05f);
				return;
			}
			if ((bool)giftbox)
			{
				...
				if (GetItemsForPlayer().Count > 0)
				{
					...
					spriteBatch.Draw(Game1.giftboxTexture, Game1.GlobalToLocal(Game1.viewport, new Vector2(draw_x * 64f + (float)((shakeTimer > 0) ? Game1.random.Next(-1, 2) : 0), draw_y * 64f - 52f)), new Rectangle(0, textureY, 16, 32), tint.Value, 0f, Vector2.Zero, 4f, SpriteEffects.None, base_sort_order);
				}
				return;
			}
		...
		public virtual void draw(SpriteBatch spriteBatch, int x, int y, float alpha = 1f, bool local = false)
		{
			...
			{
				ParsedItemData itemData2 = ItemRegistry.GetDataOrErrorItem(base.QualifiedItemId);
				spriteBatch.Draw(itemData2.GetTexture(), local ? new Vector2(x, y - 64) : Game1.GlobalToLocal(Game1.viewport, new Vector2(x * 64 + ((shakeTimer > 0) ? Game1.random.Next(-1, 2) : 0), (y - 1) * 64)), itemData2.GetSourceRect(), tint.Value * alpha, 0f, Vector2.Zero, 4f, SpriteEffects.None, local ? 0.89f : ((float)(y * 64 + 4) / 10000f));
				return;
			}
			...
			if (itemData != null)
			{
				...
				Rectangle drawRect = itemData.GetSourceRect(0, drawIndex);
				Rectangle lidRect = itemData.GetSourceRect(0, overlayIndex);
				Rectangle coloredLidRect = itemData.GetSourceRect(0, coloredLidIndex);
				Texture2D texture = itemData.GetTexture();
				...
				spriteBatch.Draw(texture, local ? new Vector2(x, y + 20) : Game1.GlobalToLocal(Game1.viewport, new Vector2(x * 64, y * 64 + 20)), new Rectangle(0, drawIndex / 8 * 32 + 53, 16, 11), Color.White * alpha, 0f, Vector2.Zero, 4f, SpriteEffects.None, local ? 0.91f : ((float)(y * 64 + 6) / 10000f));
