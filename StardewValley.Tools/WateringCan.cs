		...
		public override void DoFunction(GameLocation location, int x, int y, int power, Farmer who)
		{
			...
			else if ((int)waterLeft > 0 || who.hasWateringCanEnchantment)
			{
				...
				if (!basePosition.Equals(Vector2.Zero))
				{
					...
					for (int i = 0; i < 30; i++)
					{
						Game1.multiplayer.broadcastSprites(location, new TemporaryAnimatedSprite("", new Rectangle(0, 0, 1, 1), 999f, 1, 999, basePosition + new Vector2(Game1.random.Next(-3, 0) * 4, Game1.random.Next(2) * 4), false, false, (float)(playerBounds.Bottom + 32) / 10000f, 0.04f, Game1.random.Choose(Color.DeepSkyBlue, Color.LightBlue), 4f, 0f, 0f, 0f)
						{
