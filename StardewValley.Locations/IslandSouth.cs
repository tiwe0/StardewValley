...
namespace StardewValley.Locations
{
	public class IslandSouth : IslandLocation
	{
		public class IslandActivityAssigments
		{
			...
			public virtual void FindActivityForCharacters(Dictionary<Character, string> last_activity_assignments)
			{
				...
				foreach (NPC character2 in visitors)
				{
					...
					if (!(name == "Gus"))
					{
						if (!(name == "Sam") || !TryAssignment(character2, towelLoungePoints, "Resort_Towel", character2.name.Value.ToLower() + "_beach_towel", true, 0.5, last_activity_assignments))
						{
							continue;
						}
						foreach (NPC other_character2 in visitors)
						{
							if (!currentAssignments.ContainsKey(other_character2) && animationDescriptions.ContainsKey(other_character2.Name.ToLower() + "_beach_dance"))
							{
								string[] array = ArgUtility.SplitBySpace(currentAssignments[character2]);
								int x = int.Parse(array[0]);
								int y = int.Parse(array[1]);
								currentAssignments.Remove(other_character2);
								TryAssignment(other_character2, new List<Point>(new Point[1]
								{
									new Point(x + 1, y + 1)
					...
					foreach (NPC other_character in visitors)
					{
						...
						{
							TryAssignment(other_character, drinkPoints, "Resort_Bar", other_character.name.Value.ToLower() + "_beach_drink", false, 0.5, last_activity_assignments);
						}
					}
				}
				foreach (NPC character in visitors)
				{
					if (!currentAssignments.ContainsKey(character) && !TryAssignment(character, towelLoungePoints, "Resort_Towel", character.name.Value.ToLower() + "_beach_towel", true, 0.5, last_activity_assignments) && !TryAssignment(character, wanderPoints, "Resort_Wander", "square_3_3", false, 0.4, last_activity_assignments) && !TryAssignment(character, umbrellaPoints, "Resort_Umbrella", character.name.Value.ToLower() + "_beach_umbrella", true, (character.Name == "Abigail") ? 0.5 : 0.1) && (character.Age != 0 || !TryAssignment(character, chairPoints, "Resort_Chair", "_beach_chair", false, 0.4, last_activity_assignments)))
					{
						TryAssignment(character, shoreLoungePoints, "Resort_Shore", null, false, 1.0, last_activity_assignments);
					}
				}
			...
			{
				string assignment;
				if (currentAssignments.TryGetValue(character, out assignment))
				{
		...
		public override void UpdateWhenCurrentLocation(GameTime time)
		{
			...
			if ((float)_boatDirection != 0f)
			{
				...
				else
				{
					...
					TemporaryAnimatedSprite sprite2 = new TemporaryAnimatedSprite("TileSheets\\animations", new Microsoft.Xna.Framework.Rectangle(0, 0, 64, 64), 50f, 9, 1, position2, false, false, 0f, 0.025f, Color.White, 1f, 0f, 0f, 0f);
			...
			TemporaryAnimatedSprite sprite = new TemporaryAnimatedSprite("TileSheets\\animations", new Microsoft.Xna.Framework.Rectangle(0, 1600, 64, 128), 200f, 9, 1, position, false, false, 1f, 0.025f, Color.White, 1f, 0.025f, 0f, 0f);
		...
		protected override void resetLocalState()
		{
			...
			if (_isFirstVisit)
			{
				...
				if (!Game1.MasterPlayer.hasOrWillReceiveMail("addedParrotBoy"))
				{
					...
					temporarySprites.Add(new TemporaryAnimatedSprite("Characters\\ParrotBoy", new Microsoft.Xna.Framework.Rectangle(32, 128, 16, 32), new Vector2(29f, 15.5f) * 64f, false, 0f, Color.White)
					{
						id = 777,
						scale = 4f,
			...
			if (_exitsBlocked)
			{
				temporarySprites.Add(new TemporaryAnimatedSprite("LooseSprites\\Cursors2", new Microsoft.Xna.Framework.Rectangle(208, 94, 48, 53), new Vector2(17f, 0f) * 64f, false, 0f, Color.White)
				{
					id = 555,
					scale = 4f,
					totalNumberOfLoops = 99999,
			...
			{
				temporarySprites.Add(new TemporaryAnimatedSprite("Characters\\Monsters\\Magma Sprite", new Microsoft.Xna.Framework.Rectangle(0, 16, 16, 16), new Vector2(18f, 11f) * 64f, false, 0f, Color.White)
				{
					id = 999,
					scale = 4f,
					totalNumberOfLoops = 99999,
				...
				temporarySprites.Add(new TemporaryAnimatedSprite("LooseSprites\\shadow", new Microsoft.Xna.Framework.Rectangle(0, 0, 12, 7), new Vector2(18.2f, 12.4f) * 64f, false, 0f, Color.White)
				{
					id = 998,
					scale = 4f,
					totalNumberOfLoops = 99999,
			...
			{
				temporarySprites.Add(new TemporaryAnimatedSprite("LooseSprites\\Cursors2", new Microsoft.Xna.Framework.Rectangle(152, 101, 56, 40), new Vector2(0.5f, 10f) * 64f, false, 0f, Color.White)
				{
					id = 789,
					scale = 4f,
					totalNumberOfLoops = 99999,
		...
		public override bool HasLocationOverrideDialogue(NPC character)
		{
			...
			if (Game1.player.friendshipData.TryGetValue(character.Name, out friendship) && friendship.IsDivorced())
			{
				return false;
			}
		...
		public static void SetupIslandSchedules()
		{
			...
			if (island == null || !island.resortRestored.Value || island.IsRainingHere() || !island.resortOpenToday.Value)
			{
				return;
			}
			...
			if (seeded_random.NextDouble() < 0.4)
			{
				for (int k = 0; k < 5; k++)
				{
					NPC visitor4 = seeded_random.ChooseFrom(valid_visitors);
			...
			foreach (NPC visitor2 in visitors)
			{
				...
				schedule.Remove(0, 1);
				if (visitor2.TryLoadSchedule("island", schedule.ToString()))
				{
					visitor2.islandScheduleName.Value = "island";
					Game1.netWorldState.Value.IslandVisitors.Add(visitor2.Name);
				}
				visitor2.performSpecialScheduleChanges();
			}
		}
