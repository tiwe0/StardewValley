	...
	public class PetBowl : Building
	{
		...
		public override void draw(SpriteBatch b)
		{
			...
			if (watered.Value)
			{
				...
				float sortY2 = ((int)tileY + (int)tilesHigh) * 64;
				if (data2 != null)
				{
				...
				b.Draw(origin: new Vector2(0f, sourceRect.Height), texture: texture.Value, position: Game1.GlobalToLocal(Game1.viewport, drawPosition + drawOffset), sourceRectangle: sourceRect, color: color * alpha, rotation: 0f, scale: 4f, effects: SpriteEffects.None, layerDepth: sortY2);
			}
			if (nameTimer > 0)
			{
