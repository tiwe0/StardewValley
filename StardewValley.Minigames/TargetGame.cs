	...
	public class TargetGame : IMinigame
	{
		public class Target
		{
			...
			public void shatter(GameLocation location, Projectile stone)
			{
				...
				location.temporarySprites.Add(new TemporaryAnimatedSprite("LooseSprites\\Cursors", new Microsoft.Xna.Framework.Rectangle(304, 1183 + targetType * 16, 16, 16), 60f, 3, 0, new Vector2(Position.X - 4, Position.Y - 4), false, false, 1f, 0f, Color.White, 4f, 0f, 0f, 0f));
				location.debris.Add(new Debris(scoreToAdd, new Vector2(Position.Center.X, Position.Center.Y), new Color(255, 130, 0), 1f, null));
				score += scoreToAdd;
				BasicProjectile projectile = stone as BasicProjectile;
				if (projectile != null && (int)projectile.damageToFarmer > 0)
				{
					successShots++;
		...
		public bool tick(GameTime time)
		{
			...
			if (timerToStart > 0)
			{
				...
				if (timerToStart <= 0)
				{
					...
					Game1.changeMusicTrack("tickTock", false, MusicContext.MiniGame);
				}
			}
		...
		public void draw(SpriteBatch b)
		{
			if (showResultsTimer < 0)
			{
				...
				location.Map.RequireLayer("Back").Draw(Game1.mapDisplayDevice, Game1.viewport, Location.Origin, false, 4);
				...
				location.Map.RequireLayer("Buildings").Draw(Game1.mapDisplayDevice, Game1.viewport, Location.Origin, false, 4);
				...
				b.Begin(SpriteSortMode.Deferred, BlendState.AlphaBlend, SamplerState.PointClamp);
				Game1.mapDisplayDevice.BeginScene(b);
				location.Map.RequireLayer("Front").Draw(Game1.mapDisplayDevice, Game1.viewport, Location.Origin, false, 4);
				Game1.mapDisplayDevice.EndScene();
		...
		{
			addRowOfTargetsOnLane(0, Target.middleLane, 1500, 5, Target.mediumSpeed, false);
			addRowOfTargetsOnLane(4000, Target.nearLane, 1000, 5, Target.mediumSpeed);
			addRowOfTargetsOnLane(8000, Target.farLane, 2000, 5, Target.mediumSpeed, false, Target.bonusTarget);
			...
			addRowOfTargetsOnLane(18000, Target.middleLane, 1500, 5, Target.mediumSpeed, false);
			...
			addRowOfTargetsOnLane(41000, Target.middleLane, 1500, 5, Target.mediumSpeed, false);
			addRowOfTargetsOnLane(42000, Target.nearLane, 1000, 5, Target.mediumSpeed);
			addRowOfTargetsOnLane(43000, Target.farLane, 1000, 4, Target.mediumSpeed, false);
		}
