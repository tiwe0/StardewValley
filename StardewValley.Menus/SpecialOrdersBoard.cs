...
namespace StardewValley.Menus
{
	public class SpecialOrdersBoard : IClickableMenu
	{
		...
		public void DrawQuestDetails(SpriteBatch b, SpecialOrder order, int x)
		{
			...
			{
				Utility.drawWithShadow(b, drawn_portrait.Value.Key, new Vector2(x, header_y), drawn_portrait.Value.Value, Color.White * graphic_alpha, 0f, Vector2.Zero, 4f, false, -1f, -1, -1, shadow_intensity * 0.6f);
			}
			...
			if (boardType == "" && Game1.player.team.completedSpecialOrders.Contains(order.questKey))
			{
			...
			Utility.drawWithShadow(b, Game1.mouseCursors, new Vector2(x, due_date_y_position), new Rectangle(410, 501, 9, 9), Color.White, 0f, Vector2.Zero, 4f, false, 0.99f, -1, -1, shadow_intensity * 0.6f);
			...
			if (boardType == "Qi")
			{
				...
				{
					GemsReward gemsReward = reward2 as GemsReward;
					if (gemsReward != null)
					{
						gems = gemsReward;
						break;
				...
				if (reward != -1)
				{
					Utility.drawWithShadow(b, Game1.objectSpriteSheet, new Vector2((float)x + 512f / scale - Game1.dialogueFont.MeasureString(reward.ToString() ?? "").X - 12f - 60f, due_date_y_position - 8), new Rectangle(288, 561, 15, 15), Color.White, 0f, Vector2.Zero, 4f, false, 0.99f, -1, -1, shadow_intensity * 0.6f);
					Utility.drawTextWithShadow(b, Game1.parseText(reward.ToString() ?? "", Game1.dialogueFont, width - 128), Game1.dialogueFont, new Vector2((float)x + 512f / scale - Game1.dialogueFont.MeasureString(reward.ToString() ?? "").X - 4f, due_date_y_position), font_color, 1f, -1f, -1, -1, shadow_intensity);
			...
			{
				ObjectReward objectReward = reward3 as ObjectReward;
			...
			if (o != null)
			{
				Utility.drawWithShadow(b, ItemRegistry.GetDataOrErrorItem(o.QualifiedItemId).GetTexture(), new Vector2((float)x + 512f / scale - Game1.dialogueFont.MeasureString(o.Stack.ToString() ?? "").X - 12f - 60f, due_date_y_position - 8), ItemRegistry.GetDataOrErrorItem(o.QualifiedItemId).GetSourceRect(), Color.White, 0f, Vector2.Zero, 4f, false, 0.99f, -1, -1, shadow_intensity * 0.6f);
				Utility.drawTextWithShadow(b, Game1.parseText(o.Stack.ToString() ?? "", Game1.dialogueFont, width - 128), Game1.dialogueFont, new Vector2((float)x + 512f / scale - Game1.dialogueFont.MeasureString(o.Stack.ToString() ?? "").X - 4f, due_date_y_position), font_color, 1f, -1f, -1, -1, shadow_intensity);
				Utility.drawTextWithShadow(b, Game1.parseText(ItemRegistry.GetDataOrErrorItem(o.QualifiedItemId).DisplayName, Game1.dialogueFont, width - 128), Game1.dialogueFont, new Vector2((float)x + 512f / scale - Game1.dialogueFont.MeasureString(ItemRegistry.GetDataOrErrorItem(o.QualifiedItemId).DisplayName).X + 8f, due_date_y_position - 60), font_color * 0.6f, 1f, -1f, -1, -1, shadow_intensity);
			}
		}
	}
}
