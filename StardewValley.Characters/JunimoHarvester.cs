...
namespace StardewValley.Characters
{
	public class JunimoHarvester : NPC
	{
		...
		public JunimoHut home
		{
			...
			{
				Building building;
				if (!base.currentLocation.buildings.TryGetValue(netHome.Value, out building))
				{
					return null;
				}
		...
		public virtual bool foundCropEndFunction(PathNode currentNode, Point endPoint, GameLocation location, Character c)
		{
			...
			if (location.terrainFeatures.TryGetValue(new Vector2(currentNode.x, currentNode.y), out terrainFeature))
			{
				...
				if (bush != null && (int)bush.tileSheetOffset == 1)
				{
					return true;
				}
			}
		...
		public virtual void returnToJunimoHut(GameLocation location)
		{
			...
			if (Game1.IsMasterGame)
			{
				...
				if (controller.pathToEndPoint == null || controller.pathToEndPoint.Count == 0 || location.isCollidingPosition(nextPosition, Game1.viewport, false, 0, false, this))
				{
					destroy = true;
				}
			}
			...
			if (Game1.IsMasterGame)
			{
				if (harvestTimer > 0)
				{
					...
					else if (oldTimer >= 1000 && harvestTimer < 1000)
					{
						netAnimationEvent.Fire(2);
						JunimoHut hut2 = home;
						if (base.currentLocation != null && hut2 != null && !hut2.noHarvest && isHarvestable())
						{
							...
							{
								HoeDirt dirt = terrainFeature as HoeDirt;
								if (dirt != null && dirt.crop.harvest(base.TilePoint.X, base.TilePoint.Y, dirt, this))
								{
							...
							if (lastItemHarvested != null)
							{
								...
								{
									gotDouble = true;
									Item j = lastItemHarvested.getOne();
									j.Quality = lastItemHarvested.Quality;
									tryToAddItemToHut(j);
								}
								if (base.currentLocation.farmers.Any())
								{
									...
									if (gotDouble)
									{
										for (int i = 0; i < 2; i++)
										{
											Game1.multiplayer.broadcastSprites(base.currentLocation, new TemporaryAnimatedSprite(itemData.TextureName, itemData.GetSourceRect(), 1000f, 1, 0, base.Position + new Vector2(0f, -40f), false, false, mainDrawLayer, 0.02f, Color.White, 4f, -0.01f, 0f, 0f)
											{
												motion = new Vector2((float)((i != 0) ? 1 : (-1)) * 0.5f, -0.25f),
												delayBeforeAnimationStart = 200
											});
											ColoredObject coloredObj2 = lastItemHarvested as ColoredObject;
											if (coloredObj2 != null)
											{
												Rectangle colored_source_rect2 = ItemRegistry.GetDataOrErrorItem(lastItemHarvested.QualifiedItemId).GetSourceRect(1);
												Game1.multiplayer.broadcastSprites(base.currentLocation, new TemporaryAnimatedSprite(itemData.TextureName, colored_source_rect2, 1000f, 1, 0, base.Position + new Vector2(0f, -40f), false, false, mainDrawLayer + 0.005f, 0.02f, coloredObj2.color.Value, 4f, -0.01f, 0f, 0f)
												{
													motion = new Vector2((float)((i != 0) ? 1 : (-1)) * 0.5f, -0.25f),
													delayBeforeAnimationStart = 200
												});
									...
									else
									{
										...
										ColoredObject coloredObj = lastItemHarvested as ColoredObject;
										if (coloredObj != null)
										{
											Rectangle colored_source_rect = ItemRegistry.GetDataOrErrorItem(lastItemHarvested.QualifiedItemId).GetSourceRect(1);
											Game1.multiplayer.broadcastSprites(base.currentLocation, new TemporaryAnimatedSprite(itemData.TextureName, colored_source_rect, 1000f, 1, 0, base.Position + new Vector2(0f, -40f), false, false, mainDrawLayer + 0.005f, 0.02f, coloredObj.color.Value, 4f, -0.01f, 0f, 0f)
											{
												motion = new Vector2(0.08f, -0.25f)
											});
										}
