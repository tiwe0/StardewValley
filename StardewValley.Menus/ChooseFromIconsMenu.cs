	...
	public class ChooseFromIconsMenu : IClickableMenu
	{
		...
		public void setUpIcons(string which)
		{
			...
			if (!(which == "dwarfStatue"))
			{
				if (which == "bobbers")
				{
					...
					for (int k = 0; k < FishingRod.NUM_BOBBER_STYLES; k++)
					{
						...
						src.Height = 16;
						icons.Add(new ClickableTextureComponent(new Rectangle(0, 0, 64, 64), texture, iconBackRectangle, 4f, true)
						{
							name = (k.ToString() ?? "")
						});
						if (num)
						...
						{
							iconFronts.Add(new ClickableTextureComponent(new Rectangle(0, 0, 16, 16), Game1.bobbersTexture, src, 4f, true));
						}
					}
					icons.Add(new ClickableTextureComponent(new Rectangle(0, 0, 64, 64), null, new Rectangle(0, 0, 0, 0), 4f, true)
					{
						name = "-2"
					});
					iconFronts.Add(new ClickableTextureComponent(new Rectangle(0, 0, 10, 10), Game1.mouseCursors_1_6, new Rectangle(496, 28, 16, 16), 4f, true));
			...
			else
			{
				...
				while (icon2 == icon1);
				icons.Add(new ClickableTextureComponent(new Rectangle(0, 0, 84, 84), texture, iconBackRectangle, 4f, true)
				{
					name = (icon1.ToString() ?? ""),
			...
			if (hasTooltips)
			{
				foreach (ClickableTextureComponent j in icons)
				{
					j.hoverText = Game1.parseText(j.hoverText, Game1.smallFont, toolTipWidth - 32);
					maxTooltipHeight = Math.Max(maxTooltipHeight, (int)Game1.smallFont.MeasureString(j.hoverText).Y);
					maxTooltipWidth = Math.Max(maxTooltipWidth, (int)Game1.smallFont.MeasureString(j.hoverText).X);
				}
				maxTooltipHeight += 48;
				maxTooltipWidth += 48;
			}
			...
			for (int i = 0; i < icons.Count; i += maxIconsPerRow)
			{
				...
				{
					int index = l + i;
					icons[index].bounds.X = x + l * iconWidth;
		...
		public override void receiveLeftClick(int x, int y, bool playSound = true)
		{
			...
			for (int i = 0; i < icons.Count; i++)
			{
				...
				else
				{
					...
					for (int j = 0; j < 16; j++)
					{
						temporarySprites.Add(new TemporaryAnimatedSprite("LooseSprites\\Cursors_1_6", new Rectangle(98 + Game1.random.Next(3) * 4, 161, 4, 4), Utility.getRandomPositionInThisRectangle(c.bounds, Game1.random), false, 0f, Color.White)
						{
							local = true,
							scale = 4f,
							interval = 9999f,
