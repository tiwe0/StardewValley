	...
	public class IslandNorth : IslandLocation
	{
		...
		public virtual void ApplyIslandTraderHut()
		{
			if (map != null)
			{
				...
				temporarySprites.Add(new TemporaryAnimatedSprite("LooseSprites\\Cursors", new Microsoft.Xna.Framework.Rectangle(276, 1985, 12, 11), new Vector2(33.45f, 70.33f) * 64f + new Vector2(-16f, -32f), false, 0f, Color.White)
				{
				...
				temporarySprites.Add(new TemporaryAnimatedSprite("LooseSprites\\Cursors", new Microsoft.Xna.Framework.Rectangle(276, 1985, 12, 11), new Vector2(39.45f, 70.33f) * 64f + new Vector2(-16f, -32f), false, 0f, Color.White)
				{
					delayBeforeAnimationStart = 10,
					interval = 50f,
					totalNumberOfLoops = 99999,
		...
		public virtual void ApplyCaveOpened()
		{
			...
			for (int i = 0; i < 8; i++)
			{
				temporarySprites.Add(new TemporaryAnimatedSprite("LooseSprites\\Cursors", new Microsoft.Xna.Framework.Rectangle(372, 1956, 10, 10), Utility.getRandomPositionInThisRectangle(boulderPosition, Game1.random) + new Vector2(-32f, -32f), false, 0.007f, Color.White)
				{
					alpha = 0.75f,
		...
		public override void UpdateWhenCurrentLocation(GameTime time)
		{
			...
			if (!_sawFlameSpriteSouth && Utility.isThereAFarmerWithinDistance(new Vector2(36f, 79f), 5, this) == Game1.player)
			{
				...
				if (v2 != null)
				{
					...
					v2.motion = new Vector2(1f, -4f);
					v2.acceleration = new Vector2(0f, -0.04f);
					v2.drawAboveAlwaysFront = true;
				}
		...
		public override void DayUpdate(int dayOfMonth)
		{
			...
			{
				HoeDirt dirt = terrainFeatures[v3] as HoeDirt;
				if (dirt != null && dirt.crop != null && dirt.crop.forageCrop.Value)
				{
					gingerTiles.Add(v3);
				}
			}
		...
		private bool isTileOpenForDigSiteStone(int tileX, int tileY)
		{
			...
			{
				return CanItemBePlacedHere(new Vector2(tileX, tileY), false, CollisionMask.All, CollisionMask.None);
			}
			return false;
		}
		...
		public void digSiteUpdate()
		{
			...
			{
				for (int j = 0; j < numberOfAdditionsToTry; j++)
				{
					...
					if (!added_forced_bone_node || Game1.random.NextDouble() < 0.3)
					{
						...
						objects.Add(position2, node);
					}
					else if (Game1.random.NextDouble() < 0.1)
					{
						int xCoord = (int)position2.X;
						int yCoord = (int)position2.Y;
						if (CanItemBePlacedHere(position2, false, CollisionMask.All, CollisionMask.None) && !IsTileOccupiedBy(position2) && getTileIndexAt(xCoord, yCoord, "AlwaysFront") == -1 && getTileIndexAt(xCoord, yCoord, "Front") == -1 && !isBehindBush(position2) && doesTileHaveProperty(xCoord, yCoord, "Diggable", "Back") != null && doesTileHaveProperty(xCoord, yCoord, "Diggable", "Back") == "T")
						{
							objects.Add(position2, ItemRegistry.Create<Object>("(O)590"));
						}
					}
					...
					{
						objects.Add(position2, new Object(r.Choose("785", "676", "677"), 1));
					}
					else
					{
						string id = r.Choose("32", "38", "40", "42");
						objects.Add(position2, new Object(id, 1)
						{
							MinutesUntilReady = 2
						});
					}
		...
		protected override void resetLocalState()
		{
			...
			{
				temporarySprites.Add(new TemporaryAnimatedSprite("Characters\\Monsters\\Magma Sprite", new Microsoft.Xna.Framework.Rectangle(0, 32, 16, 16), new Vector2(36f, 79f) * 64f, false, 0f, Color.White)
				{
					id = 999,
					scale = 4f,
					totalNumberOfLoops = 99999,
				...
				temporarySprites.Add(new TemporaryAnimatedSprite("LooseSprites\\shadow", new Microsoft.Xna.Framework.Rectangle(0, 0, 12, 7), new Vector2(36.2f, 80.4f) * 64f, false, 0f, Color.White)
				{
					id = 998,
					scale = 4f,
					totalNumberOfLoops = 99999,
			...
			{
				temporarySprites.Add(new TemporaryAnimatedSprite("Characters\\Monsters\\Magma Sprite", new Microsoft.Xna.Framework.Rectangle(0, 32, 16, 16), new Vector2(41f, 30f) * 64f, false, 0f, Color.White)
				{
					id = 9999,
					scale = 4f,
					totalNumberOfLoops = 99999,
				...
				temporarySprites.Add(new TemporaryAnimatedSprite("LooseSprites\\shadow", new Microsoft.Xna.Framework.Rectangle(0, 0, 12, 7), new Vector2(41.2f, 31.4f) * 64f, false, 0f, Color.White)
				{
					id = 9998,
					scale = 4f,
					totalNumberOfLoops = 99999,
			...
			{
				temporarySprites.Add(new TemporaryAnimatedSprite("TileSheets\\critters", new Microsoft.Xna.Framework.Rectangle(141, 310, 20, 23), new Vector2(23.75f, 77.15f) * 64f, false, 0f, Color.White)
				{
