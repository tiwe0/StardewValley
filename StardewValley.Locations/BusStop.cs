	...
	public class BusStop : GameLocation
	{
		...
		protected override void resetLocalState()
		{
			...
			if (Game1.player.TilePoint.Y > 16 || Game1.eventUp || Game1.player.TilePoint.X <= 10 || Game1.player.isRidingHorse() || !arrived_from_other_location_context)
			{
				...
				busPosition = new Vector2(21f, 6f) * 64f;
				busDoor = new TemporaryAnimatedSprite("LooseSprites\\Cursors", new Microsoft.Xna.Framework.Rectangle(288, 1311, 16, 38), busPosition + new Vector2(16f, 26f) * 4f, false, 0f, Color.White)
				{
					interval = 999999f,
					animationLength = 6,
					holdLastFrame = true,
			...
			else
			{
				Game1.changeMusicTrack("silence");
				busPosition = new Vector2(21f, 6f) * 64f;
				busDoor = new TemporaryAnimatedSprite("LooseSprites\\Cursors", new Microsoft.Xna.Framework.Rectangle(368, 1311, 16, 38), busPosition + new Vector2(16f, 26f) * 4f, false, 0f, Color.White)
				{
					interval = 999999f,
					animationLength = 1,
					holdLastFrame = true,
			...
			{
				temporarySprites.Add(new TemporaryAnimatedSprite("LooseSprites\\temporary_sprites_1", new Microsoft.Xna.Framework.Rectangle(448, 546, 16, 25), new Vector2(12f, 6.5f) * 64f, true, 0f, Color.White)
				{
					scale = 4f,
				...
				if (Math.Abs(busPosition.X - 1344f) <= Math.Abs(busMotion.X * 1.5f))
				{
					...
					Game1.globalFadeToBlack(delegate
					{
						...
						if (!string.IsNullOrEmpty(Game1.player.horseName.Value))
						{
							...
							{
								Horse horse = characters[i] as Horse;
