...
namespace StardewValley
{
	public class Crop : INetObject<NetFields>, IHaveModData
	{
		...
		{
			CropData data;
			if (!TryGetData(isWildSeedCrop() ? whichForageCrop.Value : netSeedIndex.Value, out data))
			{
				return null;
			}
			return data;
		...
		public virtual bool harvest(int xTile, int yTile, HoeDirt soil, JunimoHarvester junimoHarvester = null, bool isForcedScytheHarvest = false)
		{
			...
			else if ((int)currentPhase >= phaseDays.Count - 1 && (!fullyGrown || (int)dayOfCurrentPhase <= 0))
			{
				...
				if (success)
				{
					...
					if (indexOfHarvest == "262" && r2.NextDouble() < 0.4)
					{
		...
		public virtual void newDay(int state)
		{
			...
			if ((environment is Farm || environment.HasMapPropertyWithValue("AllowGiantCrops")) && (int)currentPhase == phaseDays.Count - 1 && TryGetGiantCrops(out possibleGiantCrops))
			{
				foreach (KeyValuePair<string, GiantCropData> pair in possibleGiantCrops)
				{
					...
					{
						int x2 = tile.X;
						while (x2 < tile.X + giantCrop.TileSize.X)
						{
							Vector2 v2 = new Vector2(x2, y2);
							TerrainFeature terrainFeature2;
							if (environment.terrainFeatures.TryGetValue(v2, out terrainFeature2))
							{
								HoeDirt dirt2 = terrainFeature2 as HoeDirt;
								if (dirt2 != null && !(dirt2.crop?.indexOfHarvest != indexOfHarvest))
								{
									x2++;
									continue;
								}
			...
			if ((!fullyGrown || (int)dayOfCurrentPhase <= 0) && (int)currentPhase >= phaseDays.Count - 1)
			{
				...
				{
					Season seedSeason;
					switch ((string)whichForageCrop)
					{
					case "495":
						seedSeason = Season.Spring;
						break;
					case "496":
						seedSeason = Season.Summer;
						break;
					case "497":
						seedSeason = Season.Fall;
						break;
					case "498":
						seedSeason = Season.Winter;
						break;
					default:
						seedSeason = currentLocation.GetSeason();
						break;
					}
					Object obj;
		...
		public virtual void updateDrawMath(Vector2 tileLocation)
		{
			...
			if ((bool)forageCrop)
			{
				...
				if (!int.TryParse(whichForageCrop.Value, out which_forage_crop))
				{
					which_forage_crop = 1;
				}
