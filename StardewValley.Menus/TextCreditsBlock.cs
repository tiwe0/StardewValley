		...
		public override void draw(int topLeftX, int topLeftY, int widthToOccupy, SpriteBatch b)
		{
			if (renderNameInEnglish)
			{
				...
				if (parenthesis_index != -1 && parenthesis_index > 0)
				{
					...
					SpriteText.drawString(b, name, topLeftX, topLeftY, 999999, widthToOccupy, 99999, 1f, 0.88f, false, -1, "", color);
					SpriteText.forceEnglishFont = false;
					SpriteText.drawString(b, parenthesis_text, topLeftX + width_of_text, topLeftY, 999999, -1, 99999, 1f, 0.88f, false, -1, "", color);
				}
				else
				{
					SpriteText.forceEnglishFont = true;
					SpriteText.drawString(b, text, topLeftX, topLeftY, 999999, widthToOccupy, 99999, 1f, 0.88f, false, -1, "", color);
					SpriteText.forceEnglishFont = false;
				}
			}
			else
