...
namespace StardewValley.Menus
{
	public class MineElevatorMenu : IClickableMenu
	{
			...
			: base(0, 0, 0, 0, true)
		{
			...
			int x = xPositionOnScreen + IClickableMenu.borderWidth + IClickableMenu.spaceToClearSideBorder * 3 / 4;
			...
			x = x + 64 - 20;
			if (x > xPositionOnScreen + width - IClickableMenu.borderWidth)
			{
				x = xPositionOnScreen + IClickableMenu.borderWidth + IClickableMenu.spaceToClearSideBorder * 3 / 4;
				y += 44;
			}
			for (int i = 1; i <= numElevators; i++)
			{
				elevators.Add(new ClickableComponent(new Rectangle(x, y, 44, 44), (i * 5).ToString() ?? "")
				{
					myID = i,
		...
		public override void draw(SpriteBatch b)
		{
			...
			Game1.drawDialogueBox(xPositionOnScreen, yPositionOnScreen - 64 + 8, width + 21, height + 64, false, true);
			foreach (ClickableComponent c in elevators)
			{
