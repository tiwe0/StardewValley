	...
	public class LibraryMuseum : GameLocation
	{
		...
		public static int totalArtifacts
		{
			get
			{
				if (_totalArtifacts < 0)
				{
					...
					{
						if (IsItemSuitableForDonation("(O)" + itemId, false))
						{
							_totalArtifacts++;
						}
					}
		...
		public bool doesFarmerHaveAnythingToDonate(Farmer who)
		{
			for (int i = 0; i < (int)who.maxItems; i++)
			{
				if (i < who.Items.Count)
				{
					...
					if (obj != null && isItemSuitableForDonation(obj))
					{
						return true;
					}
				}
			...
			for (int x = 0; x < map.Layers[0].LayerWidth; x++)
			{
				for (int y = 0; y < map.Layers[0].LayerHeight; y++)
				{
					...
					if (ArgUtility.Get(action, 0) == "Notes")
					{
						int noteId;
						string error;
						if (ArgUtility.TryGetInt(action, 1, out noteId, out error))
						{
		...
		protected override void resetLocalState()
		{
			...
			foreach (KeyValuePair<int, Vector2> pair in getLostBooksLocations())
			{
				...
				{
					temporarySprites.Add(new TemporaryAnimatedSprite("LooseSprites\\Cursors", new Microsoft.Xna.Framework.Rectangle(144, 447, 15, 15), new Vector2(tile.X * 64f, tile.Y * 64f - 96f - 16f), false, 0f, Color.White)
					{
						interval = 99999f,
						animationLength = 1,
		...
		{
			Game1.activeClickableMenu = new ItemGrabMenu(getRewardsForPlayer(Game1.player), false, true, HighlightCollectableRewards, null, "Rewards", OnRewardCollected, false, true, false, false, false, 0, null, -1, this, ItemExitBehavior.ReturnToPlayer, true);
		}
		...
		public void OnRewardCollected(Item item, Farmer who)
		{
			...
			string rewardKey;
			if (item is Object && _itemToRewardsLookup.TryGetValue(item, out rewardKey))
			{
		...
		{
			string itemId;
			if (museumPieces.TryGetValue(new Vector2(tileLocation.X, tileLocation.Y), out itemId) || museumPieces.TryGetValue(new Vector2(tileLocation.X, tileLocation.Y - 1), out itemId))
			{
