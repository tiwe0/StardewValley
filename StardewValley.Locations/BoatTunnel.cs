...
namespace StardewValley.Locations
{
	public class BoatTunnel : GameLocation
	{
		...
		{
			if (doesTileHaveProperty(tileLocation.X, tileLocation.Y, "Action", "Buildings") == "BoatTicket")
			{
				...
				else if (Game1.MasterPlayer.hasOrWillReceiveMail("willyBoatFixed"))
				{
					...
					{
						createQuestionDialogueWithCustomWidth(Game1.content.LoadString("Strings\\Locations:BuyTicket", TicketPrice), createYesNoResponses(), "Boat");
					}
					else
					{
						createQuestionDialogue(Game1.content.LoadString("Strings\\Locations:BuyTicket", TicketPrice), createYesNoResponses(), "Boat");
					}
				}
				return true;
			}
		...
		public override bool answerDialogue(Response answer)
		{
			...
			{
				string questionAndAnswer = ArgUtility.SplitBySpaceAndGet(lastQuestionKey, 0) + "_" + answer.responseKey;
		...
		public virtual void StartDeparture()
		{
			...
			_boatEvent = new Event(event_string.ToString(), null, "-78765", Game1.player)
			{
			...
			_boatEvent.Update(this, Game1.currentGameTime);
			Game1.eventUp = true;
		...
		public override bool RunLocationSpecificEventCommand(Event current_event, string command_string, bool first_run, params string[] args)
		{
			...
			case "non_blocking_pause":
				if (first_run)
				{
					nonBlockingPause = ArgUtility.GetInt(args, 0);
					return false;
				}
