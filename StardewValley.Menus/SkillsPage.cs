...
namespace StardewValley.Menus
{
	public class SkillsPage : IClickableMenu
	{
		...
		{
			int xPositionOnScreen2 = xPositionOnScreen;
			int spaceToClearSideBorder2 = IClickableMenu.spaceToClearSideBorder;
			int yPositionOnScreen2 = yPositionOnScreen;
			int spaceToClearTopBorder2 = IClickableMenu.spaceToClearTopBorder;
			...
			for (int k = 4; k < 10; k += 5)
			{
				for (int l = 0; l < 5; l++)
				{
					...
					if (drawRed && (k + 1) % 5 == 0)
					{
						skillBars.Add(new ClickableTextureComponent(professionNumber.ToString() ?? "", new Rectangle(addedX + drawX - 4 + k * 36, drawY + l * 68, 56, 36), null, professionBlurb, Game1.mouseCursors, new Rectangle(159, 338, 14, 9), 4f, true)
						{
							myID = ((k + 1 == 5) ? (100 + l) : (200 + l)),
							leftNeighborID = ((k + 1 == 5) ? l : (100 + l)),
							rightNeighborID = ((k + 1 == 5) ? (200 + l) : (-1)),
							downNeighborID = -99998
						});
					}
				}
		...
		public override void performHoverAction(int x, int y)
		{
			...
			hoverTitle = "";
			professionImage = -1;
			foreach (ClickableComponent c3 in ccTrackerButtons)
			{
		...
		public override void draw(SpriteBatch b)
		{
			...
			Game1.player.FarmerRenderer.draw(b, new FarmerSprite.AnimationFrame(Game1.player.bathingClothes ? 108 : playerPanelFrames[playerPanelIndex], 0, false, false), Game1.player.bathingClothes ? 108 : playerPanelFrames[playerPanelIndex], new Rectangle(playerPanelFrames[playerPanelIndex] * 16, Game1.player.bathingClothes ? 576 : 0, 16, 32), new Vector2(x + 32, y + 16 - 4), Vector2.Zero, 0.8f, 2, Color.White, 0f, 1f, Game1.player);
			if (Game1.timeOfDay >= 1900)
			{
				Game1.player.FarmerRenderer.draw(b, new FarmerSprite.AnimationFrame(playerPanelFrames[playerPanelIndex], 0, false, false), playerPanelFrames[playerPanelIndex], new Rectangle(playerPanelFrames[playerPanelIndex] * 16, 0, 16, 32), new Vector2(x + 32, y + 16 - 4), Vector2.Zero, 0.8f, 2, Color.DarkBlue * 0.3f, 0f, 1f, Game1.player);
			}
