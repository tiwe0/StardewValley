...
namespace StardewValley.Minigames
{
	public class CalicoJack : IMinigame
	{
		...
		public bool tick(GameTime time)
		{
			...
			for (int l = 0; l < dealerCards.Count; l++)
			{
				if (dealerCards[l][1] > 0)
				{
					dealerCards[l][1] -= time.ElapsedGameTime.Milliseconds;
					if (dealerCards[l][1] <= 0)
					{
						dealerCards[l][1] = 0;
					}
		...
		public void draw(SpriteBatch b)
		{
			...
			Utility.drawWithShadow(b, Game1.mouseCursors, new Vector2(coin_draw_pos.X + 4f, coin_draw_pos.Y + 4f), new Rectangle(211, 373, 9, 10), Color.White, 0f, Vector2.Zero, 4f, false, 1f);
			if (showingResultsScreen)
			{
				...
				if (!endTitle.Equals(Game1.content.LoadString("Strings\\StringsFromCSFiles:CalicoJack.cs.11951")))
				{
					...
					Utility.drawWithShadow(b, Game1.mouseCursors, new Vector2(Game1.graphics.GraphicsDevice.Viewport.Width / 2 - 32 + SpriteText.getWidthOfString(Game1.content.LoadString("Strings\\StringsFromCSFiles:CalicoJack.cs.11965", (playerWon ? "" : "-") + currentBet + "   ")) / 2, 260f) + new Vector2(8f, 0f), new Rectangle(211, 373, 9, 10), Color.White, 0f, Vector2.Zero, 4f, false, 1f);
				}
				if (playerWon)
				{
			...
			else
			{
				...
				Utility.drawWithShadow(b, Game1.mouseCursors, new Vector2(172 + SpriteText.getWidthOfString(Game1.content.LoadString("Strings\\StringsFromCSFiles:CalicoJack.cs.11972", currentBet)), Game1.graphics.GraphicsDevice.Viewport.Height / 2 + 4 - 48), new Rectangle(211, 373, 9, 10), Color.White, 0f, Vector2.Zero, 4f, false, 1f);
				if (playButtonsActive())
				{
