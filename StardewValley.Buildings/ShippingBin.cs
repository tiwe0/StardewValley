...
namespace StardewValley.Buildings
{
	public class ShippingBin : Building
	{
		...
		public void initLid()
		{
			shippingBinLid = new TemporaryAnimatedSprite("LooseSprites\\Cursors", new Rectangle(134, 226, 30, 25), new Vector2((int)tileX, (int)tileY - 1) * 64f + new Vector2(1f, -7f) * 4f, false, 0f, Color.White)
			{
				holdLastFrame = true,
				destroyable = false,
				interval = 20f,
		...
		public override void resetLocalState()
		{
			...
			{
				Rectangle shippingBinLidOpenArea2 = shippingBinLidOpenArea;
			}
			else
			{
				initLid();
		...
		public void showShipment(Object o, bool playThrowSound = true)
		{
			if (farm != null)
			{
				...
				parentLocation.temporarySprites.Add(new TemporaryAnimatedSprite("LooseSprites\\Cursors", new Rectangle(524, 218, 34, 22), new Vector2((int)tileX, (int)tileY - 1) * 64f + new Vector2(-1f, 5f) * 4f, false, 0f, Color.White)
				{
					interval = 100f,
				...
				parentLocation.temporarySprites.Add(new TemporaryAnimatedSprite("LooseSprites\\Cursors", new Rectangle(524, 230, 34, 10), new Vector2((int)tileX, (int)tileY - 1) * 64f + new Vector2(-1f, 17f) * 4f, false, 0f, Color.White)
				{
					interval = 100f,
					totalNumberOfLoops = 1,
					animationLength = 3,
				...
				parentLocation.temporarySprites.Add(new TemporaryAnimatedSprite(itemData.TextureName, itemData.GetSourceRect(), new Vector2((int)tileX, (int)tileY - 1) * 64f + new Vector2(7 + Game1.random.Next(6), 2f) * 4f, false, 0f, Color.White)
				{
					interval = 9999f,
					scale = 4f,
					alphaFade = 0.045f,
		...
		public override bool doAction(Vector2 tileLocation, Farmer who)
		{
			if ((int)daysOfConstructionLeft <= 0 && tileLocation.X >= (float)(int)tileX && tileLocation.X <= (float)((int)tileX + 1) && tileLocation.Y == (float)(int)tileY)
			{
				...
				ItemGrabMenu itemGrabMenu = new ItemGrabMenu(null, true, false, Utility.highlightShippableObjects, shipItem, "", null, true, true, false, true, false, 0, null, -1, this);
		...
		public override void draw(SpriteBatch b)
		{
			if (!base.isMoving)
			{
				...
				if (shippingBinLid != null && (int)daysOfConstructionLeft <= 0)
				{
					...
					shippingBinLid.draw(b, false, 0, 0, alpha * (((int)newConstructionTimer > 0) ? ((1000f - (float)(int)newConstructionTimer) / 1000f) : 1f));
				}
			}
		}
	}
