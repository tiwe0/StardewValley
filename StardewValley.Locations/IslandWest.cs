	...
	public class IslandWest : IslandLocation
	{
		...
		public override bool CanPlantTreesHere(string itemId, int tileX, int tileY, out string deniedMessage)
		{
			if (getTileSheetIDAt(tileX, tileY, "Back") == "untitled tile sheet2" || Object.isWildTreeSeed(itemId))
			{
				...
				case "Grass":
				case "":
					return CheckItemPlantRules(itemId, false, true, out deniedMessage);
				}
			}
		...
		public override bool performAction(string[] action, Farmer who, Location tileLocation)
		{
			if (ArgUtility.Get(action, 0) == "FarmObelisk")
			{
				...
				{
					Point parsed;
					if (!Game1.getFarm().TryGetMapPropertyAs("WarpTotemEntry", out parsed, false))
					{
		...
		public void showShipment(Object o, bool playThrowSound = true)
		{
			...
			temporarySprites.Add(new TemporaryAnimatedSprite("LooseSprites\\Cursors", new Microsoft.Xna.Framework.Rectangle(524, 218, 34, 22), new Vector2(90f, 38f) * 64f + new Vector2(0f, 5f) * 4f, false, 0f, Color.White)
			{
				interval = 100f,
			...
			temporarySprites.Add(new TemporaryAnimatedSprite("LooseSprites\\Cursors", new Microsoft.Xna.Framework.Rectangle(524, 230, 34, 10), new Vector2(90f, 38f) * 64f + new Vector2(0f, 17f) * 4f, false, 0f, Color.White)
			{
				interval = 100f,
				totalNumberOfLoops = 1,
				animationLength = 3,
			...
			TemporaryAnimatedSprite sprite = new TemporaryAnimatedSprite(null, default(Microsoft.Xna.Framework.Rectangle), new Vector2(90f, 38f) * 64f + new Vector2(8 + Game1.random.Next(6), 2f) * 4f, false, 0f, Color.White)
			{
				interval = 9999f,
				alphaFade = 0.045f,
		...
		public override bool checkAction(Location tileLocation, xTile.Dimensions.Rectangle viewport, Farmer who)
		{
			...
			{
				ItemGrabMenu itemGrabMenu = new ItemGrabMenu(null, true, false, Utility.highlightShippableObjects, Game1.getFarm().shipItem, "", null, true, true, false, true, false, 0, null, -1, this);
			...
			if (getTileIndexAt(tileLocation.X, tileLocation.Y, "Buildings") != -1 && getTileIndexAt(tileLocation.X, tileLocation.Y, "Buildings") == 1470)
			{
				...
				if (!IsQiWalnutRoomDoorUnlocked(out actualFoundWalnutsCount))
				{
		...
		public override void monsterDrop(Monster monster, int x, int y, Farmer who)
		{
			...
			if (!Game1.MasterPlayer.hasOrWillReceiveMail("tigerSlimeNut"))
			{
				...
				{
					if (i is GreenSlime && i.name == "Tiger Slime")
					{
						numTigerSlimes++;
					}
				}
		...
		public override void spawnObjects()
		{
			...
			if (Utility.getNumObjectsOfIndexWithinRectangle(musselNodeSpawnArea, new string[1] { "(O)25" }, this) < 10)
			{
				...
				if (CanItemBePlacedHere(spawn2, false, CollisionMask.All, CollisionMask.None))
				{
					objects.Add(spawn2, new Object("25", 1)
					{
						MinutesUntilReady = 8,
			...
			if (Utility.getNumObjectsOfIndexWithinRectangle(tidePoolsArea, new string[2] { "(O)393", "(O)397" }, this) < 5)
			{
				...
				if (CanItemBePlacedHere(spawn, false, CollisionMask.All, CollisionMask.None))
				{
		...
		public override void DayUpdate(int dayOfMonth)
		{
			...
			{
				spawnWeedsAndStones(20, true);
				if (Game1.dayOfMonth % 7 == 1)
				{
					spawnWeedsAndStones(20, true, false);
				}
			}
		}
		...
		protected override void resetLocalState()
		{
			...
			shippingBinLid = new TemporaryAnimatedSprite("LooseSprites\\Cursors", new Microsoft.Xna.Framework.Rectangle(134, 226, 30, 25), new Vector2(shippingBinPosition.X, shippingBinPosition.Y - 1) * 64f + new Vector2(2f, -7f) * 4f, false, 0f, Color.White)
			{
				holdLastFrame = true,
				destroyable = false,
				interval = 20f,
			...
			if (i != null)
			{
				...
				i.Sprite.loop = true;
				i.HideShadow = true;
				i.IsInvisible = IsRainingHere();
			}
			if (Game1.timeOfDay > 1700)
			{
				temporarySprites.Add(new TemporaryAnimatedSprite("LooseSprites\\Cursors", new Microsoft.Xna.Framework.Rectangle(276, 1985, 12, 11), new Vector2(23f, 58f) * 64f + new Vector2(-16f, -32f), false, 0f, Color.White)
				{
					interval = 50f,
