...
namespace StardewValley
{
	public class CraftingRecipe
	{
		...
		public CraftingRecipe(string name, bool isCookingRecipe)
		{
			...
			string recipe;
			string info;
			if (isCookingRecipe && cookingRecipes.TryGetValue(name, out recipe))
			{
				info = recipe;
			}
			...
			string error;
			if (!ArgUtility.TryGet(fields, 0, out rawIngredients, out error, false))
			{
				rawIngredients = "";
				LogParseError(info, error);
			}
			string rawOutputItems;
			if (!ArgUtility.TryGet(fields, 2, out rawOutputItems, out error, false))
			{
				rawOutputItems = "";
				LogParseError(info, error);
			}
			string tokenizableDisplayName;
			if (!ArgUtility.TryGetOptional(fields, isCookingRecipe ? 4 : 5, out tokenizableDisplayName, out error))
			{
				LogParseError(info, error);
			}
			...
			{
				recipeList.Add(ingredients[j], ArgUtility.GetInt(ingredients, j + 1, 1));
			}
			...
			ParsedItemData itemData = GetItemData(true);
		...
		public virtual void drawMenuView(SpriteBatch b, int x, int y, float layerDepth = 0.88f, bool shadow = true)
		{
			...
			Utility.drawWithShadow(b, texture, new Vector2(x, y), sourceRect, Color.White, 0f, Vector2.Zero, 4f, false, layerDepth);
		}
		...
		public virtual Item createItem()
		{
			...
			if (isCookingRecipe)
			{
				...
				if (obj != null && Game1.player.team.SpecialOrderRuleActive("QI_COOKING"))
				{
		...
		public virtual void consumeIngredients(List<IInventory> additionalMaterials)
		{
			foreach (KeyValuePair<string, int> pair in recipeList)
			{
				...
				{
					if (ItemMatchesForCrafting(Game1.player.Items[j], itemId))
					{
		...
		{
			for (int k = additionalRecipeItems.Count - 1; k >= 0; k--)
			{
				string item_index = additionalRecipeItems[k].Key;
				int required_count = additionalRecipeItems[k].Value;
				bool foundInBackpack = false;
				for (int j = Game1.player.Items.Count - 1; j >= 0; j--)
				{
					if (ItemMatchesForCrafting(Game1.player.Items[j], item_index))
					{
						int toRemove = required_count;
						required_count -= Game1.player.Items[j].Stack;
		...
		public virtual int getCraftableCount(IList<Item> additional_materials)
		{
			...
			foreach (KeyValuePair<string, int> pair in recipeList)
			{
				...
				for (int i = Game1.player.Items.Count - 1; i >= 0; i--)
				{
					...
					if (obj != null && (obj.QualifiedItemId == itemId || obj.Category.ToString() == itemId || isThereSpecialIngredientRule(obj, itemId)))
					{
				...
				if (additional_materials != null)
				{
					for (int c = 0; c < additional_materials.Count; c++)
					{
						...
						if (obj2 != null && (obj2.QualifiedItemId == itemId || obj2.Category.ToString() == itemId || isThereSpecialIngredientRule(obj2, itemId)))
						{
		...
		public virtual string getCraftCountText()
		{
			...
			{
				int timesCooked;
				if (Game1.player.recipesCooked.TryGetValue(getIndexOfMenuView(), out timesCooked) && timesCooked > 0)
				{
		...
		private void LogParseError(string rawData, string message)
		{
			...
			defaultInterpolatedStringHandler.AppendFormatted(isCookingRecipe ? "cooking" : "crafting");
