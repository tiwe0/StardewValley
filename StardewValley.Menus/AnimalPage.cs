...
namespace StardewValley.Menus
{
	public class AnimalPage : IClickableMenu
	{
		...
		private void drawNPCSlot(SpriteBatch b, int i)
		{
			...
			if (entry.special == 1)
			{
				Utility.drawWithShadow(b, Game1.objectSpriteSheet_2, new Vector2(xPositionOnScreen + 704 - 16, sprites[i].bounds.Y + yOffset + 64 - 52), new Rectangle(0, 160, 16, 16), Color.White, 0f, Vector2.Zero, 4f, false, 0.8f, 0, 8);
			}
			if (entry.ReceivedAnimalCracker)
			{
				Utility.drawWithShadow(b, Game1.objectSpriteSheet_2, new Vector2(xPositionOnScreen + 576 - 20, sprites[i].bounds.Y + yOffset + 64 - 16), new Rectangle(16, 242, 15, 11), Color.White, 0f, Vector2.Zero, 4f, false, 0.8f);
			}
		}
		...
		public override void draw(SpriteBatch b)
		{
			...
			if (newClip.Height > 0)
			{
				...
				drawVerticalPartition(b, xPositionOnScreen + 448 + 12, true, -1, -1, -1, heightOverride);
				drawVerticalPartition(b, xPositionOnScreen + 256 + 12 + 376, true, -1, -1, -1, heightOverride);
			}
			upButton.draw(b);
			downButton.draw(b);
