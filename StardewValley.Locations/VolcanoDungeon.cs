	...
	public class VolcanoDungeon : IslandLocation
	{
		...
		public virtual void CoolLava(int x, int y, bool playSound = true)
		{
			if (Game1.currentLocation == this)
			{
				for (int i = 0; i < 5; i++)
				{
					temporarySprites.Add(new TemporaryAnimatedSprite("LooseSprites\\Cursors", new Microsoft.Xna.Framework.Rectangle(372, 1956, 10, 10), new Vector2(x, (float)y - 0.5f) * 64f + new Vector2(Game1.random.Next(64), Game1.random.Next(64)), false, 0.007f, Color.White)
					{
						alpha = 0.75f,
			...
			cooledLavaTiles.TryAdd(new Vector2(x, y), true);
		}
		...
		public virtual void UpdateLavaNeighbor(int x, int y)
		{
			if (IsCooledLava(x, y))
			{
				...
				Point offset;
				if (GetBlobLookup().TryGetValue(neighbors, out offset))
				{
		...
		public virtual void GenerateContents(bool use_level_level_as_layout = false)
		{
			...
			{
				generationSeed.Value = Utility.CreateRandomSeed(Game1.stats.DaysPlayed * (level.Value + 1), level.Value * 5152, Game1.uniqueIDForThisGame / 2uL);
				switch (level.Value)
				{
				case 0:
					layoutIndex.Value = 0;
				...
				default:
				{
					...
					if ((int)level > 1 && layout_random.NextDouble() < 0.5 * (double)luckMultiplier)
					{
						...
						{
							if (activeLevels[m].layoutIndex.Value >= 32)
							{
					...
					if ((int)level > 0 && Game1.MasterPlayer.hasOrWillReceiveMail("volcanoShortcutUnlocked") && layout_random.NextDouble() < 0.75)
					{
						for (int k = 38; k < 58; k++)
						{
							valid_layouts.Add(k);
		...
		public virtual void UpdateBridge()
		{
			...
			for (int x = 28; x <= 32; x++)
			{
				for (int y = 34; y <= 37; y++)
				{
					...
					setMapTileIndex(x, y, tile_index, "Buildings");
					setTileProperty(x, y, "Buildings", "Passable", "T");
		...
		protected override void resetLocalState()
		{
			...
			if (!_sawFlameSprite)
			{
				temporarySprites.Add(new TemporaryAnimatedSprite("Characters\\Monsters\\Magma Sprite", new Microsoft.Xna.Framework.Rectangle(0, 32, 16, 16), new Vector2(30f, 38f) * 64f, false, 0f, Color.White)
				{
					id = 999,
					scale = 4f,
				...
				temporarySprites.Add(new TemporaryAnimatedSprite("LooseSprites\\shadow", new Microsoft.Xna.Framework.Rectangle(0, 0, 12, 7), new Vector2(30.2f, 39.4f) * 64f, false, 0f, Color.White)
				{
					id = 998,
					scale = 4f,
					totalNumberOfLoops = 99999,
		...
		public override string GetLocationSpecificMusic()
		{
			...
			if ((int)level == 1 || ((Game1.random.NextDouble() < 0.25 || (int)level == 6) && (Game1.getMusicTrackName() == "none" || Game1.isMusicContextActiveButNotPlaying() || Game1.getMusicTrackName().EndsWith("_Ambient"))))
			{
				return "VolcanoMines";
			}
			return "Volcano_Ambient";
		...
		public virtual void GenerateLevel(bool use_level_level_as_layout = false)
		{
			...
			map.TileSheets[0].TileIndexProperties[1].Add("Type", "Stone");
			map.TileSheets[0].TileIndexProperties[2].Add("Type", "Stone");
			map.TileSheets[0].TileIndexProperties[3].Add("Type", "Stone");
			map.TileSheets[0].TileIndexProperties[17].Add("Type", "Stone");
			map.TileSheets[0].TileIndexProperties[18].Add("Type", "Stone");
			map.TileSheets[0].TileIndexProperties[19].Add("Type", "Stone");
			map.TileSheets[0].TileIndexProperties[528].Add("Type", "Stone");
			map.TileSheets[0].TileIndexProperties[544].Add("Type", "Stone");
			map.TileSheets[0].TileIndexProperties[560].Add("Type", "Stone");
			map.TileSheets[0].TileIndexProperties[545].Add("Type", "Stone");
			map.TileSheets[0].TileIndexProperties[561].Add("Type", "Stone");
			map.TileSheets[0].TileIndexProperties[564].Add("Type", "Stone");
			map.TileSheets[0].TileIndexProperties[565].Add("Type", "Stone");
			map.TileSheets[0].TileIndexProperties[555].Add("Type", "Stone");
			map.TileSheets[0].TileIndexProperties[571].Add("Type", "Stone");
			...
			ApplyToColor(new Color(0, 255, 0), delegate(int x, int y)
			{
				...
				if (level.Value == 0)
				{
					warps.Add(new Warp(x, y + 2, "IslandNorth", 40, 24, false));
				}
				else
				{
					warps.Add(new Warp(x, y + 2, GetLevelName(level.Value - 1), x - startPosition.Value.X, 0, false));
				}
			});
			ApplyToColor(new Color(255, 0, 0), delegate(int x, int y)
			{
				...
				if (level.Value == 9)
				{
					warps.Add(new Warp(x, y - 2, "Caldera", 21, 39, false));
				}
				else
				{
					warps.Add(new Warp(x, y - 2, GetLevelName(level.Value + 1), x - endPosition.Value.X, 1, false));
				}
			});
			setPieceAreas.Clear();
			...
			{
				int j;
				for (j = 0; j < 32 && !(GetPixel(x + j, y, Color.Black) != set_piece_color) && !(GetPixel(x, y + j, Color.Black) != set_piece_color); j++)
				{
				}
				setPieceAreas.Add(new Microsoft.Xna.Framework.Rectangle(x, y, j, j));
				for (int k = 0; k < j; k++)
				{
					for (int l = 0; l < j; l++)
					{
						SetPixelMap(x + k, y + l, Color.White);
					}
				}
			});
			...
			ApplySetPieces();
			GenerateWalls(Color.Black, 0, 4, 4, 4, true, delegate(int x, int y)
			{
			...
			GenerateBlobs(Color.Blue, 0, 16, true, true);
			if (startPosition.HasValue)
			{
			...
			if ((level.Value == 9 || generationRandom.NextDouble() < (isMonsterLevel() ? 1.0 : 0.2)) && possibleSwitchPositions.TryGetValue(0, out endSwitchPositions) && endSwitchPositions.Count > 0)
			{
			...
			foreach (int index in possibleGatePositions.Keys)
			{
				...
				if (possibleGatePositions[index].Count > 0 && possibleSwitchPositions.TryGetValue(index, out dwarfSwitchPositions) && dwarfSwitchPositions.Count > 0)
				{
			...
			if ((int)level == 0)
			{
				CreateExit(new Point(40, 48), false);
				...
				warps.Add(new Warp(44, 48, "Caldera", 11, 36, false));
				CreateEntrance(new Point(6, 48));
				warps.Add(new Warp(6, 50, "IslandNorth", 12, 31, false));
			}
			if (Game1.IsMasterGame)
			{
				GenerateEntities();
		...
		public virtual void GenerateDirtTiles()
		{
			...
			for (int j = 0; j < 8; j++)
			{
				...
				for (int k = 0; k < travel_distance; k++)
				{
			...
			foreach (Point point in dirtTiles)
			{
				...
				{
					characters.Add(new Duggy(Utility.PointToVector2(point) * 64f, true));
				}
		...
		public virtual void AddPossibleSwitchLocation(int switch_index, int x, int y)
		{
			...
			if (!possibleSwitchPositions.TryGetValue(switch_index, out positions))
			{
		...
		{
			List<Point> positions;
			if (!possibleGatePositions.TryGetValue(gate_index, out positions))
			{
		...
		public bool isTileClearForMineObjects(Vector2 v, bool ignoreRuins = false)
		{
			...
			if (!CanItemBePlacedHere(v, false, CollisionMask.All, CollisionMask.None))
			{
				return false;
			}
			...
			if (ignoreRuins)
			{
				int tileIndex = getTileIndexAt((int)v.X, (int)v.Y, "Back");
		...
		public virtual void GenerateEntities()
		{
			...
			if ((int)level > 0 && (int)level != 5 && (generationRandom.NextBool() || isMushroomLevel()))
			{
				...
				for (int j = 0; j < numBarrels; j++)
				{
					...
					{
						p2 = new Point(generationRandom.Next(map.RequireLayer("Back").LayerWidth), 0);
						motion = new Point(0, 1);
					}
					else if (generationRandom.NextBool())
					{
						p2 = new Point(0, generationRandom.Next(map.RequireLayer("Back").LayerHeight));
						motion = new Point(1, 0);
					}
					else
					{
						p2 = new Point(map.RequireLayer("Back").LayerWidth - 1, generationRandom.Next(map.RequireLayer("Back").LayerHeight));
						motion = new Point(-1, 0);
					}
					while (isTileOnMap(p2.X, p2.Y))
					{
						...
						if (isTileClearForMineObjects(new Vector2(p2.X, p2.Y)))
						{
							...
							{
								objects.Add(objectPos2, BreakableContainer.GetBarrelForVolcanoDungeon(objectPos2));
							}
							break;
						}
					}
			...
			if ((int)level != 5)
			{
				for (int x2 = 0; x2 < map.Layers[0].LayerWidth; x2++)
				{
					for (int y2 = 0; y2 < map.Layers[0].LayerHeight; y2++)
					{
						...
						if (CanItemBePlacedHere(objectPos) && generationRandom.NextDouble() < monsterChance)
						{
							if (getTileIndexAt((int)objectPos.X, (int)objectPos.Y, "Back") == 25)
							{
								if (!isMushroomLevel())
								{
									characters.Add(new Duggy(objectPos * 64f, true));
								}
							}
							...
							{
								characters.Add(new Bat(objectPos * 64f, ((int)level > 5 && generationRandom.NextBool()) ? (-556) : (-555)));
							}
						}
						else
						{
							if (!isTileClearForMineObjects(objectPos, true))
							{
								continue;
							}
							...
							{
								base.Objects.Add(objectPos, new Object("851", 1)
								{
			...
			{
				if (CanSpawnCharacterHere(new Vector2(p.X, p.Y)))
				{
					int direction = 1;
		...
		private int chooseStoneTypeIndexOnly(Vector2 tile)
		{
			...
			{
				Object obj6;
				if (objects.TryGetValue(v6, out obj6) && (obj6.QualifiedItemId == "(O)843" || obj6.QualifiedItemId == "(O)844"))
				{
					chance += 0.15;
				}
			}
			...
			else
			{
				...
				{
					Object obj5;
					if (objects.TryGetValue(v5, out obj5) && obj5.QualifiedItemId == "(O)765")
					{
						chance += 0.1;
					}
				}
				...
				else
				{
					...
					{
						Object obj4;
						if (objects.TryGetValue(v4, out obj4) && obj4.QualifiedItemId == "(O)VolcanoGoldNode")
						{
							chance += 0.2;
						}
					}
					...
					else
					{
						...
						{
							Object obj3;
							if (objects.TryGetValue(v3, out obj3) && obj3.QualifiedItemId.StartsWith("(O)VolcanoCoalNode"))
							{
								chance += 0.2;
							}
						}
						...
						else
						{
							...
							{
								Object obj2;
								if (objects.TryGetValue(v2, out obj2) && obj2.QualifiedItemId == "(O)850")
								{
									chance += 0.25;
								}
							}
							...
							else
							{
								...
								{
									Object obj;
									if (objects.TryGetValue(v, out obj) && obj.QualifiedItemId == "(O)849")
									{
										chance += 0.25;
									}
								}
		...
		public void tryToAddOreClumps()
		{
			...
			{
				if (CanItemBePlacedHere(endPoint, false, CollisionMask.All, CollisionMask.None) && isTileOnClearAndSolidGround(endPoint) && doesTileHaveProperty((int)endPoint.X, (int)endPoint.Y, "Diggable", "Back") == null)
				{
		...
		public virtual void SpawnChest(int tile_x, int tile_y)
		{
			...
			if (Game1.IsMasterGame)
			{
				...
				Chest chest = new Chest(false, position);
		...
		public virtual void PopulateChest(IList<Item> items, Random chest_random, int chest_type)
		{
			...
			{
				int random_count = 9;
				int random2 = chest_random.Next(random_count);
				if (!Game1.netWorldState.Value.GoldenCoconutCracked)
				{
					while (random2 == 3)
					{
						random2 = chest_random.Next(random_count);
					}
				}
		...
		public override void UpdateWhenCurrentLocation(GameTime time)
		{
			...
			if ((int)level == 0 && Game1.currentLocation == this)
			{
				...
				if (steamTimer < 0f)
				{
					...
					temporarySprites.Add(new TemporaryAnimatedSprite(null, new Microsoft.Xna.Framework.Rectangle(0, 0, 1, 1), new Vector2(34.5f, 30.75f) * 64f, false, 0f, Color.White)
					{
						...
						reachedStopCoordinate = delegate
						{
							...
							for (int i = 0; i < 4; i++)
							{
								temporarySprites.Add(new TemporaryAnimatedSprite("LooseSprites\\Cursors", new Microsoft.Xna.Framework.Rectangle(372, 1956, 10, 10), new Vector2(33.75f, 33.5f) * 64f + new Vector2(Game1.random.Next(64), Game1.random.Next(64)), false, 0.007f, Color.White)
								{
									alpha = 0.75f,
									motion = new Vector2(0f, -1f),
									acceleration = new Vector2(0.002f, 0f),
		...
		public override void drawAboveAlwaysFrontLayer(SpriteBatch b)
		{
			...
			if (!Game1.game1.takingMapScreenshot && (int)level > 0)
			{
				...
				SpriteText.drawString(b, txt, tsarea.Left + 16, tsarea.Top + 16, 999999, -1, 999999, 1f, 1f, false, 2, "", col);
			}
		}
		...
		public void spawnFlyingMonsterOffScreen()
		{
			...
			characters.Add(new Bat(spawnLocation, ((int)level > 5 && Game1.random.NextBool()) ? (-556) : (-555))
			{
		...
		public static VolcanoDungeon GetLevel(string name, bool use_level_level_as_layout = false)
		{
			...
			int newLevelNumber;
			if (!IsGeneratedLevel(name, out newLevelNumber))
			{
		...
		{
			if (locationName == null || !locationName.StartsWith("VolcanoDungeon", StringComparison.OrdinalIgnoreCase))
			{
				level = 0;
				return false;
			}
