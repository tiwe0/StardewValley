...
namespace StardewValley.Menus
{
	public class BuffsDisplay : IClickableMenu
	{
		...
		protected virtual void resetIcons()
		{
			...
			updatedIDs.Clear();
			arrangeTheseComponentsInThisRectangle(xPositionOnScreen, yPositionOnScreen, width / 64, 64, 64, 8, true);
		}
		...
		protected virtual string getDescription(Buff buff)
		{
			...
			{
				string description = getDescription(buff, attribute, false);
				if (description != null)
				{
		...
		public virtual IEnumerable<ClickableTextureComponent> getClickableComponents(Buff buff)
		{
			...
			{
				Rectangle sourceRect2 = Game1.getSourceRectForStandardTileSheet(buff.iconTexture, buff.iconSheetIndex, 16, 16);
				yield return new ClickableTextureComponent("", Rectangle.Empty, null, getDescription(buff), buff.iconTexture, sourceRect2, 4f);
				yield break;
			}
			foreach (BuffAttributeDisplay attribute in displayAttributes)
			{
				string description = getDescription(buff, attribute, true);
				if (description != null)
				{
