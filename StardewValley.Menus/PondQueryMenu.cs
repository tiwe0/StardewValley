...
namespace StardewValley.Menus
{
	public class PondQueryMenu : IClickableMenu
	{
		...
		{
			Random r = Utility.CreateDaySaveRandom((int)_pond.seedOffset);
			...
			if (_pond.neededItem.Value != null)
			{
				...
				{
					string item_count_string = _pond.neededItemCount.Value.ToString() ?? "";
		...
		public override void draw(SpriteBatch b)
		{
			if (!Game1.globalFade)
			{
				...
				Game1.DrawBox((int)((float)(Game1.uiViewport.Width / 2) - (text_size.X + 64f) * 0.5f), yPositionOnScreen - 4 + 128, (int)(text_size.X + 64f), 64);
				Utility.drawTextWithShadow(b, pond_name_text, Game1.smallFont, new Vector2((float)(Game1.uiViewport.Width / 2) - text_size.X * 0.5f, (float)(yPositionOnScreen - 4) + 160f - text_size.Y * 0.5f), Color.Black);
				...
				Game1.drawDialogueBox(xPositionOnScreen, yPositionOnScreen + 128, width, height - 128 + extraHeight + extraTextHeight, false, true);
				string population_text = Game1.content.LoadString("Strings\\UI:PondQuery_Population", _pond.FishCount.ToString() ?? "", _pond.maxOccupants);
				text_size = Game1.smallFont.MeasureString(population_text);
				Utility.drawTextWithShadow(b, population_text, Game1.smallFont, new Vector2(_pond.goldenAnimalCracker ? ((float)(xPositionOnScreen + IClickableMenu.borderWidth + 4)) : ((float)(xPositionOnScreen + width / 2) - text_size.X * 0.5f), yPositionOnScreen + IClickableMenu.spaceToClearTopBorder + 16 + 128), Game1.textColor);
				...
				for (int i = 0; i < slots_to_draw; i++)
				{
					...
					{
						_fishItem.drawInMenu(b, new Vector2((float)(xPositionOnScreen + width / 2) - slot_spacing * (float)Math.Min(slots_to_draw, 5) * 4f * 0.5f + slot_spacing * 4f * (float)x - 12f, (float)(yPositionOnScreen + (int)(y_offset * 4f)) + (float)(y * 4) * slot_spacing + 275.2f), 0.75f, 1f, 0f, StackDrawType.Hide, Color.White, false);
					}
					else
					{
						_fishItem.drawInMenu(b, new Vector2((float)(xPositionOnScreen + width / 2) - slot_spacing * (float)Math.Min(slots_to_draw, 5) * 4f * 0.5f + slot_spacing * 4f * (float)x - 12f, (float)(yPositionOnScreen + (int)(y_offset * 4f)) + (float)(y * 4) * slot_spacing + 275.2f), 0.75f, 0.35f, 0f, StackDrawType.Hide, Color.Black, false);
					}
					x++;
					if (x == 5)
					{
				...
				Utility.drawTextWithShadow(b, displayed_text, Game1.smallFont, new Vector2((float)(xPositionOnScreen + width / 2) - text_size.X * 0.5f, (float)(yPositionOnScreen + height + extraTextHeight - (has_unresolved_needs ? 32 : 48)) - text_size.Y), Game1.textColor);
				if (has_unresolved_needs)
				{
					...
					b.Draw(texture, new Vector2(icon_x, yPositionOnScreen + height + extraTextHeight + 4), sourceRect, Color.Black * 0.4f, 0f, Vector2.Zero, 4f, SpriteEffects.None, 1f);
					b.Draw(texture, new Vector2(icon_x + 4f, yPositionOnScreen + height + extraTextHeight), sourceRect, Color.White, 0f, Vector2.Zero, 4f, SpriteEffects.None, 1f);
					if (_pond.neededItemCount.Value > 1)
					{
						Utility.drawTinyDigits(_pond.neededItemCount.Value, b, new Vector2(icon_x + 48f, yPositionOnScreen + height + extraTextHeight + 48), 3f, 1f, Color.White);
					}
				}
				if (_pond.goldenAnimalCracker.Value && Game1.objectSpriteSheet_2 != null)
				{
					Utility.drawWithShadow(b, Game1.objectSpriteSheet_2, new Vector2((float)(xPositionOnScreen + width) - 105.6f, (float)yPositionOnScreen + 224f), new Rectangle(16, 240, 16, 16), Color.White, 0f, Vector2.Zero, 4f, false, 0.89f);
				}
				okButton.draw(b);
				emptyButton.draw(b);
