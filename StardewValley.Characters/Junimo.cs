...
namespace StardewValley.Characters
{
	public class Junimo : NPC
	{
		...
		private void placeStar(Character c, GameLocation l)
		{
			...
			friendly.Value = true;
			base.speed = 3;
			Game1.multiplayer.broadcastSprites(l, new TemporaryAnimatedSprite(Sprite.textureName, new Rectangle(0, 109, 16, 19), 40f, 8, 10, base.Position + new Vector2(0f, -64f), false, false, 1f, 0f, Color.White, 4f * scale.Value, 0f, 0f, 0f)
			{
		...
		public override void drawAboveAlwaysFrontLayer(SpriteBatch b)
		{
			if (textAboveHeadTimer > 0 && textAboveHead != null)
			{
				...
				SpriteText.drawStringWithScrollCenteredAt(b, textAboveHead, (int)local.X, (int)local.Y, "", textAboveHeadAlpha, textAboveHeadColor, 1, (float)(base.TilePoint.Y * 64) / 10000f + 0.001f + (float)base.TilePoint.X / 10000f, !sayingGoodbye);
			}
		}
		...
		public override void update(GameTime time, GameLocation location)
		{
			...
			if ((bool)sayingGoodbye)
			{
				...
				if (!base.IsInvisible && Game1.random.NextDouble() < 0.009999999776482582 && yJumpOffset == 0)
				{
					...
					{
						showTextAboveHead(Game1.random.NextBool() ? Game1.content.LoadString("Strings\\StringsFromCSFiles:Junimo.cs.6625") : Game1.content.LoadString("Strings\\StringsFromCSFiles:Junimo.cs.6626"));
					}
				}
			...
			else
			{
				...
				if (!base.IsInvisible && farmerCloseCheckTimer <= 0 && controller == null && alpha.Value >= 1f && !stayPut && Game1.IsMasterGame)
				{
