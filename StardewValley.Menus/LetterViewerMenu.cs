	...
	public class LetterViewerMenu : IClickableMenu
	{
		...
		public string HandleActionCommand(string mail)
		{
			...
			while (true)
			{
				...
				string error;
				Exception ex;
				if (!isFromCollection && !TriggerActionManager.TryRunAction(action, out error, out ex))
				{
					...
					defaultInterpolatedStringHandler.AppendFormatted(mailTitle);
		...
		public string HandleItemCommand(string mail)
		{
			...
			while (true)
			{
				...
				case "bigobject":
				{
					...
					Item o2 = ItemRegistry.Create("(BC)" + id2);
				...
				case "furniture":
				{
					...
					Item o3 = ItemRegistry.Create("(F)" + id3);
				...
				case "cookingrecipe":
				{
					...
					if (!string.IsNullOrWhiteSpace(recipeKey))
					{
						if (cookingRecipes.ContainsKey(recipeKey))
						{
							...
							learnedRecipe = new CraftingRecipe(recipeKey, true).DisplayName;
				...
				case "craftingrecipe":
				{
					...
					string rawFields;
					if (craftingRecipes.TryGetValue(args[0], out rawFields))
					{
					...
					learnedRecipe = new CraftingRecipe(learnedRecipe, false).DisplayName;
				...
				case "specialorder":
				{
					...
					bool addImmediately;
					string error;
					if (ArgUtility.TryGetBool(args, 1, out addImmediately, out error) && addImmediately)
					{
		...
		public override void draw(SpriteBatch b)
		{
			...
			if (scale == 1f)
			{
				...
				{
					SpriteText.drawString(b, mailMessage[page], xPositionOnScreen + 32, yPositionOnScreen + 32, 999999, width - 64, 999999, 0.75f, 0.865f, false, -1, "", getTextColor());
				}
				if (ShouldShowInteractable())
				{
					...
					if (moneyIncluded > 0)
					{
						...
						SpriteText.drawString(b, moneyText, xPositionOnScreen + width / 2 - SpriteText.getWidthOfString(moneyText) / 2, yPositionOnScreen + height - 96, 999999, -1, 9999, 0.75f, 0.865f, false, -1, "", getTextColor());
					}
					else
					{
						...
						if (text != null && text.Length > 0)
						{
							...
							SpriteText.drawStringHorizontallyCenteredAt(b, recipeText, xPositionOnScreen + width / 2, yPositionOnScreen + height - 32 - SpriteText.getHeightOfString(recipeText) * 2, 999999, -1, 9999, 0.65f, 0.865f, false, getTextColor());
							SpriteText.drawStringHorizontallyCenteredAt(b, Game1.content.LoadString("Strings\\UI:LetterViewer_LearnedRecipeName", learnedRecipe), xPositionOnScreen + width / 2, yPositionOnScreen + height - 32 - SpriteText.getHeightOfString("t"), 999999, -1, 9999, 0.9f, 0.865f, false, getTextColor());
						}
					}
				}
				base.draw(b);
