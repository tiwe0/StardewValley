...
namespace StardewValley.Enchantments
{
	public class MagicEnchantment : BaseWeaponEnchantment
	{
		protected override void _OnSwing(MeleeWeapon weapon, Farmer farmer)
		{
			...
			shot_velocity *= 10f;
			BasicProjectile projectile = new BasicProjectile((int)Math.Ceiling((float)weapon.minDamage.Value / 4f), 11, 0, 1, rotation_velocity * ((float)Math.PI / 180f), shot_velocity.X, shot_velocity.Y, shot_origin, null, null, null, false, true, farmer.currentLocation, farmer);
