	...
	public class ParrotUpgradePerch : INetObject<NetFields>
	{
		...
		public virtual void PerformCompleteAnimation()
		{
			if (upgradeName.Contains("Volcano"))
			{
				for (int j = 0; j < 16; j++)
				{
					...
					TemporaryAnimatedSprite t2 = new TemporaryAnimatedSprite("LooseSprites\\Cursors2", new Microsoft.Xna.Framework.Rectangle(65, 229, 16, 6), Utility.getRandomPositionInThisRectangle(upgradeRect.Value, Game1.random) * 64f, Game1.random.NextBool(), 0f, Color.White)
					{
					...
					t2 = new TemporaryAnimatedSprite("LooseSprites\\Cursors2", new Microsoft.Xna.Framework.Rectangle(65, 229, 16, 6), Utility.getRandomPositionInThisRectangle(upgradeRect.Value, Game1.random) * 64f, Game1.random.NextBool(), 0f, Color.White)
					{
						motion = new Vector2(Game1.random.Next(-2, 3), -16f),
						acceleration = new Vector2(0f, 0.5f),
						rotationChange = (float)Game1.random.Next(-4, 5) * 0.05f,
			...
			IslandLocation islandLoc = locationRef.Value as IslandLocation;
			if (islandLoc == null)
			{
				return;
			}
		...
		public static void ActivateGoldenParrot()
		{
			...
			foreach (GameLocation i in Game1.locations)
			{
				...
				IslandWestCave1 cave = i as IslandWestCave1;
				if (cave != null)
				{
		...
		public virtual void Draw(SpriteBatch b)
		{
			if (IsAvailable(true) && (parrotPresent || upgradeName == "Hut"))
			{
				int frame = 0;
		...
		{
			if (parrotPresent && IsAvailable(true) && isPlayerNearby)
			{
