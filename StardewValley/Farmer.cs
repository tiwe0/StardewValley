...
namespace StardewValley
{
	public class Farmer : Character, IComparable
	{
		...
		public static readonly EmoteType[] EMOTES = new EmoteType[22]
		{
			...
			new EmoteType("blush", "Emote_Blush", 60, null, 2, true),
			new EmoteType("angry", "Emote_Angry", 12),
			new EmoteType("yes", "Emote_Yes", 56, new FarmerSprite.AnimationFrame[7]
			{
				new FarmerSprite.AnimationFrame(0, 250, false, false).AddFrameAction(delegate(Farmer who)
				{
			...
			{
				new FarmerSprite.AnimationFrame(25, 250, false, false).AddFrameAction(delegate(Farmer who)
				{
					if (who.ShouldHandleAnimationSound())
					{
			...
			{
				new FarmerSprite.AnimationFrame(104, 350, false, false).AddFrameAction(delegate(Farmer who)
				{
					if (who.ShouldHandleAnimationSound())
					{
			...
			{
				new FarmerSprite.AnimationFrame(102, 150, false, false).AddFrameAction(delegate(Farmer who)
				{
					if (who.ShouldHandleAnimationSound())
					{
			...
			new EmoteType("surprised", "Emote_Surprised", 16, new FarmerSprite.AnimationFrame[1] { new FarmerSprite.AnimationFrame(94, 1500, false, false).AddFrameAction(delegate(Farmer who)
			{
				if (who.ShouldHandleAnimationSound())
				{
			...
			{
				new FarmerSprite.AnimationFrame(3, 250, false, false).AddFrameAction(delegate(Farmer who)
				{
					if (who.ShouldHandleAnimationSound())
					{
			...
			{
				new FarmerSprite.AnimationFrame(3, 250, false, false),
				new FarmerSprite.AnimationFrame(102, 50, false, false),
				new FarmerSprite.AnimationFrame(10, 250, false, false).AddFrameAction(delegate(Farmer who)
				{
					if (who.ShouldHandleAnimationSound())
					{
				...
				new FarmerSprite.AnimationFrame(3, 500, false, false)
			}, 2, true),
			new EmoteType("uh", "Emote_Uh", 40, new FarmerSprite.AnimationFrame[1] { new FarmerSprite.AnimationFrame(10, 1500, false, false).AddFrameAction(delegate(Farmer who)
			{
				if (who.ShouldHandleAnimationSound())
				{
			...
			{
				new FarmerSprite.AnimationFrame(98, 150, false, false).AddFrameAction(delegate(Farmer who)
				{
			...
			{
				new FarmerSprite.AnimationFrame(111, 150, false, false),
				new FarmerSprite.AnimationFrame(111, 300, false, false).AddFrameAction(delegate(Farmer who)
				{
					if (who.ShouldHandleAnimationSound())
					{
				...
				new FarmerSprite.AnimationFrame(111, 500, false, false),
				new FarmerSprite.AnimationFrame(111, 300, false, false).AddFrameAction(delegate(Farmer who)
				{
					if (who.ShouldHandleAnimationSound())
					{
				...
				new FarmerSprite.AnimationFrame(111, 500, false, false),
				new FarmerSprite.AnimationFrame(112, 1000, false, false).AddFrameAction(delegate(Farmer who)
				{
					if (who.ShouldHandleAnimationSound())
					{
		...
		public Object ActiveObject
		{
			get
			{
				...
				{
					Object obj = Items[currentToolIndex] as Object;
					if (obj != null)
					{
						return obj;
					}
		...
		public override int FacingDirection
		{
			get
			{
				if (!IsLocalPlayer && !isFakeEventActor && UsingTool)
				{
		...
		public void LearnDefaultRecipes()
		{
			...
			{
				if (!craftingRecipes.ContainsKey(recipe2.Key) && ArgUtility.Get(recipe2.Value.Split('/'), 4) == "default")
				{
					craftingRecipes.Add(recipe2.Key, 0);
				}
			}
		...
		{
			(Game1.RequireLocation(location_name) as FarmHouse)?.UpdateForRenovation();
		}
		...
		public void performPlayerEmote(string emote_string)
		{
			for (int i = 0; i < EMOTES.Length; i++)
			{
				...
				{
					isEmoting = false;
					doEmote(emote_type.emoteIconIndex, false);
				}
			}
		}
		...
		private void playHarpEmoteSound()
		{
			...
			if (Game1.IsMultiplayer && UniqueMultiplayerID % 111 == 0L)
			{
				...
				{
					DelayedAction.playSoundAfterDelay("miniharp_note", Game1.random.Next(60, 150) * j, base.currentLocation, base.Tile, notes[j]);
		...
		private static void removeLowestUpgradeLevelTool(List<Item> items, Type toolType)
		{
			...
			foreach (Item item in items)
			{
				...
				if (tool != null && tool.GetType() == toolType && (lowestItem == null || (int)tool.upgradeLevel < (int)lowestItem.upgradeLevel))
				{
					lowestItem = tool;
				}
			}
		...
		public int? tryGetFriendshipLevelForNPC(string name)
		{
			...
			if (friendshipData.TryGetValue(name, out friendship))
			{
		...
		{
			Friendship friendship;
			if (name != null && friendshipData.TryGetValue(name, out friendship))
			{
		...
		{
			Friendship friendship;
			if (spouse != null && friendshipData.TryGetValue(spouse, out friendship))
			{
		...
		{
			int curValue;
			if (!basicShipped.TryGetValue(itemId, out curValue))
			{
				curValue = 0;
			}
		...
		public void shiftToolbar(bool right)
		{
			...
			if (right)
			{
				IList<Item> toMove2 = Items.GetRange(0, 12);
			...
			{
				Toolbar toolbar = Game1.onScreenMenus[i] as Toolbar;
				if (toolbar != null)
				{
		...
		public virtual void showNutPickup()
		{
			...
			{
				base.currentLocation.temporarySprites.Add(new TemporaryAnimatedSprite("LooseSprites\\Cursors2", new Microsoft.Xna.Framework.Rectangle(0, 240, 16, 16), 100f, 4, 2, new Vector2(0f, -96f), false, false, 1f, 0f, Color.White, 4f, 0f, 0f, 0f)
				{
		...
		public void foundArtifact(string itemId, int number)
		{
			...
			int[] artifactEntry;
			if (archaeologyFound.TryGetValue(itemId, out artifactEntry))
			{
		...
		{
			int curValue;
			if (!recipesCooked.TryGetValue(itemId, out curValue))
			{
				curValue = 0;
			}
		...
		public bool caughtFish(string itemId, int size, bool from_fish_pond = false, int numberCaught = 1)
		{
			...
			{
				int[] fishEntry;
				if (fishCaught.TryGetValue(itemId, out fishEntry))
				{
		...
		public void revealGiftTaste(string npcName, string itemId)
		{
			if (npcName != null)
			{
				...
				if (!giftedItems.TryGetValue(npcName, out giftData))
				{
		...
		public void onGiftGiven(NPC npc, Object item)
		{
			...
			SerializableDictionary<string, int> giftData;
			if (!giftedItems.TryGetValue(npc.name, out giftData))
			{
				giftData = (giftedItems[npc.name] = new SerializableDictionary<string, int>());
			}
			int curValue;
			if (!giftData.TryGetValue(item.ItemId, out curValue))
			{
				curValue = 0;
			}
		...
		public bool hasItemBeenGifted(NPC npc, string itemId)
		{
			...
			int value;
			if (!giftData.TryGetValue(itemId, out value))
			{
				return false;
			}
			return value > 0;
		...
		public void MarkItemAsTailored(Item item)
		{
			if (item != null)
			{
				...
				int curValue;
				if (!tailoredItems.TryGetValue(item_key, out curValue))
				{
					curValue = 0;
				}
		...
		{
			int curValue;
			if (!mineralsFound.TryGetValue(itemId, out curValue))
			{
				curValue = 0;
			}
		...
		public void ShowSitting()
		{
			...
			if (yJumpOffset != 0)
			{
				...
				case 1:
					...
					break;
				case 3:
					FarmerSprite.setCurrentSingleFrame(6, 32000, false, true);
					break;
				case 2:
			...
			case 1:
				...
				break;
			case 3:
				FarmerSprite.setCurrentSingleFrame(117, 32000, false, true);
				xOffset = 4f;
				yOffset = -32f;
				break;
			case 2:
				FarmerSprite.setCurrentSingleFrame(107, 32000, true);
				xOffset = 0f;
				yOffset = -48f;
				break;
			}
		...
		public void showRiding()
		{
			...
			case 1:
				...
				break;
			case 3:
				FarmerSprite.setCurrentSingleFrame(106, 32000, false, true);
				xOffset = -12f;
				break;
			case 2:
		...
		public Friendship GetSpouseFriendship()
		{
			...
			Friendship friendship;
			if (string.IsNullOrEmpty(spouse) || !friendshipData.TryGetValue(spouse, out friendship))
			{
				return null;
			}
			return friendship;
		...
		public void doDivorce()
		{
			...
			if (spouse != null)
			{
				...
				if (currentSpouse != null)
				{
					...
					Friendship friendship;
					if (friendshipData.TryGetValue(currentSpouse.name, out friendship))
					{
						friendship.Points = 0;
		...
		public static void showEatingItem(Farmer who)
		{
			...
			case 1:
				...
				{
					tempSprite = new TemporaryAnimatedSprite("LooseSprites\\Cursors", new Microsoft.Xna.Framework.Rectangle(368, 16, 16, 16), 62.75f, 8, 2, who.Position + new Vector2(-21f, -112f), false, false, (float)who.StandingPixel.Y / 10000f + 0.01f, 0f, Color.White, 4f, 0f, 0f, 0f);
					break;
				}
				tempSprite = new TemporaryAnimatedSprite(textureName, sourceRect, 254f, 1, 0, who.Position + new Vector2(-21f, -112f), false, false, (float)who.StandingPixel.Y / 10000f + 0.01f, 0f, color, 4f, 0f, 0f, 0f);
				if (!coloredObjectColor.Equals(Color.White))
				{
					sourceRect.X += sourceRect.Width;
					coloredTempSprite = new TemporaryAnimatedSprite(textureName, sourceRect, 254f, 1, 0, who.Position + new Vector2(-21f, -112f), false, false, (float)(who.StandingPixel.Y + 1) / 10000f + 0.01f, 0f, coloredObjectColor, 4f, 0f, 0f, 0f);
				}
				break;
			case 2:
				if (who.IsLocalPlayer && who.itemToEat.QualifiedItemId == "(O)434")
				{
					tempSprite = new TemporaryAnimatedSprite("LooseSprites\\Cursors", new Microsoft.Xna.Framework.Rectangle(368, 16, 16, 16), 81.25f, 8, 0, who.Position + new Vector2(-21f, -108f), false, false, (float)who.StandingPixel.Y / 10000f + 0.01f, 0f, Color.White, 4f, -0.01f, 0f, 0f)
					{
				...
				tempSprite = new TemporaryAnimatedSprite(textureName, sourceRect, 650f, 1, 0, who.Position + new Vector2(-21f, -108f), false, false, (float)who.StandingPixel.Y / 10000f + 0.01f, 0f, color, 4f, -0.01f, 0f, 0f)
				{
					motion = new Vector2(0.8f, -11f),
					acceleration = new Vector2(0f, 0.5f)
				};
				...
				{
					sourceRect.X += sourceRect.Width;
					coloredTempSprite = new TemporaryAnimatedSprite(textureName, sourceRect, 650f, 1, 0, who.Position + new Vector2(-21f, -108f), false, false, (float)(who.StandingPixel.Y + 1) / 10000f + 0.01f, 0f, coloredObjectColor, 4f, 0f, 0f, 0f)
					{
						motion = new Vector2(0.8f, -11f),
						acceleration = new Vector2(0f, 0.5f)
					};
			...
			case 4:
			{
				...
				for (int i = 0; i < 8; i++)
				{
					...
					r.Width = size;
					r.Height = size;
					tempSprite = new TemporaryAnimatedSprite(textureName, r, 400f, 1, 0, who.Position + new Vector2(24f, -48f), false, false, (float)who.StandingPixel.Y / 10000f + 0.01f, 0f, color, 4f, 0f, 0f, 0f)
					{
		...
		public static void showHoldingItem(Farmer who, Item item)
		{
			...
			{
				TemporaryAnimatedSprite sprite = new TemporaryAnimatedSprite(null, default(Microsoft.Xna.Framework.Rectangle), 2500f, 1, 0, who.Position + new Vector2(0f, -124f), false, false, 1f, 0f, Color.White, 1f, 0f, 0f, 0f)
				{
			...
			if (item is Hat)
			{
				TemporaryAnimatedSprite sprite6 = new TemporaryAnimatedSprite(null, default(Microsoft.Xna.Framework.Rectangle), 2500f, 1, 0, who.Position + new Vector2(-8f, -124f), false, false, 1f, 0f, Color.White, 1f, 0f, 0f, 0f)
				{
					motion = new Vector2(0f, -0.1f)
				};
			...
			if (item is Furniture)
			{
				...
				sprite7.initialPosition = (sprite7.position = who.Position + new Vector2(32 - sprite7.sourceRect.Width / 2 * 4, -188f));
			...
			if (!(item is Tool))
			{
				...
				if (obj == null || (bool)obj.bigCraftable)
				{
					...
					{
						TemporaryAnimatedSprite sprite4 = new TemporaryAnimatedSprite(null, default(Microsoft.Xna.Framework.Rectangle), 2500f, 1, 0, who.Position + new Vector2(-4f, -124f), false, false)
						{
							motion = new Vector2(0f, -0.1f),
							layerDepth = 1f
						};
					...
					{
						TemporaryAnimatedSprite sprite3 = new TemporaryAnimatedSprite(null, default(Microsoft.Xna.Framework.Rectangle), 2500f, 1, 0, who.Position + new Vector2(0f, -124f), false, false)
						{
							motion = new Vector2(0f, -0.1f),
							layerDepth = 1f
						};
			...
			TemporaryAnimatedSprite sprite2 = new TemporaryAnimatedSprite(null, default(Microsoft.Xna.Framework.Rectangle), 2500f, 1, 0, who.Position + new Vector2(0f, -124f), false, false)
			{
				motion = new Vector2(0f, -0.1f),
				layerDepth = 1f
			};
		...
		public void holdUpItemThenMessage(Item item, bool showMessage = true)
		{
			...
			FarmerSprite.animateOnce(new FarmerSprite.AnimationFrame[3]
			{
				...
				showMessage ? new FarmerSprite.AnimationFrame((short)FarmerSprite.CurrentFrame, 500, false, false, delegate(Farmer who)
				{
					showReceiveNewItemMessage(who, item);
				}, true) : new FarmerSprite.AnimationFrame((short)FarmerSprite.CurrentFrame, 500, false, false)
			});
		...
		public void resetFriendshipsForNewDay()
		{
			foreach (string name in friendshipData.Keys)
			{
				...
				if (i == null)
				{
					i = Game1.getCharacterFromName<Child>(name, false);
				}
				if (i != null)
				{
		...
		{
			Friendship friendship;
			if (!friendshipData.TryGetValue(name, out friendship) && Game1.NPCGiftTastes.ContainsKey(name))
			{
		...
		public void showFrame(int frame, bool flip = false)
		{
			...
			animationFrames.Add(new FarmerSprite.AnimationFrame(Convert.ToInt32(frame), 100, false, flip));
		...
		public Item addItemToInventory(Item item, List<Item> affected_items_list)
		{
			...
			GetItemReceiveBehavior(item, out needsInventorySpace, out showNotification);
			...
			foreach (Item slot in Items)
			{
				...
				if (added > 0)
				{
					...
					OnItemReceived(item, added, slot, true);
			...
			if (stackLeft > 0)
			{
				for (int i = 0; i < (int)maxItems && i < Items.Count; i++)
				{
					if (Items[i] == null)
					{
						...
						Items[i] = item;
						stackLeft = 0;
						OnItemReceived(item, item.Stack, null, true);
		...
		public void addItemsByMenuIfNecessary(List<Item> itemsToAdd, ItemGrabMenu.behaviorOnItemSelect itemSelectedCallback = null, bool forceQueue = false)
		{
			...
			if (itemsToAdd.Count > 0 && itemsToAdd[0]?.QualifiedItemId == "(O)434")
			{
				...
				{
					eatObject(itemsToAdd[0] as Object, true);
				}
				else
				{
		...
		public virtual void BeginSitting(ISittable furniture)
		{
			...
			{
				Vector2? seat_position = sittingFurniture.GetSittingPosition(this, true);
		...
		{
			Vector2 mouse_pos = Utility.PointToVector2(Game1.getMousePosition(false)) + new Vector2(Game1.viewport.X, Game1.viewport.Y);
		...
		public void OnItemReceived(Item item, int countAdded, Item mergedIntoStack, bool hideHudNotification = false)
		{
			...
			if (!hideHudNotification)
			{
				...
				GetItemReceiveBehavior(actualItem, out needsInventorySpace, out showHudNotification);
			...
			case "(O)858":
				...
				base.currentLocation.temporarySprites.Add(new TemporaryAnimatedSprite("Maps\\springobjects", Game1.getSourceRectForStandardTileSheet(Game1.objectSpriteSheet, 858, 16, 16), 100f, 1, 8, new Vector2(0f, -96f), false, false, 1f, 0f, Color.White, 4f, 0f, 0f, 0f)
				{
					motion = new Vector2(0f, -6f),
					acceleration = new Vector2(0f, 0.2f),
					stopAcceleratingWhenVelocityIsZero = true,
		...
		public void ShowItemReceivedHudMessageIfNeeded(Item item, int countAdded)
		{
			bool needsInventorySpace;
			bool showHudNotification;
			GetItemReceiveBehavior(item, out needsInventorySpace, out showHudNotification);
			if (showHudNotification)
			{
		...
		public static HairStyleMetadata GetHairStyleMetadata(int hair_index)
		{
			...
			if (hairStyleMetadata.TryGetValue(hair_index, out hair_data))
			{
				return hair_data;
			}
			try
			{
				string data;
				if (hairStyleMetadataFile.TryGetValue(hair_index, out data))
				{
		...
		public void changeFriendship(int amount, NPC n)
		{
			...
			Friendship friendship;
			if (friendshipData.TryGetValue(n.Name, out friendship))
			{
		...
		public Vector2 getUniformPositionAwayFromBox(int direction, int distance)
		{
			...
			{
			case 0:
				return new Vector2(bounds.Center.X, bounds.Y - distance);
			case 1:
				return new Vector2(bounds.Right + distance, bounds.Center.Y);
			case 2:
				return new Vector2(bounds.Center.X, bounds.Bottom + distance);
			case 3:
				return new Vector2(bounds.X - distance, bounds.Center.Y);
			default:
				return Vector2.Zero;
			}
		}
		...
		{
			Friendship friendship;
			if (friendshipData.TryGetValue(npcName, out friendship))
			{
		...
		{
			Friendship friendship;
			if (friendshipData.TryGetValue(n.Name, out friendship) && !friendship.TalkedToToday)
			{
		...
		public void moveRaft(GameLocation currentLocation, GameTime time)
		{
			...
			if (xVelocity != 0f || yVelocity != 0f)
			{
				...
				if (raftPuddleCounter <= 0)
				{
					...
					currentLocation.temporarySprites.Add(new TemporaryAnimatedSprite("TileSheets\\animations", new Microsoft.Xna.Framework.Rectangle(0, 0, 64, 64), 150f - (Math.Abs(xVelocity) + Math.Abs(yVelocity)) * 3f, 8, 0, new Vector2(collidingBox.X, collidingBox.Y - 64), false, Game1.random.NextBool(), 0.001f, 0.01f, Color.White, 1f, 0.003f, 0f, 0f));
		...
		{
			_003C_003Ec__DisplayClass666_0 _003C_003Ec__DisplayClass666_ = new _003C_003Ec__DisplayClass666_0();
			_003C_003Ec__DisplayClass666_.who = who;
			_003C_003Ec__DisplayClass666_.passOutLocation = _003C_003Ec__DisplayClass666_.who.currentLocationRef.Value;
			Vector2 bed = Utility.PointToVector2(bed_point) * 64f;
			_003C_003Ec__DisplayClass666_.bed_tile = new Vector2((int)bed.X / 64, (int)bed.Y / 64);
			_003C_003Ec__DisplayClass666_.bed_sleep_position = bed;
			if (!_003C_003Ec__DisplayClass666_.who.isInBed)
			{
				...
				locationRequest.OnWarp += _003C_003Ec__DisplayClass666_._003CperformPassoutWarp_003Eg__ContinuePassOut_007C0;
				_003C_003Ec__DisplayClass666_.who.FarmerSprite.setCurrentSingleFrame(5, 3000);
				_003C_003Ec__DisplayClass666_.who.FarmerSprite.PauseForSingleAnimation = true;
			}
			else
			{
				_003C_003Ec__DisplayClass666_._003CperformPassoutWarp_003Eg__ContinuePassOut_007C0();
			}
		}
		...
		public void GetDisplayPants(out Texture2D texture, out int spriteIndex)
		{
			string id;
			Color? color;
			if (IsOverridingPants(out id, out color))
			{
		...
		public int GetPantsIndex()
		{
			Texture2D texture;
			int index;
			GetDisplayPants(out texture, out index);
			return index;
		}
		...
		public int GetShirtIndex()
		{
			Texture2D texture;
			int index;
			GetDisplayShirt(out texture, out index);
			return index;
		}
		...
		{
			string itemId;
			if (!IsOverridingShirt(out itemId))
			{
		...
		public Color GetShirtColor()
		{
			string id;
			ShirtData shirtData;
			if (IsOverridingShirt(out id) && Game1.shirtData.TryGetValue(id, out shirtData))
			{
		...
		{
			if (!Game1.IsMultiplayer || Game1.multiplayer == null || LocalMultiplayer.IsLocalMultiplayer(true) || usernameDisplayTime <= 0f)
			{
				return;
			}
		...
		public static void drinkGlug(Farmer who)
		{
			...
			if (Game1.currentLocation == who.currentLocation)
			{
				if (who.itemToEat != null)
				{
					...
					if (o != null && o.preserve.Value == Object.PreserveType.Pickle)
					{
						cueName = "eat";
						goto IL_0275;
					}
			...
			goto IL_0285;
			IL_0275:
			Game1.playSound((string)cueName);
			goto IL_0285;
			IL_0285:
			who.currentLocation.temporarySprites.Add(new TemporaryAnimatedSprite("LooseSprites\\Cursors", new Microsoft.Xna.Framework.Rectangle(653, 858, 1, 1), 9999f, 1, 1, who.Position + new Vector2(32 + Game1.random.Next(-2, 3) * 4, -48f), false, false, (float)who.StandingPixel.Y / 10000f + 0.001f, 0.04f, c, 5f, 0f, 0f, 0f)
			{
				acceleration = new Vector2(0f, 0.5f)
			});
		}
		...
		public static void showItemIntake(Farmer who)
		{
			...
			Object toShow = ((grabbedObj != null) ? grabbedObj : ((who.ActiveObject == null) ? null : who.ActiveObject));
			if (toShow == null)
			{
				return;
			}
			...
			case 2:
				...
				{
				case 1:
					tempSprite = new TemporaryAnimatedSprite(textureName, sourceRect, 100f, 1, 0, who.Position + new Vector2(0f, -32f), false, false, (float)who.StandingPixel.Y / 10000f + 0.01f, 0f, Color.White, 4f, 0f, 0f, 0f);
					break;
				case 2:
					tempSprite = new TemporaryAnimatedSprite(textureName, sourceRect, 100f, 1, 0, who.Position + new Vector2(0f, -43f), false, false, (float)who.StandingPixel.Y / 10000f + 0.01f, 0f, Color.White, 4f, 0f, 0f, 0f);
					break;
				case 3:
					tempSprite = new TemporaryAnimatedSprite(textureName, sourceRect, 100f, 1, 0, who.Position + new Vector2(0f, -128f), false, false, (float)who.StandingPixel.Y / 10000f + 0.01f, 0f, Color.White, 4f, 0f, 0f, 0f);
					break;
				case 4:
					tempSprite = new TemporaryAnimatedSprite(textureName, sourceRect, 200f, 1, 0, who.Position + new Vector2(0f, -120f), false, false, (float)who.StandingPixel.Y / 10000f + 0.01f, 0f, Color.White, 4f, 0f, 0f, 0f);
					break;
				case 5:
					tempSprite = new TemporaryAnimatedSprite(textureName, sourceRect, 200f, 1, 0, who.Position + new Vector2(0f, -120f), false, false, (float)who.StandingPixel.Y / 10000f + 0.01f, 0.02f, Color.White, 4f, -0.02f, 0f, 0f);
					break;
				}
				break;
			case 1:
				...
				{
				case 1:
					tempSprite = new TemporaryAnimatedSprite(textureName, sourceRect, 100f, 1, 0, who.Position + new Vector2(28f, -64f), false, false, (float)who.StandingPixel.Y / 10000f + 0.01f, 0f, Color.White, 4f, 0f, 0f, 0f);
					break;
				case 2:
					tempSprite = new TemporaryAnimatedSprite(textureName, sourceRect, 100f, 1, 0, who.Position + new Vector2(24f, -72f), false, false, (float)who.StandingPixel.Y / 10000f + 0.01f, 0f, Color.White, 4f, 0f, 0f, 0f);
					break;
				case 3:
					tempSprite = new TemporaryAnimatedSprite(textureName, sourceRect, 100f, 1, 0, who.Position + new Vector2(4f, -128f), false, false, (float)who.StandingPixel.Y / 10000f + 0.01f, 0f, Color.White, 4f, 0f, 0f, 0f);
					break;
				case 4:
					tempSprite = new TemporaryAnimatedSprite(textureName, sourceRect, 200f, 1, 0, who.Position + new Vector2(0f, -124f), false, false, (float)who.StandingPixel.Y / 10000f + 0.01f, 0f, Color.White, 4f, 0f, 0f, 0f);
					break;
				case 5:
					tempSprite = new TemporaryAnimatedSprite(textureName, sourceRect, 200f, 1, 0, who.Position + new Vector2(0f, -124f), false, false, (float)who.StandingPixel.Y / 10000f + 0.01f, 0.02f, Color.White, 4f, -0.02f, 0f, 0f);
					break;
				}
				break;
			case 0:
				...
				{
				case 1:
					tempSprite = new TemporaryAnimatedSprite(textureName, sourceRect, 100f, 1, 0, who.Position + new Vector2(0f, -32f), false, false, (float)who.StandingPixel.Y / 10000f - 0.001f, 0f, Color.White, 4f, 0f, 0f, 0f);
					break;
				case 2:
					tempSprite = new TemporaryAnimatedSprite(textureName, sourceRect, 100f, 1, 0, who.Position + new Vector2(0f, -43f), false, false, (float)who.StandingPixel.Y / 10000f - 0.001f, 0f, Color.White, 4f, 0f, 0f, 0f);
					break;
				case 3:
					tempSprite = new TemporaryAnimatedSprite(textureName, sourceRect, 100f, 1, 0, who.Position + new Vector2(0f, -128f), false, false, (float)who.StandingPixel.Y / 10000f - 0.001f, 0f, Color.White, 4f, 0f, 0f, 0f);
					break;
				case 4:
					tempSprite = new TemporaryAnimatedSprite(textureName, sourceRect, 200f, 1, 0, who.Position + new Vector2(0f, -120f), false, false, (float)who.StandingPixel.Y / 10000f - 0.001f, 0f, Color.White, 4f, 0f, 0f, 0f);
					break;
				case 5:
					tempSprite = new TemporaryAnimatedSprite(textureName, sourceRect, 200f, 1, 0, who.Position + new Vector2(0f, -120f), false, false, (float)who.StandingPixel.Y / 10000f - 0.001f, 0.02f, Color.White, 4f, -0.02f, 0f, 0f);
					break;
				}
				break;
			case 3:
				...
				{
				case 1:
					tempSprite = new TemporaryAnimatedSprite(textureName, sourceRect, 100f, 1, 0, who.Position + new Vector2(-32f, -64f), false, false, (float)who.StandingPixel.Y / 10000f + 0.01f, 0f, Color.White, 4f, 0f, 0f, 0f);
					break;
				case 2:
					tempSprite = new TemporaryAnimatedSprite(textureName, sourceRect, 100f, 1, 0, who.Position + new Vector2(-28f, -76f), false, false, (float)who.StandingPixel.Y / 10000f + 0.01f, 0f, Color.White, 4f, 0f, 0f, 0f);
					break;
				case 3:
					tempSprite = new TemporaryAnimatedSprite(textureName, sourceRect, 100f, 1, 0, who.Position + new Vector2(-16f, -128f), false, false, (float)who.StandingPixel.Y / 10000f + 0.01f, 0f, Color.White, 4f, 0f, 0f, 0f);
					break;
				case 4:
					tempSprite = new TemporaryAnimatedSprite(textureName, sourceRect, 200f, 1, 0, who.Position + new Vector2(0f, -124f), false, false, (float)who.StandingPixel.Y / 10000f + 0.01f, 0f, Color.White, 4f, 0f, 0f, 0f);
					break;
			...
			if (coloredObj != null && tempSprite != null)
			{
				...
				who.currentLocation.temporarySprites.Add(new TemporaryAnimatedSprite(textureName, coloredSourceRect, tempSprite.interval, 1, 0, tempSprite.Position, false, false, tempSprite.layerDepth + 0.0001f, tempSprite.alphaFade, coloredObj.color.Value, 4f, tempSprite.scaleChange, 0f, 0f));
			}
		...
		public static void showToolSwipeEffect(Farmer who)
		{
			if (!(who.CurrentTool is WateringCan))
			{
				...
				{
				case 1:
					who.currentLocation.temporarySprites.Add(new TemporaryAnimatedSprite(15, who.Position + new Vector2(20f, -132f), Color.White, 4, false, (who.stamina <= 0f) ? 80f : 40f, 0, 128, 1f, 128)
					{
						layerDepth = (float)(who.GetBoundingBox().Bottom + 1) / 10000f
					});
					break;
				case 3:
					who.currentLocation.temporarySprites.Add(new TemporaryAnimatedSprite(15, who.Position + new Vector2(-92f, -132f), Color.White, 4, true, (who.stamina <= 0f) ? 80f : 40f, 0, 128, 1f, 128)
					{
						layerDepth = (float)(who.GetBoundingBox().Bottom + 1) / 10000f
					});
					break;
				case 2:
					who.currentLocation.temporarySprites.Add(new TemporaryAnimatedSprite(19, who.Position + new Vector2(-4f, -128f), Color.White, 4, false, (who.stamina <= 0f) ? 80f : 40f, 0, 128, 1f, 128)
					{
						layerDepth = (float)(who.GetBoundingBox().Bottom + 1) / 10000f
					});
					break;
				case 0:
					who.currentLocation.temporarySprites.Add(new TemporaryAnimatedSprite(18, who.Position + new Vector2(0f, -132f), Color.White, 4, false, (who.stamina <= 0f) ? 100f : 50f, 0, 64, 1f, 64)
					{
		...
		public void toolPowerIncrease()
		{
			...
			Game1.currentLocation.temporarySprites.Add(new TemporaryAnimatedSprite(21, base.Position - new Vector2(xAnimation, yAnimation), powerUpColor, 8, false, 70f, 0, 64, (float)standingY / 10000f + 0.005f, 128));
			Game1.currentLocation.temporarySprites.Add(new TemporaryAnimatedSprite("TileSheets\\animations", new Microsoft.Xna.Framework.Rectangle(192, 1152, 64, 64), 50f, 4, 0, base.Position - new Vector2((FacingDirection != 1) ? (-64) : 0, 128f), false, FacingDirection == 1, (float)standingY / 10000f, 0.01f, Color.White, 1f, 0f, 0f, 0f));
		...
		private bool localBusMoving()
		{
			...
			Desert desert = gameLocation as Desert;
			if (desert == null)
			{
				BusStop busStop = gameLocation as BusStop;
				if (busStop != null)
				{
		...
		public void takeDamage(int damage, bool overrideParry, Monster damager)
		{
			...
			else
			{
				...
				if (damager != null && isWearingRing("839"))
				{
					...
					damager.takeDamage(damageToMonster, (int)trajectory.X, (int)trajectory.Y, false, 1.0, this);
				...
				if (Utility.GetDayOfPassiveFestival("DesertFestival") > 0 && base.currentLocation is MineShaft && Game1.mine.getMineArea() == 121)
				{
					...
					if (team.calicoStatueEffects.TryGetValue(8, out sharpTeethAmount))
					{
						adjustment += (float)sharpTeethAmount * 0.25f;
					}
					int toothFileAmount;
					if (team.calicoStatueEffects.TryGetValue(14, out toothFileAmount))
					{
				...
				{
					startGlowing(new Color(255, 255, 0), false, 0.25f);
					DelayedAction.functionAfterDelay(base.stopGlowing, 500);
					Game1.playSound("yoba");
					for (int i = 0; i < 13; i++)
					{
						float xPos = Game1.random.Next(-32, 33);
						base.currentLocation.temporarySprites.Add(new TemporaryAnimatedSprite("LooseSprites\\Cursors2", new Microsoft.Xna.Framework.Rectangle(114, 46, 2, 2), 200f, 5, 1, new Vector2(xPos + 32f, -96f), false, false, 1f, 0f, Color.White, 4f, 0f, 0f, 0f)
						{
							attachedCharacter = this,
							positionFollowsAttachedCharacter = true,
					...
					base.currentLocation.temporarySprites.Add(new TemporaryAnimatedSprite("LooseSprites\\Cursors2", new Microsoft.Xna.Framework.Rectangle(157, 280, 28, 19), 2000f, 1, 1, new Vector2(-20f, -16f), false, false, 1E-06f, 0f, Color.White, 4f, 0f, 0f, 0f)
					{
						attachedCharacter = this,
						positionFollowsAttachedCharacter = true,
						alpha = 0.1f,
				...
				if (i < location.characters.Count)
				{
					...
					if (monster != null && monster.OverlapsFarmerForDamage(this))
					{
						...
						if (monster.DamageToFarmer > 0)
						{
							...
							{
								takeDamage(monster.DamageToFarmer, false, monster);
							}
							else
							{
								takeDamage(Math.Max(1, monster.DamageToFarmer + Game1.random.Next(-monster.DamageToFarmer / 4, monster.DamageToFarmer / 4)), false, monster);
							}
						}
					}
				}
		...
		public bool checkAction(Farmer who, GameLocation location)
		{
			...
			if (Game1.CurrentEvent != null)
			{
				...
				{
					who.Halt();
					who.faceGeneralDirection(getStandingPosition(), 0, false, false);
			...
			if ((playerSpouseID.HasValue & (who.UniqueMultiplayerID == playerSpouseID)) && who.CanMove && !who.isMoving() && !isMoving() && Utility.IsHorizontalDirection(getGeneralDirectionTowards(who.getStandingPosition(), -10, false, false)))
			{
				who.Halt();
				who.faceGeneralDirection(getStandingPosition(), 0, false, false);
				who.kissFarmerEvent.Fire(UniqueMultiplayerID);
				Game1.multiplayer.broadcastSprites(base.currentLocation, new TemporaryAnimatedSprite("LooseSprites\\Cursors", new Microsoft.Xna.Framework.Rectangle(211, 428, 7, 6), 2000f, 1, 0, base.Tile * 64f + new Vector2(16f, -64f), false, false, 1f, 0f, Color.White, 4f, 0f, 0f, 0f)
				{
		...
		public void Update(GameTime time, GameLocation location)
		{
			...
			if (IsSitting())
			{
				...
				if (IsSitting() && !isStopSitting)
				{
					...
					{
						MapSeat mapSeat = sittingFurniture as MapSeat;
						if (mapSeat != null)
						{
			...
			{
				sparklingText = new SparklingText(Game1.dialogueFont, Game1.content.LoadString("Strings\\StringsFromCSFiles:Farmer.cs.2003", getSkillDisplayNameFromIndex(newLevelSparklingTexts.Peek())), Color.White, Color.White, true);
		...
		private void updateCommon(GameTime time, GameLocation location)
		{
			...
			if ((bool)swimming)
			{
				...
				if (timerSinceLastMovement == 0)
				{
					...
					{
						Game1.multiplayer.broadcastSprites(base.currentLocation, new TemporaryAnimatedSprite("TileSheets\\animations", new Microsoft.Xna.Framework.Rectangle(0, 0, 64, 64), 150f - (Math.Abs(xVelocity) + Math.Abs(yVelocity)) * 3f, 8, 0, new Vector2(base.Position.X, base.StandingPixel.Y - 32), false, Game1.random.NextBool(), 0.01f, 0.01f, Color.White, 1f, 0.003f, 0f, 0f));
					}
					if (swimTimer < 0)
					{
						swimTimer = 800;
			...
			for (int i = Items.Count - 1; i >= 0; i--)
			{
				(Items[i] as Tool)?.tickUpdate(time, this);
			}
		...
		public float getMovementSpeed()
		{
			...
			float movementSpeed = Math.Max(1f, (float)base.speed + (Game1.eventUp ? ((float)Math.Max(0, Game1.CurrentEvent.farmerAddedSpeed - 2)) : (addedSpeed + (isRidingHorse() ? 5f : temporarySpeedBuff))));
			if (movementDirections.Count > 1)
			{
		...
		public void PerformKiss(int facingDirection)
		{
			if (!Game1.eventUp && !UsingTool && (!IsLocalPlayer || Game1.activeClickableMenu == null) && !isRidingHorse() && !IsSitting() && !base.IsEmoting && CanMove)
			{
				...
				{
					new FarmerSprite.AnimationFrame(101, 1000, 0, false, FacingDirection == 3),
					new FarmerSprite.AnimationFrame(6, 1, false, FacingDirection == 3, broadcastHaltAnimation)
				});
			}
		}
		...
		protected virtual bool MovePositionImpl(int direction, float movementSpeedX, float movementSpeedY, GameTime time, xTile.Dimensions.Rectangle viewport)
		{
			...
			if (!base.currentLocation.isCollidingPosition(targetPos, viewport, true, 0, false, this) || ignoreCollisions || isCutscene)
			{
			...
			if (!base.currentLocation.isCollidingPosition(nextPositionHalf(direction), viewport, true, 0, false, this))
			{
			...
			if (movementDirections.Count == 1)
			{
				...
				if (direction == 0 || direction == 2)
				{
					...
					if (leftCorner && !rightCorner && !base.currentLocation.isCollidingPosition(nextPosition(1), viewport, true, 0, false, this))
					{
						position.X += (float)base.speed * ((float)time.ElapsedGameTime.Milliseconds / 64f);
					}
					else if (rightCorner && !leftCorner && !base.currentLocation.isCollidingPosition(nextPosition(3), viewport, true, 0, false, this))
					{
						position.X -= (float)base.speed * ((float)time.ElapsedGameTime.Milliseconds / 64f);
					}
				}
				else
				{
					...
					if (topCorner && !bottomCorner && !base.currentLocation.isCollidingPosition(nextPosition(2), viewport, true, 0, false, this))
					{
						position.Y += (float)base.speed * ((float)time.ElapsedGameTime.Milliseconds / 64f);
					}
					else if (bottomCorner && !topCorner && !base.currentLocation.isCollidingPosition(nextPosition(0), viewport, true, 0, false, this))
					{
						position.Y -= (float)base.speed * ((float)time.ElapsedGameTime.Milliseconds / 64f);
					}
				}
		...
		public void doneEating()
		{
			...
			if (consumed.QualifiedItemId == "(O)434")
			{
				...
				startGlowing(new Color(200, 0, 255), false, 0.1f);
				jitterStrength = 1f;
				Game1.staminaShakeTimer = 12000;
				Game1.screenGlowOnce(new Color(200, 0, 255), true);
				CanMove = false;
				freezePause = 8000;
				base.currentLocation.temporarySprites.Add(new TemporaryAnimatedSprite("LooseSprites\\Cursors", new Microsoft.Xna.Framework.Rectangle(368, 16, 16, 16), 60f, 8, 40, base.Position + new Vector2(-8f, -128f), false, false, 1f, 0f, Color.White, 4f, 0.0075f, 0f, 0f)
				{
					alpha = 0.75f,
					alphaFade = 0.0025f,
				...
				if (Game1.displayHUD && !Game1.eventUp)
				{
					for (int i = 0; i < 40; i++)
					{
						Game1.uiOverlayTempSprites.Add(new TemporaryAnimatedSprite(Game1.random.Next(10, 12), new Vector2((float)Game1.graphics.GraphicsDevice.Viewport.TitleSafeArea.Right / Game1.options.uiScale - 48f - 8f - (float)Game1.random.Next(64), (float)Game1.random.Next(-64, 64) + (float)Game1.graphics.GraphicsDevice.Viewport.TitleSafeArea.Bottom / Game1.options.uiScale - 224f - 16f - (float)(int)((double)(MaxStamina - 270) * 0.715)), Game1.random.Choose(Color.White, Color.Lime), 8, false, 50f)
						{
							layerDepth = 1f,
		...
		public void completelyStopAnimatingOrDoingAction()
		{
			...
			MeleeWeapon weapon = CurrentTool as MeleeWeapon;
			if (weapon != null)
			{
