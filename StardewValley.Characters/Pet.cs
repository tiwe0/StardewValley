...
namespace StardewValley.Characters
{
	public class Pet : NPC
	{
		...
		{
			PetData petData;
			if (!TryGetData(petType.Value, out petData))
			{
				return null;
			}
			return petData;
		...
		public void warpToFarmHouse(Farmer who)
		{
			...
			for (; tries < 50; tries++)
			{
				if (farmHouse.canPetWarpHere(sleepTile) && farmHouse.CanItemBePlacedHere(sleepTile, false, ~CollisionMask.Farmers) && farmHouse.CanItemBePlacedHere(sleepTile + new Vector2(1f, 0f), false, ~CollisionMask.Farmers) && !farmHouse.isTileOnWall((int)sleepTile.X, (int)sleepTile.Y))
				{
					break;
				}
		...
		public override void dayUpdate(int dayOfMonth)
		{
			...
			Halt();
			CurrentBehavior = "Sleep";
			grantedFriendshipForPet.Set(false);
			_OnNewBehavior();
			Sprite.UpdateSourceRect();
		}
		...
		public PetBowl GetPetBowl()
		{
			...
			{
				PetBowl bowl = building as PetBowl;
				if (bowl != null && bowl.petId.Value == petId.Value)
				{
					return bowl;
				}
			}
		...
		public void applyButterflyPowder(Farmer who, string responseKey)
		{
			if (responseKey.Contains("Yes"))
			{
				...
				j.temporarySprites.Add(new TemporaryAnimatedSprite(5, Utility.PointToVector2(GetBoundingBox().Center) - new Vector2(32f), Color.White, 8, false, 50f));
				for (int i = 0; i < 8; i++)
				{
					j.temporarySprites.Add(new TemporaryAnimatedSprite("LooseSprites\\Cursors", new Rectangle(372, 1956, 10, 10), base.Position + new Vector2(32f) + new Vector2(Game1.random.Next(-16, 16), Game1.random.Next(-32, 16)), false, 0.002f, Color.White)
					{
				...
				if (hat.Value != null)
				{
					Game1.createItemDebris(hat.Value, base.Position, -1, j);
				}
		...
		public override bool checkAction(Farmer who, GameLocation l)
		{
			...
			int curLastPetDay;
			if (!lastPetDay.TryGetValue(who.UniqueMultiplayerID, out curLastPetDay) || curLastPetDay != Game1.Date.TotalDays)
			{
				...
				mutex.RequestLock(delegate
				{
					...
					{
						grantedFriendshipForPet.Set(true);
						...
						if (Utility.CreateDaySaveRandom(timesPet.Value, 71928.0, petId.Value.GetHashCode()).NextDouble() < (double)GetPetData().GiftChance)
						{
							...
							if (item != null)
							{
								Game1.createMultipleItemDebris(item, base.Position, -1, l, -1, true);
							}
						}
						timesPet.Value++;
					}
		...
		public virtual void playContentSound()
		{
			...
			PlaySound(contentSound, true, -1, -1);
		...
		public override void update(GameTime time, GameLocation location)
		{
			...
			if (Game1.IsMasterGame)
			{
				...
				if (currentBehavior != null && currentBehavior.WalkInDirection)
				{
					...
					{
						tryToMoveInDirection(FacingDirection, false, -1, false);
					}
				}
			}
			flip = false;
		...
		public Item TryGetGiftItem(List<PetGift> gifts)
		{
			...
			{
				if (gift2.MinimumFriendshipThreshold <= friendshipTowardFarmer.Value)
				{
		...
		public virtual void OnNewBehavior()
		{
			...
			if (CurrentBehavior == "Sleep")
			{
				...
				{
					new FarmerSprite.AnimationFrame(28, 1000, false, local_sleep_flip),
					new FarmerSprite.AnimationFrame(29, 1000, false, local_sleep_flip)
				});
			}
			PetBehavior behavior = GetCurrentPetBehavior();
			if (behavior == null)
			...
			{
				FarmerSprite.AnimationFrame frame = new FarmerSprite.AnimationFrame(behavior.Animation[i].Frame, behavior.Animation[i].Duration, false, false);
		...
		public virtual void drawHat(SpriteBatch b, Vector2 shake)
		{
			...
			hat.Value.draw(b, getLocalPosition(Game1.viewport) + hatOffset + new Vector2(30f, -42f), scale, 1f, horse_draw_layer, direction, true);
		}
