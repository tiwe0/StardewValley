...
namespace StardewValley.Menus
{
	public class CharacterCustomization : IClickableMenu
	{
		...
		private void ResetComponents()
		{
			...
			if (source == Source.DyePots)
			{
				...
				if (Game1.player.CanDyePants())
				{
					...
					labels.Add(new ClickableComponent(new Rectangle(label_position, yPositionOnScreen + IClickableMenu.borderWidth + IClickableMenu.spaceToClearTopBorder + yOffset + 16 + pantsColorLabelYOffset2, 1, 1), Game1.content.LoadString("Strings\\UI:Character_PantsColor")));
			...
			if (Game1.gameMode == 3)
			{
				IList<GameLocation> location = Game1.locations;
			}
		...
		private void optionButtonClick(string name)
		{
			...
			else
			{
				...
				case "OK":
				{
					...
					{
						Pet pet = Game1.getCharacterFromName<Pet>(Game1.player.getPetName(), false);
		...
		public override void receiveLeftClick(int x, int y, bool playSound = true)
		{
			...
			{
				foreach (ClickableComponent c3 in leftSelectionButtons)
				{
					if (c3.containsPoint(x, y))
					{
						selectionClick(c3.name, -1);
						if (c3.scale != 0f)
						{
							c3.scale -= 0.25f;
							c3.scale = Math.Max(0.75f, c3.scale);
						}
					}
				}
			}
			...
			{
				foreach (ClickableComponent c2 in rightSelectionButtons)
				{
			...
			if (coopHelpButton != null && coopHelpButton.containsPoint(x, y))
			{
				...
				coopHelpString = Game1.parseText(rawText2, Game1.dialogueFont, width + 384 - IClickableMenu.borderWidth * 2);
		...
		public override void performHoverAction(int x, int y)
		{
			hoverText = "";
			hoverTitle = "";
			foreach (ClickableTextureComponent c5 in leftSelectionButtons)
			{
			...
			foreach (ClickableTextureComponent c4 in rightSelectionButtons)
			{
			...
			if (source == Source.NewGame || source == Source.HostNewFarm)
			{
				foreach (ClickableTextureComponent c3 in farmTypeButtons)
				{
					if (c3.containsPoint(x, y) && !c3.name.Contains("Gray"))
					{
						c3.scale = Math.Min(c3.scale + 0.02f, c3.baseScale + 0.1f);
						hoverTitle = c3.hoverText.Split('_')[0];
						hoverText = c3.hoverText.Split('_')[1];
						continue;
					}
					c3.scale = Math.Max(c3.scale - 0.02f, c3.baseScale);
					if (c3.name.Contains("Gray") && c3.containsPoint(x, y))
					{
						hoverText = "Reach level 10 " + Game1.content.LoadString("Strings\\UI:Character_" + c3.name.Split('_')[1]) + " to unlock.";
					}
				}
			}
			foreach (ClickableTextureComponent c2 in genderButtons)
			{
				if (c2.containsPoint(x, y))
				{
		...
		public override void draw(SpriteBatch b)
		{
			...
			Game1.drawDialogueBox(xPositionOnScreen, yPositionOnScreen, base.width, height, false, true);
			if (source == Source.HostNewFarm)
			{
			...
			foreach (ClickableComponent c3 in labels)
			{
				if (!c3.visible)
				{
					continue;
				...
				else if (c3 == skinLabel)
				{
					offset = 21f - Game1.smallFont.MeasureString(c3.name).X / 2f;
					sub = ((int)Game1.player.skin + 1).ToString() ?? "";
				}
				else if (c3 == hairLabel)
				{
					offset = 21f - Game1.smallFont.MeasureString(c3.name).X / 2f;
			...
			if (farmTypeButtons.Count > 0)
			{
				...
				for (int i = 0; i < farmTypeButtons.Count; i++)
				{
					...
					if (farm_is_selected)
					{
						IClickableMenu.drawTextureBox(b, Game1.mouseCursors, new Rectangle(375, 357, 3, 3), farmTypeButtons[i].bounds.X, farmTypeButtons[i].bounds.Y - 4, farmTypeButtons[i].bounds.Width, farmTypeButtons[i].bounds.Height + 8, Color.White, 4f, false);
					}
				}
			...
			PetData petData;
			if (petPortraitBox.HasValue && Pet.TryGetData(Game1.MasterPlayer.whichPetType, out petData))
			{
