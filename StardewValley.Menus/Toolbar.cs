...
namespace StardewValley.Menus
{
	public class Toolbar : IClickableMenu
	{
		...
		public override void draw(SpriteBatch b)
		{
			...
			IClickableMenu.drawTextureBox(b, Game1.menuTexture, toolbarTextSource, Game1.uiViewport.Width / 2 - 384 - 16, yPositionOnScreen - 96 - 8, 800, 96, Color.White * transparency, 1f, false);
			for (int j = 0; j < 12; j++)
			{
			...
			for (int i = 0; i < 12; i++)
			{
				...
				{
					Game1.player.Items[i].drawInMenu(b, toDraw2, (Game1.player.CurrentToolIndex == i) ? 0.9f : (buttons[i].scale * 0.8f), transparency, 0.88f);
				}
			}
			if (hoverItem != null)
			{
