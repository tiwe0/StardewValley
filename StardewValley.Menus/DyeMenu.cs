...
namespace StardewValley.Menus
{
	public class DyeMenu : MenuWithInventory
	{
		...
		public int GetPotIndex(Item item)
		{
			for (int i = 0; i < validPotColors.Length; i++)
			{
				for (int j = 0; j < validPotColors[i].Length; j++)
				{
					...
					if (colorObject != null && colorObject.preservedParentSheetIndex.Value != null && ItemContextTagManager.DoAnyTagsMatch(new List<string> { validPotColors[i][j] }, ItemContextTagManager.GetBaseContextTags(colorObject.preservedParentSheetIndex.Value)))
					{
						return i;
					}
		...
		public override void draw(SpriteBatch b)
		{
			...
			{
				int index;
				if (i < inventory.actualInventory.Count && inventory.actualInventory[i] != null && _highlightDictionary.TryGetValue(inventory.actualInventory[i], out index) && index >= 0)
				{
