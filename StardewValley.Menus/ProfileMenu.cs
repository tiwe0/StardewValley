...
namespace StardewValley.Menus
{
	public class ProfileMenu : IClickableMenu
	{
			...
			if (npc != null)
			{
				...
				foreach (ParsedItemData data in ItemRegistry.GetObjectTypeDefinition().GetAllData())
				{
					...
					for (int i = 0; i < itemCategories.Length; i++)
					{
						...
						if (!(categoryName == "Profile_Gift_Category_LikedGifts"))
						{
							if (categoryName == "Profile_Gift_Category_Misc")
							{
								...
								if (!is_accounted_for)
								{
									...
									if (!_sortedItems.TryGetValue(i, out categoryItems2))
									{
										categoryItems2 = (_sortedItems[i] = new List<Item>());
									}
							...
							else if (itemCategories[i].validCategories.Contains(item.Category))
							{
								...
								if (!_sortedItems.TryGetValue(i, out categoryItems3))
								{
									categoryItems3 = (_sortedItems[i] = new List<Item>());
								}
						...
						if (gift_taste == 2 || gift_taste == 0)
						{
							...
							if (!_sortedItems.TryGetValue(i, out categoryItems))
							{
				...
				if (isDatable || housemate)
				{
					...
					string status = text.Replace("(", "").Replace(")", "").Replace("（", "")
						.Replace("）", "");
					status = (_status = Utility.capitalizeFirstLetter(status));
				}
				_UpdateList();
			}
		...
		protected void _UpdateList()
		{
			...
			List<Item> categoryItems;
			if (_sortedItems.TryGetValue(_currentCategory, out categoryItems))
			{
