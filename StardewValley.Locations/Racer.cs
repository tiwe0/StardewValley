...
namespace StardewValley.Locations
{
	public class Racer : INetObject<NetFields>
	{
		...
		private void InitNetFields()
		{
			...
			jumpSegmentStart.Interpolated(false, false);
			jumpSegmentEnd.Interpolated(false, false);
		}
		...
		public virtual void Update(DesertFestival location)
		{
			if (Game1.IsMasterGame)
			{
				...
				if (location.currentRaceState.Value == DesertFestival.RaceState.StartingLine && currentTrackIndex < 0)
				{
					...
					currentTrackIndex = 0;
					Vector3 track_position2 = location.GetTrackPosition(currentTrackIndex, horizontalPosition);
					segmentStart = position.Value;
					segmentEnd = new Vector2(track_position2.X, track_position2.Y);
				}
