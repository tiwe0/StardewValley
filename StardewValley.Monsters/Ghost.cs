...
namespace StardewValley.Monsters
{
	public class Ghost : Monster
	{
		...
		protected override void localDeathAnimation()
		{
			...
			base.currentLocation.temporarySprites.Add(new TemporaryAnimatedSprite(Sprite.textureName, new Microsoft.Xna.Framework.Rectangle(0, 96, 16, 24), 100f, 4, 0, base.Position, false, false, 0.9f, 0.001f, Color.White, 4f, 0.01f, 0f, (float)Math.PI / 64f));
		}
		...
		protected override void updateAnimation(GameTime time)
		{
			...
			if (nextParticle <= 0f)
			{
				...
				if (variant.Value == GhostVariant.Putrid)
				{
					...
					{
						int standingY = base.StandingPixel.Y;
						TemporaryAnimatedSprite drip = new TemporaryAnimatedSprite(Sprite.textureName, new Microsoft.Xna.Framework.Rectangle(Game1.random.Next(4) * 16, 168, 16, 24), 100f, 1, 10, base.Position + new Vector2(Utility.RandomFloat(-16f, 16f), Utility.RandomFloat(-16f, 0f) - (float)yOffset), false, false, (float)standingY / 10000f, 0.01f, Color.White, 4f, -0.01f, 0f, 0f);
			...
			faceGeneralDirection(base.Player.getStandingPosition(), 0, false, false);
			resetAnimationSpeed();
		}
			...
			if (variant.Value == GhostVariant.Putrid)
			{
				...
				case 3:
					...
					if (stateTimer <= 0f)
					{
						...
						BasicProjectile projectile = new BasicProjectile(base.DamageToFarmer, 7, 0, 1, (float)Math.PI / 32f, shot_velocity.X, shot_velocity.Y, base.Position, null, null, null, false, false, base.currentLocation, this);
