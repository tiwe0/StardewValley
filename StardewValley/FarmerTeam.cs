...
namespace StardewValley
{
	public class FarmerTeam : INetObject<NetFields>
	{
		...
		public FarmerTeam()
		{
			...
			newLostAndFoundItems.Interpolated(false, false);
		...
		private void OnCalicoStatueEffectAdded(int key)
		{
			...
			{
				Game1.currentLocation.temporarySprites.Add(new TemporaryAnimatedSprite(null, Rectangle.Empty, new Vector2((float)(newVector.X * 64 + 32) - (float)SpriteText.getWidthOfString(description) / 2f, (newVector.Y - 3) * 64) + v * 4f, false, 0f, Color.Black)
				{
					text = description,
			...
			Game1.currentLocation.temporarySprites.Add(new TemporaryAnimatedSprite(null, Rectangle.Empty, new Vector2((float)(newVector.X * 64 + 32) - (float)SpriteText.getWidthOfString(description) / 2f, (newVector.Y - 3) * 64), false, 0f, Color.White)
			{
				text = description,
				extraInfoForEndBehavior = -777,
				layerDepth = 1f,
		...
		public virtual void OnKickOutOfMinesEvent(int mineshaftType)
		{
			...
			if (mineshaft != null && ((mineshaftType == 120) ? (mineshaft.mineLevel <= mineshaftType) : (mineshaft.getMineArea() == mineshaftType)))
			{
		...
		public virtual void OnRequestHorseWarp(long uid)
		{
			...
			horse.mutex.RequestLock(delegate
			{
				...
				for (int i = 0; i < 8; i++)
				{
					Game1.multiplayer.broadcastSprites(currentLocation, new TemporaryAnimatedSprite(10, new Vector2(tile.X + Utility.RandomFloat(-1f, 1f), tile.Y + Utility.RandomFloat(-1f, 0f)) * 64f, Color.White, 8, false, 50f)
					{
						layerDepth = 1f,
				...
				{
					Game1.multiplayer.broadcastSprites(currentLocation, new TemporaryAnimatedSprite(6, new Vector2(num2, tile.Y) * 64f, Color.White, 8, false, 50f)
					{
						layerDepth = 1f,
		...
		public NetIntDelta GetMoney(Farmer who)
		{
			...
			{
				NetIntDelta value;
				if (!individualMoney.TryGetValue(who.UniqueMultiplayerID, out value))
				{
		...
		public bool OnDonatedItemWithdrawn(ISalable salable, Farmer who, int amount)
		{
			...
			if (item != null && (salable.Stack <= 0 || salable.maximumStackSize() <= 1))
			{
		...
		public void OnRequestPetWarpHomeEvent(long uid)
		{
			if (Game1.IsMasterGame)
			{
				...
				Pet pet = Game1.getCharacterFromName<Pet>(farmer.getPetName(), false);
		...
		{
			int count;
			if (!limitedNutDrops.TryGetValue(key, out count) || count < limit)
			{
		...
		{
			int count;
			if (!limitedNutDrops.TryGetValue(key, out count))
			{
				return 0;
			}
			return count;
		...
		{
			Proposal proposal;
			if (proposals.TryGetValue(Game1.player.UniqueMultiplayerID, out proposal))
			{
				return proposal;
			}
			return null;
		...
		private bool locationsMatch(GameLocation location1, GameLocation location2)
		{
			...
			int mineLevel;
			if ((location1 is Mine || (MineShaft.IsGeneratedLevel(location1, out mineLevel) && mineLevel < 121)) && (location2 is Mine || (MineShaft.IsGeneratedLevel(location2, out mineLevel) && mineLevel < 121)))
			{
				return true;
			}
		...
		{
			Inventory inventory;
			if (!globalInventories.TryGetValue(id, out inventory))
			{
		...
		{
			NetMutex mutex;
			if (!globalInventoryMutexes.TryGetValue(id, out mutex))
			{
