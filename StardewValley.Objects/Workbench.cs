			...
			if (!mutex.IsLocked())
			{
				new MultipleMutexRequest(muticies, delegate(MultipleMutexRequest request)
				{
					mutex.RequestLock(delegate
					{
						...
						Game1.activeClickableMenu = new CraftingPage((int)topLeftPositionForCenteringOnScreen.X, (int)topLeftPositionForCenteringOnScreen.Y, 800 + IClickableMenu.borderWidth * 2, 600 + IClickableMenu.borderWidth * 2, false, true, inventories);
