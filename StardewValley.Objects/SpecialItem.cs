...
namespace StardewValley.Objects
{
	public class SpecialItem : Item
	{
		...
		public TemporaryAnimatedSprite getTemporarySpriteForHoldingUp(Vector2 position)
		{
			if ((int)which == 99)
			{
				return new TemporaryAnimatedSprite("LooseSprites\\Cursors", new Rectangle(((int)Game1.player.maxItems == 36) ? 268 : 257, 1436, ((int)Game1.player.maxItems == 36) ? 11 : 9, 13), position + new Vector2(16f, 0f), false, 0f, Color.White)
				{
					scale = 4f,
					layerDepth = 1f
				};
			}
			return new TemporaryAnimatedSprite("LooseSprites\\Cursors", new Rectangle(129 + 16 * (int)which, 320, 16, 16), position, false, 0f, Color.White)
			{
				layerDepth = 1f
			};
		}
