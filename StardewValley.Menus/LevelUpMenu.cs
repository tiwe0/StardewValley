	...
	public class LevelUpMenu : IClickableMenu
	{
			...
			: base(Game1.uiViewport.Width / 2 - 384, Game1.uiViewport.Height / 2 - 256, 768, 512)
		{
			...
			foreach (KeyValuePair<string, string> v2 in CraftingRecipe.craftingRecipes)
			{
				...
				if (conditions2.Contains(Farmer.getSkillNameFromIndex(currentSkill)) && conditions2.Contains(currentLevel.ToString() ?? ""))
				{
					CraftingRecipe recipe2 = new CraftingRecipe(v2.Key, false);
					newCraftingRecipes.Add(recipe2);
			...
			foreach (KeyValuePair<string, string> v in CraftingRecipe.cookingRecipes)
			{
				...
				if (conditions.Contains(Farmer.getSkillNameFromIndex(currentSkill)) && conditions.Contains(currentLevel.ToString() ?? ""))
				{
					CraftingRecipe recipe = new CraftingRecipe(v.Key, true);
		...
		public override void update(GameTime time)
		{
			...
			if (Game1.random.NextDouble() < 0.03)
			{
				...
				littleStars.Add(new TemporaryAnimatedSprite("LooseSprites\\Cursors", new Rectangle(364, 79, 5, 5), 80f, 7, 1, position, false, false, 1f, 0f, Color.White, 4f, 0f, 0f, 0f)
				{
					local = true
				});
			}
			...
			if (isActive && starIcon != null && !informationUp && (oldMouseState.LeftButton == ButtonState.Pressed || (Game1.options.gamepadControls && Game1.oldPadState.IsButtonDown(Buttons.A))) && starIcon.containsPoint(oldMouseState.X, oldMouseState.Y))
			{
				...
				currentLevel = newLevel.Y;
				currentSkill = newLevel.X;
				title = Game1.content.LoadString("Strings\\UI:LevelUp_Title", currentLevel, Farmer.getSkillDisplayNameFromIndex(currentSkill));
				extraInfoForLevel = getExtraInfoForLevel(currentSkill, currentLevel);
				switch (currentSkill)
				{
		...
		public override void draw(SpriteBatch b)
		{
			...
			else
			{
				...
				if (isProfessionChooser)
				{
					...
					Utility.drawWithShadow(b, Game1.buffsIcons, new Vector2(xPositionOnScreen + IClickableMenu.spaceToClearSideBorder + IClickableMenu.borderWidth, yPositionOnScreen + IClickableMenu.spaceToClearTopBorder + 16), sourceRectForLevelIcon, Color.White, 0f, Vector2.Zero, 4f, false, 0.88f);
					...
					Utility.drawWithShadow(b, Game1.buffsIcons, new Vector2(xPositionOnScreen + width - IClickableMenu.spaceToClearSideBorder - IClickableMenu.borderWidth - 64, yPositionOnScreen + IClickableMenu.spaceToClearTopBorder + 16), sourceRectForLevelIcon, Color.White, 0f, Vector2.Zero, 4f, false, 0.88f);
