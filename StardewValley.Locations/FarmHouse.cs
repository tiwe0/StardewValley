	...
	public class FarmHouse : DecoratableLocation
	{
		...
		private void AddStarterGiftBox(Farm farm)
		{
			...
			for (int i = 0; i < fields.Length; i += 2)
			{
				...
				string error;
				int count;
				if (!ArgUtility.TryGet(fields, i, out giftId, out error, false) || !ArgUtility.TryGetOptionalInt(fields, i + 1, out count, out error))
				{
		...
		private void AddStarterFurniture(Farm farm)
		{
			...
			if (fields.Any())
			{
				for (int i = 0; i < fields.Length; i += 4)
				{
					...
					Vector2 tile;
					int rotations;
					if (!ArgUtility.TryGetInt(fields, i, out index, out error) || !ArgUtility.TryGetVector2(fields, i + 1, out tile, out error) || !ArgUtility.TryGetInt(fields, i + 3, out rotations, out error))
					{
		...
		{
			Point position;
			if (TryGetMapPropertyAs("KitchenStandingLocation", out position, false))
			{
				return position;
			}
			switch (upgradeLevel)
		...
		{
			Point position;
			if (TryGetMapPropertyAs("EntryLocation", out position, false))
			{
				return position;
			}
			switch (upgradeLevel)
		...
		public override void UpdateWhenCurrentLocation(GameTime time)
		{
			...
			foreach (Vector2 offset in adjacentTilesOffsets)
			{
				...
				Monster monster = isCharacterAtTile(v) as Monster;
				if (monster != null)
				{
		...
		public virtual void showSpouseRoom()
		{
			...
			if (num && !displayingSpouseRoom)
			{
				...
				if (NPC.TryGetData(owner.spouse, out spouseData))
				{
		...
		protected virtual void _ApplyRenovations()
		{
			...
			if (!TryGetMapProperty("AdditionalRenovations", out propertyValue))
			{
				return;
			}
		...
		protected override void resetLocalState()
		{
			...
			if (IsOwnedByCurrentPlayer && Game1.player.team.GetSpouse(Game1.player.UniqueMultiplayerID).HasValue && Game1.player.team.IsMarried(Game1.player.UniqueMultiplayerID) && !Game1.player.mailReceived.Contains("CF_Spouse"))
			{
				...
				Chest chest = new Chest(new List<Item> { ItemRegistry.Create("(O)434") }, chestPosition, true, 1);
			...
			if (IsOwnedByCurrentPlayer && !Game1.player.activeDialogueEvents.ContainsKey("pennyRedecorating"))
			{
				...
				if (whichQuilt != -1 && !Game1.player.mailReceived.Contains("pennyRefurbished"))
				{
					...
					foreach (Furniture item in furniture)
					{
						BedFurniture bed_furniture = item as BedFurniture;
		...
		public virtual void RefreshFloorObjectNeighbors()
		{
			...
			{
				(terrainFeatures[key] as Flooring)?.OnAdded(this, key);
			}
		}
		...
		public virtual void setMapForUpgradeLevel(int level)
		{
			...
			if (previousUpgradeLevel >= 0)
			{
					...
					for (int x3 = 0; x3 < map.Layers[0].TileWidth; x3++)
					{
						for (int y3 = 0; y3 < map.Layers[0].TileHeight; y3++)
						{
							if (doesTileHaveProperty(x3, y3, "DefaultChildBedPosition", "Back") != null)
							{
								...
								furniture.Add(new BedFurniture(bedId2, new Vector2(x3, y3)));
								break;
							}
						}
					}
				...
				if (previousUpgradeLevel == 0)
				{
					foreach (Furniture item in furniture)
					{
						BedFurniture bed2 = item as BedFurniture;
						if (bed2 != null && bed2.bedType == BedFurniture.BedType.Single)
						{
							bed_furniture = bed2;
							break;
						}
				...
				else
				{
					...
					{
						BedFurniture bed = item2 as BedFurniture;
						if (bed != null && bed.bedType == BedFurniture.BedType.Double)
						{
							bed_furniture = bed;
							break;
						}
						...
						for (int y2 = 0; y2 < map.Layers[0].TileHeight; y2++)
						{
							...
							if (previousUpgradeLevel != 1 || bed_furniture == null || (bed_furniture.tileLocation.X == 39f && bed_furniture.tileLocation.Y == 22f))
							{
								...
								if (previousUpgradeLevel == 0 && bed_furniture != null)
								{
									...
									furniture.Add(new BedFurniture(bedId, new Vector2(x2, y2)));
								}
								else if (bed_furniture != null)
								{
									...
									Guid guid = furniture.GuidOf(bed_furniture);
									furniture.Remove(guid);
									furniture.Add(new BedFurniture(bed_furniture.ItemId, new Vector2(x2, y2)));
								}
							}
							break;
						}
		...
		{
			Point position;
			if (TryGetMapPropertyAs("SpouseRoomPosition", out position, false))
			{
				return position;
			}
			if (upgradeLevel != 1)
		...
		public virtual void loadSpouseRoom()
		{
			...
			{
				if (getTileIndexAt(tile2, "Paths") == 7)
				{
					spouseRoomSpot = tile2;
					break;
				}
			}
