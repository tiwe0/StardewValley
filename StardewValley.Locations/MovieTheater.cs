...
namespace StardewValley.Locations
{
	...
	public class MovieTheater : GameLocation
	{
			...
			if (paths_layer != null)
			{
				for (int x = 0; x < paths_layer.LayerWidth; x++)
				{
					for (int y = 0; y < paths_layer.LayerHeight; y++)
					{
						string property;
						int hangout_group;
						if (paths_layer.Tiles[x, y] != null && paths_layer.GetTileIndexAt(x, y) == 7 && paths_layer.Tiles[x, y].Properties.TryGetValue("group", out property) && int.TryParse(property, out hangout_group))
						{
							List<Point> points;
							if (!_hangoutPoints.TryGetValue(hangout_group, out points))
							{
		...
		public void _ShowMovieStartReady()
		{
			...
			defaultInterpolatedStringHandler.AppendFormatted(ShowingId);
		...
		protected NPC AddMoviePatronNPC(string name, int x, int y, int facingDirection)
		{
			...
			NPC i = new NPC(new AnimatedSprite("Characters\\" + textureName, 0, width, height), new Vector2(x * 64, y * 64), base.Name, facingDirection, name, null, true);
			i.EventActor = true;
		...
		private void addRandomNPCs()
		{
			...
			if (!_isJojaTheater)
			{
				...
				{
					addCritter(new Butterfly(this, new Vector2(13f, 7f)).setStayInbounds(true));
				}
				if (r.NextDouble() < 0.75)
				{
					addCritter(new Butterfly(this, new Vector2(4f, 8f)).setStayInbounds(true));
				}
				if (r.NextDouble() < 0.75)
				{
					addCritter(new Butterfly(this, new Vector2(17f, 10f)).setStayInbounds(true));
				}
			}
		}
		...
		public static MovieData GetMovieToday()
		{
			...
			{
				MovieData data;
				if (TryGetMovieData(forceMovieId, out data))
				{
					return data;
				}
		...
		public static List<MovieData> GetMoviesForSeason(WorldDate date)
		{
			...
			{
				if (MovieSeasonMatches(movie2, date.Season) && MovieYearMatches(movie2, relativeBuildYear))
				{
					movies.Add(movie2);
				}
			}
		...
		public static bool MovieYearMatches(MovieData movie, int year)
		{
			...
			if (modulus < 1)
			{
				...
				defaultInterpolatedStringHandler.AppendFormatted(movie.YearModulus);
		...
		public Dictionary<Character, MovieConcession> GetConcessionsDictionary()
		{
			...
			foreach (string npc_name in _purchasedConcessions.Keys)
			{
				...
				if (character != null && GetConcessions().TryGetValue(_purchasedConcessions[npc_name], out purchasedConcession))
				{
				...
				for (int group = 0; group < _viewingGroups.Count; group++)
				{
					for (int i = 0; i < _viewingGroups[group].Count; i++)
					{
						...
						if (patron_character != null)
						{
							...
							Point point = Game1.random.ChooseFrom(_availableHangoutPoints[group]);
							int direction;
							if (!int.TryParse(doesTileHaveProperty(point.X, point.Y, "direction", "Paths"), out direction))
							{
								direction = 2;
							}
		...
		public static MovieConcession GetConcessionItem(string id)
		{
			...
			if (id == null || !GetConcessions().TryGetValue(id, out concession))
			{
				return null;
			}
			return concession;
		...
		public override void performTouchAction(string[] action, Vector2 playerStandingPosition)
		{
			...
			if (ArgUtility.Get(action, 0) == "Theater_Exit")
			{
				Point exitTile;
				string error;
				if (!ArgUtility.TryGetPoint(action, 1, out exitTile, out error))
				{
		...
		public override bool answerDialogueAction(string questionAndAnswer, string[] questionParams)
		{
			...
			if (!(questionAndAnswer == "LeaveMovie_Yes"))
			{
				...
				{
					Utility.TryOpenShopMenu("Concessions", this, null, null, true);
