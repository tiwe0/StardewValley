	...
	public class SocialPage : IClickableMenu
	{
		...
		public List<SocialEntry> FindSocialCharacters()
		{
			...
			foreach (NPC npc in GetAllNpcs())
			{
				...
				if (!Game1.player.friendshipData.TryGetValue(npc.Name, out friendship2))
				{
					friendship2 = null;
				}
				if (npc is Child)
		...
		public IEnumerable<NPC> GetAllNpcs()
		{
				...
				else if (npc.IsVillager)
				{
					...
					else if (found.TryGetValue(npc.Name, out value) && npc != value)
					{
						...
						if (flag)
						{
							...
							defaultInterpolatedStringHandler.AppendFormatted(value.currentLocation?.NameOrUniqueName);
							defaultInterpolatedStringHandler.AppendLiteral(" ");
							defaultInterpolatedStringHandler.AppendFormatted(value.TilePoint);
							defaultInterpolatedStringHandler.AppendLiteral(", the other at ");
							defaultInterpolatedStringHandler.AppendFormatted(npc.currentLocation?.NameOrUniqueName);
							defaultInterpolatedStringHandler.AppendLiteral(" ");
							defaultInterpolatedStringHandler.AppendFormatted(npc.TilePoint);
			...
			{
				CharacterData data;
				if (nonSocial.Contains(name) || found.ContainsKey(name) || !NPC.TryGetData(name, out data))
				{
					continue;
				}
		...
		public void addTabsToClickableComponents()
		{
			...
			if (gameMenu != null && !allClickableComponents.Contains(gameMenu.tabs[0]))
			{
			...
			for (int i = 0; i < characterSlots.Count; i++)
			{
				...
				if (!entry.IsPlayer && !entry.IsChild)
				{
					...
					if (Game1.player.friendshipData.ContainsKey(character.name))
					{
						...
						{
							SocialPage socialPage = ((GameMenu)(Game1.activeClickableMenu = new GameMenu(GameMenu.socialTab, -1, false))).GetCurrentPage() as SocialPage;
							if (socialPage != null)
							{
		...
		private void drawNPCSlot(SpriteBatch b, int i)
		{
			...
			{
				Utility.drawWithShadow(b, Game1.mouseCursors2, new Vector2(xPositionOnScreen + 384 + 304, sprites[i].bounds.Y - 4), new Rectangle(166, 174, 14, 12), Color.White, 0f, Vector2.Zero, 4f, false, 0.88f, 0, -1, 0.2f);
				...
				Utility.drawWithShadow(b, Game1.mouseCursors2, new Vector2(xPositionOnScreen + 384 + 424, sprites[i].bounds.Y), new Rectangle(180, 175, 13, 11), Color.White, 0f, Vector2.Zero, 4f, false, 0.88f, 0, -1, 0.2f);
		...
		private void drawFarmerSlot(SpriteBatch b, int i)
		{
			...
			try
			{
				farmer.FarmerRenderer.draw(b, new FarmerSprite.AnimationFrame(farmer.bathingClothes ? 108 : 0, 0, false, false), farmer.bathingClothes ? 108 : 0, new Rectangle(0, farmer.bathingClothes ? 576 : 0, 16, 32), new Vector2(x, y), Vector2.Zero, 0.8f, 2, Color.White, 0f, 1f, farmer);
			}
			finally
			{
		...
		public override void draw(SpriteBatch b)
		{
			...
			drawHorizontalPartition(b, yPositionOnScreen + IClickableMenu.borderWidth + 128 + 4, true);
			drawHorizontalPartition(b, yPositionOnScreen + IClickableMenu.borderWidth + 192 + 32 + 20, true);
			drawHorizontalPartition(b, yPositionOnScreen + IClickableMenu.borderWidth + 320 + 36, true);
			drawHorizontalPartition(b, yPositionOnScreen + IClickableMenu.borderWidth + 384 + 32 + 52, true);
			...
			if (newClip.Height > 0)
			{
				...
				try
				{
					drawVerticalPartition(b, xPositionOnScreen + 256 + 12, true);
					drawVerticalPartition(b, xPositionOnScreen + 384 + 368, true);
					drawVerticalPartition(b, xPositionOnScreen + 256 + 12 + 352, true);
				}
				finally
				{
					b.GraphicsDevice.ScissorRectangle = origClip;
