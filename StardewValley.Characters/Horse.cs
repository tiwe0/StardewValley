...
namespace StardewValley.Characters
{
	public class Horse : NPC
	{
		...
		public override void update(GameTime time, GameLocation location)
		{
			...
			else if ((bool)dismounting)
			{
				...
				if (rider.yJumpOffset >= 0 && rider.yJumpVelocity <= 0f)
				{
					...
					int tries = 0;
					while (rider.currentLocation.isCollidingPosition(rider.GetBoundingBox(), Game1.viewport, true, 0, false, rider) && tries < 6)
					{
						tries++;
						rider.position.Y -= 4f;
					}
				...
				else if (Sprite.CurrentAnimation == null)
				{
					...
					case 1:
						Sprite.setCurrentAnimation(new List<FarmerSprite.AnimationFrame>
						{
							...
							new FarmerSprite.AnimationFrame(9, 70, false, false, OnMountFootstep),
							new FarmerSprite.AnimationFrame(10, 70, false, false, OnMountFootstep),
							new FarmerSprite.AnimationFrame(11, 70, false, false, OnMountFootstep),
					...
					case 3:
						Sprite.setCurrentAnimation(new List<FarmerSprite.AnimationFrame>
						{
							...
							new FarmerSprite.AnimationFrame(9, 70, false, true, OnMountFootstep),
							new FarmerSprite.AnimationFrame(10, 70, false, true, OnMountFootstep),
							new FarmerSprite.AnimationFrame(11, 70, false, true, OnMountFootstep),
					...
					case 0:
						Sprite.setCurrentAnimation(new List<FarmerSprite.AnimationFrame>
						{
							...
							new FarmerSprite.AnimationFrame(16, 70, false, false, OnMountFootstep),
							new FarmerSprite.AnimationFrame(17, 70, false, false, OnMountFootstep),
							new FarmerSprite.AnimationFrame(18, 70, false, false, OnMountFootstep),
					...
					case 2:
						Sprite.setCurrentAnimation(new List<FarmerSprite.AnimationFrame>
						{
							...
							new FarmerSprite.AnimationFrame(2, 70, false, false, OnMountFootstep),
							new FarmerSprite.AnimationFrame(3, 70, false, false, OnMountFootstep),
							new FarmerSprite.AnimationFrame(4, 70, false, false, OnMountFootstep),
		...
		public void dismount(bool from_demolish = false)
		{
			...
			{
				if (!from_demolish && TryFindStable() != null && !base.currentLocation.characters.Any(delegate(NPC c)
				{
					Horse horse = c as Horse;
					return horse != null && horse.HorseId == HorseId;
				}))
				{
		...
		public override bool checkAction(Farmer who, GameLocation l)
		{
			...
			if (rider == null)
			{
				mutex.RequestLock(delegate
				{
					...
					else
					{
							...
							else
							{
								if (who.Items.Count > who.CurrentToolIndex)
								{
									...
									if (hat != null)
									{
										...
										{
											Game1.createItemDebris(this.hat.Value, base.Position, FacingDirection);
											this.hat.Value = null;
										}
										else
										{
								...
								if (!ateCarrotToday && who.Items.Count > who.CurrentToolIndex)
								{
									Object @object = who.Items[who.CurrentToolIndex] as Object;
									if (@object != null && @object.QualifiedItemId == "(O)Carrot")
									{
										Sprite.StopAnimation();
						...
						rider.faceGeneralDirection(Utility.PointToVector2(base.StandingPixel), 0, false, false);
					}
				});
				return true;
			}
			...
			roomForHorseAtDismountTile = !base.currentLocation.isCollidingPosition(GetBoundingBox(), Game1.viewport, true, 0, false, this);
