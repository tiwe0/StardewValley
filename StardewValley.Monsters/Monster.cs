...
namespace StardewValley.Monsters
{
	...
	public class Monster : NPC
	{
		...
		public void setInvincibleCountdown(int time)
		{
			...
			startGlowing(new Color(255, 0, 0), false, 0.25f);
		...
		public virtual void InitializeForLocation(GameLocation location)
		{
			...
			if (mineShaft != null && mineShaft.mineLevel > 120 && !mineShaft.isSideBranch())
			{
		...
		private void handleParried(ParryEventArgs args)
		{
			...
			if (Game1.IsMasterGame)
			{
				...
				{
					base.currentLocation.damageMonster(GetBoundingBox(), damage / 2, damage / 2 + 1, false, 0f, 0, 0f, 0f, false, who);
				}
		...
		public override void MovePosition(GameTime time, xTile.Dimensions.Rectangle viewport, GameLocation currentLocation)
		{
			...
			if (xVelocity != 0f || yVelocity != 0f)
			{
				...
				for (int i = 1; i <= steps; i++)
				{
					...
					if (currentLocation != null && currentLocation.isCollidingPosition(nextPosition, viewport, false, DamageToFarmer, is_glider, this))
					{
			...
			if (moveUp)
			{
				if (((!Game1.eventUp || Game1.IsMultiplayer) && !currentLocation.isCollidingPosition(this.nextPosition(0), viewport, false, DamageToFarmer, isGlider, this)) || isCharging)
				{
				...
				{
					Microsoft.Xna.Framework.Rectangle tmp2 = this.nextPosition(0);
					tmp2.Width /= 4;
					bool leftCorner = currentLocation.isCollidingPosition(tmp2, viewport, false, DamageToFarmer, isGlider, this);
					tmp2.X += tmp2.Width * 3;
					bool rightCorner = currentLocation.isCollidingPosition(tmp2, viewport, false, DamageToFarmer, isGlider, this);
					if (leftCorner && !rightCorner && !currentLocation.isCollidingPosition(this.nextPosition(1), viewport, false, DamageToFarmer, isGlider, this))
					{
						position.X += (float)base.speed * ((float)time.ElapsedGameTime.Milliseconds / 64f);
					}
					else if (rightCorner && !leftCorner && !currentLocation.isCollidingPosition(this.nextPosition(3), viewport, false, DamageToFarmer, isGlider, this))
					{
						position.X -= (float)base.speed * ((float)time.ElapsedGameTime.Milliseconds / 64f);
					}
			...
			{
				if (((!Game1.eventUp || Game1.IsMultiplayer) && !currentLocation.isCollidingPosition(this.nextPosition(1), viewport, false, DamageToFarmer, isGlider, this)) || isCharging)
				{
				...
				else
				{
					...
					if (topCorner2 && !bottomCorner2 && !currentLocation.isCollidingPosition(this.nextPosition(2), viewport, false, DamageToFarmer, isGlider, this))
					{
						position.Y += (float)base.speed * ((float)time.ElapsedGameTime.Milliseconds / 64f);
					}
					else if (bottomCorner2 && !topCorner2 && !currentLocation.isCollidingPosition(this.nextPosition(0), viewport, false, DamageToFarmer, isGlider, this))
					{
						position.Y -= (float)base.speed * ((float)time.ElapsedGameTime.Milliseconds / 64f);
					}
			...
			{
				if (((!Game1.eventUp || Game1.IsMultiplayer) && !currentLocation.isCollidingPosition(this.nextPosition(2), viewport, false, DamageToFarmer, isGlider, this)) || isCharging)
				{
					position.Y += (float)base.speed + addedSpeed;
					if (!ignoreMovementAnimations)
					{
				...
				else
				{
					...
					if (leftCorner2 && !rightCorner2 && !currentLocation.isCollidingPosition(this.nextPosition(1), viewport, false, DamageToFarmer, isGlider, this))
					{
						position.X += (float)base.speed * ((float)time.ElapsedGameTime.Milliseconds / 64f);
					}
					else if (rightCorner2 && !leftCorner2 && !currentLocation.isCollidingPosition(this.nextPosition(3), viewport, false, DamageToFarmer, isGlider, this))
					{
						position.X -= (float)base.speed * ((float)time.ElapsedGameTime.Milliseconds / 64f);
					}
			...
			{
				if (((!Game1.eventUp || Game1.IsMultiplayer) && !currentLocation.isCollidingPosition(this.nextPosition(3), viewport, false, DamageToFarmer, isGlider, this)) || isCharging)
				{
					position.X -= (float)base.speed + addedSpeed;
					FacingDirection = 3;
					if (!ignoreMovementAnimations)
				...
				else
				{
					...
					tmp.Height /= 4;
					bool topCorner = currentLocation.isCollidingPosition(tmp, viewport, false, DamageToFarmer, isGlider, this);
					tmp.Y += tmp.Height * 3;
					bool bottomCorner = currentLocation.isCollidingPosition(tmp, viewport, false, DamageToFarmer, isGlider, this);
					if (topCorner && !bottomCorner && !currentLocation.isCollidingPosition(this.nextPosition(2), viewport, false, DamageToFarmer, isGlider, this))
					{
						position.Y += (float)base.speed * ((float)time.ElapsedGameTime.Milliseconds / 64f);
					}
					else if (bottomCorner && !topCorner && !currentLocation.isCollidingPosition(this.nextPosition(0), viewport, false, DamageToFarmer, isGlider, this))
					{
						position.Y -= (float)base.speed * ((float)time.ElapsedGameTime.Milliseconds / 64f);
					}
