	...
	public class Butterfly : Critter
	{
		...
		public override bool update(GameTime time, GameLocation environment)
		{
			...
			if (flapTimer <= 0 && sprite.CurrentAnimation == null)
			{
				...
				if (summerButterfly)
				{
					sprite.setCurrentAnimation(new List<FarmerSprite.AnimationFrame>
					{
						...
						new FarmerSprite.AnimationFrame(baseFrame + 2, flapSpeed),
						new FarmerSprite.AnimationFrame(baseFrame + 1, flapSpeed),
						new FarmerSprite.AnimationFrame(baseFrame, flapSpeed, false, false, doneWithFlap)
					});
				}
				else
				{
				...
				{
					Utility.makeTemporarySpriteJuicier(new TemporaryAnimatedSprite("LooseSprites\\Cursors_1_6", new Rectangle(144, 249, 7, 7), Game1.random.Next(100, 200), 6, 1, position + new Vector2(-48 + Game1.random.Next(-32, 32), -96 + Game1.random.Next(-32, 32)), false, false, Math.Max(0f, (position.Y + 64f - 24f) / 10000f) + position.X / 64f * 1E-05f, 0f, Utility.GetPrismaticColor(Game1.random.Next(7), 10f), 4f, 0f, 0f, 0f)
					{
			...
			if (prismaticCaptureTimer > 0f)
			{
				...
				{
					environment.temporarySprites.Add(new TemporaryAnimatedSprite("LooseSprites\\Cursors_1_6", new Rectangle(144, 249, 7, 7), Game1.random.Next(100, 200), 6, 1, position + new Vector2(-48f, -96f), false, false, Math.Max(0f, (position.Y + 64f - 24f) / 10000f) + position.X / 64f * 1E-05f, 0f, Utility.GetPrismaticColor(Game1.random.Next(7), 10f), 4f, 0f, 0f, 0f)
					{
						drawAboveAlwaysFront = true
					});
				...
				if (prismaticCaptureTimer <= 0f)
				{
					Utility.makeTemporarySpriteJuicier(new TemporaryAnimatedSprite("LooseSprites\\Cursors_1_6", new Rectangle(144, 249, 7, 7), Game1.random.Next(100, 200), 6, 1, position + new Vector2(-48f, -96f), false, false, Math.Max(0f, (position.Y + 64f - 24f) / 10000f) + position.X / 64f * 1E-05f, 0f, Color.White, 4f, 0f, 0f, 0f)
					{
						drawAboveAlwaysFront = true
					}, environment, 16);
