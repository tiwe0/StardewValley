	...
	public class SpriteText
	{
		...
		public static int getWidthOfString(string s, int widthConstraint = 999999)
		{
			...
			for (int i = 0; i < s.Length; i++)
			{
				...
				{
					FontChar c;
					if (characterMap.TryGetValue(s[i], out c))
					{
						width += c.XAdvance;
					}
		...
		public static int getHeightOfString(string s, int widthConstraint = 999999)
		{
			...
			if (isUsingNonSpriteSheetFont() && !forceEnglishFont)
			{
				for (int j = 0; j < s.Length; j++)
				{
					...
					FontChar c;
					if (characterMap.TryGetValue(s[j], out c))
					{
		...
		{
			drawString(b, s, x, y, 999999, -1, 999999, alpha, 0.88f, false, 0, placeHolderWidthText, color, scroll_text_alignment);
		}
		...
		public static void drawString(SpriteBatch b, string s, int x, int y, int characterPosition = 999999, int width = -1, int height = 999999, float alpha = 1f, float layerDepth = 0.88f, bool junimoText = false, int drawBGScroll = -1, string placeHolderScrollWidthText = "", Color? color = null, ScrollTextAlignment scroll_text_alignment = ScrollTextAlignment.Left)
		{
			...
			for (int i = 0; i < Math.Min(s.Length, characterPosition); i++)
			{
				...
				FontChar fc;
				if (characterMap.TryGetValue(s[i], out fc))
				{
					...
					if (LocalizedContentManager.CurrentLanguageCode == LocalizedContentManager.LanguageCode.ru)
					{
						...
						b.Draw(_texture, position2 + offset, sourcerect, color.Value * alpha * shadowAlpha, 0f, Vector2.Zero, FontPixelZoom, SpriteEffects.None, layerDepth);
						b.Draw(_texture, position2 + new Vector2(0f, offset.Y), sourcerect, color.Value * alpha * shadowAlpha, 0f, Vector2.Zero, FontPixelZoom, SpriteEffects.None, layerDepth);
						b.Draw(_texture, position2 + new Vector2(offset.X, 0f), sourcerect, color.Value * alpha * shadowAlpha, 0f, Vector2.Zero, FontPixelZoom, SpriteEffects.None, layerDepth);
					}
					b.Draw(_texture, position2, sourcerect, color.Value * alpha, 0f, Vector2.Zero, FontPixelZoom, SpriteEffects.None, layerDepth);
		...
		public static int positionOfNextSpace(string s, int index, int currentXPosition, int accumulatedHorizontalSpaceBetweenCharacters)
		{
			...
			for (int i = index; i < s.Length; i++)
			{
				if (isUsingNonSpriteSheetFont())
				{
					...
					FontChar fc2;
					currentXPosition = ((!characterMap.TryGetValue(s[i], out fc2)) ? (currentXPosition + (int)((float)FontFile.Common.LineHeight * FontPixelZoom)) : (currentXPosition + (int)((float)fc2.XAdvance * FontPixelZoom)));
					continue;
				}
				if (s[i] == ' ' || s[i] == '^')
				{
