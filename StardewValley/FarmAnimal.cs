	...
	public class FarmAnimal : Character
	{
			...
			get
			{
				if (_displayHouse == null)
				{
					...
					if (data != null)
					{
						...
						_displayHouse = (Game1.buildingData.TryGetValue(data.House, out buildingData) ? TokenParser.ParseText(buildingData.Name) : data.House);
					}
					else
					{
		...
		public void ReloadTextureIfNeeded(bool forceReload = false)
		{
			if (Sprite == null || forceReload)
			{
				...
				if (data != null)
				{
					texturePath2 = GetTexturePath(data);
					spriteWidth = data.SpriteWidth;
					spriteHeight = data.SpriteHeight;
				}
				else
				{
					texturePath2 = "Animals\\Error";
					spriteWidth = 16;
					spriteHeight = 16;
				}
				if (!Game1.content.DoesAssetExist<Texture2D>(texturePath2))
				{
					...
					defaultInterpolatedStringHandler.AppendFormatted(type.Value);
					defaultInterpolatedStringHandler.AppendLiteral("' failed to load texture path '");
					defaultInterpolatedStringHandler.AppendFormatted(texturePath2);
				...
				Sprite = new AnimatedSprite(texturePath2, 0, spriteWidth, spriteHeight)
				{
		...
		public static FarmAnimalData GetAnimalDataFromEgg(Item eggItem, GameLocation location)
		{
			string id;
			FarmAnimalData data;
			if (!TryGetAnimalDataFromEgg(eggItem, location, out id, out data))
			{
				return null;
			}
			return data;
		...
		public virtual FarmAnimalData GetAnimalData()
		{
			...
			if (!Game1.farmAnimalData.TryGetValue(type.Value, out animalData))
			{
				return null;
			}
			return animalData;
		...
		public void setRandomPosition(GameLocation location)
		{
			...
			int tries = 0;
			while (base.Position.Equals(Vector2.Zero) || location.Objects.ContainsKey(base.Position) || location.isCollidingPosition(GetBoundingBox(), Game1.viewport, false, 0, false, this))
			{
		...
		public void dayUpdate(GameLocation environment)
		{
			...
			else
			{
				...
				if (r.NextDouble() < (double)(int)happiness / 150.0)
				{
					...
					string deluxeProduce = GetProduceID(r, true);
		...
		public string getMoodMessage()
		{
			...
			default:
				...
				{
				case 3:
					return Game1.content.LoadString("Strings\\FarmAnimals:MoodMessage_Sad", displayName);
				case 2:
					return Game1.content.LoadString("Strings\\FarmAnimals:MoodMessage_Fine", displayName);
				case 1:
					return Game1.content.LoadString("Strings\\FarmAnimals:MoodMessage_Happy", displayName);
				default:
					return "";
				}
			}
		...
		public override void draw(SpriteBatch b)
		{
			...
			if (shadow == null || shadow.Visible)
			{
				...
				{
					float shadowScale2 = shadow?.Scale ?? (baby ? 2.5f : 3.5f);
					Vector2 shadowPos2 = new Vector2(base.Position.X + (float)shadowOffsetX, base.Position.Y - 24f + (float)shadowOffsetY);
					Sprite.drawShadow(b, Game1.GlobalToLocal(Game1.viewport, shadowPos2), shadowScale2, 0.5f);
		...
		public virtual void updateWhenNotCurrentLocation(Building currentBuilding, GameTime time, GameLocation environment)
		{
			...
			update(time, environment, myID.Value, false);
			if (!Game1.IsMasterGame)
			{
				return;
			}
			...
			if (currentBuilding != null && Game1.random.NextBool(0.002) && (bool)currentBuilding.animalDoorOpen && Game1.timeOfDay < 1630 && !environment.IsRainingHere() && !environment.IsWinterHere() && !environment.farmers.Any())
			{
				...
				if (buildingLocation.isCollidingPosition(doorArea, Game1.viewport, false, 0, false, this, false) || buildingLocation.isCollidingPosition(new Microsoft.Xna.Framework.Rectangle(doorArea.X, doorArea.Y + 64, doorArea.Width, doorArea.Height), Game1.viewport, false, 0, false, this, false))
				{
					return;
				}
				...
				if (location.Equals(Game1.currentLocation))
				{
					...
					case 2:
						Sprite.setCurrentAnimation(new List<FarmerSprite.AnimationFrame>
						{
							...
							new FarmerSprite.AnimationFrame(3, 250),
							new FarmerSprite.AnimationFrame(1, 250),
							new FarmerSprite.AnimationFrame(3, 250, false, false, delegate
							{
					...
					case 1:
						Sprite.setCurrentAnimation(new List<FarmerSprite.AnimationFrame>
						{
							...
							new FarmerSprite.AnimationFrame(7, 250),
							new FarmerSprite.AnimationFrame(5, 250),
							new FarmerSprite.AnimationFrame(7, 250, false, false, delegate
							{
								DigUpProduce(location, produce);
							})
						});
					...
					case 0:
						Sprite.setCurrentAnimation(new List<FarmerSprite.AnimationFrame>
						{
							...
							new FarmerSprite.AnimationFrame(11, 250),
							new FarmerSprite.AnimationFrame(9, 250),
							new FarmerSprite.AnimationFrame(11, 250, false, false, delegate
							{
								DigUpProduce(location, produce);
							})
						});
					...
					case 3:
						Sprite.setCurrentAnimation(new List<FarmerSprite.AnimationFrame>
						{
							...
							new FarmerSprite.AnimationFrame(7, 250, false, true),
							new FarmerSprite.AnimationFrame(5, 250, false, true),
							new FarmerSprite.AnimationFrame(7, 250, false, true, delegate
							{
								DigUpProduce(location, produce);
							})
						});
		...
		public virtual bool updateWhenCurrentLocation(GameTime time, GameLocation location)
		{
			...
			if (IsActuallySwimming())
			{
				...
				if (nextRipple <= 0)
				{
					...
					TemporaryAnimatedSprite ripple = new TemporaryAnimatedSprite("TileSheets\\animations", new Microsoft.Xna.Framework.Rectangle(0, 0, 64, 64), isMoving() ? 75f : 150f, 8, 0, new Vector2((float)standingPixel.X + x_offset * scale, (float)standingPixel.Y - 32f * scale), false, Game1.random.NextBool(), 0.01f, 0.01f, Color.White * 0.75f, scale, 0f, 0f, 0f);
			...
			update(time, location, myID.Value, false);
			if (hopOffset != Vector2.Zero)
			{
				Sprite.UpdateSourceRect();
				HandleHop();
		...
		public override void MovePosition(GameTime time, xTile.Dimensions.Rectangle viewport, GameLocation currentLocation)
		{
			...
			if (IsActuallySwimming())
			{
				...
				if (!currentLocation.isCollidingPosition(next_bounds, Game1.viewport, false, 0, false, this, false))
				{
					base.Position += _swimmingVelocity;
			...
			{
				if (!currentLocation.isCollidingPosition(nextPosition(0), Game1.viewport, false, 0, false, this, false))
				{
			...
			{
				if (!currentLocation.isCollidingPosition(nextPosition(1), Game1.viewport, false, 0, false, this))
				{
			...
			{
				if (!currentLocation.isCollidingPosition(nextPosition(2), Game1.viewport, false, 0, false, this))
				{
					position.Y += base.speed;
					Sprite.AnimateDown(time);
				}
			...
			else
			{
				...
				if (!currentLocation.isCollidingPosition(nextPosition(3), Game1.viewport, false, 0, false, this))
				{
					position.X -= base.speed;
					Sprite.AnimateRight(time);
				}
		...
		public virtual void Splash()
		{
			...
			Game1.multiplayer.broadcastSprites(base.currentLocation, new TemporaryAnimatedSprite(28, 100f, 2, 1, getStandingPosition() + new Vector2(-0.5f, -0.5f) * 64f, false, false)
			{
		...
		private void ValidateSpritesheetSize()
		{
			...
			if (Sprite.Texture.Height < expectedRows * Sprite.SpriteHeight)
			{
				...
				defaultInterpolatedStringHandler.AppendFormatted(Sprite.Texture.Height);
				defaultInterpolatedStringHandler.AppendLiteral("px, but expected at least ");
				defaultInterpolatedStringHandler.AppendFormatted(expectedRows * Sprite.SpriteHeight);
			...
			if (Sprite.Texture.Width != 4 * Sprite.SpriteWidth)
			{
				...
				defaultInterpolatedStringHandler.AppendFormatted(Sprite.Texture.Width);
				defaultInterpolatedStringHandler.AppendLiteral("px, but it should be exactly ");
				defaultInterpolatedStringHandler.AppendFormatted(4 * Sprite.SpriteWidth);
