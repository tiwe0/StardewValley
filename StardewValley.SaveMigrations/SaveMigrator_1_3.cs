	...
	public class SaveMigrator_1_3 : ISaveMigrator
	{
		...
		public static void ApplyLegacyChanges()
		{
			...
			Utility.ForEachVillager(delegate(NPC villager)
			{
				...
				{
					Friendship value;
					if (Game1.player.friendshipData.TryGetValue(villager.Name, out value) && !value.IsDating())
					{
				...
				{
					Friendship value2;
					if (Game1.player.friendshipData.TryGetValue(villager.Name, out value2) && !value2.IsDating() && !value2.IsDivorced())
					{
		...
		private static void MigrateHorseIds()
		{
			...
			for (int j = stablesMissingHorses.Count - 1; j >= 0; j--)
			{
				...
				GameLocation parentLocation = stable2.GetParentLocation();
				Rectangle boundingBox = stable2.GetBoundingBox();
			...
			{
				Stable stable3 = stablesMissingHorses[i];
				foreach (NPC character2 in stable3.GetParentLocation().characters)
				{
					Horse horse2 = character2 as Horse;
					if (horse2 != null && horse2.HorseId == Guid.Empty)
					{
						horse2.HorseId = stable3.HorseId;
