			...
			showDialogue = delegate
			{
				...
				else
				{
					...
					if (texture2D != null || text != null)
					{
						if (nPC != null)
						{
							nPC = new NPC(nPC.Sprite, Vector2.Zero, "", 0, nPC.Name, texture2D ?? nPC.Portrait, false);
							nPC.displayName = text ?? nPC.displayName;
						}
						else if (texture2D != null)
						{
							nPC = new NPC(new AnimatedSprite("Characters\\Abigail", 0, 16, 16), Vector2.Zero, "", 0, "???", texture2D, false)
							{
								displayName = (text ?? "???")
							};
						}
