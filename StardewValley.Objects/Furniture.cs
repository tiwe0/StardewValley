...
namespace StardewValley.Objects
{
	...
	public class Furniture : Object, ISittable
	{
		...
		{
			actionOnPlayerEntryOrPlacement(Location, true);
			return false;
		}
		...
		{
			int key;
			if (sittingFarmers.TryGetValue(who.UniqueMultiplayerID, out key))
			{
		...
		public virtual List<Vector2> GetSeatPositions(bool ignore_offsets = false)
		{
			...
			if ((int)furniture_type == 2)
			{
				...
				else if ((int)currentRotation == 1)
				{
					...
					{
						seat_positions.Add(TileLocation + new Vector2(1f, k));
					}
				}
				else
				{
					...
					{
						seat_positions.Add(TileLocation + new Vector2(0f, j));
					}
				}
			}
			if ((int)furniture_type == 3)
		...
		public override void actionOnPlayerEntry()
		{
			...
			actionOnPlayerEntryOrPlacement(Location, false);
			...
			if (r.NextDouble() < 0.06)
			{
				...
				Location.critters.Add(new Birdie(TileLocation * 64f + new Vector2(32f, 64 + Game1.random.Next(3) * 4 + yOffset), -160f, whichBird, true));
			}
		}
		...
		public virtual void actionOnPlayerEntryOrPlacement(GameLocation environment, bool dropDown)
		{
			...
			{
				addLights();
				(heldObject.Value as Furniture)?.addLights();
			}
			if (base.QualifiedItemId == "(F)1971" && !dropDown)
			{
				environment.instantiateCrittersList();
				environment.addCritter(new Butterfly(environment, environment.getRandomTile()).setStayInbounds(true));
		...
		public override bool performObjectDropInAction(Item dropInItem, bool probe, Farmer who, bool returnFalseIfItemConsumed = false)
		{
			...
			if (IsTable() && heldObject.Value == null && !dropIn.bigCraftable && !(dropIn is Wallpaper))
			{
		...
		public virtual void addLights()
		{
			...
			Furniture furniture = heldObject.Value as Furniture;
			if (furniture != null)
			{
				heldObject.Value.Location = Location;
				furniture.addLights();
		...
		{
			GameLocation environment = Location;
			(heldObject.Value as Furniture)?.removeLights();
		...
		public override bool canBePlacedHere(GameLocation l, Vector2 tile, CollisionMask collisionMask = CollisionMask.All, bool showError = false)
		{
			...
			for (int x = 0; x < tilesWide; x++)
			{
				for (int y = 0; y < tilesHigh; y++)
				{
					...
					Object tileObj;
					if (l.objects.TryGetValue(curTile, out tileObj) && (!tileObj.isPassable() || !isPassable()))
					{
						return false;
					}
					if (!isGroundFurniture())
					...
					if (l.terrainFeatures.ContainsKey(curTile))
					{
						HoeDirt dirt = l.terrainFeatures[curTile] as HoeDirt;
		...
		public override bool placementAction(GameLocation location, int x, int y, Farmer who = null)
		{
			...
			foreach (Furniture f in location.furniture)
			{
				...
				{
					f.performObjectDropInAction(this, false, who ?? Game1.player);
					return true;
				}
			}
		...
		{
			Location.temporarySprites.Add(new TemporaryAnimatedSprite("LooseSprites\\Cursors", new Rectangle(372, 1956, 10, 10), TileLocation * 64f + new Vector2(41.6f, -21f) + new Vector2(Game1.random.Next(-12, 21), Game1.random.Next(16)), false, 0.002f, Color.Lime)
			{
		...
		public override void draw(SpriteBatch spriteBatch, int x, int y, float alpha = 1f)
		{
			...
			if (isDrawingLocationFurniture)
			{
				...
				if (!_frontTextureName.TryGetValue(textureName, out frontTexturePath))
				{
			...
			if (heldObject.Value != null)
			{
				...
				else
				{
					...
					{
						heldObject.Value.drawInMenu(spriteBatch, Game1.GlobalToLocal(Game1.viewport, new Vector2(boundingBox.Center.X - 32, boundingBox.Center.Y - (drawHeldObjectLow ? 32 : 85))), 1f, 1f, (float)(boundingBox.Bottom + 1) / 10000f, StackDrawType.Hide, Color.White, false);
					}
					else
					{
			...
			{
				spriteBatch.Draw(layerDepth: (float)(GetBoundingBoxAt(x, y).Bottom - 2) / 10000f, texture: Game1.mouseCursors, position: Game1.GlobalToLocal(Game1.viewport, new Vector2(boundingBox.Center.X - 20, (float)boundingBox.Center.Y - 105.6f)), sourceRectangle: new Rectangle(276 + (int)((Game1.currentGameTime.TotalGameTime.TotalMilliseconds + (double)(x * 3047) + (double)(y * 88)) % 400.0 / 100.0) * 12, 1985, 12, 11), color: Color.White, rotation: 0f, origin: Vector2.Zero, scale: 4f, effects: SpriteEffects.None);
			}
			if (Game1.debugMode)
			{
