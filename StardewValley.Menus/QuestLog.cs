...
namespace StardewValley.Menus
{
	public class QuestLog : IClickableMenu
	{
		...
		public virtual void SetScrollFromY(int y)
		{
			...
			float percentage = (float)(y - scrollBarBounds.Y) / (float)(scrollBarBounds.Height - scrollBar.bounds.Height);
		...
		public override void receiveLeftClick(int x, int y, bool playSound = true)
		{
			...
			int yOffset = ((_shownQuest.IsTimedQuest() && _shownQuest.GetDaysLeft() > 0 && SpriteText.getWidthOfString(_shownQuest.GetName()) > width / 2) ? (-48) : 0);
		...
		public override void draw(SpriteBatch b)
		{
			...
			if (questPage == -1)
			{
				...
				for (int i = 0; i < questLogButtons.Count; i++)
				{
					...
					{
						IClickableMenu.drawTextureBox(b, Game1.mouseCursors, new Rectangle(384, 396, 15, 15), questLogButtons[i].bounds.X, questLogButtons[i].bounds.Y, questLogButtons[i].bounds.Width, questLogButtons[i].bounds.Height, questLogButtons[i].containsPoint(Game1.getOldMouseX(), Game1.getOldMouseY()) ? Color.Wheat : Color.White, 4f, false);
						if (pages[currentPage][i].ShouldDisplayAsNew() || pages[currentPage][i].ShouldDisplayAsComplete())
						{
							Utility.drawWithShadow(b, Game1.mouseCursors, new Vector2(questLogButtons[i].bounds.X + 64 + 4, questLogButtons[i].bounds.Y + 44), new Rectangle(pages[currentPage][i].ShouldDisplayAsComplete() ? 341 : 317, 410, 23, 9), Color.White, 0f, new Vector2(11f, 4f), 4f + Game1.dialogueButtonScale * 10f / 250f, false, 0.99f);
						}
						else
						{
							Utility.drawWithShadow(b, Game1.mouseCursors, new Vector2(questLogButtons[i].bounds.X + 32, questLogButtons[i].bounds.Y + 28), pages[currentPage][i].IsTimedQuest() ? new Rectangle(410, 501, 9, 9) : new Rectangle(395 + (pages[currentPage][i].IsTimedQuest() ? 3 : 0), 497, 3, 8), Color.White, 0f, Vector2.Zero, 4f, false, 0.99f);
						}
			...
			{
				int titleWidth = SpriteText.getWidthOfString(_shownQuest.GetName());
				...
				if (_shownQuest.IsTimedQuest() && _shownQuest.GetDaysLeft() > 0)
				{
					...
					Utility.drawWithShadow(b, Game1.mouseCursors, new Vector2(xPositionOnScreen + xOffset + 32, (float)(yPositionOnScreen + 48 - 8) + extraYOffset), new Rectangle(410, 501, 9, 9), Color.White, 0f, Vector2.Zero, 4f, false, 0.99f);
					Utility.drawTextWithShadow(b, Game1.parseText((pages[currentPage][questPage].GetDaysLeft() > 1) ? Game1.content.LoadString("Strings\\StringsFromCSFiles:QuestLog.cs.11374", pages[currentPage][questPage].GetDaysLeft()) : Game1.content.LoadString("Strings\\StringsFromCSFiles:Quest_FinalDay"), Game1.dialogueFont, width - 128), Game1.dialogueFont, new Vector2(xPositionOnScreen + xOffset + 80, (float)(yPositionOnScreen + 48 - 8) + extraYOffset), Game1.textColor);
				}
				...
				Utility.drawTextWithShadow(b, description, Game1.dialogueFont, new Vector2(xPositionOnScreen + 64, (float)yPositionOnScreen - scrollAmount + 96f + extraYOffset), Game1.textColor);
				float yPos = (float)(yPositionOnScreen + 96) + description_size.Y + 32f - scrollAmount + extraYOffset;
				if (_shownQuest.ShouldDisplayAsComplete())
				{
					b.End();
					b.GraphicsDevice.ScissorRectangle = cached_scissor_rect;
					b.Begin(SpriteSortMode.Deferred, BlendState.AlphaBlend, SamplerState.PointClamp);
					SpriteText.drawString(b, Game1.content.LoadString("Strings\\StringsFromCSFiles:QuestLog.cs.11376"), xPositionOnScreen + 32 + 4, rewardBox.bounds.Y + 21 + 4 + (int)extraYOffset);
					rewardBox.draw(b, Color.White, 0.9f, 0, 0, (int)extraYOffset);
					if (HasMoneyReward())
					{
						b.Draw(Game1.mouseCursors, new Vector2(rewardBox.bounds.X + 16, (float)(rewardBox.bounds.Y + 16) - Game1.dialogueButtonScale / 2f + extraYOffset), new Rectangle(280, 410, 16, 16), Color.White, 0f, Vector2.Zero, 4f, SpriteEffects.None, 1f);
						SpriteText.drawString(b, Game1.content.LoadString("Strings\\StringsFromCSFiles:LoadGameMenu.cs.11020", _shownQuest.GetMoneyReward()), xPositionOnScreen + 448, rewardBox.bounds.Y + 21 + 4 + (int)extraYOffset);
					}
				}
				else
				{
					...
					{
						string parsed_text = Game1.parseText(_objectiveText[j], width: width - 192, whichFont: Game1.dialogueFont);
						bool num2 = (_shownQuest as SpecialOrder)?.objectives[j].IsComplete() ?? false;
						...
						SpecialOrder order = _shownQuest as SpecialOrder;
						if (order != null)
						{
