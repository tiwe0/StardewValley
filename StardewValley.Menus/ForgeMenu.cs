...
namespace StardewValley.Menus
{
	public class ForgeMenu : MenuWithInventory
	{
		...
		public override void receiveLeftClick(int x, int y, bool playSound = true)
		{
			...
			foreach (ClickableComponent c in equipmentIcons)
			{
				...
				else
				{
					...
					if (item_to_place2 != Game1.player.leftRing.Value && (item_to_place2 == null || item_to_place2 is Ring))
					{
						base.heldItem = Game1.player.Equip(item_to_place2 as Ring, Game1.player.leftRing);
						if (Game1.player.leftRing.Value != null)
						{
							Game1.playSound("crit");
						}
			...
			else if (startTailoringButton.containsPoint(x, y))
			{
				if (base.heldItem == null)
				{
					...
					if (!fail && IsValidCraft(leftIngredientSpot.item, rightIngredientSpot.item) && Game1.player.Items.ContainsId("(O)848", GetForgeCost(leftIngredientSpot.item, rightIngredientSpot.item)))
					{
						...
						{
							tempSprites.Add(new TemporaryAnimatedSprite("", new Rectangle(143, 17, 14, 15), new Vector2(xPositionOnScreen + 276, yPositionOnScreen + 300), false, 0.1f, Color.White)
							{
						...
						if (rightIngredientSpot.item?.QualifiedItemId == "(O)74")
						{
							...
							for (int j = 0; j < num; j++)
							{
								...
								tempSprites.Add(new TemporaryAnimatedSprite("LooseSprites\\Cursors2", new Rectangle(114, 48, 2, 2), position, false, 0f, Color.White)
								{
			...
			else if (unforgeButton.containsPoint(x, y))
			{
				if (rightIngredientSpot.item == null)
				{
					if (IsValidUnforge())
					{
						MeleeWeapon leftWeapon = leftIngredientSpot.item as MeleeWeapon;
						if (leftWeapon != null && !Game1.player.couldInventoryAcceptThisItem("(O)848", leftWeapon.GetTotalForgeLevels() * 5 + (leftWeapon.GetTotalForgeLevels() - 1) * 2))
						{
						...
						else
						{
							...
							for (int i = 0; i < crystals; i++)
							{
								...
								tempSprites.Add(new TemporaryAnimatedSprite("", new Rectangle(143, 17, 14, 15), new Vector2(leftIngredientSpot.bounds.X, leftIngredientSpot.bounds.Y), false, 0.1f, Color.White)
								{
									alpha = 0.01f,
									alphaFade = -0.1f,
		...
		public override void gameWindowSizeChanged(Rectangle oldBounds, Rectangle newBounds)
		{
			...
			inventory = new InventoryMenu(xPositionOnScreen + IClickableMenu.spaceToClearSideBorder + IClickableMenu.borderWidth / 2 + 12, yPositionForInventory, false, null, inventory.highlightMethod);
			_CreateButtons();
		}
		...
		public override void update(GameTime time)
		{
			...
			else if (_clankEffectTimer <= 0 && !unforging)
			{
				...
				if (rightIngredientSpot.item?.QualifiedItemId == "(O)74")
				{
					...
					for (int k = 0; k < num; k++)
					{
						...
						tempSprites.Add(new TemporaryAnimatedSprite("LooseSprites\\Cursors2", new Rectangle(114, 48, 2, 2), position2, false, 0f, Color.White)
						{
							motion = new Vector2(-4f, 0f),
							yPeriodic = true,
							yPeriodicRange = 16f,
				...
				else
				{
					...
					r.Inflate(-21, -21);
					Vector2 position = Utility.getRandomPositionInThisRectangle(r, Game1.random);
					tempSprites.Add(new TemporaryAnimatedSprite("LooseSprites\\Cursors2", new Rectangle(114, 46, 2, 2), position, false, 0.015f, Color.White)
					{
			...
			if (unforging)
			{
				...
				{
					CombinedRing leftRing = leftIngredientSpot.item as CombinedRing;
					if (leftRing != null)
					{
						if (leftRing != null)
						{
			...
			Item crafted_item = CraftItem(leftIngredientSpot.item, rightIngredientSpot.item, true);
		...
		public virtual bool IsValidUnforge(bool ignore_right_slot_occupancy = false)
		{
			...
			MeleeWeapon leftWeapon = leftIngredientSpot.item as MeleeWeapon;
			if (leftWeapon != null && (leftWeapon.GetTotalForgeLevels() > 0 || leftWeapon.appearance.Value != null))
			{
				return true;
			}
