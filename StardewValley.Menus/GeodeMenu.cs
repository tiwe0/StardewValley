...
namespace StardewValley.Menus
{
	public class GeodeMenu : MenuWithInventory
	{
		...
		{
			base.update(time);
			for (int k = fluffSprites.Count - 1; k >= 0; k--)
			{
				if (fluffSprites[k].update(time))
				{
					fluffSprites.RemoveAt(k);
				}
			}
			if (alertTimer > 0)
			{
			...
			if (clint.currentFrame == 11 && frame != 11)
			{
				...
				if (geodeSpot.item != null)
				{
					...
					geodeDestructionAnimation = new TemporaryAnimatedSprite("TileSheets\\animations", new Rectangle(0, geodeDestructionYOffset, 64, 64), 100f, 8, 0, new Vector2(geodeSpot.bounds.X + 392 - 32, geodeSpot.bounds.Y + 192 - 32), false, false);
					if (geodeSpot.item?.QualifiedItemId == "(O)275")
					{
						...
						for (int j = 0; j < 6; j++)
						{
							fluffSprites.Add(new TemporaryAnimatedSprite("LooseSprites\\Cursors", new Rectangle(372, 1956, 10, 10), new Vector2(geodeSpot.bounds.X + 392 - 32 + Game1.random.Next(21), geodeSpot.bounds.Y + 192 - 16), false, 0.002f, new Color(255, 222, 198))
							{
								alphaFade = 0.02f,
					...
					else if (geodeSpot.item?.QualifiedItemId == "(O)MysteryBox" || geodeSpot.item?.QualifiedItemId == "(O)GoldenMysteryBox")
					{
						...
						for (int i = 0; i < 6; i++)
						{
							fluffSprites.Add(new TemporaryAnimatedSprite("LooseSprites\\Cursors", new Rectangle(372, 1956, 10, 10), new Vector2(geodeSpot.bounds.X + 392 - 48 + Game1.random.Next(32), geodeSpot.bounds.Y + 192 - 24), false, 0.002f, new Color(255, 222, 198))
							{
								alphaFade = 0.02f,
								motion = new Vector2((float)Game1.random.Next(-20, 21) / 10f, (float)Game1.random.Next(5, 20) / 10f),
								interval = 99999f,
					...
					if (!(geodeSpot.item.QualifiedItemId == "(O)275"))
					{
						Object mineral = geodeTreasure as Object;
						if (mineral == null || !(mineral.Type == "Minerals"))
						{
							Object artifact = geodeTreasure as Object;
			...
			if (geodeDestructionAnimation != null && ((geodeDestructionAnimation.id != 777 && geodeDestructionAnimation.currentParentTileIndex < 7) || (geodeDestructionAnimation.id == 777 && geodeDestructionAnimation.currentParentTileIndex < 5)))
			{
				...
				else
				{
					...
					{
						Object treasure = geodeTreasure as Object;
						if (treasure == null || (int)treasure.price > 75 || geodeSpot.item?.QualifiedItemId == "(O)MysteryBox" || geodeSpot.item?.QualifiedItemId == "(O)GoldenMysteryBox")
						{
							sparkle = new TemporaryAnimatedSprite("TileSheets\\animations", new Rectangle(0, 640, 64, 64), 100f, 8, 0, new Vector2(geodeSpot.bounds.X + ((geodeSpot.item.itemId == "MysteryBox") ? 94 : 98) * 4 - 32, geodeSpot.bounds.Y + 192 + yPositionOfGem - 32), false, false);
		...
		public override void draw(SpriteBatch b)
		{
			...
			if (geodeSpot.item != null)
			{
				if (geodeDestructionAnimation == null)
				{
					...
					bool flag = geodeSpot.item.QualifiedItemId == "(O)275";
