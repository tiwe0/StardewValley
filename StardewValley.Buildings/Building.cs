...
namespace StardewValley.Buildings
{
	...
	public class Building : INetObject<NetFields>, IHaveModData
	{
		...
		public virtual bool CanBePainted()
		{
			...
			if (isCabin || HasIndoorsName("Farmhouse"))
			{
				FarmHouse house = GetIndoors() as FarmHouse;
		...
		public string GetMetadata(string key)
		{
			if (buildingMetadata == null)
			{
				...
				if (data != null)
				{
					...
					{
						buildingMetadata[kvp2.Key] = kvp2.Value;
					}
		...
		public virtual bool hasCarpenterPermissions()
		{
			...
			FarmHouse farmHouse = GetIndoors() as FarmHouse;
		...
		{
			BuildingData data;
			if (!TryGetData(buildingType.Value, out data))
			{
				return null;
			}
			return data;
				...
				if (nonInstancedIndoorsName.Value == null)
				{
					...
					if (mapPath != null)
					{
						...
						else if (indoors.Value.mapPath.Value == mapPath)
						{
							if (forUpgrade)
							{
								InitializeIndoor(data, forConstruction, true);
							}
						}
						else
						{
		...
		public virtual void InitializeIndoor(BuildingData data, bool forConstruction, bool forUpgrade)
		{
			...
			foreach (IndoorItemAdd item in data.IndoorItems)
			{
				...
				if (interior.IsTileBlockedBy(tileVector, CollisionMask.Furniture | CollisionMask.Objects))
				{
					continue;
				}
				Object newObj = ItemRegistry.Create(item.ItemId) as Object;
				if (newObj != null)
				{
		...
		public bool PerformBuildingChestAction(string name, Farmer who)
		{
			...
			case BuildingChestType.Chest:
				((MenuWithInventory)(Game1.activeClickableMenu = new ItemGrabMenu(chest.Items, false, true, (Item item) => IsValidObjectForChest(item, chest), chest.grabItemFromInventory, null, chest.grabItemFromChest, false, true, true, true, true, 1, null, -1, this))).inventory.moveItemSound = chestData.Sound;
				return true;
		...
		public virtual bool doAction(Vector2 tileLocation, Farmer who)
		{
			...
			else
			{
				...
				if (data != null)
				{
					...
					door.X /= 64;
					door.Y /= 64;
					if ((int)daysOfConstructionLeft <= 0 && door != Microsoft.Xna.Framework.Rectangle.Empty && door.Contains(Utility.Vector2ToPoint(tileLocation)) && Game1.didPlayerJustRightClick(true))
					{
						ToggleAnimalDoor(who);
						return true;
					}
					if (who.IsLocalPlayer && occupiesTile(tileLocation, true) && !isTilePassable(tileLocation))
					{
				...
				else
				{
					...
					{
						return performActiveObjectDropInAction(who, false);
					}
				}
			}
			return false;
		...
		public static bool TryPerformObeliskWarp(string buildingType, Farmer who)
		{
			...
			{
			case "Desert Obelisk":
				PerformObeliskWarp("Desert", 35, 43, true, who);
				return true;
			case "Water Obelisk":
				PerformObeliskWarp("Beach", 20, 4, false, who);
				return true;
			case "Earth Obelisk":
				PerformObeliskWarp("Mountain", 31, 20, false, who);
				return true;
			case "Island Obelisk":
				PerformObeliskWarp("IslandSouth", 11, 11, false, who);
				return true;
			default:
				return false;
			}
		...
		public static void PerformObeliskWarp(string destination, int warp_x, int warp_y, bool force_dismount, Farmer who)
		{
			...
			for (int i = 0; i < 12; i++)
			{
				who.currentLocation.temporarySprites.Add(new TemporaryAnimatedSprite(354, Game1.random.Next(25, 75), 6, 1, new Vector2(Game1.random.Next((int)who.Position.X - 256, (int)who.Position.X + 192), Game1.random.Next((int)who.Position.Y - 256, (int)who.Position.Y + 192)), false, Game1.random.NextBool()));
			}
			...
			{
				who.currentLocation.temporarySprites.Add(new TemporaryAnimatedSprite(6, new Vector2(x, playerTile.Y) * 64f, Color.White, 8, false, 50f)
				{
					layerDepth = 1f,
		...
		public virtual bool isActionableTile(int xTile, int yTile, Farmer who)
		{
			...
			if (data != null)
			{
				...
				if (occupiesTile(tileLocation, true) && !isTilePassable(tileLocation) && data.GetActionAtTile(xTile - tileX.Value, yTile - tileY.Value) != null)
				{
					return true;
				}
			}
		...
		public virtual void performActionOnBuildingPlacement()
		{
			...
			foreach (BuildingPlacementTile additionalPlacementTile in GetAdditionalPlacementTiles())
			{
				...
				{
					Vector2 currentGlobalTilePosition2 = new Vector2((int)tileX + areaTile.X, (int)tileY + areaTile.Y);
					TerrainFeature feature;
					if ((!onlyNeedsToBePassable || (location.terrainFeatures.TryGetValue(currentGlobalTilePosition2, out feature) && !feature.isPassable())) && (!location.terrainFeatures.ContainsKey(currentGlobalTilePosition2) || !(location.terrainFeatures[currentGlobalTilePosition2] is Flooring) || GetData() == null || !GetData().AllowsFlooringUnderneath))
					{
		...
		{
			BuildingData data = GetData();
			LoadFromBuildingData(data, false, true);
			...
			if (!magical)
			{
				...
				for (int x2 = tileX; x2 < (int)tileX + (int)tilesWide; x2++)
				{
					for (int y2 = tileY; y2 < (int)tileY + (int)tilesHigh; y2++)
					{
						for (int j = 0; j < 5; j++)
						{
							location.temporarySprites.Add(new TemporaryAnimatedSprite(Game1.random.Choose(46, 12), new Vector2(x2, y2) * 64f + new Vector2(Game1.random.Next(-16, 32), Game1.random.Next(-16, 32)), Color.White, 10, Game1.random.NextBool())
							{
						...
						location.temporarySprites.Add(new TemporaryAnimatedSprite(14, new Vector2(x2, y2) * 64f + new Vector2(Game1.random.Next(-16, 32), Game1.random.Next(-16, 32)), Color.White, 10, Game1.random.NextBool()));
					}
				}
				for (int k = 0; k < 8; k++)
				{
					DelayedAction.playSoundAfterDelay("dirtyHit", 250 + k * 150, location, buildingCenter, -1, true);
				}
			}
			else
			{
				for (int i = 0; i < 8; i++)
				{
					DelayedAction.playSoundAfterDelay("dirtyHit", 100 + i * 210, location, buildingCenter, -1, true);
				}
				if (Game1.player == who)
				{
					Game1.flashAlpha = 2f;
			...
			Cabin cabin = GetIndoors() as Cabin;
		...
		{
			(GetIndoors() as Cabin)?.DeleteFarmhand();
			if (indoors.Value != null)
			{
		...
		public virtual void BeforeDemolish()
		{
			...
			CS_0024_003C_003E8__locals0.quest_items = new List<Item>();
			ForEachItemExcludingInterior(delegate(Item item)
			{
				CS_0024_003C_003E8__locals0._003CBeforeDemolish_003Eg__CollectQuestItem_007C0(item);
				return true;
			});
			if (indoors.Value != null)
			{
			...
			if (CS_0024_003C_003E8__locals0.quest_items.Count > 0)
			{
				Game1.showGlobalMessage(Game1.content.LoadString("Strings\\StringsFromCSFiles:NewLostAndFoundItems"));
				for (int i = 0; i < CS_0024_003C_003E8__locals0.quest_items.Count; i++)
				{
					Game1.player.team.returnedDonations.Add(CS_0024_003C_003E8__locals0.quest_items[i]);
				}
			}
		}
		...
		{
			(location as Farm)?.UnsetFarmhouseValues();
		}
		...
		public virtual void Update(GameTime time)
		{
			...
			if (farmhouse != null)
			{
				...
				if (lastHouseUpgradeLevel != upgradeLevel)
				{
					...
					if (chimneyString2 != null)
					{
						...
						chimneyPosition.X = int.Parse(split2[0]);
						chimneyPosition.Y = int.Parse(split2[1]);
					}
				}
			}
			...
			if (chimneyTimer <= 0)
			{
				if (interior.hasActiveFireplace())
				{
					...
					TemporaryAnimatedSprite sprite = TemporaryAnimatedSprite.GetTemporaryAnimatedSprite("LooseSprites\\Cursors", new Microsoft.Xna.Framework.Rectangle(372, 1956, 10, 10), new Vector2(cornerPosition.X + cornerOffset.X, cornerPosition.Y + cornerOffset.Y) + chimneyPosition * 4f + new Vector2(-8f, -12f), false, 0.002f, Color.Gray);
					sprite.alpha = 0.75f;
		...
		public virtual void showDestroyedAnimation(GameLocation location)
		{
			for (int x = tileX; x < (int)tileX + (int)tilesWide; x++)
			{
				...
				{
					location.temporarySprites.Add(new TemporaryAnimatedSprite(362, Game1.random.Next(30, 90), 6, 1, new Vector2(x * 64, y * 64) + new Vector2(Game1.random.Next(-16, 16), Game1.random.Next(-16, 16)), false, Game1.random.NextBool())
					{
					...
					location.temporarySprites.Add(new TemporaryAnimatedSprite("LooseSprites\\Cursors", new Microsoft.Xna.Framework.Rectangle(276, 1985, 12, 11), new Vector2(x, y) * 64f + new Vector2(32f, -32f) + new Vector2(Game1.random.Next(-32, 32), Game1.random.Next(-16, 16)), false, 0f, Color.White)
					{
						interval = 30f,
			...
			for (int k = 0; k < convertAmount; k++)
			{
				...
				for (int i = 0; i < conversion.ProducedItems.Count; i++)
				{
					...
					if (GameStateQuery.CheckConditions(producedItem.Condition, GetParentLocation()))
					{
						...
						Item item4 = destinationChest.addItem(item);
		...
		public virtual Microsoft.Xna.Framework.Rectangle getSourceRect()
		{
			...
			if (data != null)
			{
				...
				if (buildingType.Value == "Greenhouse")
				{
					Farm farm = GetParentLocation() as Farm;
					if (farm != null && !farm.greenhouseUnlocked)
					{
						rect.Y -= rect.Height;
					}
				}
		...
		protected virtual GameLocation createIndoors(BuildingData data, string nameOfIndoorsWithoutUnique)
		{
			...
			if (data != null && !string.IsNullOrEmpty(data.IndoorMap))
			{
				...
				if (data.IndoorMapType != null)
				{
					...
					if ((object)locationType == null || exception != null)
					{
						...
						defaultInterpolatedStringHandler.AppendFormatted(buildingType.Value);
			...
			if (lcl_indoors != null)
			{
				lcl_indoors.uniqueName.Value = nameOfIndoorsWithoutUnique + GuidHelper.NewGuid().ToString();
		...
		public virtual void load()
		{
			...
			else
			{
				...
				if (indoorInstance != null && indoors.Value != null)
				{
					...
					AnimalHouse house = indoors.Value as AnimalHouse;
					if (house != null)
					{
						(indoorInstance as AnimalHouse)?.animalsThatLiveHere.Set(house.animalsThatLiveHere);
					}
		...
		public virtual bool isTilePassable(Vector2 tile)
		{
			...
			BuildingData data = GetData();
			if (data != null && occupiesTile(tile, true))
			{
		...
		public virtual void draw(SpriteBatch b)
		{
			...
			if (data != null)
			{
				foreach (Chest chest2 in buildingChests)
				{
					BuildingChest chestData = GetBuildingChestData(data, chest2.Name);
		...
		public virtual void drawInConstruction(SpriteBatch b)
		{
			...
			if ((bool)magical || (int)daysOfConstructionLeft <= 0)
			{
				...
				if ((bool)magical)
				{
					...
					{
						b.Draw(Game1.mouseCursors, Game1.GlobalToLocal(Game1.viewport, new Vector2((int)tileX * 64 + j * 16, (float)((int)tileY * 64 - mainSourceRect.Height * 4 + (int)tilesHigh * 64) + (float)(mainSourceRect.Height * 4) * (1f - drawPercentageReal))) + new Vector2(Game1.random.Next(-1, 2), Game1.random.Next(-1, 2) - ((j % 2 == 0) ? 32 : 8)), new Microsoft.Xna.Framework.Rectangle(536 + ((int)newConstructionTimer + j * 4) % 56 / 8 * 8, 1945, 8, 8), (j % 2 == 1) ? (Color.Pink * alpha) : (Color.LightPink * alpha), 0f, new Vector2(0f, 0f), 4f + (float)Game1.random.Next(100) / 100f, SpriteEffects.None, (float)(((int)tileY + (int)tilesHigh) * 64) / 10000f + 0.0001f);
						if (j % 2 == 0)
						{
							b.Draw(Game1.mouseCursors, Game1.GlobalToLocal(Game1.viewport, new Vector2((int)tileX * 64 + j * 16, (float)((int)tileY * 64 - mainSourceRect.Height * 4 + (int)tilesHigh * 64) + (float)(mainSourceRect.Height * 4) * (1f - drawPercentageReal))) + new Vector2(Game1.random.Next(-1, 2), Game1.random.Next(-1, 2) + ((j % 2 == 0) ? 32 : 8)), new Microsoft.Xna.Framework.Rectangle(536 + ((int)newConstructionTimer + j * 4) % 56 / 8 * 8, 1945, 8, 8), Color.White * alpha, 0f, new Vector2(0f, 0f), 4f + (float)Game1.random.Next(100) / 100f, SpriteEffects.None, (float)(((int)tileY + (int)tilesHigh) * 64) / 10000f + 0.0001f);
						}
					}
					return;
				}
