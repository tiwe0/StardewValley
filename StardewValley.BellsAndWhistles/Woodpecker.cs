		...
		public override bool update(GameTime time, GameLocation environment)
		{
			...
			if (!flyingAway && peckTimer <= 0 && sprite.CurrentAnimation == null)
			{
				...
				for (int i = 0; i < nibbles; i++)
				{
					...
					anim.Add(new FarmerSprite.AnimationFrame(baseFrame + 1, 100, false, false, playPeck));
				}
				anim.Add(new FarmerSprite.AnimationFrame(baseFrame, 200, false, false, donePecking));
			...
			if (characterCheckTimer < 0)
			{
				...
				if ((f != null || (bool)tree.stump) && !flyingAway)
				{
					...
					sprite.setCurrentAnimation(new List<FarmerSprite.AnimationFrame>
					{
						...
						new FarmerSprite.AnimationFrame((short)(baseFrame + 3), 60, false, flip, playFlap),
