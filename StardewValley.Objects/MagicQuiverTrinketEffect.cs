	...
	public class MagicQuiverTrinketEffect : TrinketEffect
	{
		...
		public override void Update(Farmer farmer, GameTime time, GameLocation location)
		{
			...
			if (projectileTimer >= projectileDelayMS)
			{
				...
				Monster closest_monster = Utility.findClosestMonsterWithinRange(location, farmer.getStandingPosition(), 500, true);
				if (closest_monster != null && !closest_monster.Name.Equals("Truffle Crab"))
				{
					...
					BasicProjectile p = new BasicProjectile(Game1.random.Next(mindamage, maxdamage + 1), 16, 0, 0, 0f, motion.X, motion.Y, farmer.getStandingPosition() - new Vector2(32f, 48f), null, null, null, false, true, location, farmer);
