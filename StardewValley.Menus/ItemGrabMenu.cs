...
namespace StardewValley.Menus
{
	public class ItemGrabMenu : MenuWithInventory
	{
		...
		public class TransferredItemSprite
		{
			...
			{
				item.drawInMenu(b, position, 1f, alpha, 0.9f, StackDrawType.Hide, Color.White, false);
			}
		}
			...
			: base(null, true, true)
		{
			...
			ItemsToGrabMenu = new InventoryMenu(xPositionOnScreen + 32, yPositionOnScreen, false, inventory);
			trashCan.myID = 106;
		...
		public ItemGrabMenu(IList<Item> inventory, bool reverseGrab, bool showReceivingMenu, InventoryMenu.highlightThisItem highlightFunction, behaviorOnItemSelect behaviorOnItemSelectFunction, string message, behaviorOnItemSelect behaviorOnItemGrab = null, bool snapToBottom = false, bool canBeExitedWithKey = false, bool playRightClickSound = true, bool allowRightClick = true, bool showOrganizeButton = false, int source = 0, Item sourceItem = null, int whichSpecialButton = -1, object context = null, ItemExitBehavior heldItemExitBehavior = ItemExitBehavior.ReturnToPlayer, bool allowExitWithHeldItem = false)
			: base(highlightFunction, true, true, 0, 0, 64, heldItemExitBehavior, allowExitWithHeldItem)
		{
			this.source = source;
			...
			if (source == 1)
			{
				...
				if (sourceChest != null && (sourceChest.SpecialChestType == Chest.SpecialChestTypes.None || sourceChest.SpecialChestType == Chest.SpecialChestTypes.BigChest))
				{
			...
			if (source == 1)
			{
				...
				if (chest != null && chest.GetActualCapacity() != 36)
				{
					...
					ItemsToGrabMenu = new InventoryMenu(Game1.uiViewport.Width / 2 - containerWidth / 2, yPositionOnScreen + ((capacity < 70) ? 64 : (-21)), false, inventory, highlightFunction, capacity, rows);
			...
			ItemsToGrabMenu = new InventoryMenu(xPositionOnScreen + 32, yPositionOnScreen, false, inventory, highlightFunction);
			goto IL_04d3;
			IL_04d3:
			ItemsToGrabMenu.populateClickableComponentList();
		...
		{
			ItemGrabMenu itemGrabMenu = new ItemGrabMenu(items).setEssential(true);
		...
		public void SetupBorderNeighbors()
		{
			...
			for (int j = 0; j < GetColumnCount(); j++)
			{
				...
				{
					clickableComponent = inventory.inventory[j];
					if (!shippingBin)
					{
						...
						upNeighborID = ((ItemsToGrabMenu.inventory.Count > j) ? (53910 + j) : 53910);
					}
					else
					{
						upNeighborID = 12598;
		...
		protected override void customSnapBehavior(int direction, int oldRegion, int oldID)
		{
			...
			case 2:
			{
				...
				if (inventory != null)
				{
					int inventoryRowLength2 = inventory.capacity / inventory.rows;
					int diff2 = GetColumnCount() - inventoryRowLength2;
					currentlySnappedComponent = getComponentWithID((oldRegion != 12598) ? Math.Max(0, Math.Min((oldID - 53910) % GetColumnCount() - diff2 / 2, inventory.capacity / inventory.rows - diff2 / 2)) : 0);
				}
				else
				{
		...
		public override void receiveRightClick(int x, int y, bool playSound = true)
		{
			...
			{
				base.heldItem = ItemsToGrabMenu.rightClick(x, y, base.heldItem, false);
				...
				if (obj != null && obj?.QualifiedItemId == "(O)434")
				{
					base.heldItem = null;
					exitThisMenu(false);
		...
		public override void gameWindowSizeChanged(Rectangle oldBounds, Rectangle newBounds)
		{
			...
			if (source == 1)
			{
				...
				if (chest != null && (chest.SpecialChestType == Chest.SpecialChestTypes.None || chest.SpecialChestType == Chest.SpecialChestTypes.BigChest))
				{
					chestColorPicker = new DiscreteColorPicker(xPositionOnScreen, yPositionOnScreen - 64 - IClickableMenu.borderWidth * 2, chest.playerChoiceColor.Value, new Chest(true, sourceItem.ItemId));
				}
			}
		}
		...
		public override void receiveLeftClick(int x, int y, bool playSound = true)
		{
			...
			if (base.heldItem == null && showReceivingMenu)
			{
				base.heldItem = ItemsToGrabMenu.leftClick(x, y, base.heldItem, false);
				if (base.heldItem != null && behaviorOnItemGrab != null)
				{
					behaviorOnItemGrab(base.heldItem, Game1.player);
					ItemGrabMenu itemGrabMenu3 = Game1.activeClickableMenu as ItemGrabMenu;
					if (itemGrabMenu3 != null)
					{
						itemGrabMenu3.setSourceItem(sourceItem);
						if (Game1.options.SnappyMenus)
						{
							itemGrabMenu3.currentlySnappedComponent = currentlySnappedComponent;
							itemGrabMenu3.snapCursorToCurrentSnappedComponent();
						}
					}
				}
				...
				if (stardrop != null && stardrop?.QualifiedItemId == "(O)434")
				{
					base.heldItem = null;
					exitThisMenu(false);
			...
			if (organizeButton != null && organizeButton.containsPoint(x, y))
			{
				...
				base.heldItem = null;
				ItemGrabMenu itemGrabMenu = new ItemGrabMenu(ItemsToGrabMenu.actualInventory, false, true, InventoryMenu.highlightAllItems, behaviorFunction, null, behaviorOnItemGrab, false, true, true, true, true, source, sourceItem, whichSpecialButton, context, HeldItemExitBehavior, AllowExitWithHeldItem).setEssential(essential);
			...
			else if (junimoNoteIcon != null && junimoNoteIcon.containsPoint(x, y))
			{
				if (readyToClose())
				{
					Game1.activeClickableMenu = new JunimoNoteMenu(true)
					{
						menuToReturnTo = this
					};
				}
				...
				for (int j = 0; j < inventory.actualInventory.Count; j++)
				{
					...
					while (inventory_item.Stack > 0)
					{
						...
						if (overflow_stack == null)
						{
							...
							{
								if (ItemsToGrabMenu.actualInventory[l] != null && ItemsToGrabMenu.actualInventory[l].canStackWith(chest_item) && ItemsToGrabMenu.actualInventory[l].getRemainingStackSpace() > 0)
								{
									overflow_stack = ItemsToGrabMenu.actualInventory[l];
									break;
								}
							}
						}
		...
		public override void receiveGamePadButton(Buttons b)
		{
			...
			if (b == Buttons.RightShoulder)
			{
				...
				else
				{
					...
					foreach (ClickableComponent component2 in allClickableComponents)
					{
		...
		public override void update(GameTime time)
		{
				...
				if (source == 4)
				{
					...
					for (int j = 0; j < items.Count; j++)
					{
						if (items[j]?.QualifiedItemId == "(O)434")
						{
							...
							exitThisMenu(false);
							Game1.player.eatObject(items[j] as Object, true);
							return;
						}
					}
				}
			...
			Chest chest = sourceItem as Chest;
			if (chest != null && _sourceItemInCurrentLocation)
			{
		...
		public override void draw(SpriteBatch b)
		{
			...
			if (message != null)
			{
				Game1.drawDialogueBox(Game1.uiViewport.Width / 2, ItemsToGrabMenu.yPositionOnScreen + ItemsToGrabMenu.height / 2, false, false, message);
			}
			goto IL_03c8;
			IL_0380:
			...
			if (hoverText != null && (hoveredItem == null || hoveredItem == null || ItemsToGrabMenu == null))
			{
				if (hoverAmount > 0)
				{
					IClickableMenu.drawToolTip(b, hoverText, "", null, true, -1, 0, null, -1, null, hoverAmount);
				}
				else
				{
