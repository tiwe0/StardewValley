...
namespace StardewValley.Projectiles
{
	public class DebuffingProjectile : Projectile
	{
		...
		public override bool update(GameTime time, GameLocation location)
		{
			if (debuff == "frozen")
			{
				...
				if (periodicEffectTimer > 50f)
				{
					...
					location.temporarySprites.Add(new TemporaryAnimatedSprite("TileSheets\\Projectiles", new Rectangle(32, 32, 16, 16), 9999f, 1, 1, position.Value, false, false, 1f, 0.01f, Color.White, 4f, 0f, 0f, 0f)
					{
		...
		protected virtual void explosionAnimation(GameLocation location)
		{
			if (!(debuff == "frozen"))
			{
				Game1.multiplayer.broadcastSprites(location, new TemporaryAnimatedSprite(352, Game1.random.Next(100, 150), 2, 1, position.Value, false, false));
			}
		}

		public override void behaviorOnCollisionWithMonster(NPC n, GameLocation location)
		{
			if (!damagesMonsters || !(n is Monster) || !(debuff == "frozen"))
			{
				return;
			}
			...
			if (leaper == null || !leaper.leaping.Value)
			{
				...
				{
					location.playSound("frozen");
					Game1.multiplayer.broadcastSprites(location, new TemporaryAnimatedSprite("LooseSprites\\Cursors2", new Rectangle(118, 227, 16, 13), new Vector2(0f, 0f), false, 0f, Color.White)
					{
