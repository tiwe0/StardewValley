	...
	public class Intro : IMinigame
	{
		...
		public void draw(SpriteBatch b)
		{
			...
			case 1:
			{
				...
				SpriteText.drawString(b, Game1.content.LoadString("Strings\\StringsFromCSFiles:Game1.cs.3689"), x, y, 999, -1, 999, 1f, 1f, false, 0);
				b.End();
				break;
			}
			case 2:
		...
		public void drawRoadArea(SpriteBatch b)
		{
			...
			for (int n = 0; n < screenWidth / 112 + 2; n++)
			{
				if (sky[n] == 0)
				{
					b.Draw(texture, new Vector2(0f - skyPosition + (float)(n * 112) - (float)(n * 2), -16f), new Rectangle(129, 0, 110, 96), Color.White);
				}
				else
				{
					b.Draw(sourceRectangle: new Rectangle(128, 0, 1, 96), texture: texture, destinationRectangle: new Rectangle((int)(0f - skyPosition) - 1 + n * 112 - n * 2, -16, 114, 96), color: Color.White);
				}
			}
			for (int m = 0; m < 12; m++)
			{
				b.Draw(Game1.mouseCursors, new Vector2(-10f + (0f - valleyPosition) / 2f + (float)(m * 639) - (float)(m * 2), 70f), new Rectangle(0, 886, 639, 148), Color.White, 0f, Vector2.Zero, 1f, SpriteEffects.None, 0.08f);
				b.Draw(Game1.mouseCursors, new Vector2(0f - valleyPosition + (float)(m * 639) - (float)(m * 2), 80f), new Rectangle(0, 737, 639, 120), Color.White, 0f, Vector2.Zero, 1f, SpriteEffects.None, 0.08f);
			}
			for (int l = 0; l < 8; l++)
			{
				b.Draw(treeStripTexture, new Vector2((float)(l * 256) - treePosition, 110f), new Rectangle(0, 0, 256, 64), Color.White);
			}
