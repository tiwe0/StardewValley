...
namespace StardewValley
{
	public class Fence : Object
	{
		...
		public override bool checkForAction(Farmer who, bool justCheckingForActivity = false)
		{
			...
			foreach (Vector2 v in Utility.getAdjacentTileLocations(tileLocation.Value))
			{
				...
				if (location.objects.TryGetValue(v, out obj))
				{
					...
					if (fence != null && (bool)fence.isGate)
					{
		...
		public virtual void toggleGate(bool open, bool is_toggling_counterpart = false, Farmer who = null)
		{
			...
			case 100:
			{
				Vector2 neighborTile4 = tileLocation.Value + new Vector2(-1f, 0f);
				Object neighbor4;
				if (location.objects.TryGetValue(neighborTile4, out neighbor4))
				{
					Fence fence4 = neighbor4 as Fence;
					if (fence4 != null && (bool)fence4.isGate && fence4.getDrawSum() == 10)
					{
						fence4.toggleGate((int)gatePosition != 0, true, who);
					}
				}
				break;
			}
			case 10:
			{
				Vector2 neighborTile3 = tileLocation.Value + new Vector2(1f, 0f);
				Object neighbor3;
				if (location.objects.TryGetValue(neighborTile3, out neighbor3))
				{
					Fence fence3 = neighbor3 as Fence;
					if (fence3 != null && (bool)fence3.isGate && fence3.getDrawSum() == 100)
					{
						fence3.toggleGate((int)gatePosition != 0, true, who);
					}
				}
				break;
			}
			case 1000:
			{
				Vector2 neighborTile2 = tileLocation.Value + new Vector2(0f, 1f);
				Object neighbor2;
				if (location.objects.TryGetValue(neighborTile2, out neighbor2))
				{
					Fence fence2 = neighbor2 as Fence;
					if (fence2 != null && (bool)fence2.isGate && fence2.getDrawSum() == 500)
					{
						fence2.toggleGate((int)gatePosition != 0, true, who);
					}
				}
				break;
			}
			case 500:
			{
				...
				if (location.objects.TryGetValue(neighborTile, out neighbor))
				{
					...
					if (fence != null && (bool)fence.isGate && fence.getDrawSum() == 1000)
					{
						fence.toggleGate((int)gatePosition != 0, true, who);
					}
				}
				break;
			}
		...
		public override bool performToolAction(Tool t)
		{
			...
			else if (isGate.Value && (t is Axe || t is Pickaxe))
			{
				...
				Game1.createRadialDebris(location, 12, (int)tileLocation.X, (int)tileLocation.Y, 6, false);
			...
			else if (!isGate.Value && IsValidRemovalTool(t))
			{
				...
				Game1.createRadialDebris(location, removalDebrisType, (int)tileLocation.X, (int)tileLocation.Y, 6, false);
					...
					if (drawSum == 1500 || drawSum == 110 || drawSum == 1000 || drawSum == 10 || drawSum == 100 || drawSum == 500)
					{
						...
						case 1000:
							...
							if (location.objects.ContainsKey(neighbor) && location.objects[neighbor] is Fence && (bool)((Fence)location.objects[neighbor]).isGate)
							{
								...
								if (neighbor_sum4 != 500 && neighbor_sum4 != 1500)
								{
									return false;
								}
							}
						...
						case 500:
							...
							if (location.objects.ContainsKey(neighbor) && location.objects[neighbor] is Fence && (bool)((Fence)location.objects[neighbor]).isGate)
							{
								...
								if (neighbor_sum3 != 1000 && neighbor_sum3 != 1500)
								{
									return false;
								}
							}
						...
						})
						{
							...
							if (!(adjacent_tile == neighbor) && location.objects.TryGetValue(adjacent_tile, out adjacent))
							{
								...
								if (fence != null && (bool)fence.isGate && fence.Type == base.Type)
								{
									return false;
								}
							}
						...
						isGate.Value = true;
						FenceData gateData;
						if (TryGetData("325", out gateData))
						{
		...
		public override void draw(SpriteBatch b, int x, int y, float alpha = 1f)
		{
			...
			if (health.Value > 1f || repairQueued.Value)
			{
				...
				if ((bool)isGate)
				{
					...
					{
					case 10:
						b.Draw(fenceTexture.Value, Game1.GlobalToLocal(Game1.viewport, offset2 + new Vector2(x * 64 - 16, y * 64 - 128)), new Rectangle(((int)gatePosition == 88) ? 24 : 0, 192, 24, 48), Color.White, 0f, Vector2.Zero, 4f, SpriteEffects.None, (float)(y * 64 + 32 + 1) / 10000f);
						return;
					case 100:
						b.Draw(fenceTexture.Value, Game1.GlobalToLocal(Game1.viewport, offset2 + new Vector2(x * 64 - 16, y * 64 - 128)), new Rectangle(((int)gatePosition == 88) ? 24 : 0, 240, 24, 48), Color.White, 0f, Vector2.Zero, 4f, SpriteEffects.None, (float)(y * 64 + 32 + 1) / 10000f);
						return;
					case 1000:
						b.Draw(fenceTexture.Value, Game1.GlobalToLocal(Game1.viewport, offset2 + new Vector2(x * 64 + 20, y * 64 - 64 - 20)), new Rectangle(((int)gatePosition == 88) ? 24 : 0, 288, 24, 32), Color.White, 0f, Vector2.Zero, 4f, SpriteEffects.None, (float)(y * 64 - 32 + 2) / 10000f);
						return;
					case 500:
						b.Draw(fenceTexture.Value, Game1.GlobalToLocal(Game1.viewport, offset2 + new Vector2(x * 64 + 20, y * 64 - 64 - 20)), new Rectangle(((int)gatePosition == 88) ? 24 : 0, 320, 24, 32), Color.White, 0f, Vector2.Zero, 4f, SpriteEffects.None, (float)(y * 64 + 96 - 1) / 10000f);
						return;
					case 110:
						b.Draw(fenceTexture.Value, Game1.GlobalToLocal(Game1.viewport, offset2 + new Vector2(x * 64 - 16, y * 64 - 64)), new Rectangle(((int)gatePosition == 88) ? 24 : 0, 128, 24, 32), Color.White, 0f, Vector2.Zero, 4f, SpriteEffects.None, (float)(y * 64 + 32 + 1) / 10000f);
						return;
					case 1500:
						b.Draw(fenceTexture.Value, Game1.GlobalToLocal(Game1.viewport, offset2 + new Vector2(x * 64 + 20, y * 64 - 64 - 20)), new Rectangle(((int)gatePosition == 88) ? 16 : 0, 160, 16, 16), Color.White, 0f, Vector2.Zero, 4f, SpriteEffects.None, (float)(y * 64 - 32 + 2) / 10000f);
						b.Draw(fenceTexture.Value, Game1.GlobalToLocal(Game1.viewport, offset2 + new Vector2(x * 64 + 20, y * 64 - 64 + 44)), new Rectangle(((int)gatePosition == 88) ? 16 : 0, 176, 16, 16), Color.White, 0f, Vector2.Zero, 4f, SpriteEffects.None, (float)(y * 64 + 96 - 1) / 10000f);
						return;
					}
