	...
	public class Dialogue
	{
		...
		public Dialogue(NPC speaker, string translationKey, string dialogueText)
		{
			...
			catch (Exception ex)
			{
				...
				defaultInterpolatedStringHandler.AppendFormatted(speaker?.Name);
		...
		public int getPortraitIndex()
		{
			...
			{
				int index;
				if (!int.TryParse(CurrentEmotion.Substring(1), out index))
				{
					return 0;
				}
				return index;
		...
		protected virtual void parseDialogueString(string masterString, string translationKey)
		{
			...
			{
				masterString = multipleWeeklyDialogueSplit[(long)(Game1.stats.DaysPlayed / 7u) % (long)multipleWeeklyDialogueSplit.Length];
			}
				...
				if (curDialogue.StartsWith('$'))
				{
					...
					case "$action":
						dialogues.Add(new DialogueLine("", delegate
						{
							...
							if (!TriggerActionManager.TryRunAction(commandArgs, out error, out exception))
							{
								...
								defaultInterpolatedStringHandler.AppendFormatted(translationKey ?? speaker?.Name ?? ("\"" + masterString + "\""));
								defaultInterpolatedStringHandler.AppendLiteral(": ");
					...
					case "$t":
						dialogues.Add(new DialogueLine("", delegate
						{
							...
							string error2;
							int value2;
							if (!ArgUtility.TryGet(array2, 0, out value, out error2, false) || !ArgUtility.TryGetOptionalInt(array2, 1, out value2, out error2, 4))
							{
								IGameLogger log = Game1.log;
								DefaultInterpolatedStringHandler defaultInterpolatedStringHandler2 = new DefaultInterpolatedStringHandler(30, 3);
								defaultInterpolatedStringHandler2.AppendLiteral("Failed to parse ");
								defaultInterpolatedStringHandler2.AppendFormatted("$t");
								defaultInterpolatedStringHandler2.AppendLiteral(" token for ");
								defaultInterpolatedStringHandler2.AppendFormatted(translationKey ?? speaker?.Name ?? ("\"" + masterString + "\""));
								defaultInterpolatedStringHandler2.AppendLiteral(": ");
								defaultInterpolatedStringHandler2.AppendFormatted(error2);
								defaultInterpolatedStringHandler2.AppendLiteral(".");
					...
					case "$p":
					{
						...
						bool choseOne = false;
						for (int k = 0; k < prerequisiteSplit.Length; k++)
						{
							if (farmer.DialogueQuestionsAnswered.Contains(prerequisiteSplit[k]))
							{
								choseOne = true;
								break;
							}
					...
					case "$y":
					{
						...
						{
							playerResponses.Add(new NPCDialogueResponse(null, -1, "quickResponse" + l, Game1.parseText(rawSplit[l])));
							quickResponses.Add(rawSplit[l + 1].Replace("*", "#$b#"));
						}
						break;
					}
					default:
		...
		public virtual void prepareDialogueForDisplay()
		{
			...
			if (dialogues.Count > 0 && speaker != null && (bool)speaker.shouldWearIslandAttire && Game1.player.friendshipData.TryGetValue(speaker.Name, out friendship) && friendship.IsDivorced() && CurrentEmotion == "$u")
			{
		...
		public virtual void prepareCurrentDialogueForDisplay()
		{
			...
			if (currentDialogue.Contains('['))
			{
				...
				do
				{
					...
					if (item != null)
					{
						if (farmer.addItemToInventoryBool(item, true))
						{
							farmer.showCarrying();
						}
						else
						{
							farmer.addItemByMenuIfNecessary(item, null, true);
						}
					}
		...
		{
			str = applyGenderSwitch(str, true);
			if (str.Contains('%'))
			{
		...
		public static string applyGenderSwitchBlocks(Gender gender, string str)
		{
			...
			while (true)
			{
				...
				case Gender.Male:
					...
					break;
				case Gender.Female:
					newSubstr = ArgUtility.Get(parts, 1, parts[0]);
					break;
				default:
					newSubstr = ArgUtility.Get(parts, 2, parts[0]);
					break;
				}
		...
		public static string randomName()
		{
			...
			case LocalizedContentManager.LanguageCode.zh:
			{
				...
				return new Random().Choose(names2);
			}
			case LocalizedContentManager.LanguageCode.ru:
			{
				string[] names3 = new string[50]
				{
				...
				return new Random().Choose(names3);
			}
			default:
			{
				...
				{
					name = ((j % 2 != 0) ? (name + Game1.random.Choose(vowels)) : (name + Game1.random.Choose(consonants)));
