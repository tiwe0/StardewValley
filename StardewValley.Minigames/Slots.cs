...
namespace StardewValley.Minigames
{
	public class Slots : IMinigame
	{
		...
		public Slots(int toBet = -1, bool highStakes = false)
		{
			...
			spinButton10 = new ClickableComponent(new Rectangle((int)pos.X, (int)pos.Y, 104, 52), Game1.content.LoadString("Strings\\StringsFromCSFiles:Slots.cs.12117"));
			pos = Utility.getTopLeftPositionForCenteringOnScreen(Game1.viewport, 124, 52, -16, 96);
			spinButton100 = new ClickableComponent(new Rectangle((int)pos.X, (int)pos.Y, 124, 52), Game1.content.LoadString("Strings\\StringsFromCSFiles:Slots.cs.12118"));
			pos = Utility.getTopLeftPositionForCenteringOnScreen(Game1.viewport, 96, 52, -16, 160);
			doneButton = new ClickableComponent(new Rectangle((int)pos.X, (int)pos.Y, 96, 52), Game1.content.LoadString("Strings\\StringsFromCSFiles:NameSelect.cs.3864"));
		...
		public void draw(SpriteBatch b)
		{
			...
			if (showResult)
			{
				SpriteText.drawString(b, "+" + payoutModifier * (float)currentBet, Game1.graphics.GraphicsDevice.Viewport.Width / 2 - 372, spinButton10.bounds.Y - 64 + 8, 9999, -1, 9999, 1f, 1f, false, -1, "", SpriteText.color_White);
			}
			...
			Utility.drawWithShadow(b, Game1.mouseCursors, new Vector2(Game1.graphics.GraphicsDevice.Viewport.Width / 2 - 376 + 4, Game1.graphics.GraphicsDevice.Viewport.Height / 2 - 120 + 4), new Rectangle(211, 373, 9, 10), Color.White, 0f, Vector2.Zero, 4f, false, 1f);
			...
			SpriteText.drawString(b, "x2", (int)basePos.X + 192 + 16, (int)basePos.Y + 24, 9999, -1, 99999, 1f, 0.88f, false, -1, "", SpriteText.color_White);
			...
			SpriteText.drawString(b, "x3", (int)basePos.X + 192 + 16, (int)basePos.Y + 68 + 24, 9999, -1, 99999, 1f, 0.88f, false, -1, "", SpriteText.color_White);
			for (int j = 0; j < 8; j++)
			{
				int which = j;
				switch (j)
				...
				SpriteText.drawString(b, "x" + payout, (int)basePos.X + 192 + 16, (int)basePos.Y + (j + 2) * 68 + 24, 9999, -1, 99999, 1f, 0.88f, false, -1, "", SpriteText.color_White);
			}
			IClickableMenu.drawTextureBox(b, Game1.mouseCursors, new Rectangle(379, 357, 3, 3), (int)basePos.X - 640, (int)basePos.Y, 1024, 704, Color.Red, 4f, false);
			for (int k = 1; k < 8; k++)
			{
				IClickableMenu.drawTextureBox(b, Game1.mouseCursors, new Rectangle(379, 357, 3, 3), (int)basePos.X - 640 - 4 * k, (int)basePos.Y - 4 * k, 1024 + 8 * k, 704 + 8 * k, Color.Red * (1f - (float)k * 0.15f), 4f, false);
			}
			for (int l = 0; l < 17; l++)
			{
				IClickableMenu.drawTextureBox(b, Game1.mouseCursors, new Rectangle(147, 472, 3, 3), (int)basePos.X - 640 + 8, (int)basePos.Y + l * 4 * 3 + 12, (int)(608f - (float)(l * 64) * 1.2f + (float)(l * l * 4) * 0.7f), 4, new Color(l * 25, (l > 8) ? (l * 10) : 0, 255 - l * 25), 4f, false);
			}
