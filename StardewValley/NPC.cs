...
namespace StardewValley
{
	...
	public class NPC : Character, IComparable
	{
		...
		public virtual void reloadDefaultLocation()
		{
			...
			Point tile;
			int direction;
			if (data != null && ReadNpcHomeData(data, base.currentLocation, out locationName, out tile, out direction))
			{
		...
		public virtual void ChooseAppearance(LocalizedContentManager content = null)
		{
			...
			if (location.TryGetMapProperty("UniquePortrait", out uniquePortraitsProperty) && ArgUtility.SplitBySpace(uniquePortraitsProperty).Contains(base.Name))
			{
				...
				string errorPhrase7;
				appliedLegacyUniquePortraits = TryLoadPortraits(assetName, out errorPhrase7, content);
			...
			{
				string assetName2 = "Characters\\" + getTextureName() + "_" + location.Name;
				string errorPhrase8;
				appliedLegacyUniqueSprites = TryLoadSprites(assetName2, out errorPhrase8, content);
			...
			if (!IsMonster)
			{
				...
				if (data != null && data.Appearance?.Count > 0)
				{
					...
					{
						if (option2.Precedence > precedence || option2.IsIslandAttire != isWearingIslandAttire)
						{
							continue;
						}
			...
			if (!appliedLegacyUniquePortraits)
			{
				string defaultAsset2 = "Portraits/" + getTextureName();
				bool loaded2 = false;
				if (appearance != null && appearance.Portrait != null && appearance.Portrait != defaultAsset2)
				{
					string errorPhrase6;
					loaded2 = TryLoadPortraits(appearance.Portrait, out errorPhrase6, content);
					if (!loaded2)
					{
				...
				if (!loaded2 && isWearingIslandAttire)
				{
					string beachAsset2 = defaultAsset2 + "_Beach";
					if (content.DoesAssetExist<Texture2D>(beachAsset2))
					{
						string errorPhrase5;
						loaded2 = TryLoadPortraits(beachAsset2, out errorPhrase5, content);
						if (!loaded2)
						{
				...
				string errorPhrase4;
				if (!loaded2 && !TryLoadPortraits(defaultAsset2, out errorPhrase4, content))
				{
			...
			if (!appliedLegacyUniqueSprites)
			{
				...
				{
					string errorPhrase3;
					loaded = TryLoadSprites(appearance.Sprite, out errorPhrase3, content);
					if (!loaded)
					{
				...
				if (!loaded && isWearingIslandAttire)
				{
					...
					{
						string errorPhrase2;
						loaded = TryLoadSprites(beachAsset, out errorPhrase2, content);
						if (!loaded)
						{
		...
		{
			CharacterData data;
			TryGetData(character_name, out data);
		...
		private void doPlayRobinHammerAnimation()
		{
			...
			Sprite.AddFrame(new FarmerSprite.AnimationFrame(26, 300, false, false, robinHammerSound));
			Sprite.AddFrame(new FarmerSprite.AnimationFrame(27, 1000, false, false, robinVariablePause));
		...
		private void engagementResponse(Farmer who, bool asRoommate = false)
		{
			...
			if (asRoommate && DataLoader.EngagementDialogue(Game1.content).ContainsKey(base.Name + "Roommate0"))
			{
				...
				Dialogue attemptDialogue2 = StardewValley.Dialogue.TryGetDialogue(this, "Strings\\StringsFromCSFiles:" + base.Name + "_EngagedRoommate");
				if (attemptDialogue2 != null)
				{
					CurrentDialogue.Push(attemptDialogue2);
				}
				else
				{
		...
		public virtual bool tryToReceiveActiveObject(Farmer who, bool probe = false)
		{
			...
			ItemDeliveryQuest deliveryQuest = questOfTheDay as ItemDeliveryQuest;
			if (deliveryQuest == null)
			{
				FishingQuest fishingQuest = questOfTheDay as FishingQuest;
				if (fishingQuest != null && fishingQuest.checkIfComplete(this, -1, 1, null, activeObj.ItemId, probe))
				{
					if (!probe)
					{
						who.reduceActiveItemByOne();
			...
			case "(O)233":
				if (name == "Jas" && Utility.GetDayOfPassiveFestival("DesertFestival") > 0 && base.currentLocation is Desert && !who.mailReceived.Contains("Jas_IceCream_DF_" + Game1.year))
				{
					if (!probe)
					{
						...
						doEmote(16);
						CurrentDialogue.Clear();
						setNewDialogue("Strings\\1_6_Strings:Jas_IceCream", true);
						Game1.drawDialogue(this);
			...
			case "(O)897":
				if (!probe)
				{
					if (base.Name == "Pierre" && !Game1.player.hasOrWillReceiveMail("PierreStocklist"))
					{
						...
						setNewDialogue("Strings\\Characters:PierreStockListDialogue", true);
						Game1.drawDialogue(this);
			...
			{
				Dialogue dialogue3 = TryGetDialogue("reject_" + activeObj.ItemId);
				if (dialogue3 != null)
				{
					if (!probe)
					{
			...
			if ((bool)activeObj.questItem)
			{
				...
				{
					Dialogue dialogue2 = TryGetDialogue("accept_" + activeObj.ItemId);
					if (dialogue2 != null)
					{
						if (!probe)
						{
			...
			Dialogue dialogue = TryGetDialogue("RejectItem_" + activeObj.QualifiedItemId) ?? (from tag in activeObj.GetContextTags()
			...
			case "(O)809":
				...
				if (!probe)
				{
					faceTowardFarmerForPeriod(4000, 3, false, who);
				}
				...
				if (canReceiveGifts)
				{
					if (!probe)
					{
						...
						else
						{
							...
							{
								CurrentDialogue.Push(TryGetDialogue("RejectBouquet_VeryLowHearts") ?? TryGetDialogue("RejectBouquet") ?? (Game1.random.NextBool() ? new Dialogue(this, "Strings\\StringsFromCSFiles:NPC.cs.3958") : new Dialogue(this, "Strings\\StringsFromCSFiles:NPC.cs.3959", true)));
								Game1.drawDialogue(this);
							}
							...
							else
							{
								...
								CurrentDialogue.Push(TryGetDialogue("AcceptBouquet") ?? new Dialogue(this, "Strings\\StringsFromCSFiles:NPC.cs." + Game1.random.Choose("3962", "3963"), true));
				...
				if (canReceiveGifts)
				{
					if (!probe)
					{
						...
						if (who.isMarriedOrRoommates() || who.isEngaged())
						{
							...
							{
								CurrentDialogue.Push(TryGetDialogue("RejectMermaidPendant_PlayerWithSomeoneElse", who.getSpouse()?.displayName ?? who.spouse) ?? TryGetDialogue("RejectMermaidPendant") ?? new Dialogue(this, "Strings\\StringsFromCSFiles:NPC.cs." + Game1.random.Choose("3965", "3966"), true));
								Game1.drawDialogue(this);
							}
							else
							{
								CurrentDialogue.Push(TryGetDialogue("RejectMermaidPendant_PlayerWithSomeoneElse") ?? TryGetDialogue("RejectMermaidPendant") ?? (Game1.random.NextBool() ? new Dialogue(this, "Strings\\StringsFromCSFiles:NPC.cs.3967") : new Dialogue(this, "Strings\\StringsFromCSFiles:NPC.cs.3968", true)));
								Game1.drawDialogue(this);
							}
						}
						...
						else if ((bool)datable && friendship != null && friendship.Points < 2500)
						{
							...
							{
								CurrentDialogue.Push(TryGetDialogue("RejectMermaidPendant_Under10Hearts_AskedAgain") ?? TryGetDialogue("RejectMermaidPendant_Under10Hearts") ?? TryGetDialogue("RejectMermaidPendant") ?? new Dialogue(this, "Strings\\StringsFromCSFiles:NPC.cs." + Game1.random.Choose("3974", "3975"), true));
								Game1.drawDialogue(this);
		...
		public virtual bool checkAction(Farmer who, GameLocation l)
		{
			...
			{
				faceTowardFarmerForPeriod(6000, 3, false, who);
				return true;
			}
			...
			if (base.Name == who.spouse && who.IsLocalPlayer && Sprite.CurrentAnimation == null)
			{
				...
				{
					faceGeneralDirection(who.getStandingPosition(), 0, false, false);
					who.faceGeneralDirection(getStandingPosition(), 0, false, false);
					if (FacingDirection == 3 || FacingDirection == 1)
					{
						...
						if (who.getFriendshipHeartLevelForNPC(base.Name) > 9 && sleptInBed.Value)
						{
							...
							Sprite.ClearAnimation();
							Sprite.AddFrame(new FarmerSprite.AnimationFrame(spouseFrame, delay, false, flip, haltMe, true));
							if (!hasBeenKissedToday.Value)
							{
								who.changeFriendship(10, this);
								if (who.hasCurrentOrPendingRoommate())
								{
									Game1.multiplayer.broadcastSprites(who.currentLocation, new TemporaryAnimatedSprite("LooseSprites\\emojis", new Microsoft.Xna.Framework.Rectangle(0, 0, 9, 9), 2000f, 1, 0, base.Tile * 64f + new Vector2(16f, -64f), false, false, 1f, 0f, Color.White, 4f, 0f, 0f, 0f)
									{
								...
								{
									Game1.multiplayer.broadcastSprites(who.currentLocation, new TemporaryAnimatedSprite("LooseSprites\\Cursors", new Microsoft.Xna.Framework.Rectangle(211, 428, 7, 6), 2000f, 1, 0, base.Tile * 64f + new Vector2(16f, -64f), false, false, 1f, 0f, Color.White, 4f, 0f, 0f, 0f)
									{
										motion = new Vector2(0f, -0.5f),
										alphaFade = 0.01f
									});
			...
			if (friendship != null)
			{
				...
				if (!newCurrentDialogue)
				{
					...
					if (!newCurrentDialogue)
					{
						newCurrentDialogue = checkForNewCurrentDialogue(friendship.Points / 250, true);
					}
				}
			}
			if (who.IsLocalPlayer && friendship != null && (endOfRouteMessage.Value != null || newCurrentDialogue || (base.currentLocation != null && base.currentLocation.HasLocationOverrideDialogue(this))))
			{
				...
				{
					faceTowardFarmerForPeriod(5000, 4, false, who);
				}
				if (who.ActiveObject != null && !who.isRidingHorse() && tryToReceiveActiveObject(who))
				{
					Game1.player.checkForQuestComplete(this, -1, -1, null, null, 5);
					faceTowardFarmerForPeriod(3000, 4, false, who);
					return true;
				}
			...
			{
				if (who.ActiveObject != null && !who.isRidingHorse() && tryToReceiveActiveObject(who, true))
				{
				...
				else if (!doingEndOfRouteAnimation)
				{
					try
					{
						if (friendship != null)
						{
							faceTowardFarmerForPeriod(friendship.Points / 125 * 1000 + 1000, 4, false, who);
						}
					}
					catch (Exception)
					{
			...
			if (reacting_to_shorts)
			{
				...
				if (!(text == "Lewis"))
				{
					if (text == "Marnie")
					{
						faceTowardFarmerForPeriod(1000, 3, false, who);
						Sprite.ClearAnimation();
						Sprite.AddFrame(new FarmerSprite.AnimationFrame(33, 150, false, false, delegate
						{
							l.playSound("dustMeep");
						}));
						Sprite.AddFrame(new FarmerSprite.AnimationFrame(34, 180));
						Sprite.AddFrame(new FarmerSprite.AnimationFrame(33, 180, false, false, delegate
						{
							l.playSound("dustMeep");
						}));
						Sprite.AddFrame(new FarmerSprite.AnimationFrame(34, 180));
				...
				else
				{
					...
					jump();
					Sprite.ClearAnimation();
					Sprite.AddFrame(new FarmerSprite.AnimationFrame(26, 1000, false, false, delegate
					{
						doEmote(12);
					}, true));
					Sprite.loop = false;
		...
		{
			Friendship friendship;
			if (player != null && player.friendshipData.TryGetValue(npcName, out friendship))
			{
		...
		public virtual void performTenMinuteUpdate(int timeOfDay, GameLocation location)
		{
			...
			string rawText;
			if (Game1.random.NextDouble() < 0.1 && Dialogue != null && Dialogue.TryGetValue(location.Name + "_Ambient", out rawText))
			{
		...
		public void sayHiTo(Character c)
		{
			if (getHi(c.displayName) != null)
			{
				...
				NPC npc = c as NPC;
				if (npc != null && Game1.random.NextDouble() < 0.66 && npc.getHi(displayName) != null)
				{
		...
		{
			string rawText;
			if (!Game1.eventUp && Game1.random.NextBool() && Dialogue != null && Dialogue.TryGetValue(string.Concat(l.name, "_Entry"), out rawText))
			{
		...
		public Dialogue tryToGetMarriageSpecificDialogue(string dialogueKey)
		{
			...
			string rawText3;
			if (marriageDialogues != null && marriageDialogues.TryGetValue(dialogueKey, out rawText3))
			{
			...
			if (isRoommate())
			{
				...
				string rawText2;
				if (marriageDialogues != null && marriageDialogues.TryGetValue(key, out rawText2))
				{
		...
		private Stack<Dialogue> loadCurrentDialogue()
		{
			...
			try
			{
				...
				if (Game1.IsGreenRainingHere())
				{
					...
					if (Game1.year >= 2)
					{
						dialogue3 = TryGetDialogue("GreenRain_2");
					}
					if (dialogue3 == null)
					{
						dialogue3 = TryGetDialogue("GreenRain");
					}
					if (dialogue3 != null)
					{
						currentDialogue.Clear();
				...
				if (r.NextDouble() < 0.025 && heartLevel >= 1)
				{
					...
					if (npcData?.FriendsAndFamily != null && Utility.TryGetRandom(npcData.FriendsAndFamily, out relativeName, out relativeTitle))
					{
		...
		public bool checkForNewCurrentDialogue(int heartLevel, bool noPreface = false)
		{
			...
			foreach (string eventMessageKey in Game1.player.activeDialogueEvents.Keys)
			{
				...
				Dialogue dialogue2 = TryGetDialogue(eventMessageKey);
				if (dialogue2 == null)
				{
					continue;
				}
			...
			Dialogue dialogue = TryGetDialogue(string.Concat(preface, Game1.currentLocation.name, "_", base.TilePoint.X.ToString(), "_", base.TilePoint.Y.ToString())) ?? TryGetDialogue(string.Concat(preface, Game1.currentLocation.name, "_", Game1.shortDayNameFromDayOfSeason(Game1.dayOfMonth)));
			int hearts = 10;
		...
		public Dialogue tryToRetrieveDialogue(string preface, int heartLevel, string appendToEnd = "")
		{
			...
			if (!string.IsNullOrEmpty(Game1.player.spouse) && appendToEnd.Equals(""))
			{
				...
				{
					Dialogue s2 = tryToRetrieveDialogue(preface, heartLevel, "_roommate_" + Game1.player.spouse);
					if (s2 != null)
					{
						return s2;
					}
			...
			{
				bool flag = day_name == "Wed";
			}
			if (year == 1)
			{
				Dialogue dialogue5 = TryGetDialogue(preface + Game1.dayOfMonth + appendToEnd);
				if (dialogue5 != null)
				{
					return dialogue5;
				}
			}
			Dialogue dialogue4 = TryGetDialogue(preface + Game1.dayOfMonth + "_" + year + appendToEnd);
			if (dialogue4 != null)
			{
				return dialogue4;
			}
			Dialogue dialogue3 = TryGetDialogue(preface + Game1.dayOfMonth + "_*" + appendToEnd);
			if (dialogue3 != null)
			{
				return dialogue3;
			}
			for (int hearts = 10; hearts >= 2; hearts -= 2)
			{
				...
				{
					Dialogue dialogue = TryGetDialogue(preface + day_name + hearts + "_" + year + appendToEnd) ?? TryGetDialogue(preface + day_name + hearts + appendToEnd);
					if (dialogue != null)
					{
			...
			Dialogue dialogue2 = TryGetDialogue(preface + day_name + appendToEnd);
			if (dialogue2 != null)
			{
				Dialogue specificDialogue = TryGetDialogue(preface + day_name + "_" + year + appendToEnd);
				if (specificDialogue != null)
				{
					dialogue2 = specificDialogue;
				}
			}
			if (dialogue2 != null && base.Name.Equals("Caroline") && Game1.isLocationAccessible("CommunityCenter") && preface == "summer_" && day_name == "Mon")
			{
				dialogue2 = TryGetDialogue("summer_Wed");
			}
			if (dialogue2 != null)
			{
				return dialogue2;
		...
		private void finishEndOfRouteAnimation()
		{
			...
			if (currentlyDoingEndOfRouteAnimation && routeEndOutro != null)
			{
				...
				for (int i = 0; i < routeEndOutro.Length; i++)
				{
					...
					{
						Sprite.AddFrame(new FarmerSprite.AnimationFrame(routeEndOutro[i], 100, 0, false, false, routeEndAnimationFinished, true));
					}
					else
					{
						Sprite.AddFrame(new FarmerSprite.AnimationFrame(routeEndOutro[i], 100, 0, false, false));
					}
				}
				if (!addedFrame)
				{
		...
		protected void _PushTemporaryDialogue(string dialogue_key)
		{
			...
			catch (Exception ex)
			{
				...
				defaultInterpolatedStringHandler.AppendFormatted(dialogue_key);
				defaultInterpolatedStringHandler.AppendLiteral("'");
				defaultInterpolatedStringHandler.AppendFormatted((dialogue_key != oldDialogueKey) ? (" (from dialogue key '" + oldDialogueKey + "')") : "");
		...
		private void reallyDoAnimationAtEndOfScheduleRoute()
		{
			...
			if (!is_special_route_behavior)
			{
				...
				if (routeAnimationMetadata != null)
				{
					...
					{
						string[] metadata = ArgUtility.SplitBySpace(routeAnimationMetadata[j]);
				...
				else
				{
					...
					for (int i = 0; i < routeEndIntro.Length; i++)
					{
						...
						{
							Sprite.AddFrame(new FarmerSprite.AnimationFrame(routeEndIntro[i], 100, 0, false, false, doMiddleAnimation, true));
						}
						else
						{
							Sprite.AddFrame(new FarmerSprite.AnimationFrame(routeEndIntro[i], 100, 0, false, false));
						}
					}
				}
			}
		...
		private void doMiddleAnimation(Farmer who)
		{
			...
			{
				Sprite.AddFrame(new FarmerSprite.AnimationFrame(routeEndAnimation[i], 100, 0, false, false));
			}
			Sprite.loop = true;
		...
		private void startRouteBehavior(string behaviorName)
		{
			...
			case "abigail_videogames":
				if (Game1.IsMasterGame)
				{
					Game1.multiplayer.broadcastSprites(Utility.getGameLocationOfCharacter(this), new TemporaryAnimatedSprite("LooseSprites\\Cursors", new Microsoft.Xna.Framework.Rectangle(167, 1714, 19, 14), 100f, 3, 999999, new Vector2(2f, 3f) * 64f + new Vector2(7f, 12f) * 4f, false, false, 0.0002f, 0f, Color.White, 4f, 0f, 0f, 0f)
					{
						id = 688
					});
					doEmote(52);
			...
			case "clint_hammer":
				...
				Sprite.CurrentAnimation[14] = new FarmerSprite.AnimationFrame(9, 100, 0, false, false, clintHammerSound);
				break;
			case "birdie_fish":
				extendSourceRect(16, 0);
				Sprite.SpriteWidth = 32;
		...
		public void playSleepingAnimation()
		{
			...
			if (!isPlayingSleepingAnimation)
			{
				...
				if (DataLoader.AnimationDescriptions(Game1.content).TryGetValue(name.Value.ToLower() + "_sleep", out animationData))
				{
					int sleep_frame = Convert.ToInt32(animationData.Split('/')[0]);
					Sprite.ClearAnimation();
					Sprite.AddFrame(new FarmerSprite.AnimationFrame(sleep_frame, 100, false, false));
					Sprite.loop = true;
				}
				isPlayingSleepingAnimation = true;
			}
		...
		private void setNewDialogue(string dialogueSheetName, string dialogueSheetKey, bool clearOnMovement = false)
		{
			...
			if (dialogueSheetName.Contains("Marriage"))
			{
				...
				{
					Dialogue dialogue2 = tryToGetMarriageSpecificDialogue(dialogueSheetKey + nameToAppend) ?? new Dialogue(this, null, "");
					dialogue2.removeOnNextMove = clearOnMovement;
					CurrentDialogue.Push(dialogue2);
				}
				return;
			}
			Dialogue dialogue = StardewValley.Dialogue.TryGetDialogue(this, "Characters\\Dialogue\\" + dialogueSheetName + ":" + dialogueSheetKey + nameToAppend);
			if (dialogue != null)
			{
				dialogue.removeOnNextMove = clearOnMovement;
				CurrentDialogue.Push(dialogue);
		...
		{
			CharacterData data;
			if (!IsVillager || !TryGetData(name.Value, out data))
			{
				return null;
			}
			return data;
		...
		public virtual void receiveGift(Object o, Farmer giver, bool updateGiftLimitInfo = true, float friendshipChangeMultiplier = 1f, bool showResponse = true)
		{
			if (CanReceiveGifts())
			{
				...
				case 7:
					...
					doEmote(56);
					faceTowardFarmerForPeriod(15000, 4, false, giver);
					break;
				case 0:
				...
				case 6:
					...
					doEmote(12);
					faceTowardFarmerForPeriod(15000, 4, true, giver);
					break;
				case 2:
					giver.changeFriendship((int)(45f * friendshipChangeMultiplier * qualityChangeMultipler), this);
					faceTowardFarmerForPeriod(7000, 3, true, giver);
					break;
				case 4:
		...
		{
			string rawData;
			if (!CanReceiveGifts() || !Game1.NPCGiftTastes.TryGetValue(base.Name, out rawData))
			{
				return null;
			}
		...
		public void getHitByPlayer(Farmer who, GameLocation location)
		{
			...
			if (who.friendshipData.ContainsKey(base.Name))
			{
				...
				{
					CurrentDialogue.Clear();
					CurrentDialogue.Push(TryGetDialogue("HitBySlingshot") ?? (Game1.random.NextBool() ? new Dialogue(this, "Strings\\StringsFromCSFiles:NPC.cs.4293", true) : new Dialogue(this, "Strings\\StringsFromCSFiles:NPC.cs.4294")));
				}
				if (Sprite.Texture != null)
				{
		...
		public virtual SchedulePathDescription pathfindToNextScheduleLocation(string scheduleKey, string startingLocation, int startingX, int startingY, string endingLocation, int endingX, int endingY, int finalFacingDirection, string endBehavior, string endMessage)
		{
			...
			if (locationsRoute != null)
			{
				...
				{
					string targetLocationName2 = locationsRoute[i];
					foreach (string activePassiveFestival in Game1.netWorldState.Value.ActivePassiveFestivals)
					{
						PassiveFestivalData data2;
						string newName2;
						if (Utility.TryGetPassiveFestivalData(activePassiveFestival, out data2) && data2.MapReplacements != null && data2.MapReplacements.TryGetValue(targetLocationName2, out newName2))
						{
							targetLocationName2 = newName2;
							break;
						}
					}
					GameLocation currentLocation = Game1.RequireLocation(targetLocationName2);
			...
			else if (startingLocation.Equals(endingLocation, StringComparison.Ordinal))
			{
				...
				foreach (string activePassiveFestival2 in Game1.netWorldState.Value.ActivePassiveFestivals)
				{
					...
					string newName;
					if (Utility.TryGetPassiveFestivalData(activePassiveFestival2, out data) && data.MapReplacements != null && data.MapReplacements.TryGetValue(targetLocationName, out newName))
					{
		...
		protected virtual Dictionary<int, SchedulePathDescription> parseMasterScheduleImpl(string scheduleKey, string rawData, List<string> visited)
		{
			...
			try
			{
				...
				{
					string newKey3 = ArgUtility.SplitBySpaceAndGet(split[0], 1);
					Dictionary<string, string> allSchedules = getMasterScheduleRawData();
					if (string.Equals(newKey3, "season", StringComparison.OrdinalIgnoreCase))
					{
						newKey3 = Game1.currentSeason;
						if (!allSchedules.ContainsKey(newKey3))
						{
							newKey3 = "spring";
						}
					}
					...
					{
						string newScript;
						if (allSchedules.TryGetValue(newKey3, out newScript))
						{
							return parseMasterScheduleImpl(newKey3, newScript, visited);
						}
						IGameLogger log2 = Game1.log;
						...
						defaultInterpolatedStringHandler.AppendFormatted(newKey3);
				...
				if (split[routesToSkip].Contains("GOTO"))
				{
					...
					return parseMasterScheduleImpl(newKey2, getMasterScheduleEntry(newKey2), visited);
				}
				...
				for (int i = routesToSkip; i < split.Length; i++)
				{
					...
					if (location == "bed")
					{
						...
						else
						{
							...
							if (default_schedule != null)
							{
								try
								{
									string[] array = SplitScheduleCommands(default_schedule);
					...
					{
						int result;
						if (int.TryParse(location, out result))
						{
		...
		public bool TryLoadSchedule()
		{
			...
			{
				if (TryLoadSchedule(day + "_" + tryHearts3))
				{
					return true;
				}
			}
			...
			{
				if (TryLoadSchedule(dayName + "_" + tryHearts2))
				{
					return true;
				}
				tryHearts2--;
		...
		public string getMasterScheduleEntry(string schedule_key)
		{
			...
			string data;
			if (_masterScheduleData.TryGetValue(schedule_key, out data))
			{
				return data;
			}
		...
		public virtual void dayUpdate(int dayOfMonth)
		{
			...
			if (base.currentLocation != null && defaultMap.Value != null)
			{
				...
				catch (Exception ex)
				{
					...
					defaultInterpolatedStringHandler.AppendFormatted(defaultMap);
		...
		private void doPlaySpousePatioAnimation()
		{
			...
			for (int i = 0; i < frames.Count; i++)
			{
				...
				if (frame != null && frame.Length != 0)
				{
					...
					Sprite.AddFrame(new FarmerSprite.AnimationFrame(index, duration, 0, false, false));
				}
			}
		}
		...
		public void marriageDuties()
		{
			...
			if (spouse.GetDaysMarried() > 7 && r.NextDouble() < (double)(1f - (float)Math.Max(1, heartsWithSpouse) / (float)minHeartLevelForNegativeDialogue))
			{
				...
				if (f2 != null && f2.isGroundFurniture() && f2.furniture_type.Value != 15 && f2.furniture_type.Value != 12)
				{
					...
					if (farmHouse.CanItemBePlacedHere(new Vector2(p2.X, p2.Y)))
					{
						setTilePosition(p2);
						faceDirection(1);
						switch (r.Next(10))
				...
				spouseObstacleCheck(new MarriageDialogueReference("Strings\\StringsFromCSFiles", "NPC.cs.4406", false), farmHouse, true);
				return;
			}
			Friendship friendship = spouse.GetSpouseFriendship();
			...
			else if (!Game1.isRaining && r.NextDouble() < 0.4 && !checkTileOccupancyForSpouse(farm, Utility.PointToVector2(farmHouse.getPorchStandingSpot())) && !base.Name.Equals("Krobus"))
			{
				...
				if (!hasSomeoneFedThePet)
				{
					...
					{
						PetBowl bowl2 = building as PetBowl;
				...
				if (r.NextDouble() < 0.6 && Game1.season != Season.Winter && !hasSomeoneWateredCrops)
				{
					...
					for (; tries2 < Math.Min(50, farm.terrainFeatures.Length); tries2++)
					{
						...
						if (!Utility.TryGetRandom(farm.terrainFeatures, out tile2, out feature))
						{
							continue;
						}
						HoeDirt dirt2 = feature as HoeDirt;
						if (dirt2 != null && dirt2.needsWatering())
						{
							if (!dirt2.isWatered())
							{
								origin2 = tile2;
					...
					if (!origin2.Equals(Vector2.Zero))
					{
						foreach (Vector2 currentPosition2 in new Microsoft.Xna.Framework.Rectangle((int)origin2.X - 30, (int)origin2.Y - 30, 60, 60).GetVectors())
						{
							TerrainFeature terrainFeature;
							if (farm.isTileOnMap(currentPosition2) && farm.terrainFeatures.TryGetValue(currentPosition2, out terrainFeature))
							{
								HoeDirt dirt = terrainFeature as HoeDirt;
								if (dirt != null && Game1.IsMasterGame && dirt.needsWatering())
								{
									dirt.state.Value = 1;
								}
							}
				...
				else if (r.NextDouble() < 0.6 && !hasSomeoneFedTheAnimals)
				{
					...
					foreach (Building b in farm.buildings)
					{
						...
						if (animalHouse != null && (int)b.daysOfConstructionLeft <= 0 && Game1.IsMasterGame)
						{
					...
					if (Game1.IsMasterGame)
					{
						...
						{
							PetBowl bowl = building2 as PetBowl;
				...
				else if (!hasSomeoneRepairedTheFences)
				{
					...
					for (; tries < Math.Min(50, farm.objects.Length); tries++)
					{
						...
						Vector2 tile;
						Object obj2;
						if (Utility.TryGetRandom(farm.objects, out tile, out obj2) && obj2 is Fence)
						{
							origin = tile;
						}
					}
					if (!origin.Equals(Vector2.Zero))
					{
						...
						{
							Object obj;
							if (farm.isTileOnMap(currentPosition) && farm.objects.TryGetValue(currentPosition, out obj))
							{
			...
			else if (spouse.GetDaysMarried() >= 1 && r.NextDouble() < 0.045)
			{
				if (r.NextDouble() < 0.75)
				{
					...
					try
					{
						new_furniture = ItemRegistry.Create<Furniture>(Utility.getRandomSingleTileFurniture(r)).SetPlacement(spot2);
					}
					catch
					{
						new_furniture = null;
					}
					if (new_furniture != null && spot2.X > 0 && farmHouse.CanItemBePlacedHere(new Vector2(spot2.X - 1, spot2.Y)))
					{
			...
			else if (Game1.isRaining && r.NextDouble() < 0.08 && heartsWithSpouse < 11 && spouse.GetDaysMarried() > 7 && base.Name != "Krobus")
			{
				...
				spouseObstacleCheck(new MarriageDialogueReference("Strings\\StringsFromCSFiles", "NPC.cs.4499", false), farmHouse, true);
			}
		...
		public virtual void arriveAtFarmHouse(FarmHouse farmHouse)
		{
			...
			if (controller.pathToEndPoint == null)
			{
				base.willDestroyObjectsUnderfoot = true;
				controller = new PathFindController(this, farmHouse, farmHouse.getKitchenStandingSpot(), 0);
				setNewDialogue(TryGetDialogue("SpouseFarmhouseClutter") ?? new Dialogue(this, "Strings\\StringsFromCSFiles:NPC.cs.4500", true));
			}
			else if (Game1.timeOfDay > 1300)
			{
				if (ScheduleKey == "marriage_" + Game1.shortDayNameFromDayOfSeason(Game1.dayOfMonth))
				{
					setNewDialogue("MarriageDialogue", "funReturn_", true);
				}
				else if (ScheduleKey == "marriageJob")
				{
				...
				{
					setRandomAfternoonMarriageDialogue(Game1.timeOfDay, base.currentLocation, true);
				}
			}
		...
		private void robinVariablePause(Farmer who)
		{
			...
			{
				Sprite.CurrentAnimation[Sprite.currentAnimationIndex] = new FarmerSprite.AnimationFrame(27, 300, false, false, robinVariablePause);
			}
			else if (Game1.random.NextDouble() < 0.25)
			{
				Sprite.CurrentAnimation[Sprite.currentAnimationIndex] = new FarmerSprite.AnimationFrame(23, Game1.random.Next(500, 4000), false, false, robinVariablePause);
			}
			else
			{
				Sprite.CurrentAnimation[Sprite.currentAnimationIndex] = new FarmerSprite.AnimationFrame(27, Game1.random.Next(1000, 4000), false, false, robinVariablePause);
			}
		}
		...
		public void randomSquareMovement(GameTime time)
		{
			...
			Vector2 nextSquarePosition2 = nextSquarePosition;
