	...
	public class Squirrel : Critter
	{
		...
		public override bool update(GameTime time, GameLocation environment)
		{
			...
			if (sprite.CurrentAnimation == null && nextNibbleTimer <= 0)
			{
				...
				anim.Add(new FarmerSprite.AnimationFrame(baseFrame, 200, false, false, doneNibbling));
			...
			if (running && characterCheckTimer <= 0 && treeRunTimer <= 0)
			{
				...
				Vector2 v = new Vector2((int)(position.X / 64f), (int)position.Y / 64);
			...
			if (treeRunTimer > 0)
			{
				...
				if (treeRunTimer <= 0)
				{
					climbed.performUseAction(treeTile);
					return true;
				}
			}
