	...
	public class NPCController
	{
		...
		private bool setMoving(bool newTarget)
		{
			if (puppet != null && pathIndex < path.Count)
			{
				...
				puppet.Halt();
				puppet.faceDirection(facingDirection);
				if (CurrentPathX != 0 && CurrentPathY != 0)
				{
					pauseTime = CurrentPathY;
					facingDirection = CurrentPathX % 4;
					puppet.faceDirection(facingDirection);
					return true;
				}
		...
		public bool update(GameTime time, GameLocation location, List<NPCController> allControllers)
		{
			...
			Farmer player = puppet as Farmer;
			if (player != null)
			{
				player.setRunning(false, true);
				player.speed = 2;
				player.ignoreCollisions = true;
				if (Game1.CurrentEvent != null && Game1.CurrentEvent.farmer != puppet)
				{
