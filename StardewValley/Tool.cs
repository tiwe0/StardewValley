...
namespace StardewValley
{
	...
	public abstract class Tool : Item
	{
		...
		protected virtual void DrawAttachmentSlot(int slot, SpriteBatch b, int x, int y)
		{
			...
			GetAttachmentSlotSprite(slot, out texture, out sourceRect);
		...
		public virtual void DoFunction(GameLocation location, int x, int y, int power, Farmer who)
		{
			...
			if (isHeavyHitter() && !(this is MeleeWeapon))
			{
				...
				location.damageMonster(new Rectangle(x - 32, y - 32, 64, 64), (int)upgradeLevel + 1, ((int)upgradeLevel + 1) * 3, false, who);
			}
			MeleeWeapon weapon = this as MeleeWeapon;
			if (weapon != null && (!who.UsingTool || Game1.mouseClickPolling >= 50 || (int)weapon.type == 1 || !(weapon.ItemId != "47") || MeleeWeapon.timedHitTimer > 0 || who.FarmerSprite.currentAnimationIndex != 5 || !(who.FarmerSprite.timer < who.FarmerSprite.interval / 4f)))
			{
		...
		public virtual Object attach(Object o)
		{
			...
			for (int slot3 = 0; slot3 < attachments.Length; slot3++)
			{
				if (!canThisBeAttached(o, slot3))
				{
					continue;
				}
				Object oldObj3 = attachments[slot3];
				if (oldObj3 == null)
				{
					attachments[slot3] = o;
					o = null;
				...
				if (oldObj3.canStackWith(o))
				{
					o.Stack = oldObj3.addToStack(o);
					if (o.Stack < 1)
					{
						o = null;
						break;
			...
			for (int slot2 = 0; slot2 < attachments.Length; slot2++)
			{
				Object oldObj2 = attachments[slot2];
				attachments[slot2] = null;
				if (canThisBeAttached(o, slot2))
				{
					attachments[slot2] = o;
					Game1.playSound("button1");
					return oldObj2;
				}
				attachments[slot2] = oldObj2;
			}
			return o;
		}
		...
		public virtual bool CanForge(Item item)
		{
			...
			{
				MeleeWeapon weapon = this as MeleeWeapon;
				if (weapon != null && weapon.getItemLevel() < 15 && !Name.Contains("Galaxy"))
				{
					return true;
				}
			}
		...
		public virtual bool Forge(Item item, bool count_towards_stats = false)
		{
				...
				if (AddEnchantment(enchantment))
				{
					if (!(enchantment is DiamondEnchantment))
					{
						if (enchantment is GalaxySoulEnchantment)
						{
							...
							if (weapon2 != null && weapon2.isGalaxyWeapon() && weapon2.GetEnchantmentLevel<GalaxySoulEnchantment>() >= 3)
							{
			...
			else if (item.QualifiedItemId == "(O)852")
			{
				...
				if (weapon != null)
				{
					...
					MeleeWeapon.attemptAddRandomInnateEnchantment(weapon, Game1.random, true, oldenchantments);
					return true;
				}
			}
			return false;
