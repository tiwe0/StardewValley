...
namespace StardewValley.Menus
{
	public class MuseumMenu : MenuWithInventory
	{
			...
			: base(highlighterMethod, true)
		{
			...
			LibraryMuseum obj = Game1.currentLocation as LibraryMuseum;
			if (obj == null)
			{
		...
		public override void receiveLeftClick(int x, int y, bool playSound = true)
		{
			...
			if (oldItem != null && base.heldItem != null && (y < Game1.viewport.Height - (height - (IClickableMenu.borderWidth + IClickableMenu.spaceToClearTopBorder + 192)) || menuMovingDown))
			{
				...
				if (museum.isTileSuitableForMuseumPiece(mapXTile2, mapYTile2) && museum.isItemSuitableForDonation(base.heldItem))
				{
					string itemId2 = base.heldItem.QualifiedItemId;
					int rewardsCount = museum.getRewardsForPlayer(Game1.player).Count;
					museum.museumPieces.Add(new Vector2(mapXTile2, mapYTile2), base.heldItem.ItemId);
					...
					if (!holdingMuseumPiece)
					{
						...
						{
							Game1.multiplayer.globalChatInfoMessage("donation", Game1.player.name, TokenStringBuilder.ItemName(itemId2));
						}
					}
					ReturnToDonatableItems();
				}
		...
		private void snapCursorToCurrentMuseumSpot()
		{
			if (menuMovingDown)
			{
				Vector2 newCursorPositionTile = new Vector2((Game1.getMouseX(false) + Game1.viewport.X) / 64, (Game1.getMouseY(false) + Game1.viewport.Y) / 64);
