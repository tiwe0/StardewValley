	...
	public class Game1 : InstanceGame
	{
		...
		public static IClickableMenu activeClickableMenu
		{
			...
			set
			{
				...
				IDisposable disposable = _activeClickableMenu as IDisposable;
				if (disposable != null && !_activeClickableMenu.HasDependencies())
				{
					disposable.Dispose();
				}
		...
		public static int CurrentPlayerLimit
		{
			get
			{
				...
				{
					int currentPlayerLimit = netWorldState.Value.CurrentPlayerLimit;
		...
		public static void ResetToolSpriteSheet()
		{
			...
			Texture2D texture2D = new Texture2D(game1.GraphicsDevice, w, h, false, SurfaceFormat.Color);
		...
		private static void allocateLightmap(int width, int height)
		{
			...
			{
				_lightmap?.Dispose();
				_lightmap = new RenderTarget2D(graphics.GraphicsDevice, w, h, false, SurfaceFormat.Color, DepthFormat.None, 0, RenderTargetUsage.PreserveContents);
			}
		}
		...
		static Game1()
		{
			...
			log = new DefaultLogger(!Program.releaseBuild, false);
		...
		public virtual void SetWindowSize(int w, int h)
		{
			...
			try
			{
				...
				screen = new RenderTarget2D(graphics.GraphicsDevice, sw, sh, false, SurfaceFormat.Color, DepthFormat.None, 0, RenderTargetUsage.PreserveContents);
				screen.Name = "Screen";
				...
				uiScreen = new RenderTarget2D(graphics.GraphicsDevice, uw, uh, false, SurfaceFormat.Color, DepthFormat.None, 0, RenderTargetUsage.PreserveContents);
				uiScreen.Name = "UI Screen";
			}
			catch (Exception)
			{
			}
			updateViewportForScreenSizeChange(false, localMultiplayerWindow.Width, localMultiplayerWindow.Height);
			...
			GameMenu gameMenu = activeClickableMenu as GameMenu;
			if (gameMenu != null)
			{
				(gameMenu.GetCurrentPage() as OptionsPage)?.preWindowSizeChange();
				gameMenu = (GameMenu)(activeClickableMenu = new GameMenu(gameMenu.currentTab));
		...
		protected override void LoadContent()
		{
			...
			fadeToBlackRect = new Texture2D(base.GraphicsDevice, 1, 1, false, SurfaceFormat.Color);
			fadeToBlackRect.SetData(white2);
			Color[] white = new Color[1];
			for (int j = 0; j < white.Length; j++)
			{
				white[j] = new Color(255, 255, 255, 255);
			}
			staminaRect = new Texture2D(base.GraphicsDevice, 1, 1, false, SurfaceFormat.Color);
		...
		public static void resetPlayer()
		{
			...
			player = new Farmer(new FarmerSprite(null), new Vector2(192f, 192f), 1, "", farmersInitialTools, true);
		}
		...
		public static void setRichPresence(string friendlyName, object argument = null)
		{
			...
			case "location":
			{
				...
				debugPresenceString = defaultInterpolatedStringHandler.ToStringAndClear();
				break;
			}
			case "festival":
			{
		...
		{
			object value;
			if (!newGameSetupOptions.TryGetValue(key, out value))
			{
				return default(T);
			}
			return (T)value;
		...
		public virtual void loadForNewGame(bool loadedGame = false)
		{
			...
			if (!loadedGame)
			{
				...
				{
					(location2 as IslandLocation)?.AddAdditionalWalnutBushes();
				}
			}
			if (!loadedGame)
			{
		...
		protected override void Update(GameTime gameTime)
		{
			...
			{
				player.requestingTimePause.Value = !shouldTimePass(LocalMultiplayer.IsLocalMultiplayer(true));
				if (IsMasterGame)
				{
					...
					if (doMainGameUpdates)
					{
						...
						if (currentLocation != null)
						{
							if (!HostPaused && !showingEndOfNightStuff)
							{
								...
								if (currentMinigame == null)
								{
									UpdateViewPort(false, getViewportCenter());
								}
								else
								{
		...
		public static void updateCellarAssignments()
		{
			...
			for (int i = 2; i <= netWorldState.Value.HighestPlayerLimit; i++)
			{
				...
				if (player.team.cellarAssignments.TryGetValue(i, out assignedFarmerId))
				{
		...
		public static void playMorningSong(bool ignoreDelay = false)
		{
			...
			{
				_003C_003Ec__DisplayClass655_0 _003C_003Ec__DisplayClass655_ = new _003C_003Ec__DisplayClass655_0();
				...
				if (IsRainingHere())
				{
					if (ignoreDelay)
					{
						_003CplayMorningSong_003Eg__PlayAction_007C655_0();
					}
					else
					{
						morningSongPlayAction = DelayedAction.functionAfterDelay(_003CplayMorningSong_003Eg__PlayAction_007C655_0, 500);
					}
					return;
				}
				_003C_003Ec__DisplayClass655_.context = currentLocation?.GetLocationContext();
				if (_003C_003Ec__DisplayClass655_.context?.DefaultMusic != null)
				{
					if (_003C_003Ec__DisplayClass655_.context.DefaultMusicCondition == null || GameStateQuery.CheckConditions(_003C_003Ec__DisplayClass655_.context.DefaultMusicCondition))
					{
						if (ignoreDelay)
						{
							_003C_003Ec__DisplayClass655_._003CplayMorningSong_003Eg__PlayAction_007C1();
						}
						else
						{
							morningSongPlayAction = DelayedAction.functionAfterDelay(_003C_003Ec__DisplayClass655_._003CplayMorningSong_003Eg__PlayAction_007C1, 500);
						}
					}
				}
				else if (ignoreDelay)
				{
					_003CplayMorningSong_003Eg__PlayAction_007C655_2();
				}
				else
				{
					morningSongPlayAction = DelayedAction.functionAfterDelay(_003CplayMorningSong_003Eg__PlayAction_007C655_2, 500);
				}
			}
		...
		public static void updatePause(GameTime gameTime)
		{
			...
			else if (killScreen)
			{
				...
				{
					foreach (ReviveLocation revive_location2 in currentLocation.GetLocationContext().ReviveLocations)
					{
						if (GameStateQuery.CheckConditions(revive_location2.Condition, null, player))
						{
							warpFarmer(revive_location2.Location, revive_location2.Position.X, revive_location2.Position.Y, false);
							handledRevive = true;
							break;
						}
					}
				...
				else
				{
					foreach (ReviveLocation revive_location in LocationContexts.Default.ReviveLocations)
					{
						...
						{
							warpFarmer(revive_location.Location, revive_location.Position.X, revive_location.Position.Y, false);
							handledRevive = true;
							break;
						}
					}
				...
				if (!handledRevive)
				{
					warpFarmer("Hospital", 20, 12, false);
				}
			}
		...
		public static void toggleNonBorderlessWindowedFullscreen()
		{
			...
			graphics.ApplyChanges();
			updateViewportForScreenSizeChange(true, graphics.PreferredBackBufferWidth, graphics.PreferredBackBufferHeight);
			GameRunner.instance.OnWindowSizeChange(null, null);
		}
		...
		internal void UpdateTitleScreen(GameTime time)
		{
			...
			if (fadeToBlackAlpha >= 1f)
			{
				...
				case 0:
					...
					{
						ICue cue;
						playSound("spring_day_ambient", out cue);
						currentSong = cue;
					}
		...
		public static void performTenMinuteClockUpdate()
		{
			hooks.OnGame1_PerformTenMinuteClockUpdate(delegate
			{
				...
				if (weatherIcon == 1)
				{
					...
					{
						string value;
						if (dictionary.TryGetValue("startedMessage", out value))
						{
						...
						{
							string value2;
							if (!dictionary.TryGetValue("locationDisplayName", out value2))
							{
								value2 = array[0];
								switch (value2)
								{
								case "Forest":
									value2 = (IsWinter ? content.LoadString("Strings\\StringsFromCSFiles:Game1.cs.2634") : content.LoadString("Strings\\StringsFromCSFiles:Game1.cs.2635"));
									break;
								case "Town":
									value2 = content.LoadString("Strings\\StringsFromCSFiles:Game1.cs.2637");
									break;
								case "Beach":
									value2 = content.LoadString("Strings\\StringsFromCSFiles:Game1.cs.2639");
									break;
								default:
									value2 = TokenParser.ParseText(GameLocation.GetData(value2)?.DisplayName) ?? value2;
									break;
								}
							}
				...
				case 2600:
					...
					if (player.UsingTool)
					{
						FishingRod fishingRod = player.CurrentTool as FishingRod;
				...
				{
					PassiveFestivalData data;
					if (Utility.TryGetPassiveFestivalData(current, out data) && timeOfDay == data.StartTime && (!data.OnlyShowMessageOnFirstDay || Utility.GetDayOfPassiveFestival(current) == 1))
					{
		...
		public static void UpdateGameClock(GameTime time)
		{
			...
			else if (timeOfDay >= getStartingToGetDarkTime(currentLocation))
			{
				...
				float transparency2 = Math.Min(0.93f, 0.3f + ((float)(adjustedTime2 - getStartingToGetDarkTime(currentLocation)) + (float)gameTimeInterval / (float)realMilliSecondsPerGameTenMinutes * 16.6f) * 0.00225f);
		...
		private bool onFadeToBlackComplete()
		{
				...
				if (locationRequest != null)
				{
					...
					else
					{
						...
						if (!MineShaft.IsGeneratedLevel(locationRequest.Name, out level) && locationRequest.Location != null)
						{
							...
							checkForRunButton(GetKeyboardState(), true);
						}
						if (!eventUp)
						{
							viewportFreeze = false;
			...
			if (newDay)
			{
				newDayAfterFade(_003ConFadeToBlackComplete_003Eg__After_007C722_0);
				return true;
			}
			if (eventOver)
			{
				...
				if (dayOfMonth == 0)
				{
					newDayAfterFade(_003ConFadeToBlackComplete_003Eg__After_007C722_1);
				}
				return true;
			}
		...
		public static void OnLocationChanged(GameLocation oldLocation, GameLocation newLocation)
		{
			...
			int level;
			if (newLocation.Name != null && !MineShaft.IsGeneratedLevel(newLocation, out level) && !VolcanoDungeon.IsGeneratedLevel(newLocation.Name, out level))
			{
		...
		private static void onFadedBackInComplete()
		{
			...
			checkForRunButton(oldKBState, true);
		}
		...
		public static void UpdateOther(GameTime time)
		{
				...
				if (player.health < 20 && CurrentEvent == null)
				{
					...
					if (player.health <= 10)
					{
						...
						if (showingHealthBar && fadeToBlackAlpha <= 0f)
						{
							for (int l = 0; l < 3; l++)
							{
								uiOverlayTempSprites.Add(new TemporaryAnimatedSprite("LooseSprites\\Cursors", new Microsoft.Xna.Framework.Rectangle(366, 412, 5, 6), new Vector2(random.Next(32) + uiViewport.Width - 112, uiViewport.Height - 224 - (player.maxHealth - 100) - 16 + 4), false, 0.017f, Color.Red)
								{
			...
			int level;
			if (!MineShaft.IsGeneratedLevel(currentLocation.Name, out level))
			{
		...
		public static void updateMusic()
		{
			if (game1.IsMainInstance)
			{
				...
				{
					changeMusicTrack(important_instance_music, false, MusicContext.ImportantSplitScreenMusic);
				}
			}
			...
			if (currentLocation != null && currentLocation.IsMiniJukeboxPlaying() && (!requestedMusicDirty || requestedMusicTrackOverrideable) && currentTrackOverrideable)
			{
				...
				if (currentSong == null || !currentSong.IsPlaying || currentSong.Name != mini_jukebox_track)
				{
					if (!soundBank.Exists(mini_jukebox_track))
					{
						...
						gameLogger.Error(defaultInterpolatedStringHandler.ToStringAndClear());
		...
		public static GameLocation CreateGameLocation(string id, CreateLocationData createData)
		{
			...
			if (createData.Type != null)
			{
				...
				location = (GameLocation)Activator.CreateInstance(type, createData.MapPath, id);
			}
			else
			{
		...
		public static void AddLocations()
		{
			...
			foreach (KeyValuePair<string, LocationData> pair in DataLoader.Locations(content))
			{
				...
				{
					log.Error("Couldn't create the '" + pair.Key + "' location. Is its data in Data/Locations invalid?", ex2);
					continue;
				}
				if (location == null)
				{
		...
		public static void AddNPCs()
		{
			...
			if (location.getCharacterFromName("Mister Qi") == null)
			{
				...
				location.addCharacter(new NPC(sprite, new Vector2(448f, 256f), "QiNutRoom", 0, "Mister Qi", false, content.Load<Texture2D>("Portraits\\MrQi")));
			}
		}
		...
		public static void fixProblems()
		{
			...
			Utility.ForEachVillager(delegate(NPC n)
			{
				...
				Point tile;
				int direction;
				NPC.ReadNpcHomeData(data, n.currentLocation, out locationName, out tile, out direction);
			...
			if (divorced != null)
			{
				...
				{
					log.Warn("Fixing " + n2.Name + " who was improperly divorced and left stranded");
					n2.PerformDivorce();
				}
			}
			...
			foreach (Farmer who in getAllFarmers())
			{
				if (who.toolBeingUpgraded.Value != null)
				{
					...
					int count;
					if (missingTools.TryGetValue(key, out count))
					{
				...
				for (int m = 0; m < who.Items.Count; m++)
				{
					if (who.Items[m] != null)
					{
						checkIsMissingTool(missingTools, ref missingScythes, who.Items[m]);
					}
				}
			}
			bool allFound = true;
		...
		private static IEnumerator<int> _newDayAfterFade()
		{
			...
			if (Utility.TryGetRandom(player.friendshipData, out whichFriend, out friendship) && random.NextBool((double)(friendship.Points / 250) * 0.1) && player.spouse != whichFriend && DataLoader.Mail(content).ContainsKey(whichFriend))
			{
			...
			if (player.team.useSeparateWallets.Value)
			{
				...
				if (additional_shipped_items.TryGetValue(player.UniqueMultiplayerID, out item_list))
				{
			...
			if (player.useSeparateWallets || player.IsMainPlayer)
			{
				...
				foreach (Item item3 in shippingBin)
				{
					...
					{
						specialOrder.onItemShipped?.Invoke(player, item3, item_value2);
					}
				}
				player.Money += total2;
			}
			if (IsMasterGame)
			{
				...
				{
					GameLocation source3 = RequireLocation("Submarine");
					...
					source3 = RequireLocation("MermaidHouse");
					if (source3.objects.Length >= 0)
					{
				...
				{
					addMailForTomorrow("transferredObjectsPamHouse", true);
					GameLocation source2 = RequireLocation("Trailer");
				...
				{
					addMailForTomorrow("transferredObjectsJojaMart", true);
			...
			if (player.useSeparateWallets && player.IsMainPlayer)
			{
				foreach (Farmer who2 in getOfflineFarmhands())
				{
					...
					int total = 0;
					IInventory farmhandShippingBin = getFarm().getShippingBin(who2);
			...
			if (IsMasterGame)
			{
				...
				{
					SpecialOrder order2 = player.team.specialOrders[j];
			...
			if (IsMasterGame)
			{
				Utility.ForEachLocation(delegate(GameLocation location)
				{
					if (location.IsOutdoors && location.IsRainingHere())
					{
						...
						{
							HoeDirt hoeDirt = pair2.Value as HoeDirt;
							if (hoeDirt != null && (int)hoeDirt.state != 2)
							{
			...
			foreach (string index in new List<string>(player.team.itemsToRemoveOvernight))
			{
				if (IsMasterGame)
				{
					...
					foreach (Farmer farmer2 in getOfflineFarmhands())
					{
			...
			if (player.currentLocation != null)
			{
				...
				BedFurniture.ApplyWakeUpPosition(player);
				forceSnapOnNextViewportUpdate = true;
				UpdateViewPort(false, player.StandingPixel);
		...
		public static void UpdateWeatherForNewDay()
		{
			...
			foreach (KeyValuePair<string, LocationContextData> pair2 in locationContextData)
			{
				netWorldState.Value.GetWeatherForLocation(pair2.Key).UpdateDailyWeather(pair2.Key, pair2.Value, random);
			}
		...
		{
			ShopLocation shopLocation = getLocationFromName(location_name) as ShopLocation;
			...
			{
				Object item = shopLocation.itemsFromPlayerToSell[i] as Object;
				if (item == null)
				{
					...
					foreach (Object o2 in i.Objects.Values)
					{
						if (o2.IsWeeds())
						{
							...
							case "(O)793":
								...
								break;
							default:
								o2.SetIdAndSprite(random.Choose(678, 679));
								break;
							case "(O)882":
							case "(O)883":
							case "(O)884":
		...
		public static void drawDialogue(NPC speaker)
		{
			...
			DialogueBox dialogueBox = activeClickableMenu as DialogueBox;
		...
		public static void warpCharacter(NPC character, GameLocation targetLocation, Vector2 position)
		{
			foreach (string activePassiveFestival in netWorldState.Value.ActivePassiveFestivals)
			{
				...
				string newName;
				if (Utility.TryGetPassiveFestivalData(activePassiveFestival, out festival) && dayOfMonth >= festival.StartDay && dayOfMonth <= festival.EndDay && festival.Season == season && festival.MapReplacements != null && festival.MapReplacements.TryGetValue(targetLocation.name, out newName))
				{
		...
		public static void warpFarmer(LocationRequest locationRequest, int tileX, int tileY, int facingDirectionAfterWarp)
		{
			...
			foreach (string activePassiveFestival in netWorldState.Value.ActivePassiveFestivals)
			{
				PassiveFestivalData festival;
				string newName;
				if (Utility.TryGetPassiveFestivalData(activePassiveFestival, out festival) && dayOfMonth >= festival.StartDay && dayOfMonth <= festival.EndDay && festival.Season == season && festival.MapReplacements != null && festival.MapReplacements.TryGetValue(locationRequest.Name, out newName))
				{
			...
			case "Farm":
				...
				case "FarmCave":
				{
					...
					Point tile;
					if (getFarm().TryGetMapPropertyAs("FarmCaveEntry", out tile, false))
					{
						tileX = tile.X;
						tileY = tile.Y;
						break;
				...
				case "Forest":
				{
					...
					Point tile2;
					if (getFarm().TryGetMapPropertyAs("ForestEntry", out tile2, false))
					{
						tileX = tile2.X;
						tileY = tile2.Y;
						break;
				...
				{
					Point tile3;
					if (tileX == 79 && tileY == 17 && getFarm().TryGetMapPropertyAs("BusStopEntry", out tile3, false))
					{
						tileX = tile3.X;
						tileY = tile3.Y;
					}
				...
				{
					Point tile4;
					if (tileX == 40 && tileY == 0 && getFarm().TryGetMapPropertyAs("BackwoodsEntry", out tile4, false))
					{
						tileX = tile4.X;
						tileY = tile4.Y;
					}
			...
			case "Club":
				...
				locationRequest.OnWarp += delegate
				{
					...
					if (characterFromName != null)
					{
						...
						multiplayer.broadcastSprites(currentLocation, new TemporaryAnimatedSprite(288, 100f, 1, 24, vector * 64f, true, false, currentLocation, player)
						{
							shakeIntensity = 0.5f,
							shakeIntensityChange = 0.002f,
							extraInfoForEndBehavior = num,
							endFunction = currentLocation.removeTemporarySpritesWithID
						}, new TemporaryAnimatedSprite("LooseSprites\\Cursors", new Microsoft.Xna.Framework.Rectangle(598, 1279, 3, 4), 53f, 5, 9, vector * 64f + new Vector2(5f, 0f) * 4f, true, false, 0.0263f, 0f, Color.Yellow, 4f, 0f, 0f, 0f)
						{
							id = num
						}, new TemporaryAnimatedSprite("LooseSprites\\Cursors", new Microsoft.Xna.Framework.Rectangle(598, 1279, 3, 4), 53f, 5, 9, vector * 64f + new Vector2(5f, 0f) * 4f, true, true, 0.0263f, 0f, Color.Orange, 4f, 0f, 0f, 0f)
						{
							delayBeforeAnimationStart = 100,
							id = num
						}, new TemporaryAnimatedSprite("LooseSprites\\Cursors", new Microsoft.Xna.Framework.Rectangle(598, 1279, 3, 4), 53f, 5, 9, vector * 64f + new Vector2(5f, 0f) * 4f, true, false, 0.0263f, 0f, Color.White, 3f, 0f, 0f, 0f)
						{
			...
			if (weatherIcon == 1 && whereIsTodaysFest != null && locationRequest.Name.Equals(whereIsTodaysFest) && !warpingForForcedRemoteEvent)
			{
				...
				if (timeOfDay <= Convert.ToInt32(timeParts[1]))
				{
					...
					else
					{
						if (IsMultiplayer)
						{
							netReady.SetLocalReady("festivalStart", true);
							activeClickableMenu = new ReadyCheckDialog("festivalStart", true, delegate
							{
								exitActiveMenu();
		...
		public static T getCharacterFromName<T>(string name, bool mustBeVillager = true, bool includeEventActors = false) where T : NPC
		{
			...
			{
				T val = rawNpc as T;
				if (val != null && val.Name == name && (!mustBeVillager || val.IsVillager))
				{
					if (val.currentLocation?.IsActiveLocation() ?? false)
					{
						match = val;
		...
		public static T RequireCharacter<T>(string name, bool mustBeVillager = true) where T : NPC
		{
			...
			if (cast == null)
			{
				...
				defaultInterpolatedStringHandler.AppendFormatted(npc?.GetType().FullName);
		...
		public static TLocation RequireLocation<TLocation>(string name, bool isStructure = false) where TLocation : GameLocation
		{
			...
			if (cast == null)
			{
				...
				defaultInterpolatedStringHandler.AppendFormatted(name);
				defaultInterpolatedStringHandler.AppendLiteral(" from '");
				defaultInterpolatedStringHandler.AppendFormatted(location?.GetType().FullName);
				defaultInterpolatedStringHandler.AppendLiteral("' to the required '");
		...
		{
			return getLocationFromName(name, false);
		}
		...
		public static GameLocation getLocationFromName(string name, bool isStructure)
		{
			...
			if (_locationLookup.TryGetValue(name, out cached_location))
			{
		...
		public static GameLocation getLocationFromNameInLocationsList(string name, bool isStructure = false)
		{
			...
			int level;
			if (MineShaft.IsGeneratedLevel(name, out level))
			{
			...
			if (!isStructure)
			{
				return getLocationFromName(name, true);
			}
			return null;
		}
		...
		public static void PassOutNewDay()
		{
			...
			activeClickableMenu = new ReadyCheckDialog("sleep", false, delegate
			{
				NewDay(0f);
			});
		}
		...
		{
			ReadyCheckDialog readyCheckDialog = activeClickableMenu as ReadyCheckDialog;
		...
		{
			KeyValuePair<string, bool> trackData;
			if (_requestedMusicTracks.TryGetValue(music_context, out trackData))
			{
		...
		public static void UpdateRequestedMusicTrack()
		{
			...
			for (int i = 0; i < 6; i++)
			{
				...
				KeyValuePair<string, bool> trackData;
				if (_requestedMusicTracks.TryGetValue(context, out trackData))
				{
		...
		{
			string rawData;
			if (player.achievements.Contains(which) || gameMode != 3 || !DataLoader.Achievements(content).TryGetValue(which, out rawData))
			{
				return;
			}
		...
		public static Farmer getFarmer(long id)
		{
			...
			Farmer otherFarmer;
			if (otherFarmers.TryGetValue(id, out otherFarmer))
			{
				return otherFarmer;
			}
			if (!IsMultiplayer)
		...
		public static Farmer getFarmerMaybeOffline(long id)
		{
			...
			Farmer farmhand;
			if (netWorldState.Value.farmhandData.TryGetValue(id, out farmhand))
			{
				return farmhand;
			}
			return null;
		...
		public static void addHUDMessage(HUDMessage message)
		{
			if (message.type != null || message.whatType != 0)
			{
				...
				{
					if (message.type != null && message.type == hudMessages[j].type)
					{
						hudMessages[j].number = hudMessages[j].number + message.number;
						hudMessages[j].timeLeft = 3500f;
						hudMessages[j].transparency = 1f;
						if (hudMessages[j].number > 50000)
						{
							HUDMessage.numbersEasterEgg(hudMessages[j].number);
						}
						return;
					}
		...
		public static void showSwordswipeAnimation(int direction, Vector2 source, float animationSpeed, bool flip)
		{
			...
			{
			case 0:
				currentLocation.TemporarySprites.Add(new TemporaryAnimatedSprite(-1, animationSpeed, 5, 1, new Vector2(source.X + 32f, source.Y), false, false, !flip, -(float)Math.PI / 2f));
				break;
			case 1:
				currentLocation.TemporarySprites.Add(new TemporaryAnimatedSprite(-1, animationSpeed, 5, 1, new Vector2(source.X + 96f + 16f, source.Y + 48f), false, flip, false, flip ? (-(float)Math.PI) : 0f));
				break;
			case 2:
				currentLocation.TemporarySprites.Add(new TemporaryAnimatedSprite(-1, animationSpeed, 5, 1, new Vector2(source.X + 32f, source.Y + 128f), false, false, !flip, (float)Math.PI / 2f));
				break;
			case 3:
				currentLocation.TemporarySprites.Add(new TemporaryAnimatedSprite(-1, animationSpeed, 5, 1, new Vector2(source.X - 32f - 16f, source.Y + 48f), false, !flip, false, flip ? (-(float)Math.PI) : 0f));
				break;
			}
		}
		...
		public static bool pressActionButton(KeyboardState currentKBState, MouseState currentMouseState, GamePadState currentPadState)
		{
			...
			if (!player.UsingTool && (!eventUp || (currentLocation.currentEvent != null && currentLocation.currentEvent.playerControlSequence)) && !fadeToBlack)
			{
				if (wasMouseVisibleThisFrame && currentLocation.animals.Length > 0)
				{
					...
					if (Utility.withinRadiusOfPlayer((int)mousePosition.X, (int)mousePosition.Y, 1, player))
					{
						...
						if (didPlayerJustRightClick(true) && currentLocation.CheckInspectAnimal(mousePosition, player))
						{
							return true;
						}
					}
				...
				if (!isPlacingObject && !was_character_at_grab_tile)
				{
					...
					if (!eventUp)
					{
						Furniture furniture3 = player.ActiveObject as Furniture;
						if (furniture3 != null)
						{
							furniture3.rotate();
							playSound("dwoop");
					...
					if (!eventUp)
					{
						Furniture furniture2 = player.ActiveObject as Furniture;
						if (furniture2 != null)
						{
							furniture2.rotate();
							playSound("dwoop");
					...
					if (!eventUp)
					{
						Furniture furniture = player.ActiveObject as Furniture;
						if (furniture != null)
						{
							furniture.rotate();
							playSound("dwoop");
				...
				if (!player.isEating && player.ActiveObject != null && !dialogueUp && !eventUp && !player.canOnlyWalk && !player.FarmerSprite.PauseForSingleAnimation && !fadeToBlack && player.ActiveObject.Edibility != -300 && didPlayerJustRightClick(true))
				{
					...
					if (Game1.objectData.TryGetValue(player.ActiveObject.ItemId, out objectData))
					{
			...
			if (player.CurrentTool is MeleeWeapon && player.CanMove && !player.canOnlyWalk && !eventUp && !player.onBridge && didPlayerJustRightClick(true))
			{
		...
		public static void pressSwitchToolButton()
		{
			...
			{
				player.netItemStowed.Set(false);
		...
		public static bool pressUseToolButton()
		{
			...
			if (player.CurrentTool == null && player.ActiveObject == null)
			{
				...
				c.Y = (int)c.Y;
				Object o;
				if (currentLocation.Objects.TryGetValue(c, out o) && !o.readyForHarvest && o.heldObject.Value == null && !(o is Fence) && !(o is CrabPot) && (o.Type == "Crafting" || o.Type == "interactive") && !o.IsTwig())
				{
			...
			if (Utility.canGrabSomethingFromHere((int)position.X, (int)position.Y, player))
			{
				...
				if (currentLocation.terrainFeatures.TryGetValue(tile, out terrainFeature))
				{
			...
			if (options.allowStowing && player.netItemStowed.Value && !did_attempt_object_removal && (stow_was_initialized || didPlayerJustLeftClick(true)))
			{
				game1._didInitiateItemStow = true;
				playSound("toolSwap");
				player.netItemStowed.Set(false);
			...
			if (player.ActiveObject == null && !player.isEating && player.CurrentTool != null)
			{
				if (player.Stamina <= 20f && player.CurrentTool != null && !(player.CurrentTool is MeleeWeapon) && !eventUp)
				{
					...
					for (int i = 0; i < 4; i++)
					{
						uiOverlayTempSprites.Add(new TemporaryAnimatedSprite("LooseSprites\\Cursors", new Microsoft.Xna.Framework.Rectangle(366, 412, 5, 6), new Vector2(random.Next(32) + uiViewport.Width - 56, uiViewport.Height - 224 - 16 - (int)((double)(player.MaxStamina - 270) * 0.715)), false, 0.012f, Color.SkyBlue)
						{
				...
				if (!(player.CurrentTool is MeleeWeapon) || didPlayerJustLeftClick(true))
				{
					...
					else if (player.FarmerSprite.IsPlayingBasicAnimation(old_direction, true) || player.FarmerSprite.IsPlayingBasicAnimation(old_direction, false))
					{
						player.FarmerSprite.StopAnimation();
					}
				}
		...
		public static bool PlayEvent(string eventId, bool checkPreconditions = true, bool checkSeen = true)
		{
			...
			bool validEvent;
			if (PlayEvent(eventId, currentLocation, out validEvent, checkPreconditions, checkSeen))
			{
				return true;
			}
			if (validEvent)
		...
		{
			if (!IsMasterGame || netWorldState.Value.ActivatedGoldenParrot)
			{
				return;
			}
		...
		private void updatePanModeControls(MouseState currentMouseState, KeyboardState currentKBState)
		{
			...
			if (getMouseX(false) < 192)
			{
				viewport.X -= 8;
				viewport.X -= (192 - getMouseX()) / 8;
			}
			if (getMouseX(false) > viewport.Width - 192)
			{
				viewport.X += 8;
				viewport.X += (getMouseX() - viewport.Width + 192) / 8;
			}
			if (getMouseY(false) < 192)
			{
				viewport.Y -= 8;
				viewport.Y -= (192 - getMouseY()) / 8;
			}
			if (getMouseY(false) > viewport.Height - 192)
			{
				viewport.Y += 8;
			...
			if (currentMouseState.RightButton == ButtonState.Pressed && oldMouseState.RightButton == ButtonState.Released)
			{
				...
				if (w2 != null)
				{
					currentLocation = RequireLocation(w2.TargetName);
					currentLocation.map.LoadTileSheets(mapDisplayDevice);
					...
					else
					{
						if (((useToolButtonPressed && !isAnyGamePadButtonBeingPressed()) || (actionButtonPressed && isAnyGamePadButtonBeingPressed())) && pauseTime <= 0f && wasMouseVisibleThisFrame)
						{
							...
							foreach (IClickableMenu current2 in onScreenMenus)
							{
								...
								{
									if (!IsChatting || current2 == chatBox)
									{
										LevelUpMenu levelUpMenu = current2 as LevelUpMenu;
										if ((levelUpMenu == null || levelUpMenu.informationUp) && current2.isWithinBounds(getMouseX(), getMouseY()))
										{
						...
						else
						{
							...
							if (actionButtonPressed || (dialogueUp && useToolButtonPressed))
							{
								...
								{
									if (wasMouseVisibleThisFrame && (displayHUD || current4 == chatBox) && current4.isWithinBounds(getMouseX(), getMouseY()))
									{
										LevelUpMenu levelUpMenu2 = current4 as LevelUpMenu;
							...
							{
								if (player.CurrentTool != null && (!(player.CurrentTool is MeleeWeapon) || didPlayerJustLeftClick(true)))
								{
									player.FireTool();
								}
		...
		public virtual void DrawLoadScreen(GameTime time, RenderTarget2D target_screen)
		{
			...
			SpriteText.drawString(spriteBatch, msg, msgx, msgy, 999999, msgw, msgh, 1f, 0.88f, false, 0, largestMessage);
			spriteBatch.End();
			PopUIMode();
		}
		...
		public virtual void DrawScreenOverlaySprites(GameTime time, RenderTarget2D target_screen)
		{
			if (hooks.OnRendering(RenderSteps.OverlayTemporarySprites, spriteBatch, time, target_screen))
			{
				...
				{
					screenOverlayTempSprite.draw(spriteBatch, true);
				}
				spriteBatch.End();
				PushUIMode();
				spriteBatch.Begin(SpriteSortMode.Deferred, BlendState.AlphaBlend, SamplerState.PointClamp);
				...
				{
					uiOverlayTempSprite.draw(spriteBatch, true);
				}
				spriteBatch.End();
				PopUIMode();
			}
			...
			if (hooks.OnRendering(RenderSteps.World, spriteBatch, time, target_screen))
			{
				...
				if (hooks.OnRendering(RenderSteps.World_Background, spriteBatch, time, target_screen))
				{
					...
					for (int i = 0; i < currentLocation.backgroundLayers.Count; i++)
					{
						...
						currentLocation.backgroundLayers[i].Key.Draw(mapDisplayDevice, viewport, Location.Origin, false, 4, -1f);
						spriteBatch.End();
					}
					spriteBatch.Begin(SpriteSortMode.Deferred, BlendState.AlphaBlend, SamplerState.PointClamp);
				...
				for (int j = 0; j < currentLocation.buildingLayers.Count; j++)
				{
					...
					{
						layer = (float)j / (float)(currentLocation.buildingLayers.Count - 1);
					}
					currentLocation.buildingLayers[j].Key.Draw(mapDisplayDevice, viewport, Location.Origin, false, 4, layer_sub_sort * layer);
				}
				...
				if (hooks.OnRendering(RenderSteps.World_Sorted, spriteBatch, time, target_screen))
				{
					if (!currentLocation.shouldHideCharacters())
					{
						if (CurrentEvent == null)
						{
							foreach (NPC n2 in currentLocation.characters)
							{
								if (!n2.swimming && !n2.HideShadow && !n2.isInvisible && checkCharacterTilesForShadowDrawFlag(n2))
								{
									n2.DrawShadow(spriteBatch);
								}
							}
						}
						else
						{
							foreach (NPC n3 in CurrentEvent.actors)
							{
								if ((CurrentEvent == null || !CurrentEvent.ShouldHideCharacter(n3)) && !n3.swimming && !n3.HideShadow && checkCharacterTilesForShadowDrawFlag(n3))
								{
									n3.DrawShadow(spriteBatch);
								}
							}
						}
					...
					for (int l = 0; l < currentLocation.frontLayers.Count; l++)
					{
						float layer2 = 0f;
						if (currentLocation.frontLayers.Count > 1)
						{
							layer2 = (float)l / (float)(currentLocation.frontLayers.Count - 1);
						}
						currentLocation.frontLayers[l].Key.Draw(mapDisplayDevice, viewport, Location.Origin, false, 4, 64f + layer_sub_sort * layer2);
					}
				...
				{
					for (int k = 0; k < currentLocation.alwaysFrontLayers.Count; k++)
					{
						spriteBatch.Begin(SpriteSortMode.Texture, BlendState.AlphaBlend, SamplerState.PointClamp);
						currentLocation.alwaysFrontLayers[k].Key.Draw(mapDisplayDevice, viewport, Location.Origin, false, 4, -1f);
						spriteBatch.End();
					}
				}
		...
		public virtual void DrawCharacterEmotes(GameTime time, RenderTarget2D target_screen)
		{
			...
			if (eventUp && currentLocation.currentEvent != null)
			{
				foreach (NPC i in currentLocation.currentEvent.actors)
				{
					if (i.isEmoting)
					{
						Vector2 emotePosition = i.getLocalPosition(viewport);
						...
						spriteBatch.Draw(emoteSpriteSheet, emotePosition, new Microsoft.Xna.Framework.Rectangle(i.CurrentEmoteIndex * 16 % emoteSpriteSheet.Width, i.CurrentEmoteIndex * 16 / emoteSpriteSheet.Width * 16, 16, 16), Color.White, 0f, Vector2.Zero, 4f, SpriteEffects.None, (float)i.StandingPixel.Y / 10000f);
					}
				}
			}
			spriteBatch.End();
		...
		public virtual void DrawLighting(GameTime time, RenderTarget2D target_screen)
		{
			...
			{
				Color lighting = (currentLocation as MineShaft)?.getLightingColor(time) ?? ((ambientLight.Equals(Color.White) || ((bool)currentLocation.isOutdoors && currentLocation.IsRainingHere())) ? outdoorLight : ambientLight);
				...
				if (currentLocation.IsSnowingHere())
				{
					...
					for (float x = -64f + snowPos.X % 64f; x < (float)viewport.Width; x += 64f)
					{
						for (float y = -64f + snowPos.Y % 64f; y < (float)viewport.Height; y += 64f)
						{
							v2.X = (int)x;
							v2.Y = (int)y;
							spriteBatch.Draw(mouseCursors, v2, new Microsoft.Xna.Framework.Rectangle(368 + (int)(currentGameTime.TotalGameTime.TotalMilliseconds % 1200.0) / 75 * 16, 192, 16, 16), Color.White * 0.8f * options.snowTransparency, 0f, Vector2.Zero, 4.001f, SpriteEffects.None, 1f);
						}
					}
				}
		...
		{
			drawWithBorder(message, borderColor, insideColor, position, 0f, 1f, 1f, false);
		}
		...
		{
			drawWithBorder(message, borderColor, insideColor, position, rotate, scale, layerDepth, false);
		}
		...
		public static void drawTool(Farmer f, int currentToolIndex)
		{
			...
			{
				MeleeWeapon.drawDuringUse(farmerSprite.currentAnimationIndex, f.FacingDirection, spriteBatch, fPosition, f, f.FarmerSprite.CurrentToolIndex.ToString(), f.FarmerSprite.getWeaponTypeFromAnimation(), false);
				return;
			}
			...
			if (!(currentTool is Slingshot) && !(currentTool is Shears) && !(currentTool is MilkPail) && !(currentTool is Pan))
			{
				...
				FishingRod rod4 = f.CurrentTool as FishingRod;
				if (rod4 != null)
				{
				...
				float layerDepth = FarmerRenderer.GetLayerDepth(base_layer_depth, tool_layer);
				switch (f.FacingDirection)
				{
				case 1:
				{
				...
				case 3:
				{
					...
					currentTool = f.CurrentTool;
					FishingRod rod2 = currentTool as FishingRod;
					if (rod2 == null)
					{
						if (currentTool is WateringCan)
						{
					...
					case 0:
						...
						{
							spriteBatch.Draw(spritesheet, Utility.snapToInt(new Vector2(fPosition.X - 64f + (float)toolXOffset, fPosition.Y - 160f + (float)toolYOffset)), sourceRectangleForTool, color2, 0f, Vector2.Zero, 4f, SpriteEffects.FlipHorizontally, layerDepth);
						}
						break;
					case 1:
						spriteBatch.Draw(spritesheet, Utility.snapToInt(new Vector2(fPosition.X - 64f + (float)toolXOffset, fPosition.Y - 160f + 8f + (float)toolYOffset)), sourceRectangleForTool, color2, 0f, Vector2.Zero, 4f, SpriteEffects.FlipHorizontally, layerDepth);
						break;
					case 2:
						spriteBatch.Draw(spritesheet, Utility.snapToInt(new Vector2(fPosition.X - 96f + 32f + (float)toolXOffset, fPosition.Y - 128f - 24f + (float)toolYOffset)), sourceRectangleForTool, color2, 0f, Vector2.Zero, 4f, SpriteEffects.FlipHorizontally, layerDepth);
						break;
					case 3:
						spriteBatch.Draw(spritesheet, Utility.snapToInt(new Vector2(fPosition.X - 96f + 24f + (float)toolXOffset, fPosition.Y - 128f - 32f + (float)toolYOffset)), sourceRectangleForTool, color2, 0f, Vector2.Zero, 4f, SpriteEffects.FlipHorizontally, layerDepth);
						break;
					case 4:
						...
						{
							spriteBatch.Draw(spritesheet, Utility.snapToInt(new Vector2(fPosition.X - 64f + (float)toolXOffset, fPosition.Y - 160f + 4f + (float)toolYOffset)), sourceRectangleForTool, color2, 0f, Vector2.Zero, 4f, SpriteEffects.FlipHorizontally, layerDepth);
						}
						break;
					case 5:
						spriteBatch.Draw(spritesheet, Utility.snapToInt(new Vector2(fPosition.X - 64f + (float)toolXOffset, fPosition.Y - 160f + (float)toolYOffset)), sourceRectangleForTool, color2, 0f, Vector2.Zero, 4f, SpriteEffects.FlipHorizontally, layerDepth);
				...
				if (farmerSprite.currentAnimationIndex > 2)
				{
					FishingRod rod3 = f.CurrentTool as FishingRod;
					if (rod3 == null || rod3.isCasting || rod3.castedButBobberStillInAir || rod3.isTimingCast)
					{
						Point tileLocation6 = f.TilePoint;
						if (f.currentLocation.getTileIndexAt(tileLocation6, "Front") != -1 && f.Position.Y % 64f < 32f && f.Position.Y % 64f > 16f)
						{
							return;
						}
					}
				...
				if (farmerSprite.currentAnimationIndex <= 2)
				{
					...
					tileLocation5.Y--;
					if (f.currentLocation.getTileIndexAt(tileLocation5, "Front") != -1)
					{
						return;
					}
				}
				...
				Color color3 = fishingRod.getColor();
				switch (farmerSprite.currentAnimationIndex)
				{
				case 0:
					if (!fishingRod.showingTreasure && !fishingRod.fishCaught && (f.FacingDirection != 0 || !fishingRod.isFishing || fishingRod.isReeling))
					{
						spriteBatch.Draw(spritesheet, Utility.snapToInt(new Vector2(fPosition.X - 64f, fPosition.Y - 128f + 4f)), sourceRectangleForTool, color3, 0f, Vector2.Zero, 4f, SpriteEffects.None, layerDepth);
					}
					break;
				case 1:
					spriteBatch.Draw(spritesheet, Utility.snapToInt(new Vector2(fPosition.X - 64f, fPosition.Y - 128f + 4f)), sourceRectangleForTool, color3, 0f, Vector2.Zero, 4f, SpriteEffects.None, layerDepth);
				...
				case 4:
					...
					{
						spriteBatch.Draw(spritesheet, Utility.snapToInt(new Vector2(fPosition.X - 80f, fPosition.Y - 96f)), sourceRectangleForTool, color3, 0f, Vector2.Zero, 4f, SpriteEffects.FlipVertically, layerDepth);
					}
		...
		public static void UpdateHorseOwnership()
		{
			...
			{
				if (stable5.owner.Value == -6666666 && getFarmerMaybeOffline(-6666666L) == null)
				{
					stable5.owner.Value = player.UniqueMultiplayerID;
				}
				stable5.grabHorse();
			}
			foreach (Stable item in stables)
			{
				...
				if (horse4 != null && !claimed_horses.Contains(horse4) && horse4.getOwner() != null && !horse_lookup.ContainsKey(horse4.getOwner().UniqueMultiplayerID) && horse4.getOwner().horseName.Value != null && horse4.getOwner().horseName.Value.Length > 0 && horse4.Name == horse4.getOwner().horseName.Value)
				{
					...
					if (verbose)
					{
						log.Verbose("Assigned horse " + horse4.Name + " to " + horse4.getOwner().Name + " (Exact match)");
					}
				}
			}
			...
			foreach (Stable stable4 in stables)
			{
				...
				Farmer owner;
				if (horse3 != null && !claimed_horses.Contains(horse3) && horse3.getOwner() != null && horse3.Name != null && horse3.Name.Length > 0 && horse_name_lookup.TryGetValue(horse3.Name, out owner) && !horse_lookup.ContainsKey(owner.UniqueMultiplayerID))
				{
					...
					if (verbose)
					{
						log.Verbose("Assigned horse " + horse3.Name + " to " + horse3.getOwner().Name + " (Name match from different owner.)");
					}
				}
			}
			foreach (Stable stable3 in stables)
			{
				...
				if (horse2 != null && !claimed_horses.Contains(horse2) && horse2.getOwner() != null && !horse_lookup.ContainsKey(horse2.getOwner().UniqueMultiplayerID))
				{
					...
					if (verbose)
					{
						log.Verbose("Assigned horse " + horse2.Name + " to " + horse2.getOwner().Name + " (Owner's only stable)");
					}
				}
			}
		...
		public void _PerformRemoveNormalItemFromWorldOvernight(string itemId)
		{
			...
			{
				if (_RecursiveRemoveThisNormalItemItem(player.team.returnedDonations[k], itemId))
				{
			...
			foreach (Inventory inventory in player.team.globalInventories.Values)
			{
				...
				{
					if (_RecursiveRemoveThisNormalItemItem(((IList<Item>)inventory)[j], itemId))
					{
		...
		protected virtual void _PerformRemoveNormalItemFromFarmerOvernight(Farmer farmer, string itemId)
		{
			...
			{
				if (_RecursiveRemoveThisNormalItemItem(farmer.Items[j], itemId))
				{
					farmer.Items[j] = null;
				}
			}
		...
		protected virtual void _RecursiveRemoveThisNormalItemLocation(GameLocation l, string itemId)
		{
			...
			if (fridge != null)
			{
				...
				for (int k = 0; k < fridgeItems.Count; k++)
				{
					...
					if (item3 != null && _RecursiveRemoveThisNormalItemItem(item3, itemId))
					{
						fridgeItems[k] = null;
					}
				}
		...
		public virtual void CleanupReturningToTitle()
		{
			...
			TitleMenu titleMenu = activeClickableMenu as TitleMenu;
			if (titleMenu != null)
			{
		...
		{
			return Directory.Exists(GetScreenshotFolder(false));
		}
		...
		{
			string folderPath = GetScreenshotFolder(false);
		...
		public unsafe string takeMapScreenshot(float? in_scale, string screenshot_name, Action onDone)
		{
			...
			if (fields.Length != 0)
			{
				...
				int bottomRightX;
				int bottomRightY;
				if (!ArgUtility.TryGetInt(fields, 0, out topLeftX, out error) || !ArgUtility.TryGetInt(fields, 1, out topLeftY, out error) || !ArgUtility.TryGetInt(fields, 2, out bottomRightX, out error) || !ArgUtility.TryGetInt(fields, 3, out bottomRightY, out error))
				{
			...
			do
			{
				...
				{
					log.Error("Map Screenshot: Error trying to create Bitmap.", e2);
					failed = true;
				}
				if (failed)
				{
			...
			try
			{
				...
				for (int y_offset = 0; y_offset < chunks_high; y_offset++)
				{
					for (int x_offset = 0; x_offset < chunks_wide; x_offset++)
					{
						...
						RenderTarget2D render_target = new RenderTarget2D(graphics.GraphicsDevice, chunk_size, chunk_size, false, SurfaceFormat.Color, DepthFormat.None, 0, RenderTargetUsage.DiscardContents);
						...
						RenderTarget2D scaled_render_target = new RenderTarget2D(graphics.GraphicsDevice, current_width, current_height, false, SurfaceFormat.Color, DepthFormat.None, 0, RenderTargetUsage.DiscardContents);
